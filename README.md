...
> 轻量级 OpenAI 账号稳定调度后台。  
> 让 Codex、OpenAI SDK、Cherry Studio、NextChat 等客户端固定一个 `/v1` 入口，把多账号、授权、调度、审计和排障收进一个本地后台。
...
OpenAI OAuth、OpenAI API Key、代理、分组、额度、失败切换、统计和审计都留在后台。账号换了、限流了、过期了、代理波动了，客户端尽量不用跟着改。
...
| 可观测排障 | 使用记录、统计趋势、AI 性能、运行日志、操作日志和原始审计分层追踪 |
...
- SQLite 本地存储，默认适合单机、小服务器、家用主机或轻量云主机。
