...
- **保留完整用量记录**：记录请求、模型、Token、耗时、成本估算、错误摘要和账号命中情况，方便统计和排障。
- **查看账户消耗趋势**：按自然日范围查看最近一个月内账户请求、Token 和成本趋势，趋势图可搜索追加账户并点击账户列表筛选查看，账户统计明细随账户新增自动展示；授权用量统一在“我的授权”查看。
...
- **架构边界清晰**：当前专注 OpenAI 兼容中转和账号管理，不引入重型分布式依赖，后续供应商可按需扩展。
...
1. 登录后台。
2. 在 `AI账户管理` 添加 OpenAI OAuth 账号或 OpenAI API Key 账号。
3. 在 `API 密钥` 创建一个本地 API Key，并绑定可用分组。
4. 在客户端里填写：
