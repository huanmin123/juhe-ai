...
- 路由层级固定为 `API Key -> 路由策略 -> 分组 -> AI 账户 -> 供应商 / 协议能力`。API Key 是用户入口，只绑定路由策略；路由策略负责路由模式和分组绑定；分组协调 AI 账户；AI 账户和供应商管理真实上游语义。
- 策略路由当前目标包含 5 种模式：普通路由、混合智能路由、权重调度路由、故障回退路由和轮询路由。普通路由只能绑定一个分组且不开放复杂配置；其他模式在“策略路由”菜单中维护规则。
- 客户端画像由网关内部自动识别，不作为 API Key、路由策略、分组或普通 AI 账户的用户配置项。跨协议转换属于混合供应商账户能力，不写成 API Key 或路由策略里的显式协议桥接规则。

## 工作原则

- 保持轻量部署和清晰边界，在现有闭环上按实际需要扩展能力。
- 任何生产部署在获得用户正式上线确认后、第一次生产写操作前，必须先运行 `F:\服务部署\juhe-ai\03-部署流程\deployment-plan-lock.ps1 acquire` 领取本机部署计划锁；已有持有人时只能等待或退出，最终验收、明确回滚或未触碰生产的取消后再 `release`，超时接管按部署知识库规则执行。
- 公网 Edge 回源只允许 WireGuard 点对点数据面；禁止新增、恢复或保留 SSH 反向隧道、SSH `-R`、autossh 或等价单 TCP 回源方案。入口回滚只能使用已验证的 WireGuard/Caddy 配置版本。
...
- `accounts`：上游账号管理，保存真实上游凭据、Base URL、账号状态、并发、代理、模型限制、透传和错误策略；混合供应商账户也是真实账户，不是指向其他账户或分组的虚拟路由。
- `groups`：账号分组，承接 AI 账户集合，是账号池和分组内调度边界；分组不定义供应商、模型、协议或跨协议转换语义。
- `route-strategies`：策略路由，保存路由模式、分组绑定、权重、故障回退、轮询和混合智能路由规则；分组统一绑定到路由策略，不直接绑定到 API Key。
- `api-keys`：对外访问密钥，绑定路由策略并控制访问状态、额度、过期和时间计划；API Key 不保存分组绑定、默认客户端画像或显式跨协议路由规则。
