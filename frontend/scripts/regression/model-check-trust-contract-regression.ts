...
assert.equal((view.match(/form\.profile = qualityPolicy\.value\.profile/g) ?? []).length, 4, '加载、保存和重置都必须使用已保存的手工质量策略')
assert(!view.includes('form.profile = nextOptions.defaultProfile'), '模型检测选项不得覆盖已保存的手工质量策略')
