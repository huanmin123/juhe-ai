...
assert.match(schedulesModalSource, /v-model:checked="deepDetection"/, '每条定时计划必须独立绑定深度检测模式')
...
assert.match(schedulesModalSource, /检测模式与处罚规则均由本计划独立控制/, '定时计划必须明确不继承外部手动配置')
