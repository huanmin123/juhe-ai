...
assert(apiKeyModalSource.includes('createShortLivedQueryCache<RouteStrategyOptionSummary[]>'), 'API Key 表单策略路由下拉应使用短缓存减少重复打开和搜索请求')
assert(apiKeyModalSource.includes('missingSelectedIds'), 'API Key 表单策略路由下拉应在主窗口请求后单独补齐缺失的已选策略')
assert(apiKeyModalSource.includes('mergeRouteStrategyOptionsById(selectedStrategies, windowStrategies)'), 'API Key 表单策略路由下拉必须合并已选策略和完整候选窗口')
assert(apiKeyModalSource.includes('activeOnly: false'), 'API Key 表单策略路由下拉必须能回显停用的已绑定策略，并由前端禁用停用选项')
...
assert(apiKeysViewSource.includes('createShortLivedQueryCache<RouteStrategyOptionSummary[]>'), 'API Key 列表策略路由筛选应使用短缓存减少重复请求')
...
assert(routeStrategiesViewSource.includes('createShortLivedQueryCache<GroupOptionSummary[]>'), '策略路由分组选项应使用短缓存减少重复打开和搜索请求')
...
assert(routeStrategiesViewSource.includes('missingSelectedIds'), '策略路由分组选项应在主窗口请求后单独补齐缺失的已选分组')
assert(routeStrategiesViewSource.includes('mergeGroupOptionsById(selectedGroups, windowGroups)'), '策略路由分组选项必须合并已选分组和完整候选窗口')
