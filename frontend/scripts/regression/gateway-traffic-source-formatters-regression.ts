...
import type { ApiKeyGroupBindingFormRow } from '../../src/views/api-keys/apiKeyFormModel'
...
assert.equal(
  apiKeyGroupOptionsProviderProtocolProfileId({
    ...
    formBindings: [{ providerProtocolProfileId: 'profile_gpt_openai_v1' }] as ApiKeyGroupBindingFormRow[]
  }),
...
console.log('网关来源与供应商前端契约回归通过：混合评分来源、API Key 协议档案 scope 和 GLM 空态均符合预期')
