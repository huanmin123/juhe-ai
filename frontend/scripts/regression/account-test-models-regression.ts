...
assertNotIncludes(accountApiSource, 'testEndpointModes: AccountSupportedEndpointMode[]', '请求形态应由模型协议与账户能力推导')
...
assertIncludes(accountTestModelsSource, 'endpointModesForProtocol(protocol)', '模型协议应映射为人工测试请求形态')
assertIncludes(accountTestModelsSource, 'accountModes.filter((mode) => supportedModes.has(mode))', '模型请求形态应与账户能力取交集')
assertIncludes(accountTestModelsSource, 'refreshSelectableEndpointModes()', '切换列表模型时应同步更新请求形态')
