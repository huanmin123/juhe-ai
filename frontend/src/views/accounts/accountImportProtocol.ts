...
export const accountImportProtocolMarkdown = [
  ...
  '| `clientCompatibility` | 否 | string | `openai_standard` 或 `codex_responses`；GLM Coding OpenAI Chat、DeepSeek OpenAI-compatible、Gemini OpenAI Chat 需要承接 Codex 客户端时必须填 `codex_responses`。Anthropic v1 / Claude Code / Gemini 原生档案省略。 |',
  ...
  '| `modelMappings` | 否 | object[] | 账号模型别名列表，只允许当前账号供应商模型目录内的同协议模型名改写；条目包含 `sourceModel`、`sourceEndpointFamily`、`upstreamModel`、`upstreamEndpointFamily`、`enabled`。跨供应商或跨协议映射请在 API Key 显式混合路由配置。 |',
