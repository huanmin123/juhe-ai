...
export const accountImportProtocolMarkdown = [
  ...
  '| `providerProtocolProfileId` | 否 | string | 显式内部协议档案；省略时按供应商默认解析。DeepSeek Claude Code 填 `profile_deepseek_anthropic_v1`；Gemini OpenAI Chat 填 `profile_gemini_openai_chat_v1beta`；混合供应商省略或使用默认 `profile_hybrid_openai_chat_v1`；GLM 优先使用 `connectionType` 区分通用 API、Coding OpenAI Chat 与 Coding Anthropic。 |',
