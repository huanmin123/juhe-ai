...
export const aiConversionPrompt = [
  ...
  '6. providerCode 为 glm 时必须显式填写 connectionType：general_api_key 表示通用 GLM API Key，coding_api_key 表示 GLM Coding Plan Key；只有 GLM Coding Plan 或 DeepSeek OpenAI-compatible 且 clientCompatibility 为 codex_responses 时才启用 Codex Responses 桥接。',
...
export const accountImportProtocolMarkdown = [
  ...
  '- 当前默认使用 `providerCode: "gpt"`；确需通用 OpenAI-compatible 供应商时才填 `openai`；DeepSeek 直连填 `deepseek`；智谱 GLM 直连填 `glm`；官方 Anthropic Claude 直连填 `anthropic`。系统会按供应商自动匹配内部协议配置；通用 `openai` 供应商只填 `api_key`，`deepseek` 当前只填 `api_key`，`glm` 当前只填 `api_key` 但必须用 `connectionType` 区分通用 API 与 Coding Plan，`gpt` 供应商可填 `api_key` 或 `oauth`，`anthropic` 当前只填 `api_key`。',
  '- `providerCode: "glm"` 时必须显式填写 `connectionType: "general_api_key"` 或 `connectionType: "coding_api_key"`；不要根据 `base_url` 猜测接入类型。',
  ...
  '| `providerCode` | 是 | string | 默认填 `gpt`。当前支持 `gpt`、`openai`、`deepseek`、`glm` 和 `anthropic`；`gpt` 表示 GPT 专属供应商，`openai` 表示通用 OpenAI-compatible 供应商，`deepseek` 表示 DeepSeek OpenAI-compatible 直连供应商，`glm` 表示智谱 GLM 直连供应商，`anthropic` 表示官方 Anthropic Claude 直连供应商。 |',
  '| `connectionType` | 条件必填 | string | `glm` 必填 `general_api_key` 或 `coding_api_key`；其他供应商省略。 |',
  '| `type` | 是 | string | `api_key` 或 `oauth`。 |',
  '| `clientCompatibility` | 否 | string | `openai_standard` 或 `codex_responses`；GLM Coding Plan、DeepSeek OpenAI-compatible 需要承接 Codex 客户端时必须填 `codex_responses`。 |',
  '| `providerProtocolProfileId` | 否 | string | 显式内部协议档案；省略时按供应商默认解析。GLM 优先使用 `connectionType` 区分通用 API 与 Coding Plan。 |',
  ...
  '- GLM Coding Plan 的 `clientCompatibility: "codex_responses"` 表示启用 Codex Responses -> Chat Completions 桥接，`credentials.supported_endpoint_modes` 仍填写真实上游能力 `chat_json`、`chat_sse`。',
  ...
  '- `credentials.base_url` 必须显式填写服务根地址或 `/v1` 版本根地址，OpenAI / GPT 通常为 `https://api.openai.com/v1`，DeepSeek 通常为 `https://api.deepseek.com`，GLM 通用 API 通常为 `https://open.bigmodel.cn/api/paas/v4/`，GLM Coding 通常为 `https://open.bigmodel.cn/api/coding/paas/v4`，Anthropic 通常为 `https://api.anthropic.com/v1`；不要填写 `/chat/completions`、`/responses` 或 `/messages` 等具体接口路径。',
  ...
  '- `credentials.supported_endpoint_modes` 可限制接口能力，OpenAI / GPT 枚举为 `chat_json`、`chat_sse`、`responses_json`、`responses_sse`，DeepSeek 和 GLM 只支持 `chat_json`、`chat_sse`，Anthropic 枚举为 `messages_json`、`messages_sse`、`message_token_counting`；省略时通用 `openai` API Key 默认 Chat JSON/SSE，DeepSeek API Key 默认 Chat JSON/SSE，GLM API Key 默认 Chat JSON/SSE，GLM Coding 和 DeepSeek 的 Codex bridge 也必须保存真实 Chat 能力，GPT API Key 默认四项全开，GPT OAuth 默认 Responses JSON/SSE，Anthropic API Key 默认 Messages JSON/SSE/Count Tokens。',
