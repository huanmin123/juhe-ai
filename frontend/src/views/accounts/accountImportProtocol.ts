...
export const accountImportProtocolMarkdown = [
  ...
  '| `healthCheckModel` | 否 | string | 账户级检查模型；省略时按个人默认、管理员系统默认、协议档案默认的顺序初始化，最终必须属于账户 `supportedModels`。 |',
  '| `healthCheckEndpointMode` | 否 | string | 后台健康检查精确请求形态；可填 `chat_json`、`chat_sse`、`responses_json`、`responses_sse`、`messages_json`、`messages_sse`、`generate_content_json`、`generate_content_sse`、`interactions_json`、`interactions_sse`。 |',
  ...
  '- `healthCheckModel` 只在账户创建阶段按默认链补齐；账户落库后后台检查严格读取该字段，不会动态跟随供应商默认值。',
  '- `healthCheckEndpointMode` 必须是账户已启用的请求形态；省略时 GPT 官方默认 `responses_sse`、OpenAI-compatible 默认 `chat_json`、Anthropic 默认 `messages_json`、Gemini Native 默认 `generate_content_json`。',
  ...
  '```',
  '',
  'Gemini Code Assist / Google One OAuth 把 `oauth_type` 改为 `code_assist` 或 `google_one`，必须填写 `project_id`，`base_url` 使用 `https://cloudcode-pa.googleapis.com`，`supported_endpoint_modes` 只能填写 `generate_content_json`、`generate_content_sse`。导入的 Client ID / Secret 必须与 Refresh Token 的签发 client 一致。',
  '',
  ...
  '- `providerCode = xai` 必须使用 `profile_xai_openai_v1`，可选 `api_key` 或 `oauth`。API Key 必须有 `credentials.api_key`，通常使用 `https://api.x.ai/v1`；Grok OAuth 必须有 `credentials.access_token`，通常使用 `https://cli-chat-proxy.grok.com/v1`，`supported_endpoint_modes` 只能是 `responses_json`、`responses_sse`。导入 JSON 不接受 SSO、`sso`、`sso-rw` 或 Cookie 字段。',
