...
export const aiConversionPrompt = [
  ...
  '6. providerCode 为 glm 时必须显式填写 providerProtocolProfileId：profile_glm_general_openai_v1 表示通用 GLM API Key，profile_glm_coding_openai_v1 表示 GLM Coding OpenAI Chat，profile_glm_coding_anthropic_v1 表示 GLM Coding Anthropic；DeepSeek Claude Code 必须显式填写 providerProtocolProfileId: profile_deepseek_anthropic_v1。',
...
export const accountImportProtocolMarkdown = [
  ...
  '- 当前默认使用 `providerCode: "gpt"`；确需通用 OpenAI-compatible 供应商时才填 `openai`；DeepSeek 直连填 `deepseek`；智谱 GLM 直连填 `glm`；官方 Anthropic Claude 直连填 `anthropic`；Google Gemini 直连填 `gemini`；跨协议真实上游账户填 `hybrid`。系统会按供应商和协议档案匹配内部配置；通用 `openai` 供应商只填 `api_key`，`deepseek` 当前只填 `api_key` 但可选 OpenAI-compatible 或 Anthropic v1 档案，`glm` 当前只填 `api_key` 但必须用 `providerProtocolProfileId` 区分通用 API、Coding OpenAI Chat 与 Coding Anthropic，`gpt` 供应商可填 `api_key` 或 `oauth`，`anthropic` 当前只填 `api_key`，`gemini` 当前只填 `api_key` 但可选 Gemini native 或 Gemini OpenAI Chat 档案，`hybrid` 当前只填 `api_key`，使用默认混合供应商档案；真实上游协议由账户模型映射的目标协议和接口能力表达。',
  '- `providerCode: "glm"` 时必须显式填写 `providerProtocolProfileId: "profile_glm_general_openai_v1"`、`providerProtocolProfileId: "profile_glm_coding_openai_v1"` 或 `providerProtocolProfileId: "profile_glm_coding_anthropic_v1"`；不要根据 `base_url` 猜测接入类型。',
  ...
  '| `providerProtocolProfileId` | 条件必填 | string | 显式内部协议档案；DeepSeek Claude Code 填 `profile_deepseek_anthropic_v1`；GLM 必填 `profile_glm_general_openai_v1`、`profile_glm_coding_openai_v1` 或 `profile_glm_coding_anthropic_v1`；Gemini OpenAI Chat 填 `profile_gemini_openai_chat_v1beta`；混合供应商省略或使用默认 `profile_hybrid_openai_chat_v1`。 |',
