...
export const accountImportProtocolMarkdown = [
  ...
  '| `clientCompatibility` | 否 | string | `openai_standard` 或 `codex_responses`；GLM Coding OpenAI Chat、DeepSeek OpenAI-compatible、Gemini OpenAI Chat 需要承接 Codex 客户端时必须填 `codex_responses` 或配置显式 `responses -> chat_completions` 模型映射。Anthropic v1 / Claude Code / Gemini 原生档案省略。 |',
  '| `providerProtocolProfileId` | 否 | string | 显式内部协议档案；省略时按供应商默认解析。DeepSeek Claude Code 填 `profile_deepseek_anthropic_v1`；Gemini OpenAI Chat 填 `profile_gemini_openai_chat_v1beta`；GLM 优先使用 `connectionType` 区分通用 API、Coding OpenAI Chat 与 Coding Claude Code。 |',
  ...
  '- `credentials.supported_endpoint_modes` 可限制接口能力，OpenAI / GPT 枚举为 `chat_json`、`chat_sse`、`responses_json`、`responses_sse`；DeepSeek OpenAI-compatible、GLM 通用、GLM Coding OpenAI Chat、Gemini OpenAI Chat 只支持 `chat_json`、`chat_sse`；DeepSeek Claude Code、GLM Coding Claude Code 只支持 `messages_json`、`messages_sse`；官方 Anthropic 枚举为 `messages_json`、`messages_sse`、`message_token_counting`；Gemini native 枚举为 `generate_content_json`、`generate_content_sse`、`count_tokens`、`embed_content`。省略时通用 `openai` API Key 默认 Chat Completions (JSON/Streaming)，DeepSeek OpenAI-compatible 默认 Chat Completion (JSON/Streaming)，DeepSeek Claude Code 默认 Messages (JSON/Streaming)，GLM 通用 API Key 默认对话补全 (JSON/Streaming)，GLM Coding OpenAI Chat 默认 OpenAI Chat Completions (JSON/Streaming)，GLM Coding Claude Code 默认 Messages (JSON/Streaming)，Gemini OpenAI Chat 默认 OpenAI Chat Completions (JSON/Streaming)，Gemini native 默认 GenerateContent (JSON/Streaming) 和 CountTokens，GLM Coding、DeepSeek 和 Gemini OpenAI Chat 的 Codex / Gemini native bridge 也必须保存真实上游能力，GPT API Key 默认四项全开，GPT OAuth 默认 Responses API (JSON/Streaming)，官方 Anthropic API Key 默认 Messages API (JSON/Streaming) 和 Count tokens。',
