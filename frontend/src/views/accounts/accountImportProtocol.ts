...
export const accountImportProtocolMarkdown = [
  ...
  '- 当前默认使用 `providerCode: "gpt"`；确需通用 OpenAI-compatible 供应商时才填 `openai`；DeepSeek 直连填 `deepseek`；智谱 GLM 直连填 `glm`；官方 Anthropic Claude 直连填 `anthropic`；Google Gemini 直连填 `gemini`；跨协议真实上游账户填 `hybrid`。系统会按供应商和协议档案匹配内部配置；所有账户都必须显式填写 `providerProtocolProfileId`，真实上游协议由账户模型映射的目标协议和上游接口能力表达。',
  ...
  '- `credentials.supported_endpoint_modes` 用于声明上游接口能力，OpenAI / GPT 枚举为 `chat_json`、`chat_sse`、`responses_json`、`responses_sse`；Anthropic 枚举为 `messages_json`、`messages_sse`、`message_token_counting`；Gemini native 枚举为 `generate_content_json`、`generate_content_sse`、`count_tokens`、`embed_content`；混合供应商账户可在这些枚举中按真实上游支持情况选择，省略时默认全量开放。字段名保持 snake_case，不要改成 camelCase。',
