...
export const accountImportProtocolMarkdown = [
  ...
  '- GLM Coding OpenAI Chat 的 `clientCompatibility: "codex_responses"` 表示启用 Codex Responses -> Chat Completions 桥接，`credentials.supported_endpoint_modes` 仍填写真实上游能力 `chat_json`、`chat_sse`。',
  '- DeepSeek OpenAI-compatible 的 `clientCompatibility: "codex_responses"` 表示启用 Codex Responses -> Chat Completions 桥接；DeepSeek bridge 账号的 `credentials.supported_endpoint_modes` 仍保存真实上游能力 `chat_json`、`chat_sse`。',
