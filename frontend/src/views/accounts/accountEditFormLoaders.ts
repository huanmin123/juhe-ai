...
export function buildAccountEditFormLoad(input: AccountFormLoadInput): AccountEditFormLoadResult {
  ...
  const patch: AccountFormModel = {
    ...
    baseUrl,
    anthropicVersion: asString(credentials.anthropic_version) ?? defaults.anthropicVersion,
    anthropicBeta: asString(credentials.anthropic_beta) ?? defaults.anthropicBeta,
