...
export function accountTypeDescription(providerCode: string, type: AccountType, providerProtocolProfileId?: string) {
  ...
  if (isGeminiProviderCode(providerCode) && type === 'api_key' && providerProtocolProfileId === GEMINI_OPENAI_CHAT_V1BETA_PROFILE_ID) return '适合 Gemini OpenAI Chat 兼容入口；跨协议 Codex / Gemini native 调度请在 API Key 显式混合路由中配置。'
