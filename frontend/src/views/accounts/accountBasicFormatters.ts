...
export function accountTypeDescription(providerCode: string, type: AccountType, providerProtocolProfileId?: string) {
  ...
  if (isGeminiProviderCode(providerCode) && type === 'google_oauth') return '支持 Code Assist、Google One 与 AI Studio OAuth 托管授权，也可导入 Refresh Token 或 Access Token；AI Studio 模式使用用户自己的 Client ID 与 Client Secret。'
  if (isXaiProviderCode(providerCode) && type === 'oauth') return '支持 Grok 官方 OAuth 托管授权、Refresh Token 或 Access Token 导入；使用 OpenAI-compatible 请求形态并按账户能力启用 Responses。'
