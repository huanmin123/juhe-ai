...
export type OAuthAccessTokenPayload = AccountOAuthCreateCommonPayload & {
  accessToken: string
  refreshToken?: string
}
...
export type ReauthorizeAccessTokenPayload = {
  accessToken: string
  refreshToken?: string
}
...
export function buildOAuthCreatePayload(input: {
  ...
  sessionId?: string
}): OAuthCodePayload | OAuthRefreshTokenPayload | OAuthAccessTokenPayload {
  ...
  if (input.form.oauthMode === 'access_token') {
    return {
      ...input.commonPayload,
      accessToken: input.form.accessToken,
      ...(input.form.refreshToken.trim() ? { refreshToken: input.form.refreshToken } : {})
    }
  }
  return {
    ...input.commonPayload,
...
export function buildReauthorizePayload(input: {
  form: AccountOAuthAuthorizeForm
  sessionId?: string
}): ReauthorizeCodePayload | ReauthorizeRefreshTokenPayload | ReauthorizeAccessTokenPayload {
  if (input.form.oauthMode === 'manual') {
    return {
      sessionId: input.sessionId,
      callbackUrl: input.form.callbackUrl
