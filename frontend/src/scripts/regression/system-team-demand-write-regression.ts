...
const saveSource = sourceBetween(viewSource, "const saveTeam = submitAction('system_teams.save'", 'function openMemberModal')
...
assert.match(saveSource, /commitSystemTeamPatch\(updated\)/, '编辑必须把后端最小 mutation 交给列表协调器')
...
assert.match(saveSource.slice(createBranch), /const created = await api\.systemTeams\.create[\s\S]*commitCreatedSystemTeam\(created\)/, '创建成功必须直接消费服务端生成的最小列表行')
assert.doesNotMatch(saveSource.slice(createBranch), /await loadData\(\)/, '创建成功不得无条件重载列表')
