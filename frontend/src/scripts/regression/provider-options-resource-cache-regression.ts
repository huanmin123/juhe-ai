...
const responseInspectionPoliciesSource = readFileSync(fileURLToPath(new URL('../../views/response-inspection-policies/ResponseInspectionPoliciesView.vue', import.meta.url)), 'utf8')
assert.match(responseInspectionPoliciesSource, /api\.responseInspectionPolicies\.providerOptions\(\{ signal \}\)/, '响应检查策略必须按操作请求专用三字段 provider options')
assert.doesNotMatch(responseInspectionPoliciesSource, /loadProviderOptionsResource|api\.providers\.(?:options|definitions)\(/, '响应检查策略不得为表单选项复用完整供应商 definitions/catalog')
