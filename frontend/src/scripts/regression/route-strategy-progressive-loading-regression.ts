...
const createBranch = sourceBetween(saveSource, '} else {\n      const created = await routeStrategiesApi.create', 'message.success(\'策略路由已创建\')')
assert.doesNotMatch(createBranch, /invalidateUserReferenceData/, '普通路由创建不得无条件失效共享默认资源缓存')
const createCompletionBranch = sourceBetween(saveSource, '} else {\n      const created = await routeStrategiesApi.create', '\n    }\n  } catch')
assert.match(createCompletionBranch, /applyCreatedRouteStrategyListItem\(created, operationScopeParams\?\.systemAccountId\)/, '创建成功必须直接消费服务端窄列表行')
assert.doesNotMatch(createCompletionBranch, /loadRouteStrategies\(/, '创建成功不得重新加载策略路由列表')
assert.match(viewSource, /function applyCreatedRouteStrategyListItem[\s\S]*routeStrategyMatchesCurrentFilters\(created\)[\s\S]*total\.value \+= 1[\s\S]*items\.value = \[created, \.\.\.items\.value\]\.slice/, '新建列表行必须仅在匹配当前筛选时本地插入并维护分页上限')
assert.match(apiSource, /create: .*unwrap<RouteStrategyListItem>/, '创建 API 必须声明返回可直接插入列表的窄行')
...
console.log('策略路由渐进加载回归通过：编辑字段级 PATCH 本地合并，创建返回窄列表行且不再刷新列表')
