...
assert.match(modelCheckOptionsSource, /createModelCheckDemandRequestCoordinator/, '模型检测账户选项应保留相同在途请求合并协调器')
assert.match(modelCheckOptionsSource, /targetAbortController\?\.abort\(\)/, '模型检测账户选项重新请求前必须中止旧在途请求')
