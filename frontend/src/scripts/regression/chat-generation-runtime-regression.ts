...
assert.equal(first.userProjection?.role, 'user', 'runtime.start 必须同步返回 optimistic user projection')
assert.equal(first.userProjection?.contentText, 'hello')
assert.equal(first.projection.role, 'assistant', 'runtime.start 必须同步返回 optimistic assistant projection')
assert.deepEqual(first.userProjection?.contentBlocks, [{ type: 'input_text', text: 'hello', order: 0 }])
