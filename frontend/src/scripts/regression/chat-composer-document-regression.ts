...
assert.match(composerSource, /<strong>\/\{\{ item\.key \}\}<\/strong><span class="ai-composer-command-description">\{\{ item\.description \}\}<\/span>/, '命令菜单必须只展示命令与说明两列')
assert.doesNotMatch(composerSource, /<strong>\/\{\{ item\.key \}\}<\/strong><span>\{\{ item\.label \}\}<\/span>/, '命令菜单不得恢复命令名称的第三列')
assert.match(composerSource, /grid-template-columns:\s*minmax\(96px, auto\) minmax\(0, 1fr\)/, '命令菜单必须为命令与说明保留两列布局')
