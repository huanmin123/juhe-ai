...
const legacyImagePreparationStateUrl = new URL('../../views/chat/composer/chatImagePreparationState.ts', import.meta.url)
assert.equal(existsSync(fileURLToPath(legacyImagePreparationStateUrl)), false, '旧图片准备状态 helper 已被有界队列替代，必须删除')
...
assert.match(composerSource, /import \{ ChatImagePreparationQueue \} from '\.\/chatImagePreparationQueue'/, 'AIComposer 必须保留当前有界图片准备队列接线')
assert.doesNotMatch(composerSource, /chatImagePreparationState/, 'AIComposer 不得恢复旧 generation/token 状态 helper')
