...
assertMatch(accountImportProtocolMarkdown, /GLM Coding OpenAI Chat、DeepSeek OpenAI-compatible 如需承接 Codex 客户端必须显式填写 `codex_responses`/, '协议 Markdown 应说明 GLM 与 DeepSeek Codex bridge 的显式开关')
...
assertMatch(accountImportProtocolMarkdown, /DeepSeek Claude Code、GLM Coding Claude Code 和官方 Anthropic 走 Anthropic v1 Messages 原生协议，不填写 `clientCompatibility`/, '协议 Markdown 应说明 Anthropic v1 Claude Code 档案不填写客户端兼容字段')
assertMatch(accountImportProtocolMarkdown, /GLM Coding OpenAI Chat 和 DeepSeek OpenAI-compatible 的 `credentials\.supported_endpoint_modes` 仍填写真实上游能力 `chat_json`、`chat_sse`/, '协议 Markdown 应说明 GLM 与 DeepSeek Codex bridge 仍保存 Chat endpoint modes')
