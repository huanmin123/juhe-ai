...
const glmCodingClaudeCodeAccount = template.accounts?.find((account) => account.providerCode === GLM_PROVIDER_CODE && account.connectionType === 'coding_anthropic_api_key')
...
assertDefined(glmCodingClaudeCodeAccount, '导入模板应包含 GLM Coding Claude Code Key 账号示例')
...
assertFalse(Object.prototype.hasOwnProperty.call(glmCodingClaudeCodeAccount, 'clientCompatibility'), 'GLM Coding Claude Code 示例不应暴露客户端兼容字段')
assertEqual(glmCodingClaudeCodeAccount.credentials?.base_url, 'https://open.bigmodel.cn/api/anthropic', 'GLM Coding Claude Code 示例应使用 Anthropic-compatible base URL')
assertEqual(JSON.stringify(glmCodingClaudeCodeAccount.credentials?.supported_endpoint_modes), JSON.stringify(['messages_json', 'messages_sse']), 'GLM Coding Claude Code 示例应只启用 Messages JSON/Streaming')
...
assertMatch(accountImportProtocolMarkdown, /GLM Coding OpenAI Chat、DeepSeek OpenAI-compatible、Gemini OpenAI Chat 如需承接 Codex 客户端必须显式填写 `codex_responses` 或配置 `responses -> chat_completions` 模型映射/, '协议 Markdown 应说明 GLM、DeepSeek 与 Gemini OpenAI Chat Codex bridge 的显式开关')
assertMatch(accountImportProtocolMarkdown, /`connectionType: "coding_anthropic_api_key"`/, '协议 Markdown 应说明 GLM Coding Claude Code 接入类型')
...
assertMatch(accountImportProtocolMarkdown, /DeepSeek Claude Code、GLM Coding Claude Code、官方 Anthropic 和 Gemini 原生档案不填写 `clientCompatibility`/, '协议 Markdown 应说明 Anthropic v1 Claude Code 与 Gemini 原生档案不填写客户端兼容字段')
assertMatch(accountImportProtocolMarkdown, /需要把 Gemini native `generateContent` \/ `streamGenerateContent` 请求桥接到 Chat Completions 上游时，在目标 OpenAI 协议 Chat 上游账号的 `modelMappings` 中显式配置 `sourceEndpointFamily: "generate_content"` 或 `"stream_generate_content"`/, '协议 Markdown 应说明 Gemini native 到 Chat Completions 显式映射')
assertMatch(accountImportProtocolMarkdown, /GLM Coding、DeepSeek 和 Gemini OpenAI Chat 的 Codex \/ Gemini native bridge 也必须保存真实上游能力/, '协议 Markdown 应说明 Gemini native bridge 仍保存 Chat endpoint modes')
assertMatch(accountImportProtocolMarkdown, /右侧 `upstreamEndpointFamily: "responses"` 只允许 `sourceEndpointFamily: "responses"` 的原生 Responses 直连/, '协议 Markdown 应说明右侧 Responses 只允许 Responses 原生直连')
assertMatch(accountImportProtocolMarkdown, /该方向承接 Codex Responses 客户端时仍只填写真实 Chat 能力 `chat_json`、`chat_sse`/, '协议 Markdown 应说明 Responses 到 Chat bridge 不伪造 Responses endpoint mode')
assertMatch(accountImportProtocolMarkdown, /不要配置 `chat_completions -> responses`，该方向不支持/, '协议 Markdown 应说明 Chat 到 Responses 不支持')
...
assertMatch(accountImportProtocolMarkdown, /需要把 OpenAI Chat Completions、OpenAI Responses 或 Anthropic Messages 请求桥接到 Gemini native 上游时，在 Gemini native 账号的 `modelMappings` 中显式配置 `upstreamEndpointFamily: "generate_content"`/, '协议 Markdown 应说明 OpenAI/Anthropic 到 Gemini native 显式映射')
assertMatch(accountImportProtocolMarkdown, /右侧不单独配置 `stream_generate_content`，流式由请求自身决定/, '协议 Markdown 应说明 Gemini native 右侧目标不单独配置 stream_generate_content')
assertMatch(accountImportProtocolMarkdown, /`generate_content` \/ `stream_generate_content` 来源必须来自 Gemini 协议模型池/, '协议 Markdown 应说明 Gemini source 模型池')
assertMatch(accountImportProtocolMarkdown, /Gemini native 账号右侧模型来自 Gemini 模型目录/, '协议 Markdown 应说明 Gemini native target 模型池')
...
assertMatch(formalProtocolMarkdown, /右侧 `upstreamEndpointFamily: "responses"` 只允许 `sourceEndpointFamily: "responses"` 的原生 Responses 直连/, '正式协议文档应说明右侧 Responses 只允许 Responses 原生直连')
assertMatch(formalProtocolMarkdown, /该方向承接 Codex Responses 客户端时仍只填写真实 Chat 能力 `chat_json`、`chat_sse`/, '正式协议文档应说明 Responses 到 Chat bridge 不伪造 Responses endpoint mode')
assertMatch(formalProtocolMarkdown, /需要把 Gemini native `generateContent` \/ `streamGenerateContent` 请求桥接到 Chat Completions 上游时/, '正式协议文档应说明 Gemini native 到 Chat bridge')
assertMatch(formalProtocolMarkdown, /不要配置 `chat_completions -> responses`，该方向不支持/, '正式协议文档应说明 Chat 到 Responses 不支持')
...
assertMatch(formalProtocolMarkdown, /需要把 OpenAI Chat Completions、OpenAI Responses 或 Anthropic Messages 请求桥接到 Gemini native 上游时，在 Gemini native 账号的 `modelMappings` 中显式配置 `upstreamEndpointFamily: "generate_content"`/, '正式协议文档应说明 OpenAI/Anthropic 到 Gemini native 显式映射')
assertMatch(formalProtocolMarkdown, /右侧不单独配置 `stream_generate_content`，流式由请求自身决定/, '正式协议文档应说明 Gemini native 右侧目标不单独配置 stream_generate_content')
