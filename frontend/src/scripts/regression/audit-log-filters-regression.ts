...
const traceParams = auditLogListParams({
  ...
  sessionClientTypeFilter: 'codex_cli',
...
assert(traceParams.sessionClientType === undefined, 'traceId 直接定位不应叠加缓存的会话客户端筛选')
...
assert(
  /function clearFiltersForDirectTraceLookup[\s\S]*?sessionClientTypeFilter\.value = defaults\.sessionClientTypeFilter[\s\S]*?sessionIdFilter\.value = defaults\.sessionIdFilter/.test(viewSource),
  'traceId 直接定位必须清除后续新增的会话客户端和 sessionId 筛选'
)
assert(
...
console.log('审计日志筛选回归通过：traceId 直接定位不会叠加缓存的其他筛选')
