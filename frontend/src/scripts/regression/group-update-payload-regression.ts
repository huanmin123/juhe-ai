...
assert.match(saveSource, /const target = editingTarget[\s\S]*groupEditPatch\(\)/, '编辑保存必须使用打开弹窗时保存的目标与差异基线')
assert.match(saveSource, /if \(!Object\.keys\(payload\)\.length\)[\s\S]*modalOpen\.value = false[\s\S]*return/, '空差异必须在请求前直接结束')
