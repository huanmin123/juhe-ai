...
try {
  await waitForServer()
  const browser = spawn(browserPath(), ['--headless=new', '--disable-gpu', '--no-first-run', '--no-default-browser-check', `--remote-debugging-port=${debuggingPort}`, `--user-data-dir=${profile}`, `http://127.0.0.1:${port}/__aisys__/chat-local-cache-indexeddb-regression.html`], { stdio: 'ignore' })
  try {
    ...
    for (let attempt = 0; attempt < 300; attempt += 1) { response = await evaluate(); if (response.result?.result?.value?.status) break; await new Promise((resolve) => setTimeout(resolve, 100)) }
    socket.close()
