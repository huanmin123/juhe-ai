...
const sessionFilterCounts = auditLogFilterCounts({
  accountIdFilter: '',
  outcomeFilter: 'all',
  pathFilter: '',
  sessionIdFilter: sessionId,
...
assert(sessionFilterCounts.active === 1 && sessionFilterCounts.advanced === 1, 'sessionId 必须计入更多筛选状态')
...
assert(source('../../views/audit-logs/useAuditLogModeBridge.ts').includes('输入完整 traceId，精确查找请求'), '主搜索入口必须明确按完整 traceId 精确查询')
assert(filterFormSource.includes('label="会话 ID"'), '更多筛选必须提供完整会话 ID 输入框')
assert(!filterFormSource.includes('客户端类型'), '更多筛选不应提供客户端类型')
assert(!filterFormSource.includes('HTTP 状态码'), '更多筛选不应提供 HTTP 状态码')
...
console.log('审计会话身份前端回归通过：原始 sessionId 精确查询，客户端类型仅保留详情展示')
