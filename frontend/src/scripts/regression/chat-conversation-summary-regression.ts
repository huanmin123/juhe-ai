...
function conversation(title: string, activeTurnId?: string): ChatConversation {
  return { id: 'conv_1', systemAccountId: 'sys_1', apiKeyNameSnapshot: 'Key', title, isPinned: true, activeTurnId, lastMessageAt: '2026-07-13T00:00:00.000Z', createdAt: '2026-07-13T00:00:00.000Z', updatedAt: '2026-07-13T00:00:00.000Z' }
}
...
const merged = mergeChatConversationSummary(conversation('本地旧标题'), { ...conversation('后端新标题'), isPinned: false })
...
assert.match(viewSource, /event\.type === 'message\.started'[\s\S]{0,500}refreshConversationSummary\(requestContext\.conversationId\)/, 'message.started 后必须非阻塞刷新权威标题')
