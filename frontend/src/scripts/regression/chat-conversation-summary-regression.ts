...
const contextStatusTimerStart = viewSource.indexOf('contextStatusTimer = window.setInterval')
const contextStatusTimerSource = viewSource.slice(contextStatusTimerStart, viewSource.indexOf('subscribeSelectedRuntime()', contextStatusTimerStart))
assert.match(contextStatusTimerSource, /if \(!conversation \|\| !runtime\?\.reconciliationReason\) return/, '空闲会话不得因为固定定时器无条件刷新上下文状态')
assert.match(contextStatusTimerSource, /if \(!conversation \|\| !runtime\?\.reconciliationReason\) return\s*if \(!requestRuntimeReconciliationSync\(runtime\)\) return\s*void refreshContextStatus\(conversation\.id\)/, '上下文状态轮询必须先取得运行态对账锁，避免重复对账时继续刷新')
assert.doesNotMatch(contextStatusTimerSource, /setInterval\(\(\) => \{\s*void refreshContextStatus\(\)/, '上下文状态轮询只能在确有运行态需要重协调时触发')
assert.match(viewSource, /function requestRuntimeReconciliationSync\(turn: RunningTurn\): boolean \{\s*if \(!runtimeReconciliationScheduler\.begin\(turn\)\) return false\s*void refreshConversationFromSync\(turn\.conversationId\)\.finally\(\(\) => \{\s*runtimeReconciliationScheduler\.complete\(turn, chatGenerationRuntime\.get\(turn\.systemAccountId, turn\.conversationId\)\)\s*\}\)\s*return true\s*\}/, '运行态对账请求必须以 boolean 返回是否实际取得调度锁')
