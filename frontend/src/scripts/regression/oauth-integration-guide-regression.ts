...
assertContains(viewSource, 'Client 类型说明', '新建 Client 时必须提供类型说明入口')
assertContains(viewSource, '适合浏览器网页、桌面客户端、手机 App 和 CLI', '类型说明必须明确公开 Client 的适用场景')
assertContains(viewSource, '适合有可信服务端或 BFF 的网站与服务端集成', '类型说明必须明确机密 Client 的适用场景')
assertContains(viewSource, '只有第三方服务端兑换 Token 时选择机密 Client', '类型说明必须给出可执行的选择建议')
