...
assert.deepEqual(XAI_PROVIDER.accountTypes, ['api_key'])
assert.deepEqual(XAI_PROVIDER.protocolProfiles[0]?.accountTypes, ['api_key'])

const profile = XAI_PROVIDER.protocolProfiles[0]
assert(profile, 'xAI fallback provider 应包含 OpenAI v1 档案')
assert.equal(canCreateOAuthAccount({ provider: XAI_PROVIDER, profile }), false, 'xAI 不应出现 OAuth 创建入口')
assert.deepEqual(
...
console.log('xAI 前端账户能力回归通过：仅 API Key、默认 Base URL、Chat/Responses 能力与中文创建文案符合预期')
