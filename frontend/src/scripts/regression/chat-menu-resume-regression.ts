...
assert.match(viewSource, /shouldAttemptRuntimeReconciliationSync/, 'reconnect_exhausted 权威同步必须有去重门禁')
assert.doesNotMatch(viewSource, /setInterval\([\s\S]{0,500}reconciliationReason\) void refreshConversationFromSync/, '5 秒轮询不得对同一 reconnect_exhausted 状态无限同步')
