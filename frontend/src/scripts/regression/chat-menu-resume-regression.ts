...
assert.match(viewSource, /turn\.error\?\.status === 403[\s\S]{0,500}messages\.value = \[\]/, '初始 POST\/SSE 403 必须立即隐藏正文')
assert.match(viewSource, /resolveChatSubmitFailure[\s\S]{0,900}resolution\.restoreSubmittedDraft[\s\S]{0,300}composer\.value\?\.restore\(input\.request\.snapshot\)/, '未接受的初始 POST/SSE 失败必须按权威提交结果恢复草稿')
