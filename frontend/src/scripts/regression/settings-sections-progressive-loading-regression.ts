...
assert(/onActivated\(\(\) => \{\s*pageActive = true[\s\S]*sectionRequestGate\.activate\(\)[\s\S]*sectionSaveRequestGate\.activate\(\)\s*\}\)/.test(view), 'KeepAlive 恢复时只能恢复 section 请求门禁')
assert(!view.includes('reloadVisibleSections'), 'KeepAlive 恢复和身份变化不得自动重载 settings section')
