...
assert.match(viewSource, /conversation-new-button[\s\S]{0,220}onClick:\s*openCreateDialogFromConversationPane/, '会话面板新建必须走抽屉协调入口')
assert.match(viewSource, /function openCreateDialogFromConversationPane[\s\S]{0,420}pendingCreateAfterDrawerClose\.value\s*=\s*true[\s\S]{0,180}conversationDrawerOpen\.value\s*=\s*false[\s\S]{0,80}return/, '手机入口必须只关闭 Drawer 并延后 Modal')
assert.match(viewSource, /function handleConversationDrawerAfterOpenChange\(open:\s*boolean\)[\s\S]{0,300}if \(open \|\| !pendingCreateAfterDrawerClose\.value\) return[\s\S]{0,180}openCreateDialog\(\)/, 'Drawer 确认关闭后才能打开 Modal')
assert.doesNotMatch(viewSource, /function openCreateDialogFromConversationPane[\s\S]{0,420}conversationDrawerOpen\.value\s*=\s*false[\s\S]{0,160}(?:nextTick|setTimeout|requestAnimationFrame)/, '不得用猜测延迟代替 Drawer afterClose')
