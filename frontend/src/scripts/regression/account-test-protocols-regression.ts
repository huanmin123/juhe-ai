...
assertIncludes(accountTestModelsSource, 'option.testEndpointModes', '模型目录必须携带服务端计算后的请求形态')
assertIncludes(updateSelectableTestModelSource, 'applyTestEndpointModes(option.testEndpointModes, false)', '切换模型必须立即同步该模型的请求形态')
assertIncludes(applyTestEndpointModesSource, "endpointModes[0] ?? 'account_default'", '模型无可用形态时必须清除前一模型遗留的无效检查协议')
assertIncludes(applyTestEndpointModesSource, 'endpointModes.includes(currentEndpointMode)', '模型列表加载后必须保留仍然有效的账户默认请求形态')
