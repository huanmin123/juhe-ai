...
assert.doesNotMatch(performanceSource, /cacheTtlMilliseconds|this\.cache\.set/, '聊天模型和能力不得保留 TTL 结果缓存')
assert.match(chatViewSource, /async function loadModelsOnOpen[\s\S]*modelLoadCoordinator\.load/, '模型列表只能由下拉展开触发，并在每次展开时重读')
