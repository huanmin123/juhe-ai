...
import {
  extractGatewayMultipartAudioResponseFormat,
  extractGatewayMultipartImageModel
} from './multipart-image-metadata.js'
...
export async function captureGatewayRawBody(
  ...
  try {
    ...
    if (rawBody.length === 0) {
      ...
      const [model, responseFormat] = await Promise.all([
        extractGatewayMultipartImageModel({
          rawBody,
          contentType: String(contentType),
      ...
      req.gatewayRequestBody = createGatewayRequestBodyState({ rawBody, contentType, jsonParseStatus: 'not_json', model, responseFormat })
      req.body = undefined
