...
export function planImportProxies(rawProxies: unknown[], context: AccountImportResourceContext): AccountImportProxyPlanResult {
  ...
  return buildProxyPlanResult(proxyPlans)
}

export async function planImportProxiesAsync(rawProxies: unknown[], context: AccountImportResourceContext): Promise<AccountImportProxyPlanResult> {
  const proxyPlans: AccountImportProxyPlan[] = []
  for (let index = 0; index < rawProxies.length; index += 1) {
    proxyPlans.push(await planImportProxyAsync(rawProxies[index], index + 1, context))
  }
  return buildProxyPlanResult(proxyPlans)
}

function buildProxyPlanResult(proxyPlans: AccountImportProxyPlan[]): AccountImportProxyPlanResult {
...
async function planImportProxyAsync(value: unknown, index: number, context: AccountImportResourceContext): Promise<AccountImportProxyPlan> {
  ...
  const existing = await findProxyOptionByNameAsync(source.name, context)
  if (existing) {
    item.action = 'reuse'
