...
export const accountCreateSchema = z.object({
  ...
  status: z.enum(['active', 'pending_test', 'disabled', 'error', 'rate_limited', 'temporary_unavailable', 'quality_isolated']).optional(),
...
export const accountBatchEditSchema = z.object({
  ...
  updates: z.object({
    ...
    reasoningEffortOverride: batchUpdateFieldSchema(
      z.enum(['none', 'minimal', 'low', 'medium', 'high', 'xhigh', 'max', '']).nullable()
    ).optional()
  }).strict()
}).strict().superRefine((value, context) => {
