...
import { accountExportMaxAccounts } from './account-export.service.js'
...
export function registerAccountExportRoutes(router: Router): void {
  router.post('/export', async (req, res) => {
    ...
    if (!parsed.success) {
      res.status(400).json(badRequest(`账户导出参数无效，单次最多导出 ${accountExportMaxAccounts} 个账户`))
      return
    }
    ...
    try {
      ...
      await recordOperationLogAsync({
        ...
        summary: `导出 AI 账户：${result.summary.accounts} 个账户，${result.summary.proxies} 个代理${matchedText}`,
