...
export const accountModelCatalogDiagnosticTimeoutMs = [10_000] as const

export function accountDiagnosticRetryTimeouts(probeKind: 'generation' | 'image_generation' | 'models_catalog'): readonly number[] {
  if (probeKind === 'image_generation') return accountImageDiagnosticRetryTimeoutMs
  if (probeKind === 'models_catalog') return accountModelCatalogDiagnosticTimeoutMs
