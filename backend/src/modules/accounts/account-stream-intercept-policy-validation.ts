...
export function validateAccountStreamInterceptRules(value: unknown): AccountStreamInterceptPolicyValidationResult {
  ...
  for (const [index, item] of value.entries()) {
    ...
    const retryEnabled = rule.retryEnabled ?? false
    const dataHandling = rule.dataHandling ?? 'discard_stream'
    if (retryEnabled && dataHandling === 'discard_event') {
      return { valid: false, message: `第 ${ruleIndex} 条流式拦截规则需要重试时不能只丢弃命中事件` }
    }
  }
  return { valid: true }
}
