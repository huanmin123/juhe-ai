...
const scheduleModeSchema = z.string({
  required_error: 'API Key 时间计划模式必须为 allow_windows',
  invalid_type_error: 'API Key 时间计划模式必须为 allow_windows'
}).refine((value) => value === 'allow_windows', 'API Key 时间计划模式必须为 allow_windows')

const scheduleWindowSchema = z.object({
  daysOfWeek: z.array(z.number({
    invalid_type_error: '重复日期必须是数字'
  }).int('API Key 时间计划重复日期无效').min(1, 'API Key 时间计划重复日期无效').max(7, 'API Key 时间计划重复日期无效')).min(1, 'API Key 时间计划至少需要选择一个重复日期'),
  start: scheduleTimeSchema,
  end: scheduleTimeSchema
}).strict().refine((value) => value.start !== value.end, {
  message: 'API Key 时间计划开始时间和停止时间不能相同'
})
...
const scheduleExceptionSchema = z.object({
  ...
  if (value.action === 'allow' && !value.windows?.length) {
    context.addIssue({
      ...
      path: ['windows'],
      message: 'API Key 时间计划允许例外至少需要一个允许时段'
    })
  }
  if (value.action === 'deny' && value.windows !== undefined) {
    context.addIssue({
      code: z.ZodIssueCode.custom,
      path: ['windows'],
      message: 'API Key 时间计划拒绝例外不能配置允许时段'
    })
  }
})
...
export const apiKeyAvailabilityScheduleSchema = z.object({
  enabled: z.literal(true, {
    invalid_type_error: '时间计划启用状态必须为 true'
  }),
  timezone: z.string().trim().min(1, 'API Key 时间计划时区不能为空').optional(),
  mode: scheduleModeSchema,
  windows: z.array(scheduleWindowSchema).min(1, '时间计划至少需要一个允许时段').max(32),
  dateRange: z.object({
