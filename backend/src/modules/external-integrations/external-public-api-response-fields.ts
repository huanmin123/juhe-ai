...
function publicUsageMetricFields(prefix: string): ExternalPublicApiField[] {
  return [
    ...
    apiDocField(`${prefix}.cacheWriteTokens`, 'number', true, '缓存写入 token 数。', 0),
    apiDocField(`${prefix}.cacheWrite1hTokens`, 'number', true, '1 小时缓存写入 token 数。', 0),
    apiDocField(`${prefix}.cacheWriteCost`, 'number', true, '缓存写入成本，单位 USD。', 0),
    apiDocField(`${prefix}.thinkingTokens`, 'number', true, '思考 token 数。', 0),
    apiDocField(`${prefix}.inputImageTokens`, 'number', true, '输入图片 token 数。', 0),
    apiDocField(`${prefix}.outputImageTokens`, 'number', true, '输出图片 token 数。', 0),
