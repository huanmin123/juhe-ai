  ...
  switch (runtimeConfig.workerRole) {
    ...
    case 'stats-worker':
      ...
      if (isPostgresHighPerformanceMode()) {
        ...
        scheduler.schedule({ name: backgroundScheduledJobName('usage-stats-consistency-check'), intervalMs: 60 * minuteMs, initialDelayMs: 11 * minuteMs, passiveJitter: true, task: ({ signal }) => runWithPostgresScheduledLease('usage-stats-consistency-check', 5 * minuteMs, signal, () => runUsageStatsConsistencyCheck()) })
        return
      }
      scheduler.schedule({ name: backgroundScheduledJobName('system-metrics-sample'), intervalMs: settingsNumber('systemMetricsSampleIntervalSeconds', 5, 3600) * secondMs, initialDelayMs: 4 * secondMs, passiveJitter: true, overlapPolicy: 'coalesceOne', timeoutMs: 20 * secondMs, task: ({ signal }) => runWithPostgresScheduledLease('system-metrics-sample', 15 * secondMs, signal, () => runSystemMetricsSample()) })
      scheduler.schedule({ name: backgroundScheduledJobName('usage-stats-aggregation'), intervalMs: usageStatsOnlineAggregationIntervalSeconds() * secondMs, initialDelayMs: 3 * secondMs, stablePhaseWindowMs: 2 * secondMs, passiveJitter: true, overlapPolicy: 'coalesceOne', resourceLane: 'stats-online', timeoutMs: 20 * secondMs, failureBackoff: { baseMs: secondMs, maxMs: minuteMs }, task: ({ signal }) => runWithPostgresScheduledLease('usage-stats-aggregation', minuteMs, signal, runUsageStatsAggregation) })
    ...
    case 'ops-worker':
      ...
      scheduler.schedule({ name: backgroundScheduledJobName('api-key-availability-schedule-status-sync'), intervalMs: 10 * secondMs, initialDelayMs: secondMs, passiveJitter: true, task: ({ signal }) => runWithPostgresScheduledLease('api-key-availability-schedule-status-sync', 30 * secondMs, signal, () => runApiKeyAvailabilityScheduleStatusSync()) })
      ...
      scheduler.schedule({ name: backgroundScheduledJobName('openai-oauth-access-token-refresh'), intervalMs: settingsNumber('oauthAccessTokenRefreshIntervalSeconds', 10, 3600) * secondMs, initialDelayMs: 35 * secondMs, stablePhaseWindowMs: 5 * secondMs, passiveJitter: true, overlapPolicy: 'coalesceOne', resourceLane: 'external-account-maintenance', timeoutMs: 90 * secondMs, failureBackoff: { baseMs: 10 * secondMs, maxMs: 5 * minuteMs }, task: ({ signal }) => runWithPostgresScheduledLease('openai-oauth-access-token-refresh', 2 * minuteMs, signal, (leaseSignal) => runOpenAIOAuthAccessTokenRefresh(leaseSignal)) })
      return
    default:
      return
  }
...
function scheduleUsageIngestJobs(): void {
  scheduler.schedule({ name: backgroundScheduledJobName('api-key-record-cleanup-retry'), intervalMs: minuteMs, initialDelayMs: 24 * secondMs, passiveJitter: true, task: ({ signal }) => runWithPostgresScheduledLease('api-key-record-cleanup-retry', 2 * minuteMs, signal, () => runApiKeyRecordCleanupRetry()) })
  scheduler.schedule({ name: backgroundScheduledJobName('account-record-cleanup-retry'), intervalMs: minuteMs, initialDelayMs: 42 * secondMs, passiveJitter: true, task: ({ signal }) => runWithPostgresScheduledLease('account-record-cleanup-retry', 2 * minuteMs, signal, () => runAccountRecordCleanupRetry()) })
  scheduler.schedule({
