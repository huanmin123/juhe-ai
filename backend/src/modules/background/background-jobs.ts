  ...
  switch (runtimeConfig.workerRole) {
    ...
    case 'stats-worker':
      ...
      if (isPostgresHighPerformanceMode()) {
        scheduler.schedule({ name: backgroundScheduledJobName('system-metrics-sample'), intervalMs: settingsNumber('systemMetricsSampleIntervalSeconds', 5, 3600) * secondMs, initialDelayMs: 4 * secondMs, passiveJitter: true, overlapPolicy: 'coalesceOne', timeoutMs: 20 * secondMs, task: runSystemMetricsSample })
        ...
        scheduler.schedule({ name: backgroundScheduledJobName('usage-stats-consistency-check'), intervalMs: 60 * minuteMs, initialDelayMs: 11 * minuteMs, passiveJitter: true, task: runUsageStatsConsistencyCheck })
        return
      }
      scheduler.schedule({ name: backgroundScheduledJobName('system-metrics-sample'), intervalMs: settingsNumber('systemMetricsSampleIntervalSeconds', 5, 3600) * secondMs, initialDelayMs: 4 * secondMs, passiveJitter: true, overlapPolicy: 'coalesceOne', timeoutMs: 20 * secondMs, task: runSystemMetricsSample })
      scheduler.schedule({ name: backgroundScheduledJobName('usage-stats-aggregation'), intervalMs: usageStatsOnlineAggregationIntervalSeconds() * secondMs, initialDelayMs: 3 * secondMs, stablePhaseWindowMs: 2 * secondMs, passiveJitter: true, overlapPolicy: 'coalesceOne', resourceLane: 'stats-online', timeoutMs: 20 * secondMs, failureBackoff: { baseMs: secondMs, maxMs: minuteMs }, task: ({ signal }) => runWithPostgresScheduledLease('usage-stats-aggregation', minuteMs, signal, runUsageStatsAggregation) })
    ...
    case 'ops-worker':
      installDefaultScheduledAccountCircuitRecoveryResolver()
      scheduler.schedule({ name: backgroundScheduledJobName('chat-retention-cleanup'), intervalMs: 10 * minuteMs, initialDelayMs: 270 * secondMs, stablePhaseWindowMs: 30 * secondMs, passiveJitter: true, scheduleMode: 'fixedDelay', resourceLane: 'storage-maintenance', timeoutMs: 2 * minuteMs, failureBackoff: { baseMs: 30 * secondMs, maxMs: 10 * minuteMs }, task: ({ signal }) => runWithPostgresScheduledLease('chat-retention-cleanup', 5 * minuteMs, signal, runChatRetentionCleanup) })
      scheduler.schedule({ name: backgroundScheduledJobName('api-key-availability-schedule-status-sync'), intervalMs: 10 * secondMs, initialDelayMs: secondMs, passiveJitter: true, task: runApiKeyAvailabilityScheduleStatusSync })
      scheduler.schedule({ name: backgroundScheduledJobName('account-availability-schedule-status-sync'), intervalMs: 10 * secondMs, initialDelayMs: 2 * secondMs, passiveJitter: true, task: runAccountAvailabilityScheduleStatusSync })
      scheduler.schedule({ name: backgroundScheduledJobName('resource-authorization-expiry-sweep'), intervalMs: minuteMs, initialDelayMs: 54 * secondMs, passiveJitter: true, task: runResourceAuthorizationExpirySweep })
      scheduler.schedule({ name: backgroundScheduledJobName('expired-deleted-account-cleanup'), intervalMs: dailyIntervalMs, initialDelayMs: 14 * minuteMs, passiveJitter: true, task: runExpiredDeletedAccountCleanup })
...
function scheduleUsageIngestJobs(): void {
  scheduler.schedule({ name: backgroundScheduledJobName('api-key-record-cleanup-retry'), intervalMs: minuteMs, initialDelayMs: 24 * secondMs, passiveJitter: true, task: runApiKeyRecordCleanupRetry })
  scheduler.schedule({ name: backgroundScheduledJobName('account-record-cleanup-retry'), intervalMs: minuteMs, initialDelayMs: 42 * secondMs, passiveJitter: true, task: runAccountRecordCleanupRetry })
  scheduler.schedule({
