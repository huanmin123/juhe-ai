...
function shouldOpenSqliteDatabaseReadOnly(kind: SqliteMainDatabaseKind): boolean {
	// Go owns the dedicated passive-feature SQLite files. Node only serves
	// query-side consumers of these databases.
...
function assertDistinctStoragePaths(): void {
  ...
  for (const target of targets) {
    ...
    if (existingRole) {
      throw new Error(`${target.role} 与 ${existingRole} 指向同一个 SQLite 文件，请分别配置 JUHE_AI_DATABASE_PATH、JUHE_AI_CHAT_DATABASE_PATH、JUHE_AI_DATASET_DATABASE_PATH、JUHE_AI_RUNTIME_LOG_DATABASE_PATH、JUHE_AI_USAGE_CATALOG_DATABASE_PATH、JUHE_AI_STATS_DATABASE_PATH、JUHE_AI_TABLE_MONITOR_DATABASE_PATH、JUHE_AI_CODEX_CONTEXT_STATE_SHARD_ROOT 和 JUHE_AI_CODEX_CONTEXT_STATE_SHARD_COUNT`)
    }
