...
export function auditLogSummaryFromRow(row: AuditLogRow, systemAccountNames: Map<string, string>): AuditLogSummary {
  ...
  return {
    ...
    sessionClientType: optionalString(row.session_client_type),
    ...
    sourceEndpointFamily: optionalString(row.source_endpoint_family) as AuditLogSummary['sourceEndpointFamily'],
    upstreamEndpointFamily: optionalString(row.upstream_endpoint_family) as AuditLogSummary['upstreamEndpointFamily'],
    stream: row.stream === 1,
...
export function auditLogAttemptFromRow(
  ...
  return {
    ...
    providerCode: optionalString(row.provider_code),
    model: optionalString(row.attempt_model),
    upstreamModel: optionalString(row.attempt_upstream_model),
    pricingModel: optionalString(row.attempt_pricing_model),
    modelMappingApplied: row.attempt_model_mapping_applied === 1,
    modelMappingSource: optionalString(row.attempt_model_mapping_source),
    sourceEndpointFamily: optionalString(row.attempt_source_endpoint_family) as AuditLogAttemptSummary['sourceEndpointFamily'],
    upstreamEndpointFamily: optionalString(row.attempt_upstream_endpoint_family) as AuditLogAttemptSummary['upstreamEndpointFamily'],
