...
interface AuthorizedAccountSummaryContext {
  ...
  apiKeyRuntimeByAccount: Awaited<ReturnType<typeof loadAccountApiKeyRuntimeSummariesByAccountIdsAsync>>
}
...
async function authorizedAccountSummaryFromRowAsync(
  ...
  return accountSummaryWithEffectiveAvailability({
    ...
    apiKeyRuntime: factAccountId ? apiKeyRuntimeByAccount.get(factAccountId) : undefined,
    ...
    authorizationInstanceSourceAccountCooldownRetestLastStatusCode: optionalNumber(row.source_cooldown_retest_last_status_code),
    ...
    authorizationInstanceSourceAccountLastHealthCheckStatusCode: optionalNumber(row.source_last_health_check_status_code),
  ...
  const rows = await client.query<AccountListRow>(`
    WITH account_rows AS (
      SELECT
        ...
        NULL AS source_temporary_unavailable_continuous_probe_enabled,
        NULL AS source_last_error_code,
        NULL AS source_last_error_message,
        NULL AS source_last_error_trace_id,
        NULL AS source_cooldown_retest_last_at,
        NULL AS source_cooldown_retest_last_status_code,
        NULL AS source_last_health_check_at,
        NULL AS source_next_health_check_at,
        NULL AS source_last_health_check_status_code,
        NULL AS source_last_health_check_error_code,
        NULL AS source_last_health_check_error_message,
        NULL AS source_last_health_check_trace_id,
...
async function ownerAccountSummariesFromRowsAsync(
  ...
  return rows.map((row) => {
    ...
    return accountSummaryWithEffectiveAvailability({
      ...
      streamFailureCount: Math.max(0, Number(row.stream_failure_count ?? 0)),
      streamFailureWindowStartedAt: row.stream_failure_window_started_at ?? undefined,
      lastUsedAt: usage.lastUsedAt ?? row.last_used_at ?? undefined,
      todayUsage,
      usage,
      accessType: 'owner',
      boundGroupId: groupBinding?.groupId,
...
function accountSummariesFromRows(
  ...
  return rows.map((row) => {
    ...
    return accountSummaryWithEffectiveAvailability({
      ...
      lastHealthCheckErrorMessage: row.last_health_check_error_message ?? undefined,
      lastHealthCheckTraceId: row.last_health_check_trace_id ?? undefined,
      apiKeyRuntime: apiKeyRuntimeByAccount.get(accountResourceFactAccountId(row)),
      ...
      authorizationInstanceSourceAccountLastErrorTraceId: isAuthorizedView ? row.source_last_error_trace_id ?? undefined : undefined,
      authorizationInstanceSourceAccountCooldownRetestLastAt: isAuthorizedView ? row.source_cooldown_retest_last_at ?? undefined : undefined,
      authorizationInstanceSourceAccountCooldownRetestLastStatusCode: isAuthorizedView ? optionalNumber(row.source_cooldown_retest_last_status_code) : undefined,
      authorizationInstanceSourceAccountLastHealthCheckAt: isAuthorizedView ? row.source_last_health_check_at ?? undefined : undefined,
      authorizationInstanceSourceAccountNextHealthCheckAt: isAuthorizedView ? row.source_next_health_check_at ?? undefined : undefined,
      authorizationInstanceSourceAccountLastHealthCheckStatusCode: isAuthorizedView ? optionalNumber(row.source_last_health_check_status_code) : undefined,
      authorizationInstanceSourceAccountLastHealthCheckErrorCode: isAuthorizedView ? row.source_last_health_check_error_code ?? undefined : undefined,
      authorizationInstanceSourceAccountLastHealthCheckErrorMessage: isAuthorizedView ? row.source_last_health_check_error_message ?? undefined : undefined,
      authorizationInstanceSourceAccountLastHealthCheckTraceId: isAuthorizedView ? row.source_last_health_check_trace_id ?? undefined : undefined,
      boundGroupId: groupBinding?.groupId,
      boundGroupName: groupBinding?.groupName,
      groupBindStatus: groupBinding?.groupBindStatus,
