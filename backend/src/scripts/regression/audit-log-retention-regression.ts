...
function assertAuditPayloadCleanupUsesAsyncFiles(): void {
  ...
  assert(payloadBlobSource.includes('runtimeConfig.auditPayloadBlobRoot'), '审计 payload blob 必须支持跨 release 共享存储根目录')
  ...
  assert.match(auditRepositorySource, /writePostgresAuditPayloadBlobFiles\([\s\S]*await client\.transaction/, 'PG 审计 payload 文件写入必须发生在引用事务之外')
