...
function assertAuditPayloadCleanupUsesAsyncFiles(): void {
  ...
  assert(payloadBlobSource.includes('postgresAuditPayloadBlobMetadataDeleteSubBatchLimit'), 'PG 审计 payload blob 元数据清理必须限制单条 DELETE 的 id 批次')
  assert.match(payloadBlobSource, /deleteAuditPayloadBlobMetadataRowsPostgresAsync[\s\S]*chunkAuditPayloadBlobIds\(blobIds, postgresAuditPayloadBlobMetadataDeleteSubBatchLimit\)[\s\S]*DELETE FROM juhe_dataset\.audit_payload_blobs WHERE id = ANY\(\?::text\[\]\)', \[chunk\]/, 'PG 审计 payload blob 元数据必须按 chunk 删除，避免整批 id 长尾')
