...
const server = http.createServer((req, res) => {
  ...
  req.on('end', () => {
    ...
    assert.match(JSON.stringify(body.input), /海王星-7349/, '每次压缩请求都必须携带来源消息或已有长期记忆')
    const summary = JSON.stringify({
      durableMemory: [`用户项目代号是${rememberedCode}`],
      currentGoal: '持续维护项目发布计划',
...
assert.deepEqual(afterFailedTurn.history.map((message) => message.content), ['失败后这轮必须保留', '失败后的成功回答'], '失败轮不能打乱后续成功轮的配对')
