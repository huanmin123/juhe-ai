...
try {
  ...
  const adminTargetOption = adminOptions.find((account) => account.id === seed.userAccountId)
  ...
  assert.equal(sortedAdminOptions.length >= 1, true, '账户选项应仍返回可用账户')
  assert.equal(sortedAdminOptions.every((account) => account.ownerSystemAccountId === seed.userId), true, '账户选项不应因质量分排序请求混入其他账户')
  ...
  const userTargetOption = userOptions.find((account) => account.id === seed.userAccountId)
...
function seedData(): SeedState {
  ...
  const userAccountId = 'acc_account_options_lightweight'
  ...
  databaseModule.getDatabase()
    .prepare(`
      INSERT INTO accounts (
        ...
        NULL, 0, NULL, ?, ?)
    `)
    .run(userAccountId, user.id, '账户选项种子', now, now)
  return {
