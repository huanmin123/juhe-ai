...
assert.match(maintenanceCleanupJobsSource, /cleanupPostgresDatasetRetainedData[\s\S]*cleanupPublicApiLogsBeforeAsync/, 'PG 高性能 data-retention 必须按保留设置清理仍由 Node 拥有的 dataset 日志')
assert.doesNotMatch(maintenanceCleanupJobsSource, /cleanupModelCheckRunsBeforeAsync|model_check_runs|model_check_items|model_check_observations/, 'J3b 归档后 Node data-retention 不得再清理 Gateway-owned 模型检测事实')
assert.doesNotMatch(maintenanceCleanupJobsSource, /cleanupOperationLogsBeforeAsync/, 'F4 Go 接管后 Node data-retention 不得调用已删除的操作日志清理入口')
