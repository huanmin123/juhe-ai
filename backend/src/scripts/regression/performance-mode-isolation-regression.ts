...
assert.match(maintenanceCleanupJobsSource, /cleanupPostgresDatasetRetainedData[\s\S]*cleanupPublicApiLogsBeforeAsync[\s\S]*cleanupModelCheckRunsBeforeAsync/, 'PG 高性能 data-retention 必须按保留设置清理仍由 Node 拥有的 dataset 日志和模型检测历史')
