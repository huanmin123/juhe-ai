...
assert.equal(process.env.JUHE_AI_ALLOW_RUNTIME_LOG_POSTGRES_SMOKE, '1', '真实运行日志 smoke 必须显式设置 JUHE_AI_ALLOW_RUNTIME_LOG_POSTGRES_SMOKE=1')
assert.equal(new URL(runtimeConfig.postgres.url).hostname, '192.168.1.203', '真实运行日志 smoke 只允许连接 192.168.1.203 测试 PostgreSQL')
assert.equal(new URL(runtimeConfig.redis.queueUrl).hostname, '192.168.1.203', '真实运行日志 smoke 只允许连接 192.168.1.203 测试 Redis')
...
const lines = Array.from({ length: expectedRowCount }, (_, index) => JSON.stringify({ time: new Date().toISOString(), level: 30, event: `${marker}_${index}`, msg: `postgres smoke ${index}` })).join('\n') + '\n'
...
const beforeStreamExists = Number(await redis.sendCommand(['EXISTS', legacyStreamKey]))
const beforeStreamLength = Number(await redis.sendCommand(['XLEN', legacyStreamKey]))
assert.equal(beforeStreamExists, 0, '隔离 namespace 下不得预先存在 legacy runtime-log Stream')
try {
  const cursorGenerationColumn = await pool.query(`
    SELECT column_name
    FROM information_schema.columns
    WHERE table_schema = 'juhe_dataset'
      AND table_name = 'runtime_log_file_cursors'
      AND column_name = 'truncation_generation'
  `)
  assert.equal(cursorGenerationColumn.rowCount, 1, '真实 PostgreSQL 必须由迁移提供 truncation_generation')
  ...
  assert.equal(rows.rowCount, expectedRowCount, '真实 PostgreSQL 应批量写入全部 runtime log 行')
  ...
  const afterStreamExists = Number(await redis.sendCommand(['EXISTS', legacyStreamKey]))
  assert.equal(afterStreamExists, 0, '文件消费不得创建已删除的 runtime-log Redis Stream key')
