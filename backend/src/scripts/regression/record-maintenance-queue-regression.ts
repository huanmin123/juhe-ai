...
function assertRecordMaintenanceCleanupRunsAsync(): void {
  ...
  assert.doesNotMatch(queueSource, /recordMaintenanceRedisStreamClaimIdleMs\s*=\s*60 \* 60 \* 1000/, '数据维护 Redis Stream 不得把所有任务恢复时间统一延长到 1 小时')
  assert.match(queueSource, /claimIdleMs:\s*runtimeConfig\.queue\.redisStreamClaimIdleMs/, '数据维护 Redis Stream 应沿用统一的短故障恢复阈值')
  assert.match(queueSource, /runWithRecordMaintenancePendingHeartbeat[\s\S]*queue\.heartbeatPending\(messageIds\)/, '数据维护消息执行期间必须刷新 pending idle，保护长任务不被重复 claim')
  ...
  databaseModule.getBusinessDatabase()
    .prepare(`
      INSERT INTO accounts (
        ...
        name, type, status, credentials_encrypted, health_check_model, health_check_endpoint_mode, schedulable, created_at, updated_at
      )
      VALUES (?, ?, 'gpt', 'profile_gpt_openai_v1', 'openai', 'v1', ?, 'oauth', 'active', ?, 'gpt-5.5', 'responses_sse', 1, ?, ?)
    `)
