...
assert.match(gatewayUsageRecordsSource, /usageSemanticForProviderCode/)
...
assert.match(gatewayFixedResponsesSource, /usageSemanticForProviderCode/)
...
assert.match(oauthAccessTokenRefreshSource, /isForkedBackgroundWorkerProcess/)
assert.match(oauthAccessTokenRefreshSource, /isSingleProcessWorkerRole/)
...
assert.match(usageStatsWritersSource, /function shouldRecordAccountQualityStats\(row: UsageStatsRecordRow\): boolean \{[\s\S]+row\.traffic_source !== 'cooldown_retest'[\s\S]+row\.traffic_source !== 'hybrid_scoring'[\s\S]+row\.traffic_source !== 'hybrid_quality_scoring'[\s\S]+\}/, '恢复探活、混合评分和混合质量评分应计入用量统计但不写入账号质量分钟样本')
