...
assert.match(modelChecksServiceSource, /maxAttempts:\s*1/, '快速模型检测必须显式限制为单次探针尝试')
...
assert.match(modelChecksGatewayProbeSource, /normalizedProbeMaxAttempts/, '模型检测网关探针必须支持按 profile 收窄尝试次数')
