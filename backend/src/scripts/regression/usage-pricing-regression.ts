...
for (const id of anthropicOfficialModelIds) {
  ...
  assert(item.releaseDate, `Anthropic ${id} 必须有发布日期`)
  assert(item.contextWindowTokens, `Anthropic ${id} 必须有上下文容量`)
  assert(item.maxInputTokens, `Anthropic ${id} 必须有最大输入容量`)
  assert(item.maxOutputTokens, `Anthropic ${id} 必须有最大输出容量`)
  assert(item.inputModalities.length > 0 && item.outputModalities.length > 0, `Anthropic ${id} 必须有输入输出模态`)
  assert(item.supportedTools.length > 0, `Anthropic ${id} 必须有工具能力`)
  assert.deepEqual(item.supportedServiceTiers, [], `Anthropic ${id} 不得伪造 priority/flex 服务等级`)
}
for (const id of ['claude-mythos-preview', 'best', 'fable', 'opus', 'opus[1m]', 'opusplan', 'sonnet', 'sonnet[1m]', 'haiku', 'claude-fake-5']) {
...
assert.equal(anthropicPricingById.get('claude-sonnet-4-6')?.inputUsdPer1M, 3)
...
}), undefined, 'Anthropic 不支持 priority 服务等级')
...
assert.equal(openAIModelPricingById.has('codex-auto-review'), false, '缺少完整元数据的 Codex 自动审查模型必须移出内置目录')
...
assert.equal(openAIModelPricingById.has('gpt-5-search-api'), false, '缺少完整计价事实的搜索专用模型必须移出内置目录')
assert.equal(openAIModelPricingById.get('gpt-4.1')?.releaseDate, '2025-04-14')
assert.equal(openAIModelPricingById.has('babbage-002'), false)
...
assert.match(gatewayUsageRecordsSource, /recordDownstreamClosedUpstreamAttempt/)
...
assert.match(modelCatalogServiceSource, /getProviderModelPricing/)
assert.match(modelCatalogServiceSource, /buildProviderBillingCostBreakdown/)
assert.doesNotMatch(modelCatalogServiceSource, /modelPricingProviderDriverForProvider/)
...
assert.match(memoryGatewayHttpSource, /gatewayStreamInspectionReceiver/)
assert.match(memoryGatewayHttpSource, /gatewayStreamParsedEventReceiver/)
...
const diagnosticRetrySource = sourceBetween(accountTestSource, 'export async function testOpenAIAccountWithDiagnosticRetries', 'export async function discoverAccountUpstreamModels')
...
assert.match(accountRequestSchemasSource, /status:\s*z\.enum\(\['active', 'pending_test', 'disabled', 'error', 'rate_limited', 'temporary_unavailable', 'quality_isolated'\]\)\.optional\(\)/)
...
const modelChecksProbeRetrySource = readSource('modules/model-checks/model-checks-probe-retry.ts')
...
assert.match(modelChecksProbeRetrySource, /modelCheckProbeMaxAttempts\s*=\s*3/)
assert.match(modelChecksGatewayProbeSource, /accountDiagnosticRetryTimeoutMs\[Math\.min\(attempt - 1, accountDiagnosticRetryTimeoutMs\.length - 1\)\]/)
...
assert.match(modelChecksGatewayProbeSource, /function modelCheckProbeAttemptDelayMs\(\)/)
assert.match(modelChecksGatewayProbeSource, /1_000 \+ Math\.floor\(Math\.random\(\) \* 2_001\)/)
assert.match(modelChecksGatewayProbeSource, /return result\.statusCode !== 200/)
...
assert.doesNotMatch(modelChecksGatewayProbeSource, /requests-per-minute|rate_limit|rate limit/i, '模型检测探针不能按上游限流文案决定重试')
...
async function assertRetryQueueRetainsFailedItems(): Promise<void> {
  ...
  assert.deepEqual(scheduledDelays, [1, 3])
}
