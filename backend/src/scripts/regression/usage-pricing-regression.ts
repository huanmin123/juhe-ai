...
assert.match(modelChecksServiceSource, /probeMaxAttempts\s*=\s*accountDiagnosticRetryTimeoutMs\.length/)
assert.match(modelChecksServiceSource, /diagnosticAccountTestGatewaySettingsOverride\(undefined,\s*timeoutMs\)/)
assert.match(modelChecksServiceSource, /diagnosticAttemptSignal\(signal,\s*timeoutMs\)/)
assert.doesNotMatch(modelChecksServiceSource, /probeRetryDelayMs|waitForProbeRetryDelay|45_000/, '模型检测探针必须复用 10s/20s/30s 诊断策略，不能额外 sleep 或回退旧超时')
