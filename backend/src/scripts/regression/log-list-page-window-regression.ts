...
try {
  ...
  for (const call of calls) {
    ...
    assert.equal(limit, 2, `pageSize=1 时列表 SQL 只应多取 1 条，SQL: ${call.sql}`)
    assert(offset <= 999, `高增长日志列表 offset 必须被固定窗口限制在 1000 行内，实际 offset=${offset}，SQL: ${call.sql}`)
  }

  console.log('高增长日志列表页码窗口回归通过：审计、操作和运行日志深翻页 offset 被限制在 1000 行内')
} finally {
  try {
...
function isListSql(sql: string): boolean {
  return /\bFROM\s+audit_logs\s+al\b[\s\S]*\bLIMIT\s+\?\s+OFFSET\s+\?/i.test(sql)
    ...
    || /\bFROM\s*\(\s*SELECT[\s\S]*\bFROM\s+operation_logs\s+ol\b[\s\S]*\bLIMIT\s+\?\s+OFFSET\s+\?/i.test(sql)
