...
assertRoute(routesSource, 'expectedConfigRevision: z.number().int().min(1)', 'Anthropic OAuth 手动刷新与重授权必须要求客户端配置版本')
assertRoute(routesSource, 'runWithProviderOAuthRefreshLock(', 'Anthropic OAuth 手动刷新与重授权必须加入跨进程刷新锁')
assert.ok((routesSource.match(/current\.configRevision !== parsed\.data\.expectedConfigRevision/gu) ?? []).length >= 3, 'Anthropic 手动刷新与两条重授权路径都必须在上游请求前拦截版本变化')
...
assertRoute(routesSource, 'rotateOAuthCredentialsAsync({', 'Anthropic OAuth 凭据写回必须使用用途专用窄写 repository')
assertRoute(routesSource, 'oauthRotationReceipt(updated', 'Anthropic OAuth 凭据更新只返回最小 mutation 回执')
assert.equal(routesSource.includes('updateAccountAsync'), false, 'Anthropic OAuth 凭据更新不得复用账户宽写')
assert.equal(routesSource.includes('clearAccountFailureStateAsync'), false, 'Anthropic OAuth 手动刷新不得清除限流或临时不可用等业务状态')
assert.equal(routesSource.includes('sanitizeAccountCredentialCarrierResponse'), false, 'Anthropic OAuth 凭据更新不得返回完整账户载体')
