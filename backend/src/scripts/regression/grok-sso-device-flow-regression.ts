...
assert.doesNotMatch(finalCookie, /(?:^|; )session=web-session(?:;|$)/u, 'accounts.x.ai 的 host-only Cookie 不得泄漏给 auth.x.ai')
...
assert.equal(routeSource.includes('}, abortController.signal)'), true, '客户端断开后批量 worker 必须停止领取新任务')
