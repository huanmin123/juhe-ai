...
assertFunctionIncludes(gatewayApiKeyRepositorySource, 'clearGatewayApiKeyValidationCache', 'gatewayApiKeyProcessCache.clear()', '网关 API Key 校验全量失效应清理进程内热缓存')
...
assert(gatewayApiKeyRepositorySource.includes('registerGatewayRuntimeCacheInvalidator((reason) => {'), '跨进程网关配置失效必须同步检查 API Key 进程内热缓存')
assertFunctionIncludes(gatewayApiKeyRepositorySource, 'shouldInvalidateGatewayApiKeyProcessCache', "'api_key_updated'", 'API Key 更新必须清理进程内校验缓存')
assertFunctionIncludes(gatewayApiKeyRepositorySource, 'shouldInvalidateGatewayApiKeyProcessCache', "'route_strategy_updated'", '路由策略更新必须清理进程内校验缓存')
assertFunctionIncludes(gatewayApiKeyRepositorySource, 'shouldInvalidateGatewayApiKeyProcessCache', "'group_updated'", '分组更新必须清理进程内校验缓存')
assertFunctionIncludes(gatewayApiKeyRepositorySource, 'shouldInvalidateGatewayApiKeyProcessCache', "'resource_authorization_updated'", '资源授权更新必须清理进程内校验缓存')
assertFunctionIncludes(gatewayApiKeyRepositorySource, 'shouldInvalidateGatewayApiKeyProcessCache', "'team_members_changed'", '团队成员更新必须清理进程内校验缓存')
assertFunctionIncludes(gatewayApiKeyRepositorySource, 'shouldInvalidateGatewayApiKeyProcessCache', "'system_account_status_changed'", '系统账户状态更新必须清理进程内校验缓存')
assertFunctionExcludes(gatewayApiKeyRepositorySource, 'shouldInvalidateGatewayApiKeyProcessCache', "'account_api_key_runtime'", 'AI 账户运行态变化不得驱逐 API Key 结构缓存')
assertFunctionIncludes(gatewayApiKeyRepositorySource, 'prewarmGatewayApiKeyValidationCacheAsync', 'api_keys.key_hash', '网关启动预热必须按持久化 key hash 建立校验缓存')
assertFunctionIncludes(gatewayApiKeyRepositorySource, 'prewarmGatewayApiKeyValidationCacheAsync', 'setGatewayApiKeyCacheEntryAsync(keyHash', '网关启动预热必须同时填充进程内和 Redis 校验缓存')
assert(serverSource.includes('prewarmGatewayApiKeyValidationCacheAsync()'), '主服务 DB ready 后必须预热 API Key 校验热路径')
