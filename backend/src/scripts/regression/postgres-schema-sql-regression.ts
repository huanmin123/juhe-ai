...
assert.match(sql, /proxy_profiles\.enabled contains values outside 0\/1[\s\S]+ALTER COLUMN enabled TYPE boolean USING \(enabled = 1\)/, 'Node PG 既有代理启用整数列必须校验 0/1 后升级为 boolean')
assert.match(sql, /ALTER COLUMN last_tested_at TYPE timestamptz[\s\S]+ALTER COLUMN created_at TYPE timestamptz[\s\S]+ALTER COLUMN updated_at TYPE timestamptz/, 'Node PG 既有代理时间文本列必须升级为 timestamptz')
...
assert.match(sql, /ALTER TABLE system_accounts ADD COLUMN IF NOT EXISTS request_limits_json text/, '既有 Node PostgreSQL 业务库必须补用户请求限制列')
assert.match(sql, /ALTER TABLE accounts ADD COLUMN IF NOT EXISTS dispatch_revision bigint NOT NULL DEFAULT 1/, '既有 Node PostgreSQL 业务库必须补账户派发版本')
assert.match(sql, /ALTER TABLE accounts ADD COLUMN IF NOT EXISTS circuit_projection_revision bigint NOT NULL DEFAULT 0/, '既有 Node PostgreSQL 业务库必须补账户熔断投影版本')
assert.match(sql, /ALTER TABLE accounts ADD COLUMN IF NOT EXISTS cooldown_retest_generation text/, '既有 Node PostgreSQL 业务库必须补冷却复测代次')
assert.match(sql, /ALTER TABLE account_api_key_runtime_states ADD COLUMN IF NOT EXISTS probe_claim_token text/, '既有 Node PostgreSQL 业务库必须补 Key 探针 claim token')
assert.match(sql, /ALTER TABLE account_api_key_runtime_states ADD COLUMN IF NOT EXISTS probe_claimed_until text/, '既有 Node PostgreSQL 业务库必须补 Key 探针 claim 租约')
assert.match(sql, /ALTER TABLE api_keys ADD COLUMN IF NOT EXISTS purpose text NOT NULL DEFAULT 'general'/, '既有 Node PostgreSQL 业务库必须补 API Key purpose')
assert.match(sql, /ADD CONSTRAINT accounts_dispatch_revision_check[\s\S]+CHECK \(dispatch_revision >= 1\)/, '既有 Node PostgreSQL 业务库必须约束账户派发版本')
assert.match(sql, /ADD CONSTRAINT accounts_circuit_projection_revision_check[\s\S]+circuit_projection_revision <= dispatch_revision/, '既有 Node PostgreSQL 业务库必须约束账户熔断投影版本')
assert.match(sql, /ADD CONSTRAINT api_keys_purpose_check[\s\S]+purpose IN \('general', 'chat'\)/, '既有 Node PostgreSQL 业务库必须约束 API Key purpose')
assert.match(sql, /DROP CONSTRAINT IF EXISTS custom_provider_models_scope_check[\s\S]+scope IN \('personal', 'global'\)/, '既有 Node PostgreSQL 业务库必须扩展自定义模型作用域')
assert.match(sql, /ALTER TABLE provider_protocol_profiles ALTER COLUMN default_health_check_model SET NOT NULL/, '既有 Node PostgreSQL 业务库必须固定协议默认检查模型非空')
assert.match(sql, /CREATE UNIQUE INDEX IF NOT EXISTS idx_api_keys_chat_purpose_unique[\s\S]+WHERE purpose = 'chat'/, '既有 Node PostgreSQL 业务库必须补聊天 API Key 唯一索引')
assert.match(sql, /CREATE INDEX IF NOT EXISTS idx_account_api_key_runtime_probe_claim[\s\S]+probe_claimed_until ASC/, '既有 Node PostgreSQL 业务库必须补 Key 探针 claim 索引')
assert.match(sql, /DROP INDEX IF EXISTS idx_accounts_health_check_candidate_order[\s\S]+google_oauth/, '既有 Node PostgreSQL 业务库必须重建支持 Google OAuth 的健康检查候选索引')
assert.match(sql, /DROP INDEX IF EXISTS idx_accounts_cooldown_retest_candidate_order[\s\S]+google_oauth/, '既有 Node PostgreSQL 业务库必须重建支持 Google OAuth 的冷却复测索引')
...
const schemaWithoutApprovedRuntimeUpgrades = sql
  .replace(/ALTER TABLE system_accounts ADD COLUMN IF NOT EXISTS request_limits_json text;/g, '')
  .replace(/ALTER TABLE accounts ADD COLUMN IF NOT EXISTS dispatch_revision bigint NOT NULL DEFAULT 1;/g, '')
  .replace(/ALTER TABLE accounts ADD COLUMN IF NOT EXISTS circuit_projection_revision bigint NOT NULL DEFAULT 0;/g, '')
  .replace(/ALTER TABLE accounts ADD COLUMN IF NOT EXISTS cooldown_retest_generation text;/g, '')
  .replace(/ALTER TABLE account_api_key_runtime_states ADD COLUMN IF NOT EXISTS probe_claim_token text;/g, '')
  .replace(/ALTER TABLE account_api_key_runtime_states ADD COLUMN IF NOT EXISTS probe_claimed_until text;/g, '')
  .replace(/ALTER TABLE api_keys ADD COLUMN IF NOT EXISTS purpose text NOT NULL DEFAULT 'general';/g, '')
