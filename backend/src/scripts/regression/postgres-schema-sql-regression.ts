...
assert.match(sql, /CREATE INDEX IF NOT EXISTS idx_audit_logs_trace_c_created_sort ON audit_logs\(\(trace_id COLLATE "C"\), created_at DESC, id DESC\)/, 'PG 审计日志 trace 前缀查询必须有 C collation 前缀索引')
...
assert.match(sql, /CREATE INDEX IF NOT EXISTS idx_audit_logs_client_ip_c_created_sort ON audit_logs\(\(client_ip COLLATE "C"\), created_at DESC, id DESC\)/, 'PG 审计日志 client IP 前缀查询必须有 C collation 前缀索引')
assert.match(sql, /CREATE INDEX IF NOT EXISTS idx_public_api_logs_trace_c_created_sort ON public_api_logs\(\(trace_id COLLATE "C"\), created_at DESC, id DESC\)/, 'PG 公开接口日志 trace 前缀查询必须有 C collation 前缀索引')
