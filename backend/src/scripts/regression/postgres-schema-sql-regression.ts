...
const providerAuthProtocolCatchUpMigration = readFileSync('../backend-go/db/migrations/000058_w2_provider_auth_protocol_schema_20260718.sql', 'utf8')
...
assert.match(postgresSeedDefaultsSource, /model\.supportsPromptCaching,\s*model\.catalogVisible !== false,/, 'Node PG 模型目录 seed 必须向 boolean 字段传递 boolean，不能传 0/1')
...
assert.match(goPublicAccountsMigration, /health_check_endpoint_mode text NOT NULL[\s\S]+?'interactions_json', 'interactions_sse'/, 'Go/PG 当前 accounts migration 必须允许 Gemini Interactions 健康检查模式')
