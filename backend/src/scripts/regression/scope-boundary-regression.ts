...
async function main(): Promise<void> {
  ...
  try {
    ...
    const userProviderOptions = await getEnvelope<Array<{ id: string; code: string; name: string; enabled: boolean; defaultHealthCheckModel?: string }>>(baseUrl, '/__aisys__/api/providers/options', seed.userACookie)
    const userGptProviderOption = userProviderOptions.find((item) => item.code === 'gpt')
    assert(userGptProviderOption?.id === 'gpt' && userGptProviderOption.enabled === true, '普通用户应能读取供应商轻量安全选项')
    assert(!Object.prototype.hasOwnProperty.call(userGptProviderOption, 'defaultHealthCheckModel'), '供应商轻量选项不应回流默认检查模型等完整定义字段')
    const userProviderDefinitions = await getEnvelope<Array<{ code: string; defaultHealthCheckModel: string }>>(baseUrl, '/__aisys__/api/providers/definitions', seed.userACookie)
    assert(userProviderDefinitions.some((item) => item.code === 'gpt' && typeof item.defaultHealthCheckModel === 'string'), '普通用户应能按需读取包含默认检查模型的供应商定义')
    ...
    const userProviderModelOptions = await getEnvelope<Array<{ id: string; name: string; providerCode?: string; model?: string }>>(baseUrl, '/__aisys__/api/providers/models/options?providerCode=gpt&selectedIds=gpt-5.5', seed.userACookie)
    const userGptModelOption = userProviderModelOptions.find((item) => item.id === 'gpt-5.5' && item.name === 'gpt-5.5')
    assert(userGptModelOption, '普通用户应能查询全部模型名称轻量选项用于使用记录模型筛选')
    assert(!Object.prototype.hasOwnProperty.call(userGptModelOption, 'providerCode') && !Object.prototype.hasOwnProperty.call(userGptModelOption, 'model'), '模型轻量选项不应回流供应商目录详情字段')
