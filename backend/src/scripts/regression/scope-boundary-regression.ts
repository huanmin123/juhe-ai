...
async function main(): Promise<void> {
  try {
    ...
    const userAApiKeyUsage = await getEnvelope<AccountUsageStatsOverview>(baseUrl, '/__aisys__/api/my-stats/account-usage?type=api_key&page=1&pageSize=10', seed.userACookie)
    assert(userAApiKeyUsage.total === 2 && userAApiKeyUsage.rows.every((row) => row.type === 'api_key'), '用户侧账号用量统计类型筛选异常')
