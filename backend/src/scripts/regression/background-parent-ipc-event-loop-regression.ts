...
try {
  ...
  assert.deepEqual(samplesAfterThrow, [], '父进程 IPC 同步发送失败时应清理 pending 并返回空样本')
  ...
  assert.deepEqual(samplesAfterCallbackError, [], '父进程 IPC 异步发送失败时应清理 pending 并返回空样本')
  ...
  console.log('后台 worker 父进程 IPC 事件循环采样回归通过：发送失败会清 pending 并退出等待恢复')
} finally {
