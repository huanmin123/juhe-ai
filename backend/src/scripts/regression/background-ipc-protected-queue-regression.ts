...
try {
  ...
  assert.equal(backgroundIpc.getBackgroundWorkerState().pendingMessageCount, 5000, '操作日志不应突破 IPC 队列上限继续排队')
  ...
  assert.equal(backgroundIpc.getBackgroundWorkerState().pendingMessageCount, 5000, '审计日志不应突破 IPC 队列上限继续排队')
