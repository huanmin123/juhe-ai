...
assertRoute(routesSource, "clearAccountFailureStateAsync(account.id, requestAccess) ?? updatedAccount", 'Grok OAuth 手动刷新成功后必须尝试清理旧失败态')
assertRoute(routesSource, 'sanitizeAccountCredentialCarrierResponse(restoredAccount)', 'Grok OAuth 刷新接口必须返回脱敏后的账户响应')
