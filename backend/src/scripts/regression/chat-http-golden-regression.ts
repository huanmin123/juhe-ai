...
const sourceGuards: Array<[string, RegExp]> = [
  ...
  ['模型列表必须是轻量发布快照', /loadOwnedChatModelListAsync[\s\S]{0,3000}readPublishedChatModelListAsync/],
  ['模型列表响应不得返回完整能力', /chatRouter\.get\('\/conversations\/:conversationId\/models'[\s\S]{0,360}res\.json\(ok\(modelOptions\)\)/],
  ['模型详情必须定点读取能力并保留 404 code', /chatRouter\.get\('\/conversations\/:conversationId\/models\/:modelId'[\s\S]{0,800}readPublishedChatModelCapabilitiesAsync[\s\S]{0,500}chat_model_not_found/],
