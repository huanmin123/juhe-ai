...
try {
  ...
  const deferredCleanupResult = repositories.cleanupExpiredLogicallyDeletedAccounts({ limit: 10 })
  assert.equal(deferredCleanupResult.attempted, 1, '过期物理清理应按父账户聚合处理一次')
  assert.equal(deferredCleanupResult.completed, 0, '关联记录未清空前不应物理删除业务行')
  assert.equal(deferredCleanupResult.deferred, 1, '关联记录未清空时应延迟物理清理')
  assert.equal(deferredCleanupResult.recordCleanupTargets.length, 1, '过期物理清理应返回待投递的记录清理目标')
  assert.equal(deferredCleanupResult.physicallyDeletedAccounts, 0, '延迟阶段不应删除父账户和被授权实例账户')
  assert.equal(usageRecordExists(ownerUsageId), true, '延迟阶段仍应保留原账户关联使用记录等待 ingest 清理')

  const recordCleanupResult = repositories.cleanupDeletedAccountRelatedRecordData(deferredCleanupResult.recordCleanupTargets[0])
  assert.equal(recordCleanupResult.hasMore, false, '记录清理完成后不应继续延迟')
  assert.equal(recordCleanupResult.blockedReason, undefined, '统计游标已追平时记录清理不应 blocked')

  const cleanupResult = repositories.cleanupExpiredLogicallyDeletedAccounts({ limit: 10 })
  assert.equal(cleanupResult.attempted, 1, '记录清理完成后过期物理清理应再次处理父账户')
  assert.equal(cleanupResult.completed, 1, '过期物理清理应完成父账户和授权实例清理')
  assert.equal(cleanupResult.deferred, 0, '记录数据已清空时不应延迟物理清理')
  assert.equal(cleanupResult.recordCleanupTargets.length, 0, '记录清理完成后不应再返回清理目标')
