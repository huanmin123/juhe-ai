...
assert.match(runtimeSource, /globalLeaseDurationMs: number/, '全局并发租约时长必须显式配置')
assert.match(runtimeSource, /globalAcquirePollMs: number/, '全局并发槽轮询间隔必须显式配置')
assert.match(runtimeSource, /upstreamAgentMaxSockets = numberConfig\([\s\S]*globalConcurrencyMax/, 'HTTP Agent 单源连接容量默认必须跟随全局并发')
assert.match(runtimeSource, /upstreamAgentMaxTotalSockets = numberConfig\([\s\S]*globalConcurrencyMax/, 'HTTP Agent 总连接容量默认必须跟随全局并发')
const globalConcurrencyGovernorSource = source('shared/concurrency-governor.ts')
assert.match(globalConcurrencyGovernorSource, /runtimeConfig\.runtimeStateDriver === 'memory'/, '全局并发治理器必须区分本地与 Redis runtime state')
assert.match(globalConcurrencyGovernorSource, /runRedisOperationWithDeadline/, 'performance 模式全局并发必须以 Redis 作为共享事实源')
assert.match(globalConcurrencyGovernorSource, /redisNamespacedKey\('juhe-ai:concurrency:global:v1'\)/, '全局并发 Redis key 必须进入项目 namespace')
assert.match(globalConcurrencyGovernorSource, /ZREMRANGEBYSCORE[\s\S]*ZCARD[\s\S]*ZADD/, 'Redis 全局并发获取必须先清理过期租约再原子计数')
assert.match(globalConcurrencyGovernorSource, /redis_global_concurrency_lease_refresh_failed/, 'Redis 共享池续租失败必须留下可观测错误')
const upstreamRequestSource = source('modules/gateway/upstream/request.ts')
assert.match(functionBody(upstreamRequestSource, 'requestUpstream'), /requestUpstreamWithFetch\(safeRequest\.url, options, slot\.release\)/, 'fetch 上游请求必须把全局并发槽交给响应体生命周期')
assert.doesNotMatch(functionBody(upstreamRequestSource, 'requestUpstream'), /return await requestUpstreamWithFetch\(safeRequest\.url, options\)\s*\n\s*} finally \{\s*\n\s*slot\.release\(\)/, 'fetch 上游请求不得在仅收到响应头后释放全局并发槽')
assert.match(upstreamRequestSource, /fetchReadableStreamBody\(this\.response\.body, this\.options, \(\) => this\.releaseGlobalConcurrencySlot\(\)\)/, 'fetch 响应体完成、取消或出错后必须释放全局并发槽')
...
assert.match(runtimeProbeStateSource, /redisAcquireProbeGenerationLeaseScript[\s\S]*decoded\['phase'\] ~= 'precheck_pending'[\s\S]*halfOpenPreviousNextProbeAtMs[\s\S]*nextProbeAtMs.*math\.max/, 'Redis 半开 acquire 必须在单条 Lua 内校验 phase/generation 并推迟后台 due')
assert.match(runtimeProbeStateSource, /redisAcquireProbeGenerationLeaseScript[\s\S]*probeRunId[\s\S]*probeRunUntilMs[\s\S]*return ''/, 'Redis half-open acquire 必须拒绝有效后台 run')
assert.match(runtimeProbeStateSource, /redisReleaseProbeGenerationLeaseScript[\s\S]*halfOpenLeaseId.*ARGV\[3\][\s\S]*nextProbeAtMs.*halfOpenPreviousNextProbeAtMs/, 'Redis 半开 release 必须按 leaseId CAS 并恢复原后台探针时间')
assert.match(runtimeProbeStateSource, /redisCompleteProbeGenerationLeaseScript[\s\S]*generation.*ARGV\[2\][\s\S]*halfOpenLeaseId.*ARGV\[3\][\s\S]*ZREM/, 'Redis 半开完整成功必须按 generation + leaseId 原子清理 state 和 due')
...
assert.match(runtimeProbeStateSource, /acquireGenerationRun\([\s\S]*redisAcquireProbeGenerationRunScript/, 'Redis 探针状态必须提供原子后台 run acquire')
assert.match(runtimeProbeStateSource, /redisAcquireProbeGenerationRunScript[\s\S]*generation[\s\S]*halfOpenLeaseId[\s\S]*probeRunId[\s\S]*probeRunUntilMs[\s\S]*probeRunPreviousNextProbeAtMs[\s\S]*ZADD/, 'Redis 后台 run acquire 必须校验 generation/half-open/run 并原子推迟 due')
assert.match(runtimeProbeStateSource, /commitGenerationRun\([\s\S]*redisCommitProbeGenerationRunScript/, 'Redis 探针状态必须提供 generation + runId 条件提交')
assert.match(runtimeProbeStateSource, /redisCommitProbeGenerationRunScript[\s\S]*generation[\s\S]*probeRunId[\s\S]*SET[\s\S]*ZADD/, 'Redis 后台 run commit 必须按 generation + runId 原子写入 state 与 due')
assert.match(runtimeProbeStateSource, /deleteGenerationRun\([\s\S]*redisDeleteProbeStateGenerationRunScript/, 'Redis 探针状态必须提供 generation + runId 条件删除')
assert.match(runtimeProbeStateSource, /scheduledRuntimeKeys\([\s\S]*ZMSCORE/, 'Redis 必须通过 ZMSCORE 批量读取 due membership，避免逐 key 往返')
...
assert.match(accountConcurrencySource, /redisAcquireAccountConcurrencyScript[\s\S]*redis\.call\('TIME'\)/, 'Node 与 Go 共存写同一账号并发 key 时必须统一使用 Redis TIME')
assert.match(accountConcurrencySource, /redisAcquireAccountConcurrencyScript[\s\S]*latest_expiry_ms[\s\S]*expire_at_latest_slot/, 'Node 新占槽不得缩短共享 key 上已有的更长租约 TTL')
assert.match(accountConcurrencySource, /redisRefreshAccountConcurrencySlotsScript[\s\S]*redis\.call\('TIME'\)[\s\S]*total_expiry_ms > now_ms/, 'Redis 账号并发续租只能刷新仍有效的 token，禁止复活已过期租约')
assert.match(accountConcurrencySource, /redisRefreshAccountConcurrencySlotsScript[\s\S]*math\.max\(requested_expires_at_ms, total_expiry_ms, lane_expiry_ms\)/, 'Node 账号并发续租不得缩短当前 token 的到期时间')
assert.doesNotMatch(functionBody(accountConcurrencySource, 'acquireRedisAccountConcurrency'), /Date\.now\(\)/, 'Node 账号并发占槽禁止把进程本机时钟写入共享 Redis score')
assert.doesNotMatch(functionBody(accountConcurrencySource, 'refreshRedisAccountConcurrencySlots'), /Date\.now\(\)/, 'Node 账号并发续租禁止把进程本机时钟写入共享 Redis score')
...
const clientSourceAvoidanceSource = source('modules/gateway/client-profiles/client-source-avoidance.service.ts')
assert.match(functionBody(clientSourceAvoidanceSource, 'orderOpenAIAccountsByClientSourceAvoidanceAsync'), /await orderOpenAIAccountsByCodexTurnAvoidanceAsync/, '通用 client source 避让入口必须等待 Redis Codex turn retry 排序')
assert.match(functionBody(clientSourceAvoidanceSource, 'rememberGatewayClientSourceFailureAsync'), /await rememberCodexTurnStreamFailureAsync/, '通用 client source 失败入口必须等待 Redis Codex turn retry 失败记录')
assert.match(source('modules/gateway/dispatch/preparation.ts'), /await orderOpenAIAccountsByClientSourceAvoidanceAsync/, '调度准备必须等待通用 client source 避让排序')
assert.match(source('modules/gateway/response/finalization.ts'), /await rememberGatewayClientSourceFailureAsync/, '响应最终化必须等待通用 client source 失败记录')
assert.match(functionBody(source('modules/gateway/routes.ts'), 'rememberCodexTurnFailureWhenClientRetryIsVisible'), /await rememberGatewayClientSourceFailureAsync/, '可见客户端重试响应前必须等待通用 client source 失败记录')
...
function assertStrictRedisCacheBoundaries(): void {
  ...
  const sharedCacheReadBatchingSource = source('storage/shared-cache-read-batching.ts')
  assert.match(functionBody(sharedCacheReadBatchingSource, 'loadSharedCacheEntriesByBatches'), /chunkValues\(ids, 100\)[\s\S]*Promise\.all\(chunk\.map/, 'shared cache lookup 必须按 100 个 ID 的有界批次并发读取')
  ...
  assert.match(accountApiKeyMutationAuthorizationBody, /context\.authority === 'explicit_user_policy'[\s\S]*mutation === 'failure'[\s\S]*context\.authority === 'confirmed_same_account_key_rotation'[\s\S]*mutation === 'failure'[\s\S]*context\.authority !== 'automatic_probe'[\s\S]*context\.probeOutcome === 'upstream_failure'[\s\S]*context\.probeOutcome === 'complete_success'/, '账户内 API Key 持久状态只允许用户显式失败策略、同账户后继 Key 成功确认或独立自动探针的确定结果授权')
  ...
  assert.match(accountSideEffectsSource, /runWithGlobalBackgroundConcurrencySlot/, '网关自动恢复探针必须进入进程级共享并发池')
  assert.doesNotMatch(accountSideEffectsSource, /pLimit|automaticProbeMaxConcurrency|recoveryProbeMaxConcurrentRuns/, '网关自动恢复探针不得保留业务局部并发门禁')
  ...
  assert.match(functionBody(accountSideEffectsSource, 'runDistributedGatewayAccountRecoveryProbe'), /acquireGenerationRun[\s\S]*loadDistributedRecoveryProbeStateWithAccount[\s\S]*runSingleGatewayAccountPrecheck[\s\S]*commitDistributedRecoveryProbeRun/, 'Redis runtime state 下恢复探针必须原子取得 runId、重载账号凭据、使用账号测试健康探针并按 runId 提交')
  ...
  assert.match(functionBody(sessionAffinitySource, 'orderOpenAIAccountsBySessionAffinityAsync'), /Promise\.all\([\s\S]*trafficMigrationPreferenceForAccountsAsync[\s\S]*getRedisSessionAffinityBindingForOrdering/, '迁移偏好与会话绑定互不依赖，Redis 模式必须并行读取')
  assert.match(functionBody(sessionAffinitySource, 'areOpenAIHighConcurrencyAccountsBusyForLaneAsync'), /Promise\.all\([\s\S]*loadAccountCurrentConcurrencyByIdsAsync\(accountIds\)[\s\S]*loadAccountCurrentConcurrencyByIdsAsync\(accountIds, 'image'\)/, '高并发 image lane 的 total/image 并发快照必须并行读取')
  assert.match(functionBody(gatewayCapacitySource, 'orderGatewayAccountsByLaneCapacityAvailabilityAsync'), /Promise\.all\([\s\S]*loadAccountCurrentConcurrencyByIdsAsync\(accountIds\)[\s\S]*loadAccountCurrentConcurrencyByIdsAsync\(accountIds, 'image'\)/, '网关容量排序的 total/image 并发快照必须并行读取')
  assert.match(functionBody(gatewayCapacitySource, 'areGatewayAccountsCapacityBusyForLaneAsync'), /Promise\.all\([\s\S]*loadAccountCurrentConcurrencyByIdsAsync\(accountIds\)[\s\S]*loadAccountCurrentConcurrencyByIdsAsync\(accountIds, 'image'\)/, '网关容量判定的 total/image 并发快照必须并行读取')
  ...
  assert.match(usageFinalizationSource, /gatewayUsageFinalizationMaxItems\s*=\s*runtimeConfig\.gateway\.usageFinalizationMaxItems/, '网关使用记录异步收尾数量上限必须来自 runtimeConfig')
  assert.match(usageFinalizationSource, /gatewayUsageFinalizationMaxBytes\s*=\s*64 \* 1024 \* 1024/, '网关使用记录异步收尾必须有字节上限')
  assert.match(usageFinalizationSource, /gatewayUsageFinalizationMaxConcurrency\s*=\s*runtimeConfig\.concurrency\.globalMax/, '网关使用记录异步收尾必须使用全局共享并发')
