...
function assertStrictRedisCacheBoundaries(): void {
  ...
  assert.match(accountSideEffectsSource, /recoveryProbeMaxConcurrentRuns\s*=\s*runtimeConfig\.gateway\.automaticProbeMaxConcurrency/, '网关自动恢复探针并发上限必须来自 runtimeConfig')
  ...
  assert.match(usageFinalizationSource, /gatewayUsageFinalizationMaxConcurrency\s*=\s*runtimeConfig\.gateway\.usageFinalizationMaxConcurrency/, '网关使用记录异步收尾并发上限必须来自 runtimeConfig')
