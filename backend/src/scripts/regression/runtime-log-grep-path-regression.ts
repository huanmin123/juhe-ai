...
try {
  ...
  assert.equal(result.items.length, 2, '日志搜索应过滤自身请求日志，只返回业务日志')
  assert(result.items.some((item) => item.event === 'gateway_test_event'), '应保留非 runtime-logs 路径的业务日志')
  const largeItem = result.items.find((item) => item.rawJson.includes('大日志 needle'))
  assert(largeItem, '应保留命中的超长业务日志')
  assert.equal(largeItem.event, undefined, '超长命中日志不应同步解析整行 JSON 提取字段')
  assert(largeItem.rawJson.length < 21_000, '超长命中日志应截断后返回')
  assert(largeItem.rawJson.endsWith('...'), '超长命中日志应带有截断标记')
