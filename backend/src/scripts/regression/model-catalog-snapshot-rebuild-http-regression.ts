...
function assertServerWiring(): void {
  ...
  assert(source.includes("name: 'model-catalog-snapshot-reconcile-lease'"), '双槽 control 必须共享 dirty 扫描分布式租约')
  assert(source.includes('modelCatalogSnapshotReconcileLease.acquireLease'), 'dirty 扫描前必须先取得分布式租约')
  assert(source.includes('modelCatalogSnapshotReconcileLease.releaseLease'), 'dirty 扫描结束必须释放分布式租约')
