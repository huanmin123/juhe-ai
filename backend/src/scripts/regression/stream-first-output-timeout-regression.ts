...
async function main(): Promise<void> {
  ...
  try {
    ...
    assert(overloadedAfterOutputResult.streamText.includes('upstream_retryable_error'), `输出后精确客户端应收到可重试失败终态：${overloadedAfterOutputResult.streamText}`)
    assert.equal((overloadedAfterOutputResult.streamText.match(/event: response\.failed/g) ?? []).length, 1, '输出后只能追加一个失败终态')
    usageRecordQueue.flushAllUsageRecordQueue()
    await accountSideEffects.flushGatewayAccountSideEffectsForTest()
