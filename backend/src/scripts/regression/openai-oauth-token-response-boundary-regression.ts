...
assert.match(routesSource, /updated\.status !== 'error' \|\| !updated\.lastErrorCode[\s\S]*expectedLastErrorCodes:\s*\[updated\.lastErrorCode\]/, 'OAuth 重新授权成功只能按写回时的精确旧错误码清理 error 状态')
...
assert.match(routesSource, /operationKey: 'openai_oauth\.create_from_refresh_token'[\s\S]*?fingerprint:[\s\S]*?status: 'pending_test'/, 'OAuth Refresh Token 创建的幂等指纹必须使用实际固定的待检查语义')
