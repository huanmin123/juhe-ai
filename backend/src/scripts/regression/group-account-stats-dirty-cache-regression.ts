...
const [
  ...
  import('../../storage/account-runtime-status.js')
])
...
try {
  ...
  assert.equal(repositories.projectAccountHealthFixtureSuccess(account.id, {
    ...
    failureThreshold: 3,
    statusCode: 200
  }), true, 'Go outcome projector 应写入账户激活状态')
  ...
  assert.deepEqual(
    dirtyRows(),
    [{ group_id: primaryGroup.id, reason: 'j1_account_health_projection' }],
    'J1 projector 实际改变状态或调度后应按账户反查标记关联分组统计为脏'
  )
  ...
  assert.equal(repositories.projectAccountHealthFixtureSuccess(account.id, {
    ...
    failureThreshold: 3,
    statusCode: 200
  }), true, '重复 Go outcome projector 应刷新健康检查时间')
  ...
  withStatsWriteLock(() => {
    assert.doesNotThrow(() => accountRuntimeStatus.disableExpiredAccounts(undefined, 20), '统计结果库写锁占用时，过期账户维护不应因分组统计脏标记失败')
...
function assertSourceGuards(): void {
  ...
  assert.match(source, /client\.transaction\(async \(tx\) => \{\s*await pinScheduledJobLeaseInTransaction\(tx, scheduledLease\)/, 'PG 分组统计刷新必须在写事务内 pin scheduled lease')
  assert.match(source, /refreshDirtyGroupAccountStatsCacheInClient\(tx, normalizedLimit\)/, 'PG 分组统计缓存写入和 dirty CAS 删除必须共享已 fenced 的事务')
  const jobsSource = readFileSync(resolve('src/modules/background/background-jobs.ts'), 'utf8')
  assert.match(jobsSource, /runWithPostgresScheduledLease\('group-account-stats-refresh', 2 \* minuteMs, signal, runGroupAccountStatsRefresh\)/, '分组账户统计周期任务必须取得跨部署 scheduled lease')
  assert.match(jobsSource, /requestStatsWriter\(\{ type: 'refresh_group_account_stats', scheduledLease \}\)/, '分组账户统计任务必须把 fencing token 传给 stats writer')
  const statsWriterSource = readFileSync(resolve('src/modules/background/background-stats-writer.ts'), 'utf8')
  assert.match(statsWriterSource, /refreshGroupAccountStats\(requiredPostgresScheduledLease\(operation\)\)/, 'PG stats writer 必须拒绝缺失 scheduled lease 的分组统计写操作')
  ...
  assert.match(healthProjectionSource, /refreshGroupAccountStatsAfterWrite\(\{ accountIds: \[outcome\.account_id\], reason: 'j1_account_health_projection' \}\)/, 'SQLite J1 projector 激活后必须按账户 ID 标记分组统计脏队列')
  assert.match(healthProjectionSource, /await refreshGroupAccountStatsAfterWriteAsync\(\{ accountIds: \[outcome\.account_id\], reason: 'j1_account_health_projection' \}, tx\)/, 'PostgreSQL J1 projector 激活后必须在当前事务按账户 ID 标记分组统计脏队列')
  ...
  assert.match(dbServiceSource, /case 'project_account_health_jobs_outcome':[\s\S]*projectAccountHealthJobsOutcome/, 'DB service 只能投影 Go J1 outcome')
