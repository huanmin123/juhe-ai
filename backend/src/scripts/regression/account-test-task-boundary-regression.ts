...
assert(
  ...
  '前端测试弹窗应创建测试 session、保持心跳，并仅在用户手动停止时批量取消 session'
)
assert(
  !frontendAccountTestModalSource.includes('beforeunload')
    ...
    && frontendAccountTestModalSource.includes('completeActiveAccountTestSession')
  ...
  '前端刷新、切换菜单或关闭浏览器时不应取消后台 session，并应通过 sessionStorage 恢复 12 小时内的终端状态'
)
assert(
