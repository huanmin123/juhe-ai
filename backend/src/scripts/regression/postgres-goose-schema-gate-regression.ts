...
assert.equal(EXPECTED_POSTGRES_GOOSE_SCHEMA_VERSION, 69)
...
async function assertAcceptsRolledBackNewerVersion(): Promise<void> {
  ...
  assert.equal(ended, true, '69 up, 70 up, 70 down 折叠后必须允许启动并关闭连接池')
}
...
function assertRejectsInvalidCurrentStates(): void {
  const invalidStates: Array<{
    ...
    { label: 'not applied', currentRows: [{ version_id: '69', is_applied: false }] },
