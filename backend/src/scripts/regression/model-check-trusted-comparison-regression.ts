...
assert.equal(modelCheckSupportedProtocolLabel, 'OpenAI Responses', '模型检测当前能力边界应表达 Responses 检测')
assert.equal(isModelCheckSupportedProtocolProfile({ providerCode: 'openai', protocolCode: 'openai', protocolVersion: 'v1' }), true, 'OpenAI-compatible Responses 账户应支持模型检测')
assert.equal(isModelCheckSupportedProtocolProfile({ providerCode: 'deepseek', protocolCode: 'openai', protocolVersion: 'v1' }), false, 'DeepSeek Chat-only 账户不应进入 Responses 模型检测')
...
assert.match(trustedComparisonMessage, /OpenAI Responses/, '可信对比文案应描述 Responses 能力边界')
