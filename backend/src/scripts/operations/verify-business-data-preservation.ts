...
async function main(): Promise<void> {
  ...
  const allowedAdditionTables = new Set(
    (process.env.JUHE_AI_BUSINESS_VERIFY_ALLOWED_ADDITION_TABLES ?? '')
      .split(',')
      .map((value) => value.trim())
      .filter(Boolean)
  )
  const allowedNewTables = new Set(
    (process.env.JUHE_AI_BUSINESS_VERIFY_ALLOWED_NEW_TABLES ?? '')
      .split(',')
      .map((value) => value.trim())
      .filter(Boolean)
  )
  const discardedSourceTables = new Set(
    (process.env.JUHE_AI_BUSINESS_VERIFY_DISCARDED_SOURCE_TABLES ?? '')
      .split(',')
      .map((value) => value.trim())
      .filter(Boolean)
  )
  ...
  try {
    await Promise.all([
      beginReadOnlySnapshot(source),
      beginReadOnlySnapshot(target)
    ])
    const report = await verifyBusinessData(
      source,
      target,
      allowedNewTables,
      allowedAdditionTables,
      discardedSourceTables
    )
...
async function verifyBusinessData(
  ...
  discardedSourceTables: Set<string>
  ...
  for (const table of sourceTables) {
    if (discardedSourceTables.has(table)) continue
    if (!targetTableSet.has(table)) {
      violations.push(`${table}: target table missing`)
      continue
    }
    const [sourceColumns, targetColumns, primaryKey] = await Promise.all([
    ...
    const [sourceRows, targetRows] = await Promise.all([
      readRows(source, table, selectedColumns, primaryKey),
      readRows(target, table, selectedColumns, primaryKey)
    ])
    ...
    const result: TableComparison = {
      table,
      primaryKey,
      ...compareKeyedRows(normalizedSourceRows, normalizedTargetRows, primaryKey)
    }
  ...
  return {
    ...
    discardedSourceTables: [...existingDiscardedSourceTables(sourceTableSet, discardedSourceTables, violations)].sort(),
    comparisons,
    violations
  }
}
...
function existingDiscardedSourceTables(
  sourceTableSet: Set<string>,
  discardedSourceTables: Set<string>,
  violations: string[]
): Set<string> {
  const existing = new Set<string>()
  for (const table of discardedSourceTables) {
    if (sourceTableSet.has(table)) existing.add(table)
    else violations.push(`${table}: configured discarded source table does not exist`)
  }
  return existing
}
...
function compareUnkeyedRows(
  ...
  return {
    ...
    preservedDigest: digestHashList([...targetCounts.entries()].flatMap(([digest, count]) => Array(Math.min(count, sourceCounts.get(digest) ?? 0)).fill(digest)))
  }
}
