...
// runtime.
const BusinessSQLiteSchemaVersion = "business-sqlite-gateway-v13"
...
var BusinessSQLiteSchema = map[string]SQLiteTableSpec{
	...
	"accounts":                     {Columns: []string{"id", "config_revision", "dispatch_revision", "circuit_projection_revision", "system_account_id", "provider_code", "provider_protocol_profile_id", "protocol_code", "protocol_version", "name", "type", "status", "credentials_encrypted", "credential_fingerprint", "credential_mask", "oauth_access_token_expires_at", "oauth_refresh_token_present", "proxy_profile_id", "concurrency_limit", "priority", "super_priority_enabled", "fallback_enabled", "client_compatibility", "schedulable", "availability_schedule_json", "availability_schedule_next_check_at", "notes", "account_expires_at", "last_used_at", "cooldown_until", "last_error_code", "last_error_message", "last_error_trace_id", "cooldown_retest_failure_count", "cooldown_retest_observation_started_at", "cooldown_retest_generation", "cooldown_retest_last_at", "cooldown_retest_last_status_code", "temporary_unavailable_continuous_probe_enabled", "health_check_model", "health_check_endpoint_mode", "last_health_check_at", "next_health_check_at", "last_health_success_at", "health_check_failure_count", "health_check_failure_started_at", "last_health_check_status_code", "last_health_check_error_code", "last_health_check_error_message", "last_health_check_trace_id", "stream_failure_count", "stream_failure_window_started_at", "balance_query_enabled", "balance_query_config_json", "balance_query_next_refresh_at", "authorization_instance_source_account_id", "authorization_instance_authorization_id", "authorization_instance_owner_system_account_id", "deleted_at", "deleted_by", "created_at", "updated_at"}},
	...
	"model_quality_policies":       {Columns: []string{"system_account_id", "revision", "profile", "manual_enforcement_enabled", "penalty_threshold", "penalty_action", "recovery_interval_minutes", "created_at", "updated_at", "custom_question_ids"}},
	"model_quality_schedules":      {Columns: []string{"id", "system_account_id", "account_id", "model", "interval_minutes", "profile", "penalty_threshold", "penalty_action", "recovery_interval_minutes", "enabled", "revision", "next_run_at", "last_run_id", "last_run_at", "last_run_status", "lease_owner", "lease_until", "created_at", "updated_at", "custom_question_ids"}, UniqueConstraints: [][]string{{"system_account_id", "account_id"}}, Indexes: []string{"idx_model_quality_schedules_due"}},
