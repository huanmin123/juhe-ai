...
// Store contract tests migrated from the former jobs gometricsstore suite
// (the Store has always lived in this shared package).
...
func TestSQLiteStoreSchemaInsertIdempotencyAndTrend(t *testing.T) {
	...
	sample := RuntimeSnapshot{SampledAt: when, ProcessPID: 7, Service: "jobs", Role: "active", Goroutines: 2, HeapAllocBytes: 100, HeapLiveBytes: 90, HeapObjects: 10, Threads: 3}
...
func TestSQLiteStoreAggregatesOptionalAndSchedulerMetrics(t *testing.T) {
	...
	for _, sample := range []RuntimeSnapshot{
...
func TestInsertRequiresExplicitSchema(t *testing.T) {
	...
	_, err = store.InsertSnapshot(context.Background(), RuntimeSnapshot{SampledAt: time.Now().UTC(), ProcessPID: 1, Service: "jobs", Role: "active"})
	if err == nil {
...
func TestTrendRangeIsBounded(t *testing.T) {
	...
	if _, err := store.QueryTrend(context.Background(), "jobs", "active", from, to); !errors.Is(err, ErrTrendRangeTooLarge) {
