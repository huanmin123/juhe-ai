...
//   - 任务认领由 accounttest.ManualTestQueue 完成（mark_running → 执行器 →
...
func (e *Executor) Execute(ctx context.Context, task accounttest.ManualTestTaskRecord, report accounttest.ProgressReporter) (accounttest.ManualTestTaskExecutorResult, error) {
	startedAt := e.now()
	view, failMessage, err := e.resolveView(ctx, task)
	if err != nil {
		return accounttest.ManualTestTaskExecutorResult{}, err
	}
	if failMessage != "" {
...
func (e *Executor) buildResult(task accounttest.ManualTestTaskRecord, view *accountprobe.View, observation *accountquality.ProbeObservation, poolAttempts []accountprobe.PoolKeyAttempt, startedAt time.Time) builtResult {
...
func reportProgress(report accounttest.ProgressReporter, view *accountprobe.View) {
	...
	report(accounttest.DiagnosticAttemptProgressMessage(total, string(mode)))
}
