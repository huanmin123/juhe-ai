...
func encryptDraft(t *testing.T, draft *DraftSnapshot) string {
	t.Helper()
	envelope, err := accountcrypto.EncryptJSON(testSecret, draft)
	if err != nil {
		t.Fatal(err)
	}
	return envelope
...
func manualTestTask(envelope string) accounttest.ManualTestTaskRecord {
	startedAt := "2030-01-01T00:00:00.000Z"
	return accounttest.ManualTestTaskRecord{
		ID:                    "task-1",
		AccountID:             "acc-1",
		Model:                 "gpt-test",
