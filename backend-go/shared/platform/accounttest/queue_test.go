...
// 进程内派发语义：EnqueueLocal 去重拒绝（同 key 已 pending/running）是幂等
// 成功而非失败信号（调用方不得把 running 任务置败）；Stopped 后拒绝新入队。
func TestEnqueueLocalIdempotentAndStopped(t *testing.T) {
	repo := newFakeTestTaskRepo()
	queue, err := NewManualTestQueue(repo, func(context.Context, ManualTestTaskRecord, ProgressReporter) (ManualTestTaskExecutorResult, error) {
		return ManualTestTaskExecutorResult{Success: true}, nil
	}, testQueueConfig())
	...
	if queue.EnqueueLocal("  ") {
		t.Fatal("空白 ID 应拒绝")
	}
	if !queue.EnqueueLocal("  task-1  ") {
		t.Fatal("trim 后应接受")
	}
	// 同 key 二次入队被去重 → false（幂等成功，不产生任何失败写入）。
	if queue.EnqueueLocal("task-1") {
		t.Fatal("重复 key 应返回 false")
	}
	if len(repo.failed) != 0 {
		t.Fatalf("去重拒绝不得触发失败写入: %v", repo.failed)
	}
	if queue.Stopped() {
		t.Fatal("运行中队列不应报告 stopped")
	}
	...
	queue.Stop(stopCtx)
	if !queue.Stopped() {
		t.Fatal("Stop 后应报告 stopped")
	}
	if queue.EnqueueLocal("task-2") {
		t.Fatal("停止后应拒绝新入队")
	}
}
