...
// Repo 实现 accounttest.ManualTestTaskRepo。
type Repo struct {
	db       *sql.DB
	postgres bool
	now      func() time.Time
...
func (row *taskRow) record() *accounttest.ManualTestTaskRecord {
	record := &accounttest.ManualTestTaskRecord{
		ID:                     row.id,
...
// ---- accounttest.ManualTestTaskRepo ----

// Maintenance 对齐 handleAccountTestTaskMaintenanceAsync：
// cleanup → idle session 收口 →（sweep）queued 超限自动失败 →（start）
// 中断回收 /（sweep）可运行补充。
func (r *Repo) Maintenance(ctx context.Context, input accounttest.ManualTestMaintenanceInput) (accounttest.ManualTestMaintenanceResult, error) {
	if input.Action != "start" && input.Action != "sweep" {
		return accounttest.ManualTestMaintenanceResult{}, fmt.Errorf("手动测试维护 action 无效: %s", input.Action)
	}
	...
	result := accounttest.ManualTestMaintenanceResult{TaskIDs: []string{}, CanceledTaskIDs: []string{}, ExpiredQueuedTaskIDs: []string{}}
...
// claim + started_at 围栏）。
func (r *Repo) MarkRunning(ctx context.Context, taskID string) (*accounttest.ManualTestTaskRecord, error) {
...
func (r *Repo) Complete(ctx context.Context, taskID string, resultValue accounttest.ManualTestTaskExecutorResult, expectedStartedAt *string) error {
	id, ok := normalizedText(taskID, 256)
	if !ok {
		return nil
	}
