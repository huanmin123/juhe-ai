// Package accounttest 是手动账号测试执行链的共享实现（原 jobs internal
// accountquality/accountprobe/proberepo/manualtestrepo/manualtest 与
// opsjobs.ManualTestQueue 的收敛，gateway 与 jobs 共用）。本文件承载队列与
// 执行 port：逐语义对齐 Node
...
//
// 去跨进程战役后，手动测试执行权归 gateway 进程内单持有者（组合根装配
// repo → probe → executor → queue，进程内入队），jobs 不再运行本队列。
package accounttest
...
// Stopped 报告队列是否已停止（停止后 EnqueueLocal 恒拒绝；进程内派发
// 适配器据此保留路由 503 拒收契约）。
func (q *ManualTestQueue) Stopped() bool {
	q.mu.Lock()
	defer q.mu.Unlock()
	return q.stopped
}
...
// EnqueueLocal 本地入队（去重）。返回是否新入队；任务已在本进程
// pending/running 时返回 false（调用方按幂等成功处理，不得置败任务）。
