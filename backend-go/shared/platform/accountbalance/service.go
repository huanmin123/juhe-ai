...
func NewService(config RuntimeConfig, logger *slog.Logger) (*Service, error) {
	...
	if inputPool == nil {
		...
		opened.SetMaxOpenConns(config.InputPostgresMaxOpenConns)
		opened.SetMaxIdleConns(config.InputPostgresMaxIdleConns)
		inputPool = directPool{db: opened}
	}
	db := inputPool.DB()
...
// directPool adapts a directly-opened *sql.DB to the PoolHandle seam (same
// shape as the gateway/jobs pgpool handles).
type directPool struct{ db *sql.DB }

func (p directPool) DB() *sql.DB  { return p.db }
func (p directPool) Close() error { return p.db.Close() }
