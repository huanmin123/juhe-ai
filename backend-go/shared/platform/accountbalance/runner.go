...
// ApplyManualQueryResult maps one direct query result onto the manual-trigger
// snapshot semantics (applyQueryResult's manual branch, no persistence and no
// prior-snapshot participation). The non-persisted draft probe in-process
// callers use it so the draft and the committed manual refresh share one
// mapping implementation.
func ApplyManualQueryResult(query QueryResult, now time.Time) Snapshot {
	return applyQueryResult(query, SnapshotRecord{}, false, TriggerManual, now)
}
