...
`../shared/contracts` 是稳定无业务契约库，`../shared/platform` 是无业务编排的基础设施库（包括统一上游传输 `upstreamhttp` 和 SQL pool 生命周期 `sqlpool`）；二者都不是部署项目。三个项目之间禁止直接 import。`gateway` 当前承载 F3/F4，`jobs` 当前承载 F1/F2，`maintenance` 仅提供一次性命令骨架。
