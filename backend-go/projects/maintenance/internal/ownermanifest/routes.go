...
// isArchivedNodeSourcePath reports whether a manifest path points into one of
// the archived Node source trees. The final Node archive is split across
// migration-backup/node/ and migration-backup-1/node/, so archived-source
// detection must accept either root; a live backend/src path is never
// archived.
func isArchivedNodeSourcePath(value string) bool {
	slash := filepath.ToSlash(value)
	return strings.HasPrefix(slash, "migration-backup/node/") || strings.HasPrefix(slash, "migration-backup-1/node/")
}
...
func VerifyGatewayRouteOwnerManifest(manifestPath, repositoryRoot string) (GatewayRouteOwnerReport, error) {
	...
	sourceAppArchived := isArchivedNodeSourcePath(manifest.SourceApp)
	...
	for _, family := range manifest.Families {
		...
		archivedSource := isArchivedNodeSourcePath(family.NodeRouterFile)
		if family.Status == "implemented-archive-pending" {
