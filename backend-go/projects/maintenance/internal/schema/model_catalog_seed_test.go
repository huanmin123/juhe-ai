...
func TestModelCatalogSeedRowsPinnedCount(t *testing.T) {
	// 113 rows after the 2026-09-20 manual sync from the Go pricing catalog
	// (gpt 58, xai 10, deepseek 3, anthropic 13, gemini 13, glm 16).
	if len(modelCatalogSeedRows) != 113 {
		t.Fatalf("model catalog seed rows = %d, want 113 (stale snapshot? sync from backend-go/projects/gateway/internal/pricing/data_*.go)", len(modelCatalogSeedRows))
	}
	perProvider := map[string]int{}
	for _, row := range modelCatalogSeedRows {
		perProvider[row.ProviderCode]++
	}
	want := map[string]int{"gpt": 58, "xai": 10, "deepseek": 3, "anthropic": 13, "gemini": 13, "glm": 16}
