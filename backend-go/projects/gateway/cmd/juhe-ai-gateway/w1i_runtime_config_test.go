...
func TestW1iEnvBoolTrue(t *testing.T) {
	...
	for _, v := range trueVals {
		if !envBoolTrue(v) { t.Fatalf("envBoolTrue(%q) = false", v) }
	}
	falseVals := []string{"false", "", "no", "0", "x", "   "}
	for _, v := range falseVals {
		if envBoolTrue(v) { t.Fatalf("envBoolTrue(%q) = true", v) }
	}
}
...
func TestW1iStrictEnvBool(t *testing.T) {
	for _, v := range []string{"true", "1", "yes", "on", "TRUE", "On"} {
		...
		if err != nil || !got { t.Fatalf("strictEnvBool(%q): %v %v", v, got, err) }
	}
	...
	if err != nil || !fallbackTrue { t.Fatalf("fallback true: %v %v", fallbackTrue, err) }
	fallbackFalse, err := strictEnvBool("TEST", "", false)
	if err != nil || fallbackFalse { t.Fatalf("fallback false: %v %v", fallbackFalse, err) }
	_, err = strictEnvBool("TEST", "bogus", false)
	if err == nil { t.Fatal("bogus value should fail") }
}

func TestW1iParseTruncatedInt(t *testing.T) {
	got, err := parseTruncatedInt("N", "10.5", 1, 100)
	if err != nil || got != 10 { t.Fatalf("trunc: %d %v", got, err) }
	got, err = parseTruncatedInt("N", "0.9", 1, 10)
	if err == nil { t.Fatalf("trunc below min: %d", got) }
	got, err = parseTruncatedInt("N", "abc", 1, 100)
	if err == nil { t.Fatal("non-number accepted") }
	got, err = parseTruncatedInt("N", "100.1", 1, 100)
	if err != nil || got != 100 { t.Fatalf("trunc at max: %d %v", got, err) }
}
...
func TestW1iLoadRuntimeConfigDefaults(t *testing.T) {
	...
	if err != nil { t.Fatalf("default config: %v", err) }
	if cfg.RuntimeMode != "standalone" { t.Fatalf("mode = %q", cfg.RuntimeMode) }
	if cfg.DatabaseDriver != "sqlite" { t.Fatalf("db driver = %q", cfg.DatabaseDriver) }
	if cfg.CacheDriver != "memory" { t.Fatalf("cache driver = %q", cfg.CacheDriver) }
	if cfg.RuntimeStateDriver != "memory" { t.Fatalf("state driver = %q", cfg.RuntimeStateDriver) }
	if cfg.Host != "127.0.0.1" { t.Fatalf("host = %q", cfg.Host) }
	if cfg.Port != 3000 { t.Fatalf("port = %d", cfg.Port) }
	if cfg.ChatAssetsRoot != "data/chat-assets" { t.Fatalf("chat assets = %q", cfg.ChatAssetsRoot) }
}
...
func TestW1iLoadRuntimeConfigPerformanceHints(t *testing.T) {
	...
	if cfg.RuntimeMode != "performance" { t.Fatalf("mode = %q", cfg.RuntimeMode) }
	if cfg.CacheDriver != "redis" { t.Fatalf("cache = %q", cfg.CacheDriver) }
}
...
func TestW1iLoadRuntimeConfigPort(t *testing.T) {
	...
	if err == nil { t.Fatal("non-numeric port accepted") }
}
