...
// aliases.go keeps every symbol that moved to openaicompatcore (REFACTOR-0008
// 阶段 A-1) resolvable from this package so consumers of the root package see
// an unchanged surface: type aliases for the shared vocabulary, constant and
// function forwards for the protocol / error / parsing / time primitives.
// The bridge (阶段 A-2) and storage (阶段 B) splits will retire most of these
// forwards; until then both domains keep calling the original in-package
// spellings.
