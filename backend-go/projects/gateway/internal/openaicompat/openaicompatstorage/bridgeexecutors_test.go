...
func TestFileResolver(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			resolved, err := resolver.ResolveFile(ctx, openaicompatbridge.FileResolveInput{FileID: tc.fileID})
			if err != nil {
				t.Fatal(err)
			}
			if tc.wantNil {
	...
	if resolved, err := other.ResolveFile(ctx, openaicompatbridge.FileResolveInput{FileID: "file-text"}); err != nil || resolved != nil {
...
func TestFileResolverOversize(t *testing.T) {
	...
	_ = record
	resolver := env.Deps.FileResolverForScope(&GatewayScope{SystemAccountID: testScopeA, APIKeyID: testKeyA})
	_, err = resolver.ResolveFile(t.Context(), openaicompatbridge.FileResolveInput{FileID: "file-big"})
	if err == nil {
...
func TestFileSearchExecutor(t *testing.T) {
	...
	output, err := executor.Search(ctx, openaicompatbridge.FileSearchInput{VectorStoreIDs: []string{"vs-2", "vs-1"}, Query: "shared"})
	if err != nil {
		t.Fatal(err)
	}
	...
	// Max results clamp.
	output, err = executor.Search(ctx, openaicompatbridge.FileSearchInput{VectorStoreIDs: []string{"vs-1", "vs-2"}, Query: "shared", MaxNumResults: floatPtr(1)})
	if err != nil {
		t.Fatal(err)
	}
	...
	_, err = executor.Search(ctx, openaicompatbridge.FileSearchInput{VectorStoreIDs: []string{"vs-none"}, Query: "shared"})
	bridgeErr, ok := err.(*BridgeRequestError)
	...
	_, err = executor.Search(ctx, openaicompatbridge.FileSearchInput{VectorStoreIDs: []string{"vs-1"}, Query: "shared"})
	bridgeErr, ok = err.(*BridgeRequestError)
	...
	_, err = other.Search(ctx, openaicompatbridge.FileSearchInput{VectorStoreIDs: []string{"vs-1"}, Query: "shared"})
	bridgeErr, ok = err.(*BridgeRequestError)
