...
// HTTPDoer abstracts the transport for mock-first tests. Structurally
// identical to the storage-domain interface declared next to the image
// generation executor (both accept *http.Request / return (*http.Response,
// error)), so callers satisfy both with the same value (e.g. *http.Client).
