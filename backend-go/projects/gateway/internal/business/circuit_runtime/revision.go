...
type AccountCircuitRevisionProjector struct {
	...
	maxIndexMembers int
	now             func() time.Time
	project         func(context.Context, accountCircuitRevisionKeys, GatewayAccountCircuitOutboxEvent, time.Duration, time.Time) ([]byte, error)
}
...
func NewAccountCircuitRevisionProjector(client *Client, retention time.Duration) (*AccountCircuitRevisionProjector, error) {
	...
	projector.project = func(ctx context.Context, keys accountCircuitRevisionKeys, event GatewayAccountCircuitOutboxEvent, retention time.Duration, now time.Time) ([]byte, error) {
		...
		var status GatewayAccountCircuitRevisionProjectionStatus
		switch response.Status {
		case string(GatewayAccountCircuitMutationApplied):
			status = GatewayAccountCircuitRevisionApplied
		case string(GatewayAccountCircuitMutationIdempotent):
			status = GatewayAccountCircuitRevisionIdempotent
		case string(GatewayAccountCircuitMutationStaleDispatchRevision):
			status = GatewayAccountCircuitRevisionStale
		default:
		...
		return json.Marshal(GatewayAccountCircuitRevisionProjection{Status: status, CurrentRevision: response.CurrentDispatchRevision, ClosedStates: response.ClosedScopeCount})
	}
...
func (p *AccountCircuitRevisionProjector) ProjectGatewayAccountCircuitRevision(ctx context.Context, event GatewayAccountCircuitOutboxEvent) (GatewayAccountCircuitRevisionProjection, error) {
	if p == nil || p.project == nil {
		return GatewayAccountCircuitRevisionProjection{}, fmt.Errorf("account circuit revision projector is required")
	}
	if ctx == nil {
		return GatewayAccountCircuitRevisionProjection{}, fmt.Errorf("account circuit revision context is required")
	}
	...
	if err != nil {
		return GatewayAccountCircuitRevisionProjection{}, fmt.Errorf("project account circuit dispatch revision: %w", err)
	}
	var result GatewayAccountCircuitRevisionProjection
	if err := json.Unmarshal(raw, &result); err != nil {
		return GatewayAccountCircuitRevisionProjection{}, fmt.Errorf("decode account circuit revision projection: %w", err)
	}
...
func validateAccountCircuitRevisionEvent(event GatewayAccountCircuitOutboxEvent) error {
	if event.EventType != GatewayAccountCircuitDispatchRevisionChanged || event.ProjectionKey != GatewayAccountCircuitProjectionKey || event.AccountRuntimeKey != event.AccountID || !validAccountCircuitRevisionText(event.AccountID, 1024) || !validAccountCircuitRevisionText(event.TransitionID, 256) || event.DispatchRevision < 1 {
		return fmt.Errorf("account circuit revision event is invalid")
	}
	return nil
}

func validateAccountCircuitRevisionProjection(value GatewayAccountCircuitRevisionProjection) error {
	if value.CurrentRevision < 1 || value.ClosedStates < 0 {
		return fmt.Errorf("account circuit revision projection result is invalid")
	}
	switch value.Status {
	case GatewayAccountCircuitRevisionApplied, GatewayAccountCircuitRevisionIdempotent, GatewayAccountCircuitRevisionStale:
		return nil
	default:
...
var _ GatewayAccountCircuitRevisionProjector = (*AccountCircuitRevisionProjector)(nil)
