...
const defaultAccountCircuitRuntimeCapacity = 100000
...
type AccountCircuitIncidentRestorer struct {
	...
	capacity  int
	now       func() time.Time
	restore   func(context.Context, accountCircuitRevisionKeys, GatewayAccountCircuitIncident, []byte, time.Time, time.Duration, int) ([]byte, error)
}
...
func (r *AccountCircuitIncidentRestorer) RestoreGatewayAccountCircuitIncident(ctx context.Context, incident GatewayAccountCircuitIncident) (GatewayAccountCircuitRevisionProjection, error) {
	if r == nil || r.restore == nil {
		if r == nil || r.runtime == nil {
			return GatewayAccountCircuitRevisionProjection{}, fmt.Errorf("account circuit incident restorer is required")
		}
	...
	if err != nil {
		return GatewayAccountCircuitRevisionProjection{}, fmt.Errorf("encode account circuit incident runtime state: %w", err)
	}
	raw, err := r.restore(ctx, r.keys, incident, rawState, r.now().UTC(), r.retention, r.capacity)
	if err != nil {
		return GatewayAccountCircuitRevisionProjection{}, fmt.Errorf("restore account circuit incident: %w", err)
	}
	var result GatewayAccountCircuitRevisionProjection
	if err := json.Unmarshal(raw, &result); err != nil {
		return GatewayAccountCircuitRevisionProjection{}, fmt.Errorf("decode account circuit incident restore: %w", err)
	}
	if result.Status == "capacity_exhausted" {
		return GatewayAccountCircuitRevisionProjection{}, fmt.Errorf("account circuit runtime capacity exhausted")
	}
	if result.Status == "ledger_conflict" {
		return GatewayAccountCircuitRevisionProjection{}, fmt.Errorf("account circuit runtime generation is ahead of durable ledger")
	}
...
func (r *AccountCircuitIncidentRestorer) restoreWithRuntimeOwner(ctx context.Context, incident GatewayAccountCircuitIncident) (GatewayAccountCircuitRevisionProjection, error) {
	if r.runtime == nil {
		return GatewayAccountCircuitRevisionProjection{}, fmt.Errorf("account circuit runtime owner restorer is required")
	}
	...
	if err != nil {
		return GatewayAccountCircuitRevisionProjection{}, fmt.Errorf("encode account circuit runtime owner incident: %w", err)
	}
	var wire accountCircuitRuntimeStateWire
	if err := json.Unmarshal(raw, &wire); err != nil {
		return GatewayAccountCircuitRevisionProjection{}, fmt.Errorf("decode account circuit runtime owner incident: %w", err)
	}
	state, err := runtimeStateFromWire(wire)
	if err != nil {
		return GatewayAccountCircuitRevisionProjection{}, err
	}
	result, err := r.runtime.RestoreGatewayAccountCircuit(ctx, GatewayAccountCircuitRestoreInput{
	...
	case GatewayAccountCircuitMutationApplied:
		return GatewayAccountCircuitRevisionProjection{Status: GatewayAccountCircuitRevisionApplied, CurrentRevision: incident.DispatchRevision}, nil
	case GatewayAccountCircuitMutationIdempotent:
		return GatewayAccountCircuitRevisionProjection{Status: GatewayAccountCircuitRevisionIdempotent, CurrentRevision: result.State.DispatchRevision}, nil
	case GatewayAccountCircuitMutationStaleDispatchRevision:
		return GatewayAccountCircuitRevisionProjection{Status: GatewayAccountCircuitRevisionStale, CurrentRevision: result.State.DispatchRevision}, nil
	case GatewayAccountCircuitMutationStaleGeneration:
		return GatewayAccountCircuitRevisionProjection{}, fmt.Errorf("account circuit runtime generation is ahead of durable ledger")
	case GatewayAccountCircuitMutationCapacityExhausted:
		return GatewayAccountCircuitRevisionProjection{}, fmt.Errorf("account circuit runtime capacity exhausted")
	default:
		return GatewayAccountCircuitRevisionProjection{}, fmt.Errorf("account circuit runtime owner restore result is invalid")
	}
}

func (r *AccountCircuitIncidentRestorer) runRestore(ctx context.Context, keys accountCircuitRevisionKeys, incident GatewayAccountCircuitIncident, state []byte, now time.Time, retention time.Duration, capacity int) ([]byte, error) {
...
func accountCircuitIncidentRuntimeStateFromIncident(incident GatewayAccountCircuitIncident) (accountCircuitIncidentRuntimeState, error) {
	...
	case "key_model":
		scope.KeyFingerprint = incident.KeyFingerprint
		scope.ClientModel = incident.ClientModel
		scope.CapabilityHash = incident.CapabilityHash
		scope.CredentialSourceAccountID = incident.CredentialSourceAccountID
		scope.ClientEndpointFamily = incident.ClientEndpointFamily
		scope.FinalUpstreamModel = incident.FinalUpstreamModel
		scope.UpstreamEndpointMode = incident.UpstreamEndpointMode
		for name, value := range map[string]string{
			"key fingerprint":              scope.KeyFingerprint,
			...
			if !validAccountCircuitRevisionText(value, 256) {
				return accountCircuitIncidentRuntimeState{}, fmt.Errorf("account circuit key-model %s is invalid", name)
			}
		}
	default:
...
var _ GatewayAccountCircuitIncidentRestorer = (*AccountCircuitIncidentRestorer)(nil)
