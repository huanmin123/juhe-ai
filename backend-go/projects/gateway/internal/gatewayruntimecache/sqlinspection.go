...
var systemDefaultInspectionRules = []inspectionDefaultRule{
	...
	defaultRule("default_openai_transient_precommit_error_type", "OpenAI 首输出前短暂错误（官方原生 type）", 0, "protocol", "openai", "",
		ResponseInspectionPolicyMatch{
			ClientProfiles: []string{"generic_openai", "codex"},
			ErrorTypes:     []string{"server_error"},
		},
		"retry_next_account",
		"直连官方账户的流内错误帧为官方原生形态 type=server_error 且 code 通常为空，仅配 errorCodes 的瞬态规则接不到（改写型中转见 default_gpt_upstream_error / upstream_error）。server_error 是官方定义的 5xx 可重试错误；语义与 Anthropic/Gemini 的 errorTypes 瞬态规则同构。匹配谓词为 AND，故与仅配 errorCodes 的规则拆分为两条。"),
