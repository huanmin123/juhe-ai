...
var systemDefaultInspectionRules = []inspectionDefaultRule{
	...
	defaultRule("default_gpt_upstream_error", "GPT 上游限流/过载错误", 0, "provider", "openai", "gpt",
		...
		"retry_no_avoidance",
		"upstream_error 是网关桥层（openaicompat FailXxxChatStream 等）把上游失败转换为 chat completions 失败帧时写入的固定 type——命中它即识别「上游失败经桥转换、尚未向客户端提交语义输出」的瞬间，零猜测。gpt 供应商场景（上游限流/过载，如 'Our servers are currently overloaded'）：命中后原地有界重试（瞬态白名单授予同账号重试资格），不切换账户；耗尽后交客户端统一可重试失败。供应商可预知，按供应商级配置（同 default_gpt_cyber_policy 先例）。"),
