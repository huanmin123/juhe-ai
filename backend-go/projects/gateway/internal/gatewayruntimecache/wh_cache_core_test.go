...
// 快照服务：探测、redis 直读优先、服务器快照缓存与陈旧窗口。
func TestWhRuntimeSnapshotService(t *testing.T) {
	clock := newManualClock()
	availabilityOK := func(ctx context.Context, keys []string) (AccountRuntimeAvailabilitySnapshot, bool, error) {
		values := AccountRuntimeAvailabilitySnapshot{}
		for _, key := range keys {
			values[key] = json.RawMessage(`{"ok":true}`)
		}
		return values, true, nil
	}
	concurrencyOK := func(ctx context.Context, ids []string) (AccountConcurrencySnapshot, bool, error) {
	...
	service := NewRuntimeSnapshotService(clock, availabilityOK, concurrencyOK, nil)

	if status := AllRuntimeSnapshotAvailable(); !status.AccountConcurrencyAvailable || !status.AccountRuntimeAvailabilityAvailable || !status.AccountCircuitSummaryAvailable {
		t.Fatalf("全可用状态 = %+v", status)
	}
	// 探针：合成 key，直读依赖。
	concurrencyAvailable, availabilityAvailable := service.ProbeAccountRuntimeState(context.Background())
	if !concurrencyAvailable || !availabilityAvailable {
		t.Fatalf("探测 = %v/%v", concurrencyAvailable, availabilityAvailable)
	}
	// 依赖不可用 → false。
	service2 := NewRuntimeSnapshotService(clock, nil, nil, nil)
	if c, a := service2.ProbeAccountRuntimeState(context.Background()); c || a {
		t.Fatalf("无依赖探测 = %v/%v", c, a)
	}
	// 服务器快照缓存路径 + 陈旧窗口（Peek）。
	loads := 0
	service3 := NewRuntimeSnapshotService(clock, nil, nil, func(ctx context.Context) (*AccountRuntimeSnapshot, error) {
		loads++
		return &AccountRuntimeSnapshot{
			AccountConcurrency:         AccountConcurrencySnapshot{"a1": 7},
			AccountRuntimeAvailability: AccountRuntimeAvailabilitySnapshot{"k1": json.RawMessage(`{}`)},
		}, nil
	})
	if values, ok := service3.LoadServerAccountRuntimeAvailabilitySnapshot(context.Background()); !ok || len(values) != 1 {
		t.Fatalf("服务器快照 = %v ok=%v", values, ok)
	}
	if values, ok := service3.PeekServerAccountRuntimeAvailabilitySnapshot(context.Background()); !ok || len(values) != 1 {
		t.Fatalf("peek = %v ok=%v", values, ok)
	}
	// 推进超过 max-stale：peek 返回 false（但已调度刷新）。
	clock.Advance(6 * time.Second)
	if _, ok := service3.PeekServerAccountRuntimeAvailabilitySnapshot(context.Background()); ok {
		t.Fatal("超过 max-stale 的 peek 必须失败")
	}
	if loads != 1 {
		t.Fatalf("加载次数 = %d", loads)
	}
	// 键工具。
	if got := AccountRuntimeAvailabilityKey("a1", "authorized", "auth1", "g1", "", "sys", "owner"); got != "a1:authorized:sys:g1:auth1" {
		t.Fatalf("授权键 = %q", got)
	}
	if got := AccountRuntimeAvailabilityKey("a1", "owner", "", "", "", "sys", "owner"); got != "a1" {
		t.Fatalf("owner 键 = %q", got)
	}
	if got := AccountRuntimeAvailabilityKey("a1", "authorized", "auth1", "g1", "binder", "sys", "owner"); got != "a1:authorized:binder:g1:auth1" {
		t.Fatalf("绑定者优先键 = %q", got)
	}
	if got := SortSnapshotKeys([]string{"b", "a"}); len(got) != 2 || got[0] != "a" {
		t.Fatalf("排序 = %v", got)
	}
	// 数值归一化表。
	numeric := []struct {
		...
		{7, 7}, {int64(8), 8}, {9.9, 9},
		{json.Number("4"), 4}, {"5", 5}, {"bad", 0},
		{nil, 0}, {true, 0},
	}
	for _, c := range numeric {
		if got := numberValue(c.in); got != c.want {
			t.Fatalf("numberValue(%#v) = %d, want %d", c.in, got, c.want)
		}
	}
}
