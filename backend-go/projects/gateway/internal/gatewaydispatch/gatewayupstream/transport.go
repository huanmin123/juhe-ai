...
func RequestUpstream(ctx context.Context, upstreamURL string, options UpstreamRequestOptions, deps TransportDeps) (*GatewayUpstreamResponse, error) {
	...
	if err != nil {
		...
		if upstreamStarted {
			started := &StartedTransportError{Err: normalized}
			// Go can tell the pre-connection dial phase apart (DNS / TCP dial
			// inside client.Do): the request never reached the upstream. Mark
			// it so the attempt loop keeps candidate exclusion but skips the
			// account-circuit transport accounting (a gateway-side DNS /
			// egress outage must not poison every account's circuit).
			if IsPreConnectionDialError(err) {
				return nil, &DialPhaseTransportError{Err: started}
			}
			return nil, started
		}
...
// DialPhaseTransportError marks a started transport failure that happened in
// the pre-connection dial phase (DNS resolution / TCP dial inside client.Do,
// before any byte reached the upstream). Node's started WeakSet cannot tell
// this phase apart (request.end() runs before connect); Go can, so
// handleUpstreamAttemptError keeps the candidate-exclusion semantics for
// these failures but skips the account-circuit transport accounting.
type DialPhaseTransportError struct{ Err error }

func (e *DialPhaseTransportError) Error() string { return e.Err.Error() }
func (e *DialPhaseTransportError) Unwrap() error { return e.Err }

// IsDialPhaseStartedTransportError reports whether err is a started transport
// failure proven to be a pre-connection dial failure.
func IsDialPhaseStartedTransportError(err error) bool {
	...
	return errors.As(err, &dialPhase)
}

// IsPreConnectionDialError inspects the raw client.Do error: the net/http
// chain wraps dial-phase failures as *url.Error -> *net.OpError with
// Op == "dial" (DNS resolution failures surface as *net.DNSError, nested
// inside that OpError or — on some resolver paths — directly under the
// *url.Error). Post-connection phases (TLS handshake, request write, response
// read) use other Ops or no OpError at all and stay ordinary started
// transport failures. The raw error is inspected before
// NormalizeTransportError, which may replace the *url.Error wrapper with an
