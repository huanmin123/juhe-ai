...
func NormalizeOpenAIOAuthCodexParsedBody(parsedBody any, input OpenAIOAuthCodexNormalizeInput) (NormalizedCodexBody, error) {
	...
	if input.Compact {
		...
		gatewayupstream.NormalizeOpenAICodexResponsesLiteBody(cloned, model, nil)
...
func cloneJSONObject(source map[string]any) map[string]any {
	...
	for key, value := range source {
		cloned[key] = gatewayupstream.JSONCloneValue(value)
	}
	return cloned
}
...
func ResolveOpenAIOAuthCodexSession(
	...
	rawPromptCacheKey := gatewayupstream.FirstNonEmptyString(
		stringValueOf(body["prompt_cache_key"]),
		gatewayupstream.HeaderValueOf(inputHeaders, "prompt_cache_key"),
		gatewayupstream.HeaderValueOf(inputHeaders, "x-prompt-cache-key"),
	)
	rawSessionID := gatewayupstream.FirstNonEmptyString(gatewayupstream.HeaderValueOf(inputHeaders, "session-id"))
	rawConversationID := gatewayupstream.FirstNonEmptyString(gatewayupstream.HeaderValueOf(inputHeaders, "thread-id"))
	rawPrimary := gatewayupstream.FirstNonEmptyString(rawSessionID, rawConversationID, rawPromptCacheKey)
