...
func BuildOpenAIOAuthCodexRequestParts(
	...
	compact := gatewayupstream.IsOpenAIOAuthCodexCompactRequest(req)
...
func buildOpenAIOAuthCodexHeaders(
	...
	headers := gatewayupstream.CopyOfficialOAuthClientRequestHeaders(inputHeaders, gatewayupstream.OAuthHeaderProfileOpenAICodex)
	if attestation := validatedAttestation.Get("X-Oai-Attestation"); attestation != "" {
		headers.Set("X-Oai-Attestation", attestation)
	}
	nativeCodexClient := gatewayupstream.IsOpenAICodexClientHeaders(headers)
	gatewayupstream.NormalizeOpenAICodexClientHeaders(headers, input.model)
	...
	accountID := gatewayupstream.StringCredential(account.Credentials, "account_id")
	if accountID == "" {
		accountID = gatewayupstream.StringCredential(account.Credentials, "chatgpt_account_id")
	}
	if accountID != "" {
...
func copyOpenAIOAuthCodexAttestationHeader(output http.Header, inputHeaders http.Header) error {
	value := gatewayupstream.HeaderValueOf(inputHeaders, "x-oai-attestation")
	if value == "" {
		return nil
	}
