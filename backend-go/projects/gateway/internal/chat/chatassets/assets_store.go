...
// Asset subdomain persistence for the generation wave: upload pipeline
// persistence (createChatAsset / completeChatAssetProcessing /
// assertChatAssetUploadSlotAvailable), generated-image commits
// (commitChatGeneratedAsset) and image lineage reads
// (listRecentChatImageGenerations). The compaction source page loader
// stayed in the facade root (context.go vocabulary). SQL semantics and
// Chinese error strings mirror the Node repositories.
...
func (s *AssetStore) assertUncommittedChatAssetCountAvailable(q queryer, ownerID, conversationID, now string) (int, error) {
	var total int64
	err := q.QueryRow(s.ports.Bind(`SELECT COUNT(*) AS total FROM `+s.ports.Table("chat_assets")+`
...
func (s *AssetStore) AssertChatAssetUploadSlotAvailable(ownerID, conversationID, nowValue string) (int, error) {
	now, err := s.ports.RequireRFC3339Instant(nowValue, "聊天资产 now")
	if err != nil {
		return 0, err
	}
	tx, err := s.db.Begin()
...
func (s *AssetStore) CreateChatAsset(input CreateChatAssetInput) (*Asset, error) {
	...
	FROM `+s.ports.Table("chat_conversations")+`
...
func (s *AssetStore) GetAssetNoExpiryGuard(assetID, ownerID, conversationID string) (*Asset, error) {
...
func (s *AssetStore) CompleteChatAssetProcessing(input CompleteAssetProcessingInput) (*Asset, error) {
	now, err := s.ports.RequireRFC3339Instant(input.Now, "聊天资产 now")
	if err != nil {
		return nil, err
	}
...
func (s *AssetStore) FailChatAssetProcessing(assetID, ownerID, conversationID, errorCode, nowValue string) (bool, error) {
	now, err := s.ports.RequireRFC3339Instant(nowValue, "聊天资产 now")
	if err != nil {
		return false, err
	}
	result, err := s.db.Exec(s.ports.Bind(`UPDATE `+s.ports.Table("chat_assets")+`
		...
		NormalizedErrorCode(errorCode, s.ports.TrimSpace), now, assetID, ownerID, conversationID)
	if err != nil {
		return false, err
	}
...
func (s *AssetStore) CommitChatGeneratedAsset(input GeneratedAssetCommitInput) (*Asset, error) {
	now, err := s.ports.RequireRFC3339Instant(input.Now, "聊天资产 now")
	if err != nil {
		return nil, err
	}
	...
	sourceAssetIDs := s.ports.UniqueStrings(input.Generation.SourceAssetIDs)
	...
	if _, err := tx.Exec(s.ports.Bind(`INSERT INTO `+s.ports.Table("chat_image_generations")+` (
...
func (s *AssetStore) ListRecentImageGenerations(conversationID, ownerID, nowValue string, limit int) ([]ImageGenerationRecord, error) {
	now, err := s.ports.RequireRFC3339Instant(nowValue, "聊天上下文 now")
	if err != nil {
		return nil, err
	}
	if limit < 1 {
	...
	rows, err := s.db.Query(s.ports.Bind(`SELECT asset_id, operation, model, prompt, source_asset_ids_json,
			root_asset_id, size, quality, output_format, created_at, expires_at
		FROM `+s.ports.Table("chat_image_generations")+`
