...
func (s *AssetStore) QueryAssets(q queryer, query string, args ...any) ([]*Asset, error) {
...
func (s *AssetStore) GetAsset(assetID, ownerID, conversationID, now string) (*Asset, error) {
	normalizedNow, err := s.ports.RequireRFC3339Instant(now, "聊天资产 now")
	if err != nil {
		return nil, err
	}
...
func (s *AssetStore) normalizedAssetID(value string) (string, error) {
	normalized := s.ports.TrimSpace(value)
	if !assetIDPattern.MatchString(normalized) {
		return "", s.ports.DomainError("聊天资产 ID 无效")
	}
	return normalized, nil
}

func (s *AssetStore) normalizedAssetIDs(values []string) ([]string, error) {
	out := make([]string, 0, len(values))
	for _, value := range values {
		normalized, err := s.normalizedAssetID(value)
		if err != nil {
			return nil, err
		}
...
func (s *AssetStore) ListReadyAssetsByID(assetIDs []string, ownerID, conversationID, now string) ([]*Asset, error) {
	normalizedNow, err := s.ports.RequireRFC3339Instant(now, "聊天资产 now")
	if err != nil {
		return nil, err
	}
	ids, err := s.normalizedAssetIDs(assetIDs)
	if err != nil {
		return nil, err
	}
	if len(ids) == 0 {
	...
	rows, err := s.QueryAssets(s.db, s.ports.Bind(`SELECT `+AssetColumns+` FROM `+s.ports.Table("chat_assets")+`
		WHERE id IN (`+s.ports.Placeholders(len(ids))+`)
...
func (s *AssetStore) ClaimUncommittedAssetForDeletion(assetID, ownerID, conversationID, now string) (*AssetDeletionClaim, error) {
	normalizedNow, err := s.ports.RequireRFC3339Instant(now, "聊天资产 now")
	if err != nil {
		return nil, err
	}
	claimID := cleanupClaimID()
	result, err := s.db.Exec(s.ports.Bind(`UPDATE `+s.ports.Table("chat_assets")+` AS asset
		...
		WHERE asset.id = ? AND asset.system_account_id = ? AND asset.conversation_id = ?
			...
			AND NOT EXISTS (
				SELECT 1 FROM `+s.ports.Table("chat_asset_references")+` AS reference
...
func (s *AssetStore) GetClaimedAsset(assetID, claimID string) (*Asset, error) {
	assets, err := s.QueryAssets(s.db, s.ports.Bind(`SELECT `+AssetColumns+` FROM `+s.ports.Table("chat_assets")+`
...
func (s *AssetStore) getClaimedAssetTx(tx queryer, assetID, claimID string) (*Asset, error) {
	assets, err := s.QueryAssets(tx, s.ports.Bind(`SELECT `+AssetColumns+` FROM `+s.ports.Table("chat_assets")+`
		WHERE id = ? AND cleanup_status = 'claimed' AND cleanup_claim_id = ?
		LIMIT 1`), assetID, claimID)
	if err != nil {
		return nil, err
...
func (s *AssetStore) CompleteAssetDeletion(assetID, claimID string) (bool, error) {
	tx, err := s.db.Begin()
	if err != nil {
		return false, err
	}
	...
	result, err := tx.Exec(s.ports.Bind(`DELETE FROM `+s.ports.Table("chat_assets")+`
	...
	if err := s.decrementAssetUserUsage(tx, asset.SystemAccountID, asset.QuotaBytes, s.ports.NowISO()); err != nil {
		return false, err
	}
...
func (s *AssetStore) LockAssetUserQuota(tx queryer, ownerID string) error {
	if !s.pg {
		return nil
	}
	_, err := tx.Exec(s.ports.Bind(`SELECT pg_advisory_xact_lock(hashtextextended(?, 0))`), "juhe-ai:chat-asset-usage:"+ownerID)
	return err
}

func (s *AssetStore) decrementAssetUserUsage(tx queryer, ownerID string, quotaBytes int64, now string) error {
	_, err := tx.Exec(s.ports.Bind(`UPDATE `+s.ports.Table("chat_user_asset_usage")+`
...
func (s *AssetStore) ReleaseAssetDeletionClaim(assetID, claimID, errorCode, retryAt, now string) (bool, error) {
	normalizedRetryAt, err := s.ports.RequireRFC3339Instant(retryAt, "聊天资产 cleanupRetryAt")
	if err != nil {
		return false, err
	}
	normalizedNow, err := s.ports.RequireRFC3339Instant(now, "聊天资产 now")
	...
	result, err := s.db.Exec(s.ports.Bind(`UPDATE `+s.ports.Table("chat_assets")+`
		SET cleanup_status = 'failed', cleanup_claim_id = NULL, cleanup_claimed_at = NULL,
			cleanup_retry_at = ?, cleanup_error_code = ?, updated_at = ?
		WHERE id = ? AND cleanup_status = 'claimed' AND cleanup_claim_id = ?`),
		normalizedRetryAt, NormalizedErrorCode(errorCode, s.ports.TrimSpace), normalizedNow, assetID, claimID)
	if err != nil {
		return false, err
	}
...
func NormalizedErrorCode(value string, trimSpace func(string) string) string {
...
func (s *AssetStore) ExpireChatAssetsForConversation(tx queryer, conversationID, ownerID, now string) error {
	normalizedNow, err := s.ports.RequireRFC3339Instant(now, "聊天资产 now")
	if err != nil {
		return err
	}
	_, err = tx.Exec(s.ports.Bind(`UPDATE `+s.ports.Table("chat_assets")+`
...
func (s *AssetStore) RemoveChatAssetReferencesForMessage(tx queryer, ownerID, conversationID, messageID, now string) error {
	_, err := tx.Exec(s.ports.Bind(`DELETE FROM `+s.ports.Table("chat_asset_references")+` AS reference
		WHERE reference.conversation_id = ? AND reference.message_id = ?
			AND EXISTS (
				SELECT 1 FROM `+s.ports.Table("chat_assets")+` AS asset
...
func (s *AssetStore) QueryUserInputAssetIDs(tx queryer, conversationID, messageID, now string) ([]string, error) {
	rows, err := tx.Query(s.ports.Bind(`SELECT asset_id FROM `+s.ports.Table("chat_asset_references")+`
	...
	for rows.Next() {
		...
		normalized := s.ports.TrimSpace(assetID)
		if normalized != "" {
			out = append(out, normalized)
		}
	}
...
func (s *AssetStore) CommitChatAssetsToMessage(tx queryer, rawAssetIDs []string, ownerID, conversationID, messageID, now string, retentionDays int) error {
	normalizedNow, err := s.ports.RequireRFC3339Instant(now, "聊天资产 now")
	if err != nil {
		return err
	}
	assetIDs, err := s.normalizedAssetIDs(rawAssetIDs)
	if err != nil {
		return err
	}
	if len(assetIDs) == 0 {
	...
	err = tx.QueryRow(s.ports.Bind(`SELECT turn_id FROM `+s.ports.Table("chat_messages")+`
		...
		LIMIT 1`+s.ports.LockSuffix()), messageID, conversationID, ownerID).Scan(&turnID)
	if errors.Is(err, sql.ErrNoRows) {
		return s.ports.DomainError("聊天资产只能绑定到已写入的用户消息")
	}
	if err != nil {
		return err
	}
	...
	rows, err := s.QueryAssets(tx, s.ports.Bind(`SELECT `+AssetColumns+` FROM `+s.ports.Table("chat_assets")+`
		WHERE id IN (`+s.ports.Placeholders(len(assetIDs))+`)
			AND system_account_id = ? AND conversation_id = ?`+s.ports.LockSuffix()),
	...
	nowMs, okNow := s.ports.RFC3339Millis(normalizedNow)
	if !okNow {
		return s.ports.DomainError("聊天资产 now 必须是带 Z 或数值 offset 的 RFC3339 时间")
	}
	for _, assetID := range assetIDs {
		asset := byID[assetID]
		if asset == nil || asset.ProcessingStatus != "ready" || asset.CleanupStatus != "active" {
			return s.ports.DomainError("聊天资产不存在、未处理完成或已过期")
		}
		expiresMs, ok := s.ports.RFC3339Millis(asset.ExpiresAt)
		if !ok {
			return s.ports.DomainError("聊天资产 expiresAt 必须是带 Z 或数值 offset 的 RFC3339 时间")
		}
		if expiresMs <= nowMs {
			return s.ports.DomainError("聊天资产不存在、未处理完成或已过期")
		}
		if asset.SourceKind == "user_upload" &&
			...
			return s.ports.DomainError("聊天资产已绑定其他消息")
		}
	}
	expiresAt, err := s.ports.AddDays(normalizedNow, retentionDays, "聊天资产 expiresAt 基准时间")
	if err != nil {
		return err
	}
	for _, assetID := range assetIDs {
		...
		if asset.SourceKind == "assistant_generated" {
			result, err = tx.Exec(s.ports.Bind(`UPDATE `+s.ports.Table("chat_assets")+`
		...
		if affected, _ := result.RowsAffected(); affected != 1 {
			return s.ports.DomainError("聊天资产绑定消息时发生并发冲突")
		}
	}
	...
	if len(filteredRoots) > 0 {
		result, err := tx.Exec(s.ports.Bind(`UPDATE `+s.ports.Table("chat_assets")+`
			...
			WHERE id IN (`+s.ports.Placeholders(len(filteredRoots))+`)
				AND system_account_id = ? AND conversation_id = ?
				AND source_kind = 'assistant_generated' AND processing_status = 'ready' AND cleanup_status = 'active'`),
		...
		if affected, _ := result.RowsAffected(); affected != int64(len(filteredRoots)) {
			return s.ports.DomainError("聊天图片根谱系保留期限更新失败")
		}
	}
...
func (s *AssetStore) ImageGenerationRootAssetIDs(q queryer, assetIDs []string, conversationID, ownerID string) ([]string, error) {
	ids := s.ports.UniqueStrings(assetIDs)
	if len(ids) == 0 {
		return []string{}, nil
	}
	rows, err := q.Query(s.ports.Bind(`SELECT root_asset_id FROM `+s.ports.Table("chat_image_generations")+`
		WHERE asset_id IN (`+s.ports.Placeholders(len(ids))+`)
	...
	for rows.Next() {
		...
		normalized, err := s.normalizedAssetID(root)
		if err != nil {
			return nil, err
		}
		out = append(out, normalized)
	...
	return s.ports.UniqueStrings(out), nil
}

// renewImageGenerationExpiry mirrors renewChatImageGenerationExpiryInClient.
func (s *AssetStore) renewImageGenerationExpiry(q queryer, assetIDs []string, conversationID, ownerID, expiresAt string) error {
	ids := s.ports.UniqueStrings(assetIDs)
	if len(ids) == 0 {
		return nil
	}
	...
	_, err := q.Exec(s.ports.Bind(`UPDATE `+s.ports.Table("chat_image_generations")+`
		SET expires_at = CASE WHEN expires_at < ? THEN ? ELSE expires_at END
		WHERE asset_id IN (`+s.ports.Placeholders(len(ids))+`)
