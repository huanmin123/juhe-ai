...
const (
	// QuotaRecoveryFixedJitterMinutes mirrors quotaRecoveryFixedJitterMinutes
	// （门面 m11_reads.go 展示面共用，导出供桥别名）.
	QuotaRecoveryFixedJitterMinutes = 15
	// QuotaRecoveryMaxPolicyBytes mirrors quotaRecoveryMaxPolicyBytes。
...
func NormalizeQuotaRecoveryPolicy(value any) (map[string]any, error) {
	...
	if !ok {
		return nil, &accountscore.ValidationError{Message: "额度恢复策略必须是对象"}
	}
	allowed := map[string]bool{"api_key": true, "oauth": true, "google_oauth": true}
	for key := range input {
		if !allowed[key] {
			return nil, &accountscore.ValidationError{Message: "额度恢复策略字段 " + key + " 不受支持"}
		}
	}
	...
	if encoded > QuotaRecoveryMaxPolicyBytes {
		return nil, &accountscore.ValidationError{Message: "额度恢复策略过大"}
	}
	return output, nil
}

// NormalizeQuotaRecoverySchedule mirrors normalizeQuotaRecoverySchedule.
func NormalizeQuotaRecoverySchedule(value any) (map[string]any, error) {
	input, ok := value.(map[string]any)
	if !ok {
		return nil, &accountscore.ValidationError{Message: "额度恢复策略项必须是对象"}
	}
	strategy, _ := input["reset_strategy"].(string)
	if strategy != "duration" && strategy != "daily" && strategy != "weekly" {
		return nil, &accountscore.ValidationError{Message: "额度恢复策略 reset_strategy 必须是 duration、daily 或 weekly"}
	}
	...
	if !ok || number != float64(QuotaRecoveryFixedJitterMinutes) {
		return nil, &accountscore.ValidationError{Message: "额度恢复策略 jitter_minutes固定15，仅作为兼容字段"}
	}
	...
	if !ok || strings.TrimSpace(text) == "" {
		return nil, &accountscore.ValidationError{Message: "额度恢复策略 timezone 无效"}
	}
	trimmed := strings.TrimSpace(text)
	if !ValidQuotaRecoveryTimezone(trimmed) {
		return nil, &accountscore.ValidationError{Message: "额度恢复策略 timezone 无效：" + trimmed}
	}
...
// QuotaRecoveryIntegerInRange mirrors quotaRecoveryIntegerInRange.
func QuotaRecoveryIntegerInRange(value any, min, max int, label string) (float64, error) {
	...
	if !ok || number != float64(int64(number)) || number < float64(min) || number > float64(max) {
		return 0, &accountscore.ValidationError{Message: "额度恢复策略 " + label + " 必须是 " + accountscore.Itoa(min) + "-" + accountscore.Itoa(max) + " 的整数"}
	}
	return number, nil
}
