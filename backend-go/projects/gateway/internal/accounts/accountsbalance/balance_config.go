...
// BalanceConfigError renders the balance config validation failure.
func BalanceConfigError(message string) error {
	return &accountscore.ValidationError{Message: message}
}
...
func EffectiveAccountApiKeys(credentials accountscore.Credentials) []string {
	pool := []any{}
	if credentials != nil {
...
func EffectiveAccountApiKeyCount(credentials accountscore.Credentials) int {
