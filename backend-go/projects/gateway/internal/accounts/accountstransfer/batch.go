...
func (s *Service) LoadBatchEditContext(ctx context.Context, accountIDs []string, fields []string, access accountscore.AccessScope) ([]BatchEditContextItem, error) {
	...
	if len(accountIDs) < 2 || len(accountIDs) > 100 || len(ids) != len(accountIDs) {
		return nil, &accountscore.ValidationError{Message: batchRangeUniqueMessage}
	}
	fieldSet := map[string]bool{}
	for _, field := range fields {
		if !batchEditContextFields[field] {
			return nil, &accountscore.ValidationError{Message: "不支持的批量编辑上下文字段：" + field}
		}
		fieldSet[field] = true
	}
	...
	rows, err := s.store.DB().QueryContext(ctx, s.store.Bind(`SELECT `+columns+` FROM `+s.store.Table("accounts")+`
		WHERE id IN (`+accountscore.Placeholders(len(ids))+`)
	...
	var (
		...
		mappings map[string][]accountscore.ModelMapping
	)
	if fieldSet["supportedModels"] {
		if models, err = s.LoadBatchSupportedModels(ctx, s.store.DB(), ids); err != nil {
			return nil, err
		}
	}
	if fieldSet["modelMappings"] {
		if mappings, err = s.LoadBatchModelMappings(ctx, s.store.DB(), ids); err != nil {
			return nil, err
		}
	}
	...
	for _, id := range ids {
		...
		if fieldSet["modelMappings"] {
			...
			if relations == nil {
				relations = []accountscore.ModelMapping{}
			}
		...
		if fieldSet["supportedEndpointModes"] {
			...
			if row.credentials.Valid && strings.TrimSpace(row.credentials.String) != "" {
				if err := accountscore.DecryptJSON(s.store.Secret(), row.credentials.String, &credentials); err != nil {
					return nil, err
				}
			}
...
// branch (account_id ASC, model ASC). The accountscore.Queryer stays explicit so
// transactional callers never touch s.db while the tx holds the only SQLite
// connection.
func (s *Service) LoadBatchSupportedModels(ctx context.Context, q accountscore.Queryer, ids []string) (map[string][]string, error) {
	out := map[string][]string{}
	if len(ids) == 0 {
		return out, nil
	}
	rows, err := q.QueryContext(ctx, s.store.Bind(`SELECT account_id, model FROM `+s.store.Table("account_supported_models")+`
		WHERE account_id IN (`+accountscore.Placeholders(len(ids))+`) ORDER BY account_id ASC, model ASC`), accountscore.AnySlice(ids)...)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (s *Service) LoadBatchModelMappings(ctx context.Context, q accountscore.Queryer, ids []string) (map[string][]accountscore.ModelMapping, error) {
	out := map[string][]accountscore.ModelMapping{}
	if len(ids) == 0 {
		return out, nil
	}
	rows, err := q.QueryContext(ctx, s.store.Bind(`SELECT account_id, source_model, source_endpoint_family,
			upstream_model, upstream_endpoint_family, enabled
		FROM `+s.store.Table("account_model_mappings")+`
		WHERE account_id IN (`+accountscore.Placeholders(len(ids))+`)
		ORDER BY account_id ASC, source_model ASC, source_endpoint_family ASC`), accountscore.AnySlice(ids)...)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (s *Service) BatchUpdate(ctx context.Context, input BatchUpdateInput, access accountscore.AccessScope) (*BatchUpdateResult, error) {
	ctx = accountscore.EnsureCtx(ctx)
	...
	if len(requestedFields) == 0 {
		return nil, &accountscore.ValidationError{Message: "请至少选择一项需要覆盖的配置"}
	}
	accountscore.SortStrings(requestedFields)
	...
	batchID := s.store.NewID("account_batch")
	...
	for _, account := range prepared {
		...
		if account.supportedModels != nil {
			if err := s.deps.Writer.ReplaceAccountSupportedModels(ctx, tx, account.accountID, current.providerCode, account.supportedModels, nowISO); err != nil {
				return nil, err
			}
		}
		if account.hasModelMappings {
			if err := s.deps.Writer.ReplaceAccountModelMappings(ctx, tx, account.accountID, current.providerCode, account.modelMappings, nowISO); err != nil {
				return nil, err
			}
		}
		if account.hasTags {
			if err := s.deps.Writer.ReplaceTags(ctx, tx, account.accountID, current.systemAccountID, account.tags, nowISO); err != nil {
				return nil, err
			}
		}
		...
		if account.dispatchBinding != nil {
			if _, err := tx.ExecContext(ctx, s.store.Bind(`UPDATE `+s.store.Table("group_accounts")+`
				...
				account.dispatchBinding.priority, accountscore.BoolInt(account.dispatchBinding.superPriorityEnabled),
				accountscore.BoolInt(account.dispatchBinding.fallbackEnabled), nowISO,
	...
	accountscore.SortStrings(result.ChangedFields)
...
func AssertBatchTargets(targets []BatchUpdateTarget) error {
	if len(targets) < 2 || len(targets) > 100 {
		return &accountscore.ValidationError{Message: BatchRangeMessage}
	}
	seen := map[string]bool{}
	for _, target := range targets {
		id := strings.TrimSpace(target.AccountID)
		if id == "" || seen[id] {
			return &accountscore.ValidationError{Message: BatchDuplicateMessage}
		}
		seen[id] = true
		if target.ConfigRevision < 1 {
			return &accountscore.ValidationError{Message: batchRevisionInvalidPrompt}
		}
	}
	return nil
}
...
func (s *Service) loadBatchLockedAccounts(ctx context.Context, q accountscore.Queryer, targets []BatchUpdateTarget, requestedFields []string, scoped string, access accountscore.AccessScope) ([]batchLockedAccount, error) {
...
func (s *Service) queryBatchAccountRows(ctx context.Context, q accountscore.Queryer, ids []string, scoped string) ([]batchLockedAccount, error) {
	scopeClause := ""
	args := accountscore.AnySlice(ids)
	if scoped != "" {
	...
	rows, err := q.QueryContext(ctx, s.store.Bind(`SELECT accounts.id, accounts.config_revision, accounts.system_account_id,
	...
	for rows.Next() {
		...
		account.credentials = accountscore.Credentials{}
		if strings.TrimSpace(credentialsEncrypted) != "" {
			if err := accountscore.DecryptJSON(s.store.Secret(), credentialsEncrypted, &account.credentials); err != nil {
				return nil, err
			}
		}
		...
		if schedule, err := s.deps.ParseScheduleJSON(scheduleJSON.String); err == nil {
...
func (s *Service) LoadBatchTags(ctx context.Context, q accountscore.Queryer, ids []string) (map[string][]string, error) {
	out := map[string][]string{}
	if len(ids) == 0 {
		return out, nil
	}
	rows, err := q.QueryContext(ctx, s.store.Bind(`SELECT account_tag_bindings.account_id, account_tags.name
		...
		WHERE account_tag_bindings.account_id IN (`+accountscore.Placeholders(len(ids))+`)
		ORDER BY account_tag_bindings.account_id ASC, account_tags.name ASC`), accountscore.AnySlice(ids)...)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func assertPreparedAccountsMatchTargets(prepared []batchPreparedAccount, targets []BatchUpdateTarget) error {
	if len(prepared) != len(targets) {
		return &accountscore.ValidationError{Message: "批量编辑最终配置数量不匹配"}
	}
	ids := map[string]bool{}
	for _, account := range prepared {
	...
	for _, target := range targets {
		if !ids[strings.TrimSpace(target.AccountID)] {
			return &accountscore.ValidationError{Message: "批量编辑最终配置目标不匹配"}
		}
	}
	return nil
}
...
func (s *Service) prepareBatchUpdates(ctx context.Context, q accountscore.Queryer, accounts []batchLockedAccount, updates map[string]BatchUpdateField) ([]batchPreparedAccount, error) {
	homogeneous := false
	...
	if homogeneous {
		...
		if len(signatures) > 1 {
			return nil, &accountscore.ValidationError{Message: "模型与协议配置只能批量覆盖到相同供应商、协议档案和账户类型的账户"}
		}
	}
	resolvedProxy := ""
	resolveProxy := false
	if update, ok := updates["proxyProfileId"]; ok && update.Enabled {
		...
		if update.Value != nil {
			...
			if text == "" {
				return nil, &accountscore.ValidationError{Message: "代理配置不能为空"}
			}
			owner := ""
			if len(accounts) > 0 {
				owner = accounts[0].systemAccountID
...
func ApplyNullableCredentialOverride(credentials accountscore.Credentials, updates map[string]BatchUpdateField, updateKey, credentialKey string) {
...
func JSONValueDeepEqual(left, right any) bool {
	leftEncoded, err := json.Marshal(accountscore.CanonicalizeJSONValue(left))
	if err != nil {
		return false
	}
	rightEncoded, err := json.Marshal(accountscore.CanonicalizeJSONValue(right))
	if err != nil {
		return false
	}
...
func (s *Service) prepareBatchAccount(ctx context.Context, q accountscore.Queryer, account *batchLockedAccount, updates map[string]BatchUpdateField, resolvedProxy string, resolveProxy bool) (batchPreparedAccount, error) {
	...
	if hasCredentialConfigUpdate {
		merged := accountscore.Credentials{}
		for key, value := range account.credentials {
			merged[key] = value
		}
		if hasOwn("errorHandlingRules") {
			rules, err := s.deps.NormalizeErrorHandlingRules(fieldValue("errorHandlingRules"))
			if err != nil {
				return result, err
			}
		...
		if hasOwn("responseInspectionRules") {
			rules, err := s.deps.NormalizeResponseInspectionRules(fieldValue("responseInspectionRules"))
			if err != nil {
				return result, err
			}
		...
		normalized, err := s.deps.NormalizeCredentialsForWrite(account.accountType, merged, &accountscore.EndpointModeDefaultContext{
		...
		if credentialsChanged {
			sealed, err := accountscore.EncryptJSON(s.store.Secret(), map[string]any(nextCredentials))
			if err != nil {
				return result, err
			}
	...
	if hasOwn("supportedModels") {
		models, err := s.deps.NormalizeSupportedModels(fieldValue("supportedModels"))
		if err != nil {
			return result, err
		}
		if len(models) == 0 {
	...
	if modelConfigurationRelevant {
		if err := s.deps.AssertSupportedModelsRequired(nextSupportedModels); err != nil {
			return result, err
		}
	}
	...
	if supportedModelsChanged {
		...
		if err := s.deps.AssertSupportedModelsInProviderCatalog(ctx, q, nextSupportedModels, account.providerCode, account.systemAccountID, accountscore.ProtocolPredicate{
			ProviderCode:              account.providerCode,
			ProtocolCode:              account.protocolCode,
			ProtocolVersion:           account.protocolVersion,
			ProviderProtocolProfileID: account.providerProtocolProfileID,
	...
	if hasOwn("healthCheckModel") {
		...
		if text == "" {
			return result, &accountscore.ValidationError{Message: "账户检查模型不能为空"}
		}
	...
	if hasOwn("healthCheckModel") || supportedModelsChanged {
		...
		if !found {
			return result, &accountscore.ValidationError{Message: "账户 " + account.id + " 的检查模型必须属于最终支持模型"}
		}
	}
	...
	if resolveHealthMode {
		resolved, err := s.deps.ResolveHealthCheckEndpointMode(
	...
	if shouldValidateMappings {
		if err := s.deps.AssertModelMappingsInProviderCatalog(ctx, q, account.providerCode, account.systemAccountID, accountscore.ProtocolPredicate{
			ProviderCode:              account.providerCode,
			ProtocolCode:              account.protocolCode,
			ProtocolVersion:           account.protocolVersion,
			ProviderProtocolProfileID: account.providerProtocolProfileID,
	...
	if shouldValidateMappings || hasOwn("healthCheckEndpointMode") {
		if err := s.deps.AssertEndpointModesCompatible(account.providerCode, account.accountType, account.clientCompatibility,
	...
	if hasOwn("tags") {
		names, err := s.deps.NormalizeTagNames(fieldValue("tags"))
		if err != nil {
			return result, err
		}
		nextTags = names
	...
	if resolveProxy {
		...
		currentProxy := accountscore.NullPtrString(account.proxyProfileID)
		...
		if proxyChanged {
			...
			if account.balanceQueryEnabled {
				setColumn("balance_query_next_refresh_at", accountscore.IsoMillis(s.store.Now()))
			}
		}
	}
	...
	if nextSuper && nextFallback {
		if hasOwn("superPriorityEnabled") && BoolValue(fieldValue("superPriorityEnabled"), false) && !hasOwn("fallbackEnabled") {
			...
			return result, &accountscore.ValidationError{Message: "超级优先和降级备用不能同时开启"}
		}
	}
	...
	if superChanged {
		...
		setColumn("super_priority_enabled", accountscore.BoolInt(nextSuper))
	}
	if fallbackChanged {
		addChange("fallbackEnabled")
		setColumn("fallback_enabled", accountscore.BoolInt(nextFallback))
	}
	...
	if hasOwn("accountExpiresAt") {
		...
		if value == nil {
			...
			canonical, valid := accountscore.CanonicalRFC3339(textString(value))
			if !valid {
				return result, &accountscore.ValidationError{Message: "账户套餐到期时间必须是有效时间字符串"}
			}
	...
	if hasOwn("availabilitySchedule") {
		schedule, err := s.deps.NormalizeSchedule(fieldValue("availabilitySchedule"))
		if err != nil {
			return result, AccountScheduleError(err)
		}
		nextSchedule = schedule
		currentJSON, _ := s.deps.ScheduleJSON(account.availabilitySchedule)
		nextJSON, _ := s.deps.ScheduleJSON(nextSchedule)
		scheduleChanged = currentJSON != nextJSON
		if scheduleChanged {
			addChange("availabilitySchedule")
			if raw, ok := s.deps.ScheduleJSON(nextSchedule); ok {
				setColumn("availability_schedule_json", raw)
			} else {
				setColumn("availability_schedule_json", nil)
			}
			if nextCheck, ok := s.deps.NextScheduleCheckAt(nextSchedule, s.store.Now()); ok {
	...
	expiredByChangedPackage := expiresAtChanged && accountscore.IsAccountExpired(nextExpiresAt.String, s.store.Now())
	if expiredByChangedPackage {
		...
		if override, ok := s.deps.ScheduleStatus(nextSchedule, s.store.Now()); ok &&
	...
	if nextSchedulable != account.schedulable {
		...
		setColumn("schedulable", accountscore.BoolInt(nextSchedulable))
	}
...
func AccountScheduleError(err error) error {
	var validation *accountscore.ValidationError
	if errors.As(err, &validation) {
		return &accountscore.ValidationError{Message: strings.ReplaceAll(validation.Message, "API Key 时间计划", "账户时间计划")}
	}
	return err
}

// executeBatchCasUpdate mirrors executeAccountBatchCasUpdate.
func (s *Service) executeBatchCasUpdate(ctx context.Context, q accountscore.Queryer, prepared *batchPreparedAccount, ownerSystemAccountID, updatedAt string) error {
	...
	exec, err := q.ExecContext(ctx, s.store.Bind(`UPDATE `+s.store.Table("accounts")+` SET
...
func NormalizeBatchModelMappings(value any) ([]accountscore.ModelMapping, error) {
	list, ok := value.([]any)
	if !ok {
		return nil, &accountscore.ValidationError{Message: "账户 modelMappings 必须是模型映射数组"}
	}
	out := []accountscore.ModelMapping{}
	seenSources := map[string]bool{}
	for _, item := range list {
		record, ok := item.(map[string]any)
		if !ok {
			return nil, &accountscore.ValidationError{Message: "账户 modelMappings 条目必须是对象"}
		}
		mapping := accountscore.ModelMapping{
		...
		if mapping.SourceModel == "" || mapping.SourceEndpointFamily == "" ||
			...
			return nil, &accountscore.ValidationError{Message: "账户 modelMappings 条目必须包含 sourceModel、sourceEndpointFamily、upstreamModel 和 upstreamEndpointFamily"}
		}
		if !batchMappingSourceFamilies[mapping.SourceEndpointFamily] || !batchMappingUpstreamFamilies[mapping.UpstreamEndpointFamily] {
			return nil, &accountscore.ValidationError{Message: BatchFieldsPrompt}
		}
		...
		if seenSources[sourceKey] {
			return nil, &accountscore.ValidationError{Message: "账户 modelMappings 不能重复配置同一个 sourceModel 和 sourceEndpointFamily：" + mapping.SourceModel + " / " + mapping.SourceEndpointFamily}
		}
...
func AssertMappingUpstreamsAllowed(mappings []accountscore.ModelMapping, supportedModels []string) error {
	...
	if len(invalid) > 0 {
		limit := accountscore.MinInt(5, len(invalid))
		return &accountscore.ValidationError{Message: "映射上游模型只能选择账户支持模型：" + strings.Join(invalid[:limit], "、")}
	}
	return nil
}
...
func ModelMappingsEqual(left, right []accountscore.ModelMapping) bool {
	keys := func(mappings []accountscore.ModelMapping) []string {
