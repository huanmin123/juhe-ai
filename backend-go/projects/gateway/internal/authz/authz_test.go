...
var ddl = []string{
	...
	// NOTE: the real schema backs the instance-name ladder with the partial
	// unique index idx_accounts_owner_name_unique (system_account_id, name)
	// WHERE deleted_at IS NULL. The fixture deliberately skips it: several
	// existing seeders insert nameless (DEFAULT '') accounts in one namespace,
	// and the ladder itself dedupes by lookup before insert.
