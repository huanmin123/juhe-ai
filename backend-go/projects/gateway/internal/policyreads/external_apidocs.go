...
func catalogRouteStrategyFields(prefix string) []catalogField {
	return []catalogField{
		...
		catalogFieldEx(prefix+".mode", "string", false, "路由模式：normal、weighted、failover、round_robin 或 merge。", "normal"),
	...
	items := []catalogItem{
		...
		},
		{
			...
			Query: append([]catalogField{catalogTargetQuery,
				...
				catalogFieldEx("mode", "string", false, "路由模式筛选：normal、weighted、failover、round_robin、merge 或 all。", "normal"),
				catalogFieldEx("status", "string", false, "状态筛选：active、disabled 或 all。", "active")},
				catalogPageQuery()...),
		...
		},
		{
			...
			Summary: "按路由策略 ID 修改名称、状态、模式、分组绑定或调度配置。",
