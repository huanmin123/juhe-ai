...
// (accounts reuse the same evaluation through
// account-availability-schedule.ts) and the stored JSON shape
// (enabled/timezone/mode/windows/dateRange/exceptions).
...
// ScheduleDueEvent mirrors ApiKeyAvailabilityScheduleDueEvent: a boundary
// crossing at the current minute with the dedupe key the sync writes into the
// *_schedule_status_events tables.
type ScheduleDueEvent struct {
	EventKey string
...
// ParseScheduleJSON mirrors parseApiKeyAvailabilityScheduleJson /
// parseAccountAvailabilityScheduleJson (stored rows are re-normalized on read;
// invalid rows surface as errors).
func ParseScheduleJSON(raw string) (*AvailabilitySchedule, error) {
	...
	if err := json.Unmarshal([]byte(raw), &decoded); err != nil {
		return nil, fmt.Errorf("API Key 时间计划参数无效")
	}
	return normalizeSchedule(decoded)
}

// ScheduleStatus mirrors apiKeyAvailabilityScheduleStatus: absent=false when
// the schedule is absent/disabled, otherwise active/disabled for now.
...
// DueScheduleEvent mirrors dueApiKeyAvailabilityScheduleEvent: when the current
// zoned minute sits on a window boundary, the event (start wins over end) with
// the composite event key; absent for non-boundary minutes.
func DueScheduleEvent(schedule *AvailabilitySchedule, now time.Time) (ScheduleDueEvent, bool) {
	if schedule == nil || !schedule.Enabled {
		return ScheduleDueEvent{}, false
	}
	current := scheduleZonedParts(now, schedule.Timezone)
	events := dueScheduleWindowEvents(schedule, current)
	if len(events) == 0 {
		return ScheduleDueEvent{}, false
	}
	...
	status := "disabled"
	if scheduleAllows(schedule, current) {
		status = "active"
	}
	...
	sort.Strings(keys)
	return ScheduleDueEvent{
		Action:   action,
		Status:   status,
		EventKey: fmt.Sprintf("%s:%d:%s:%s", current.dateKey, current.minuteOfDay, action, strings.Join(keys, "|")),
	}, true
}

// NextScheduleCheckAt mirrors nextApiKeyAvailabilityScheduleCheckAt: the
// earliest UTC window boundary after now, or now + 7 days when the horizon has
// no boundary. valid=false for absent/disabled schedules (NULL column).
...
func normalizeSchedule(input any) (*AvailabilitySchedule, error) {
	...
	if enabled, ok := object["enabled"].(bool); !ok || !enabled {
		return nil, fmt.Errorf("API Key 时间计划启用状态必须为 true")
	}
	if mode, _ := object["mode"].(string); mode != scheduleModeAllowWindows {
		return nil, fmt.Errorf("API Key 时间计划模式必须为 allow_windows")
	}
	...
	if len(windows) == 0 {
		return nil, fmt.Errorf("API Key 时间计划至少需要一个允许时段")
	}
...
func normalizeScheduleTimezone(value any) (string, error) {
	...
	if !ok || strings.TrimSpace(text) == "" {
		return "", fmt.Errorf("API Key API Key 时间计划时区不能为空")
	}
	timezone := strings.TrimSpace(text)
	if _, err := time.LoadLocation(timezone); err != nil {
		return "", fmt.Errorf("API Key API Key 时间计划时区无效")
	}
	return timezone, nil
}

// defaultScheduleTimezone mirrors defaultScheduleTimezone: the deployment
// timezone when resolvable, otherwise UTC.
...
func normalizeScheduleWindows(input any, requireDays bool) ([]ScheduleWindow, error) {
	list, ok := input.([]any)
	if !ok {
		return nil, fmt.Errorf("API Key 时间计划时段无效")
	}
	if len(list) > maxScheduleWindows {
		return nil, fmt.Errorf("API Key 时间计划最多支持 %d 个时段", maxScheduleWindows)
	}
...
func normalizeScheduleWindow(input any, requireDays bool) (ScheduleWindow, error) {
	object, ok := input.(map[string]any)
	if !ok {
		return ScheduleWindow{}, fmt.Errorf("API Key 时间计划时段无效")
	}
	...
	if start == end {
		return ScheduleWindow{}, fmt.Errorf("API Key API Key 时间计划开始时间和停止时间不能相同")
	}
	if !requireDays {
...
func normalizeScheduleTime(value any, label string) (string, error) {
	...
	if !ok || !scheduleTimePattern.MatchString(strings.TrimSpace(text)) {
		return "", fmt.Errorf("API Key 时间计划%s格式应为 HH:mm", label)
	}
...
func normalizeDaysOfWeek(value any) ([]int, error) {
	list, ok := value.([]any)
	if !ok {
		return nil, fmt.Errorf("API Key 时间计划重复日期无效")
	}
	seen := map[int]bool{}
	days := []int{}
	for _, item := range list {
	...
	if len(days) == 0 {
		return nil, fmt.Errorf("API Key 时间计划至少需要选择一个重复日期")
	}
	sort.Ints(days)
	return days, nil
}
...
func normalizeScheduleDateRange(input any) (*ScheduleDateRange, error) {
	...
	object, ok := input.(map[string]any)
	if !ok {
		return nil, fmt.Errorf("API Key 时间计划生效日期范围无效")
	}
	...
	if startDate != "" && endDate != "" && startDate > endDate {
		return nil, fmt.Errorf("API Key API Key 时间计划开始日期不能晚于结束日期")
	}
...
func normalizeScheduleExceptions(input any) ([]ScheduleException, error) {
	...
	list, ok := input.([]any)
	if !ok {
		return nil, fmt.Errorf("API Key API Key 时间计划例外日期无效")
	}
	if len(list) > maxScheduleExceptions {
		return nil, fmt.Errorf("API Key 时间计划最多支持 %d 个例外日期", maxScheduleExceptions)
	}
...
func normalizeScheduleException(input any) (ScheduleException, error) {
	object, ok := input.(map[string]any)
	if !ok {
		return ScheduleException{}, fmt.Errorf("API Key API Key 时间计划例外日期无效")
	}
	...
	if date == "" {
		return ScheduleException{}, fmt.Errorf("API Key API Key 时间计划例外日期不能为空")
	}
	action, _ := object["action"].(string)
	if action != scheduleExceptionAllow && action != scheduleExceptionDeny {
		return ScheduleException{}, fmt.Errorf("API Key 时间计划例外动作无效")
	}
	if action == scheduleExceptionDeny && object["windows"] != nil {
		return ScheduleException{}, fmt.Errorf("API Key 时间计划拒绝例外不能配置允许时段")
	}
	if action == scheduleExceptionAllow {
		...
		if len(windows) == 0 {
			return ScheduleException{}, fmt.Errorf("API Key 时间计划允许例外至少需要一个允许时段")
		}
...
func normalizeDateKey(value any, label string) (string, error) {
	...
	if !ok || !scheduleDatePattern.MatchString(strings.TrimSpace(text)) {
		return "", fmt.Errorf("API Key 时间计划%s格式应为 YYYY-MM-DD", label)
	}
	date := strings.TrimSpace(text)
	parsed, err := time.Parse("2006-01-02", date)
	if err != nil || parsed.UTC().Format("2006-01-02") != date {
		return "", fmt.Errorf("API Key 时间计划%s无效", label)
	}
	return date, nil
}
...
// allowedScheduleOccurrences mirrors allowedScheduleWindowOccurrences: windows
// may spill across midnight, so both today and yesterday are start candidates.
...
// scheduleWindowEvent mirrors ScheduleWindowEvent.
type scheduleWindowEvent struct {
	action string
	key    string
}
...
// dueScheduleWindowEvents mirrors dueScheduleWindowEvents: window boundary
// events (start/end) landing exactly on the current zoned minute, today and
// yesterday as start candidates.
func dueScheduleWindowEvents(schedule *AvailabilitySchedule, current zonedParts) []scheduleWindowEvent {
	events := []scheduleWindowEvent{}
	for _, startDateKey := range []string{current.dateKey, previousDateKey(current.dateKey)} {
		if !dateInScheduleRange(startDateKey, schedule) {
			continue
		}
		...
		if exception != nil && exception.Action == scheduleExceptionAllow {
			for index, window := range exception.Windows {
				events = append(events, windowEvents(current, startDateKey, window.Start, window.End,
					fmt.Sprintf("exception:%s:%d", startDateKey, index))...)
			}
			continue
		}
		for index, window := range schedule.Windows {
			...
			events = append(events, windowEvents(current, startDateKey, window.Start, window.End,
				fmt.Sprintf("window:%d", index))...)
		}
	}
	return events
}

func windowEvents(current zonedParts, startDateKey, startText, endText, token string) []scheduleWindowEvent {
	start, end := minuteOfDay(startText), minuteOfDay(endText)
	endDateKey := startDateKey
	if start >= end {
		endDateKey = nextDateKey(startDateKey)
	}
	events := []scheduleWindowEvent{}
	if current.dateKey == startDateKey && current.minuteOfDay == start {
		events = append(events, scheduleWindowEvent{action: "start", key: token + ":start:" + startText})
	}
	if current.dateKey == endDateKey && current.minuteOfDay == end {
		events = append(events, scheduleWindowEvent{action: "end", key: token + ":end:" + endText})
	}
	return events
}
...
// zonedLocalMinuteToUTC mirrors zonedLocalMinuteToUtcTime: iteratively snaps a
// naive zoned wall-clock minute onto the real UTC instant (DST safe).
