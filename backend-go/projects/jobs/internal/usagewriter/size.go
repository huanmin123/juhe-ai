...
// the snapshot bounding helpers of record-queue.service.ts (same
// implementation as gateway/internal/gatewayusage/size.go).
...
// []byte buffers, time.Time dates, typed structs via their json tags).
...
func visitJSONLikeValue(value any, context *jsonLikeEstimateContext) {
	...
	case json.Number:
		addEstimatedBytes(context, len(string(typed)))
	case []byte:
