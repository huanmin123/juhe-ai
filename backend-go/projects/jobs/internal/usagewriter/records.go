...
// (backend/src/storage/usage-records.repository.ts) field for field — the
// same frozen contract as gateway/internal/gatewayusage records.go, so the
// G20 assembly adapter is a mechanical value copy. Empty strings and nil
// pointers mean the Node undefined fields; Success, TraceID and TrafficSource
// are always set by the record builders.
...
// Usage traffic source values (usage-records.repository.ts). Invalid values
// are rejected with the Node copy "使用记录来源无效".
const (
	TrafficSourceGateway             = "gateway"
	TrafficSourceManualAccountTest   = "manual_account_test"
	TrafficSourceAccountHealthCheck  = "account_health_check"
	TrafficSourceRuntimeRecoveryProb = "runtime_recovery_probe"
	TrafficSourceCooldownRetest      = "cooldown_retest"
	TrafficSourceHybridScoring       = "hybrid_scoring"
	TrafficSourceHybridQualityScore  = "hybrid_quality_scoring"
)
...
// the G20 adapter stays mechanical. The concrete id format lives in
// shards.go.
type UsageRecordIDFactory interface {
	GenerateUsageRecordID(createdAt string) string
}

// IDFactoryFunc adapts a function to UsageRecordIDFactory.
type IDFactoryFunc func(createdAt string) string

// GenerateUsageRecordID implements UsageRecordIDFactory.
func (f IDFactoryFunc) GenerateUsageRecordID(createdAt string) string { return f(createdAt) }

// NewRandomUUID mirrors randomUUID(): a random RFC 4122 version-4 UUID
// string (Node node:crypto randomUUID).
...
// Ctx is the dispatch context alias keeping signatures compact.
...
// account/group scope integrity rules. Byte-for-byte the same contract as
// gateway/internal/gatewayusage NormalizeUsageRecordInput.
...
// boundSnapshotStruct walks typed struct snapshots with their JSON names in
// declaration order, the equivalent of the Node plain objects. Nil
// pointer/interface fields are skipped like undefined properties. Returns
// false for non-structs.
...
// budget is supplied by the writer config (Node
