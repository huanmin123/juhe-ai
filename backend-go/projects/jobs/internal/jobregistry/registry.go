...
func ScheduledEntries() []Entry {
	return []Entry{
		...
		},
		{
			...
			GoStatus: GoPartial, GoPackage: "retention",
			GoBinding: "RecordCleanupRetryJob.RunAPIKey 已迁移；缺 APIKeyRecordCleanupRetryer 适配器：api-key-record-cleanup.ts（dataset targets/关联行清理/统计扣减）尚无 jobs 侧等价迁移",
		},
		{
			...
			GoStatus: GoPartial, GoPackage: "retention",
			GoBinding: "RecordCleanupRetryJob.RunAccount 已迁移；缺 AccountRecordCleanupRetryer 适配器：account-record-cleanup.ts（dataset targets/关联行清理/统计扣减）尚无 jobs 侧等价迁移",
		},
		{
		...
		},
		{
			...
			GoStatus: GoPartial, GoPackage: "accountquality",
			GoBinding: "RefreshRunner 已迁移；探针链适配器未接：find_account_for_test 的 effectiveAvailability 派生与 testOpenAIAccountDiagnosticAttempt 协议诊断栈属网关域，jobs 侧无等价实现且两 module 不可互 import",
		},
		{
		...
		},
		{
			...
			GoStatus: GoPartial, GoPackage: "accountquality",
			GoBinding: "CooldownRetestRunner 已迁移；缺 Prober/CooldownCandidateSource/CooldownMutation 适配器：协议诊断探针与 account_api_key_runtime_states CAS 仓储（record/defer）尚无 jobs 侧等价实现",
		},
		{
			...
			GoStatus: GoPartial, GoPackage: "opsjobs",
			GoBinding: "SpeedFirstProbeRunner 已迁移；缺 SpeedFirstClaimStore（Redis 降级运行态 normal-route-latency-degradation.service.ts 为 gateway runtime 单实现，复制契约会引入第二 writer）与首字探针传输",
		},
		{
			...
			GoStatus: GoPartial, GoPackage: "opsjobs",
			GoBinding: "ControlPlaneMaintenance 已迁移；缺 Redis CircuitStore（gatewaycircuit store_redis+lua 单实现）与 ControlPlaneLedger/Outbox 适配器；跨模块不可 import，复制实现有状态分歧风险",
		},
		{
			...
			GoStatus: GoPartial, GoPackage: "opsjobs",
			GoBinding: "RunListAvailabilityMaintenance 已迁移；缺 ListAvailabilityRepo（PostgreSQL 读模型仓储）与 LoadItems 物化载荷来源（ProjectionItem payload 由网关域组装）",
		},
		{
			...
			GoStatus: GoPartial, GoPackage: "opsjobs",
			GoBinding: "CircuitRecoveryService 已迁移；缺 Redis CircuitStore（gatewaycircuit 单实现）与恢复探针目标解析（账户域读取链）",
		},
		{
		...
		},
		{
			...
			GoStatus: GoPartial, GoPackage: "retention",
			GoBinding: "DataRetentionJob 已迁移；缺 StatsWriter/PublicApiLogsCleaner/UsageRecordsCleaner/DbService 适配器：data-retention.repository.ts 与 codex-context-state.repository.ts 尚无 jobs 侧等价迁移",
		},
		{
			...
			Writes:   []string{"chat:*"},
			GoStatus: GoPartial, GoPackage: "retention",
			GoBinding: "ChatRetentionJob 已迁移；缺 DbService.CleanupChatRetention 适配器：chat.repository.ts cleanupChatRetention（chat 库分区/资产/检查点链）尚无 jobs 侧等价迁移",
		},
		{
			...
			GoStatus: GoPartial, GoPackage: "retention",
			GoBinding: "ExpiredDeletedAccountJob 已迁移；缺 DbService.CleanupExpiredDeletedAccounts/RecordMaintenanceEnqueuer 适配器：逻辑删除物理清理仓储尚无 jobs 侧等价迁移",
		},
	}
}
...
func QueueEntries() []Entry {
	...
	return []Entry{
		...
		{JobName: "background_worker_record_maintenance", Category: CategoryIPCQueue, Kind: "maintenance", DefaultRole: "ingest-worker",
			Writes: []string{"dataset:*", "stats:*", "usage-shards:usage_records"}, GoStatus: GoPartial, GoPackage: "retention",
			GoBinding: "recordmaintenance 已迁移；缺 DB service 适配器"},
		batch("background_worker_account_test_tasks", []string{"business:account_test_tasks"}, GoPartial, "opsjobs",
			"ManualTestQueue 已迁移；缺 ManualTestTaskRepo/Executor 适配器"),
		control("background_worker_account_test_cancel", "ops-worker", GoPartial, "ManualTestQueue.CancelLocal 待适配器接线"),
		control("background_worker_codex_source_fence_settled", "ops-worker", GoPartial, "fence settlement 随 J1/J3 适配器接线"),
		...
		{JobName: "manual-account-test-queue", Category: CategoryLocalQueue, Kind: "probe", DefaultRole: "ops-worker",
			Writes: []string{"business:account_test_tasks"}, GoStatus: GoPartial, GoPackage: "opsjobs",
			GoBinding: "ManualTestQueue.EnqueueLocal/DispatchAccountTestTask 待 repo/executor 适配器"},
		{JobName: "account-api-key-cooldown-retest-queue", Category: CategoryLocalQueue, Kind: "probe", DefaultRole: "ops-worker",
			...
			GoStatus: GoPartial, GoPackage: "accountquality", GoBinding: "CooldownRetestRunner 队列待探针适配器"},
		{JobName: "account-quality-failure-precheck-queue", Category: CategoryLocalQueue, Kind: "probe", DefaultRole: "ops-worker",
			LeaseRequired: true, Writes: []string{"business:accounts"}, GoStatus: GoPartial, GoPackage: "accountquality",
			GoBinding: "PrecheckRunner 队列待探针适配器"},
		...
		{JobName: "record-maintenance:api_key_related_cleanup", Category: CategoryMaintenanceTask, Kind: "maintenance", DefaultRole: "ops-worker",
			...
			GoStatus: GoPartial, GoPackage: "retention", GoBinding: "recordmaintenance 任务包已迁移，待 DB service 适配器"},
		...
		{JobName: "record-maintenance:usage_records_cleanup", Category: CategoryMaintenanceTask, Kind: "maintenance", DefaultRole: "ingest-worker",
			...
			GoStatus: GoPartial, GoPackage: "retention", GoBinding: "retention UsageRecordsCleaner 待适配器"},
		{JobName: "record-maintenance:non_business_data_cleanup", Category: CategoryMaintenanceTask, Kind: "maintenance", DefaultRole: "ingest-worker",
			...
			GoStatus: GoPartial, GoPackage: "retention", GoBinding: "retention non-business 清理待适配器（audit payload 归 F3）"},
		{JobName: "record-maintenance:account_usage_snapshot_upsert", Category: CategoryMaintenanceTask, Kind: "maintenance", DefaultRole: "ingest-worker",
			Writes: []string{"stats:account_usage_snapshots"}, GoStatus: GoPartial, GoPackage: "retention",
			GoBinding: "StatsWriter.UpsertAccountUsageSnapshots 待适配器"},
	}
}
