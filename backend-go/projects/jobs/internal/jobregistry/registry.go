...
func ScheduledEntries() []Entry {
	return []Entry{
		...
		},
		{
			...
			GoStatus: GoWired, GoPackage: "oauthrefresh",
			GoBinding: "Store.RunAuthorizationExpirySweep + 组合根 GrantFinalizer（authorizationGrantHealthFanout：事务内 J1 输入 fanout，kind=snapshot/reason=authorization_grant_changed）+ expired>0 后 markAllGroupAccountStatsDirty('authorization_expired')（对齐 Node refreshAfterResourceAuthorizationBusinessWriteAsync）。T6d 冻结交接：grant 翻转后的 runtime 投影（resource_authorizations 行 + effective source 重算）与 quota 窗口 scope bindings 属 gateway authz sync 域（gateway internal/authz，jobs 不可 import）；Go gateway 读路径以 expires_at > now 门禁兜底（chain_accounts activeResourceAuthorization*），gateway 侧消费面已接入（审查轮换六 `compose_authz_expiry_sync.go`：authz-expiry-runtime-sync 组件按固定节拍调用 authz.Store.ReconcileExpiredGrants，对窗口内已翻转 grant 幂等重放 runtime 投影与 quota scope bindings 重同步）",
		},
		{
...
func QueueEntries() []Entry {
	...
	return []Entry{
		...
		{JobName: "client-ip-policy-hit-buffer", Category: CategoryLocalQueue, Kind: "stats", DefaultRole: "stats-worker", Hotspot: true,
			Writes: []string{"stats:client_ip_policy_hits"}, GoStatus: GoPartial, GoPackage: "gateway/internal/gatewayclientip",
			GoBinding: "gateway 进程内缓冲+flush（policy.go 双方言 upsert、policycache flush、retention 覆盖该表）；与 Node client-ip-policy-cache.service.ts 的缓冲语义逐项等价性（flush 节拍/批量上限/丢弃计数）尚未核验（2026-09-06 复查登记）"},
