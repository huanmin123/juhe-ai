...
func ScheduledEntries() []Entry {
	return []Entry{
		...
		},
		{
			...
			GoBinding: "RecordCleanupRetryJob.RunAPIKey + cleanuprepo.RecordCleanupStore（SQLite 全链：targets/分片游标/统计扣减结算）；PostgreSQL 统计扣减链（subtractPostgresUsageStatsRows）显式报错未迁移，不静默回落",
		},
		{
			...
			GoStatus: GoWired, GoPackage: "cleanuprepo + retention",
			GoBinding: "RecordCleanupRetryJob.RunAccount + cleanuprepo.RecordCleanupStore（SQLite 全链：targets/分片游标/统计扣减结算）；PostgreSQL 统计扣减链（subtractPostgresUsageStatsRows）显式报错未迁移，不静默回落",
		},
		{
