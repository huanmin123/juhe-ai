// Package modelcheckprofile freezes the J3b protocol/model catalog at the
// jobs boundary. It is deliberately pure: no Node import, database lookup,
// network call, or mutable global registry is involved.
...
func Profiles() []ProtocolProfile {
	...
	for i, profile := range catalog {
		result[i] = clone(profile)
	}
	return result
}
...
func Find(providerCode, profileID string) (ProtocolProfile, bool) {
	...
	for _, profile := range catalog {
		if NormalizeToken(profile.ProviderCode) != providerCode {
			continue
		}
		for _, candidate := range profile.ProviderProtocolProfileIDs {
			if NormalizeToken(candidate) == profileID {
...
func FindForModel(providerCode, profileID, model string) (ProtocolProfile, bool) {
	profile, ok := Find(providerCode, profileID)
	if !ok {
		return ProtocolProfile{}, false
	}
