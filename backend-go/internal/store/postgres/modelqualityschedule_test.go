...
func TestModelQualityScheduleSQLUsesLockedRowsAndOpaqueCompletionFence(t *testing.T) {
	...
	for _, fragment := range []string{
		"lease_token = $2",
		"revision = $6",
		"accounts.config_revision = $7",
		"accounts.status = 'active'",
		"next_run_at <= $4",
		"lease_until IS NULL OR lease_until <= $4",
	} {
...
func TestClaimDueModelQualitySchedulesReturnsCommittedTokenizedFacts(t *testing.T) {
	...
	claims, err := claimDueModelQualitySchedules(
		...
		port.ModelQualityScheduleClaimInput{OwnerID: "ops-1", Now: now, LeaseUntil: leaseUntil},
	...
	if len(tx.execCalls) != 1 || !reflect.DeepEqual(tx.execCalls[0].args, []any{"ops-1", "claim_token_1", modelQualityPolicyTimeText(leaseUntil), modelQualityPolicyTimeText(now), "mqs_1", int64(4), int64(9)}) {
		t.Fatalf("claim update args = %+v", tx.execCalls)
	}
...
func TestClaimDueModelQualitySchedulesSkipsAccountThatChangedAfterSelection(t *testing.T) {
	...
	claims, err := claimDueModelQualitySchedules(
		context.Background(),
		func(context.Context, pgx.TxOptions) (pgx.Tx, error) { return tx, nil },
		port.ModelQualityScheduleClaimInput{OwnerID: "ops-1", Now: now, LeaseUntil: now.Add(5 * time.Minute), Limit: 1},
		func(string) (string, error) { return "claim_token_1", nil },
	)
...
func TestCompleteModelQualityScheduleUsesAllFencesAndRejectsExpiredLease(t *testing.T) {
	now := time.Date(2026, 7, 26, 10, 0, 0, 0, time.UTC)
	input := port.ModelQualityScheduleCompleteInput{
		...
		CompletedAt: now, Interval: time.Hour,
	}
	...
	wantArgs := []any{"run_1", modelQualityPolicyTimeText(now), "completed", modelQualityPolicyTimeText(now.Add(time.Hour)), "mqs_1", int64(4), 60, "ops-1", "claim_token_1", modelQualityPolicyTimeText(now.Add(5 * time.Minute))}
	if !reflect.DeepEqual(execer.args, wantArgs) {
		t.Fatalf("completion args = %#v, want %#v", execer.args, wantArgs)
	}

	input.Lease.Until = now
	execer.calls = 0
	if _, err := completeModelQualitySchedule(context.Background(), execer, input); err == nil || execer.calls != 0 {
		t.Fatalf("expired lease error=%v calls=%d", err, execer.calls)
	}
}
...
func TestModelQualityScheduleValidationBoundsDatabaseAndLeaseInputs(t *testing.T) {
	now := time.Date(2026, 7, 26, 10, 0, 0, 0, time.UTC)
	validClaim := port.ModelQualityScheduleClaimInput{OwnerID: "ops-1", Now: now, LeaseUntil: now.Add(time.Minute), Limit: 1}
	...
	for _, mutate := range []func(*port.ModelQualityScheduleClaimInput){
		...
		func(v *port.ModelQualityScheduleClaimInput) { v.LeaseUntil = v.Now.Add(time.Minute - time.Nanosecond) },
		func(v *port.ModelQualityScheduleClaimInput) {
			v.LeaseUntil = v.Now.Add(port.ModelQualityClaimMaximumLease + time.Nanosecond)
		},
