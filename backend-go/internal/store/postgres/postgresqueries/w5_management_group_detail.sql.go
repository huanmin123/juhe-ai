...
// versions:
//   sqlc v1.31.1
// source: w5_management_group_detail.sql
...
const findManagementGroupDetail = `-- name: FindManagementGroupDetail :one
WITH visible_group AS (
  ...
  WHERE groups.id = $1::text
    ...
    AND groups.system_account_id <> $2::text
)
SELECT
  visible_group.id,
...
type FindManagementGroupDetailParams struct {
	GroupID         string
	SystemAccountID string
}

type FindManagementGroupDetailRow struct {
	ID                      string
	SystemAccountID         string
...
func (q *Queries) FindManagementGroupDetail(ctx context.Context, arg FindManagementGroupDetailParams) (FindManagementGroupDetailRow, error) {
	row := q.db.QueryRow(ctx, findManagementGroupDetail, arg.GroupID, arg.SystemAccountID)
	var i FindManagementGroupDetailRow
	err := row.Scan(
		&i.ID,
		&i.SystemAccountID,
		&i.SystemAccountName,
...
const listManagementGroupDetailAccountIDs = `-- name: ListManagementGroupDetailAccountIDs :many
      ...
      OR (
        ...
        AND EXISTS (
          ...
          WHERE group_authorizations.resource_type = 'group'
            ...
            AND group_authorizations.grantee_system_account_id = $2::text
...
type ListManagementGroupDetailAccountIDsParams struct {
	GroupID         string
	SystemAccountID string
}

func (q *Queries) ListManagementGroupDetailAccountIDs(ctx context.Context, arg ListManagementGroupDetailAccountIDsParams) ([]string, error) {
	rows, err := q.db.Query(ctx, listManagementGroupDetailAccountIDs, arg.GroupID, arg.SystemAccountID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const listManagementGroupDetailAuthorizationSources = `-- name: ListManagementGroupDetailAuthorizationSources :many
...
type ListManagementGroupDetailAuthorizationSourcesParams struct {
	GroupID         string
	SystemAccountID string
}

type ListManagementGroupDetailAuthorizationSourcesRow struct {
	ID              string
...
func (q *Queries) ListManagementGroupDetailAuthorizationSources(ctx context.Context, arg ListManagementGroupDetailAuthorizationSourcesParams) ([]ListManagementGroupDetailAuthorizationSourcesRow, error) {
	rows, err := q.db.Query(ctx, listManagementGroupDetailAuthorizationSources, arg.GroupID, arg.SystemAccountID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []ListManagementGroupDetailAuthorizationSourcesRow
	for rows.Next() {
		var i ListManagementGroupDetailAuthorizationSourcesRow
		if err := rows.Scan(
			&i.ID,
			&i.AuthorizationID,
			&i.SourceType,
