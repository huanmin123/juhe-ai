...
SELECT a.id,a.system_account_id,sa.display_name AS system_account_name,a.name,a.provider_code,a.type,a.status,a.schedulable,a.concurrency_limit,a.priority,a.super_priority_enabled,a.fallback_enabled,a.health_check_model,a.health_check_endpoint_mode,a.account_expires_at,a.last_used_at,a.updated_at,'owner'::text AS access_type,NULL::text AS account_authorization_id,NULL::text AS authorization_status,NULL::timestamptz AS authorization_expires_at
...
UNION ALL
SELECT a.id,a.system_account_id,ga.display_name,a.name,src.provider_code,src.type,a.status,a.schedulable,src.concurrency_limit,a.priority,a.super_priority_enabled,a.fallback_enabled,a.health_check_model,a.health_check_endpoint_mode,a.account_expires_at,a.last_used_at,a.updated_at,'authorized'::text,ra.id,ra.status,ra.expires_at
...
func (q managementAccountListPoolQueries) ListManagementAccounts(ctx context.Context, arg postgresqueries.ListManagementAccountsParams) ([]postgresqueries.ListManagementAccountsRow, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&row.ID, &row.SystemAccountID, &row.SystemAccountName, &row.Name, &row.ProviderCode, &row.Type, &row.Status, &row.Schedulable, &row.ConcurrencyLimit, &row.Priority, &row.SuperPriorityEnabled, &row.FallbackEnabled, &row.HealthCheckModel, &row.HealthCheckEndpointMode, &row.AccountExpiresAt, &row.LastUsedAt, &row.AccessType, &row.AccountAuthorizationID, &row.AuthorizationStatus, &row.AuthorizationExpiresAt, &row.RequestCount, &row.InputTokens, &row.OutputTokens, &row.TotalCost, &row.QualityScore); err != nil {
			return nil, err
		}
