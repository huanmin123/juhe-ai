...
func TestRequireGooseSchemaVersion(t *testing.T) {
	tests := []struct {
		...
		{
			name: "version 59 applied",
			rows: []fakeSchemaVersionRow{
				{version: "59", applied: true},
				{err: pgx.ErrNoRows},
			},
			wantCalls: 2,
		},
		{
			name: "version 60 rollback history resolves to version 59",
			rows: []fakeSchemaVersionRow{
				{version: "59", applied: true},
				{err: pgx.ErrNoRows},
			},
		...
		},
		{
			...
			wantError: "expected 59",
			wantCalls: 1,
		},
		{
			name:      "version 60",
			rows:      []fakeSchemaVersionRow{{version: "60", applied: true}},
			wantError: "expected 59",
			wantCalls: 1,
		},
		{
			name:      "version 59 unapplied",
			rows:      []fakeSchemaVersionRow{{version: "59", applied: false}},
		...
		},
		{
			...
			rows: []fakeSchemaVersionRow{
				{version: "59", applied: true},
				{version: "60", applied: true},
			},
			wantError: "newer applied version 60",
			wantCalls: 2,
		},
		{
			name:      "missing table",
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			err := requireGooseSchemaVersion(t.Context(), querier, 59)
			if test.wantError == "" {
				if err != nil {
...
LIMIT 1`
	...
	if want := []any{int64(59)}; !reflect.DeepEqual(calls[1].args, want) {
