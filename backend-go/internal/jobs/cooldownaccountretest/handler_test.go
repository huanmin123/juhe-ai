...
func TestHandleTaskRejectsInvalidPayload(t *testing.T) {
	err := HandleTask(context.Background(), module.Processor{}, []byte(`{bad`), nil)
	if err == nil {
...
func TestEncodeTaskRequiresStableIdentity(t *testing.T) {
	if _, _, err := EncodeTask(port.CooldownAccountRetestTask{AccountID: "acct", ConfigRevision: 0}); err == nil {
