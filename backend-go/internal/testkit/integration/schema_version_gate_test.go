...
func TestGoServerSchemaVersionGateSmokeUsesGooseForNextVersion(t *testing.T) {
	...
	if strings.Contains(sourceText, "db."+"ExecContext") {
		t.Fatal("next-version setup must use Goose instead of directly writing goose_db_version")
	}
	for _, want := range []string{
		...
		if !strings.Contains(sourceText, want) {
			t.Fatalf("next-version setup source missing %q", want)
		}
	}
}
...
func TestGoServerSchemaVersionGatePostgresSmoke(t *testing.T) {
	...
	if err := store.RequireGooseSchemaVersion(ctx, version.SchemaVersion); err == nil || !strings.Contains(err.Error(), "expected 59") {
...
`
	if err := os.WriteFile(
		filepath.Join(schemaGateMigrationDir, "000060_schema_gate_test.sql"),
		[]byte(schemaGateMigration),
		0o600,
	); err != nil {
		t.Fatalf("write schema gate version 60 migration: %v", err)
	}
	...
	if err := store.RequireGooseSchemaVersion(ctx, version.SchemaVersion); err != nil {
		t.Fatalf("schema gate after version 60 rollback to %d: %v", version.SchemaVersion, err)
	}
	if err := goose.UpTo(db, schemaGateMigrationDir, version.SchemaVersion+1); err != nil {
