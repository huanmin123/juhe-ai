...
func TestRouterRegistersW2ManagementGroupOptions(t *testing.T) {
	...
	for _, path := range []string{"/__aisys__/api/groups/options", "/__aisys__/api/my-groups/options"} {
...
func TestRouterDoesNotRegisterW2ManagementGroupOptionsWhenDisabled(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodGet, "/__aisys__/api/groups/options", nil)
	req.Header.Set("Cookie", "juhe_ai_session=session-token")
	rec := httptest.NewRecorder()
