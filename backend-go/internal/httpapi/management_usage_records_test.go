...
func TestRouterRegistersOnlyManagementAndSelfUsageRecordListRoutes(t *testing.T) {
	service := &managementUsageRecordServiceStub{listResult: managementusagerecords.ListResult{Items: []managementusagerecords.Summary{}, Page: 1, PageSize: 50}}
	...
	for _, path := range []string{"/__aisys__/api/usage-records/usage_1", "/__aisys__/api/my-usage-records/usage_1"} {
		req := httptest.NewRequest(http.MethodGet, path, nil)
		req.Header.Set("Cookie", "juhe_ai_session=session-token")
		rec := httptest.NewRecorder()
		router.ServeHTTP(rec, req)
		if rec.Code != http.StatusNotFound {
			t.Fatalf("retired detail route %s status = %d; body = %s", path, rec.Code, rec.Body.String())
		}
	}
}
