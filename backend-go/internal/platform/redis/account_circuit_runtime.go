...
const (
	DefaultAccountCircuitRuntimeReplayLimit = port.GatewayAccountCircuitRuntimeMaxReplayIDs
...
func (s *AccountCircuitRuntimeStore) GetGatewayAccountCircuit(ctx context.Context, input port.GatewayAccountCircuitGetInput) (port.GatewayAccountCircuitState, error) {
	if err := s.validateIdentity(input.AccountID, input.Scope); err != nil {
		return port.GatewayAccountCircuitState{}, err
	}
...
func (s *AccountCircuitRuntimeStore) SuspectGatewayAccountCircuit(ctx context.Context, input port.GatewayAccountCircuitSuspectInput) (port.GatewayAccountCircuitMutationResult, error) {
	if err := s.validateMutationIdentity(input.AccountID, input.Scope, input.DispatchRevision, input.TransitionID); err != nil {
		return port.GatewayAccountCircuitMutationResult{}, err
	}
...
func (s *AccountCircuitRuntimeStore) AcquireGatewayAccountCircuitConfirmationLease(ctx context.Context, input port.GatewayAccountCircuitAcquireConfirmationLeaseInput) (port.GatewayAccountCircuitMutationResult, error) {
...
func (s *AccountCircuitRuntimeStore) CompleteGatewayAccountCircuitConfirmation(ctx context.Context, input port.GatewayAccountCircuitCompleteConfirmationInput) (port.GatewayAccountCircuitMutationResult, error) {
	...
	if err := validateRuntimeText(input.LeaseID, 256, "lease id"); err != nil || !validRuntimeOutcome(input.Outcome) {
		return port.GatewayAccountCircuitMutationResult{}, fmt.Errorf("account circuit confirmation completion is invalid")
	}
	if input.Reason != "" {
		if err := validateRuntimeText(input.Reason, 1024, "reason"); err != nil {
			return port.GatewayAccountCircuitMutationResult{}, err
...
func (s *AccountCircuitRuntimeStore) AcquireGatewayAccountCircuitCanaryLease(ctx context.Context, input port.GatewayAccountCircuitAcquireCanaryLeaseInput) (port.GatewayAccountCircuitMutationResult, error) {
	if err := s.validateLeaseIdentity(input.GatewayAccountCircuitTransitionIdentity, input.LeaseID, input.LeaseUntil); err != nil {
		return port.GatewayAccountCircuitMutationResult{}, err
	}
...
func (s *AccountCircuitRuntimeStore) CompleteGatewayAccountCircuitCanary(ctx context.Context, input port.GatewayAccountCircuitCompleteCanaryInput) (port.GatewayAccountCircuitMutationResult, error) {
	if err := s.validateTransitionIdentity(input.GatewayAccountCircuitTransitionIdentity); err != nil {
		return port.GatewayAccountCircuitMutationResult{}, err
	}
	if err := validateRuntimeText(input.LeaseID, 256, "lease id"); err != nil || !validRuntimeOutcome(input.Outcome) {
		return port.GatewayAccountCircuitMutationResult{}, fmt.Errorf("account circuit canary completion is invalid")
	}
	if input.Reason != "" {
		if err := validateRuntimeText(input.Reason, 1024, "reason"); err != nil {
			return port.GatewayAccountCircuitMutationResult{}, err
...
func (s *AccountCircuitRuntimeStore) ReplaceGatewayAccountCircuitDispatchRevision(ctx context.Context, input port.GatewayAccountCircuitReplaceDispatchRevisionInput) (port.GatewayAccountCircuitMutationResult, error) {
	if err := s.validateMutationIdentity(input.AccountID, input.Scope, input.DispatchRevision, input.TransitionID); err != nil {
		return port.GatewayAccountCircuitMutationResult{}, err
	}
...
func (s *AccountCircuitRuntimeStore) RestoreGatewayAccountCircuit(ctx context.Context, input port.GatewayAccountCircuitRestoreInput) (port.GatewayAccountCircuitMutationResult, error) {
	if err := s.validateIdentity(input.AccountID, input.State.Scope); err != nil {
		return port.GatewayAccountCircuitMutationResult{}, err
	}
	if err := port.ValidateGatewayAccountCircuitState(input.State); err != nil {
		return port.GatewayAccountCircuitMutationResult{}, err
	}
	if input.State.DispatchRevision < 1 {
		return port.GatewayAccountCircuitMutationResult{}, fmt.Errorf("account circuit restore dispatch revision is invalid")
	}
...
func (s *AccountCircuitRuntimeStore) RecordGatewayAccountCircuitProtocolModelOpenEvidence(ctx context.Context, input port.GatewayAccountCircuitProtocolModelOpenEvidenceInput) (port.GatewayAccountCircuitEscalationResult, error) {
	if input.Scope.Kind != port.GatewayAccountCircuitScopeProtocolModel || input.Generation < 0 || input.DispatchRevision < 1 || input.ConfirmedFailureCount < 1 || input.MaxProtocolScopes < 1 || input.MaxProtocolScopes > port.GatewayAccountCircuitRuntimeMaxEvidenceScopes || input.Window <= 0 || input.Window > port.GatewayAccountCircuitRuntimeMaxEvidenceWindow {
		return port.GatewayAccountCircuitEscalationResult{}, fmt.Errorf("account circuit escalation input is invalid")
	}
	if err := s.validateIdentity(input.AccountID, input.Scope); err != nil {
		return port.GatewayAccountCircuitEscalationResult{}, err
	}
	...
	accountScope := port.GatewayAccountCircuitScope{Kind: port.GatewayAccountCircuitScopeAccount, AccountRuntimeKey: input.Scope.AccountRuntimeKey}
	accountScopeKey, err := port.GatewayAccountCircuitScopeKey(accountScope)
	if err != nil {
		return port.GatewayAccountCircuitEscalationResult{}, err
	}
	...
	if err != nil {
		return port.GatewayAccountCircuitEscalationResult{}, fmt.Errorf("encode account circuit escalation: %w", err)
	}
	...
	if err != nil {
		return port.GatewayAccountCircuitEscalationResult{}, fmt.Errorf("record account circuit escalation: %w", err)
	}
	...
	if err := json.Unmarshal(encoded, &response); err != nil {
		return port.GatewayAccountCircuitEscalationResult{}, fmt.Errorf("decode account circuit escalation: %w", err)
	}
	state, err := runtimeStateFromWire(response.AccountState)
	if err != nil {
		return port.GatewayAccountCircuitEscalationResult{}, err
	}
	status := port.GatewayAccountCircuitEscalationStatus(response.Status)
	switch status {
	case port.GatewayAccountCircuitEscalationRecorded, port.GatewayAccountCircuitEscalationEscalated, port.GatewayAccountCircuitEscalationAlreadyActive, port.GatewayAccountCircuitEscalationIdempotent, port.GatewayAccountCircuitEscalationNotFound, port.GatewayAccountCircuitEscalationStateMismatch, port.GatewayAccountCircuitEscalationStaleGeneration, port.GatewayAccountCircuitEscalationStaleDispatchRevision, port.GatewayAccountCircuitEscalationCapacityExhausted:
	default:
		return port.GatewayAccountCircuitEscalationResult{}, fmt.Errorf("account circuit escalation status is invalid")
	}
	if response.ProtocolScopeCount < 0 || response.ConfirmedFailureCount < 0 {
		return port.GatewayAccountCircuitEscalationResult{}, fmt.Errorf("account circuit escalation result is invalid")
	}
	return port.GatewayAccountCircuitEscalationResult{Status: status, AccountState: state, ProtocolScopeCount: response.ProtocolScopeCount, ConfirmedFailureCount: response.ConfirmedFailureCount}, nil
}

func (s *AccountCircuitRuntimeStore) ClearGatewayAccountCircuitEscalationEvidence(ctx context.Context, input port.GatewayAccountCircuitClearAccountEscalationEvidenceInput) (bool, error) {
...
func (s *AccountCircuitRuntimeStore) ReplaceGatewayAccountCircuitAccountDispatchRevision(ctx context.Context, input port.GatewayAccountCircuitReplaceAccountDispatchRevisionInput) (port.GatewayAccountCircuitAccountRevisionResult, error) {
	if !validAccountCircuitRevisionText(input.AccountID, 256) || input.DispatchRevision < 1 || validateRuntimeText(input.TransitionID, 256, "transition id") != nil {
		return port.GatewayAccountCircuitAccountRevisionResult{}, fmt.Errorf("account circuit account revision input is invalid")
	}
	...
	if err != nil {
		return port.GatewayAccountCircuitAccountRevisionResult{}, fmt.Errorf("replace account circuit dispatch revision: %w", err)
	}
	encoded, err := runtimeRedisBytes(value)
	if err != nil {
		return port.GatewayAccountCircuitAccountRevisionResult{}, err
	}
	var response accountCircuitRuntimeAccountRevisionResponseWire
	if err := json.Unmarshal(encoded, &response); err != nil {
		return port.GatewayAccountCircuitAccountRevisionResult{}, fmt.Errorf("decode account circuit account revision: %w", err)
	}
	status := port.GatewayAccountCircuitMutationStatus(response.Status)
	if status != port.GatewayAccountCircuitMutationApplied && status != port.GatewayAccountCircuitMutationIdempotent && status != port.GatewayAccountCircuitMutationStaleDispatchRevision || response.CurrentDispatchRevision < 1 || response.ClosedScopeCount < 0 {
		return port.GatewayAccountCircuitAccountRevisionResult{}, fmt.Errorf("account circuit account revision result is invalid")
	}
	return port.GatewayAccountCircuitAccountRevisionResult{Status: status, CurrentDispatchRevision: response.CurrentDispatchRevision, ClosedScopeCount: response.ClosedScopeCount}, nil
}

func (s *AccountCircuitRuntimeStore) ListDueGatewayAccountCircuits(ctx context.Context, input port.GatewayAccountCircuitListDueInput) ([]port.GatewayAccountCircuitState, error) {
	if input.Limit < 1 || input.Limit > port.GatewayAccountCircuitRuntimeMaxDuePage {
		return nil, fmt.Errorf("account circuit due limit is invalid")
	}
	value, err := accountCircuitRuntimeListDueScript.Run(ctx, s.client.client, []string{s.keys.states, s.keys.due, s.keys.closed, s.keys.revisions, s.keys.scopeRuntime, s.keys.runtimeScopes, s.keys.runtimeAccounts, s.keys.accountRuntimes, s.keys.indexMeta}, strconv.FormatInt(s.resolveNow(input.Now).UnixMilli(), 10), strconv.Itoa(input.Limit), strconv.Itoa(min(port.GatewayAccountCircuitRuntimeMaxDuePage*4, input.Limit*4)), strconv.Itoa(s.capacity)).Result()
	if err != nil {
	...
	states := make([]port.GatewayAccountCircuitState, 0, len(response.States))
	for _, value := range response.States {
		state, err := runtimeStateFromWire(value)
		if err != nil {
			return nil, err
		}
		if state.Phase != port.GatewayAccountCircuitPhaseOpen && state.Phase != port.GatewayAccountCircuitPhaseRecovering {
...
func (s *AccountCircuitRuntimeStore) runTransition(ctx context.Context, operation string, identity port.GatewayAccountCircuitTransitionIdentity, leaseID string, leaseUntil time.Time, outcome port.GatewayAccountCircuitCompletionOutcome, reason, evidenceScopeKey string) (port.GatewayAccountCircuitMutationResult, error) {
...
func (s *AccountCircuitRuntimeStore) runMutation(ctx context.Context, payload accountCircuitRuntimeMutationWire) (port.GatewayAccountCircuitMutationResult, error) {
	if s == nil || s.client == nil || s.client.client == nil {
		return port.GatewayAccountCircuitMutationResult{}, fmt.Errorf("account circuit runtime store is required")
	}
	if ctx == nil {
		return port.GatewayAccountCircuitMutationResult{}, fmt.Errorf("account circuit runtime context is required")
	}
	raw, err := json.Marshal(payload)
	if err != nil {
		return port.GatewayAccountCircuitMutationResult{}, fmt.Errorf("encode account circuit runtime mutation: %w", err)
	}
	...
	if err != nil {
		return port.GatewayAccountCircuitMutationResult{}, fmt.Errorf("mutate account circuit runtime: %w", err)
	}
	encoded, err := runtimeRedisBytes(value)
	if err != nil {
		return port.GatewayAccountCircuitMutationResult{}, err
	...
	if err := json.Unmarshal(encoded, &response); err != nil {
		return port.GatewayAccountCircuitMutationResult{}, fmt.Errorf("decode account circuit runtime mutation: %w", err)
	}
	...
	status := port.GatewayAccountCircuitMutationStatus(response.Status)
	switch status {
	case port.GatewayAccountCircuitMutationApplied, port.GatewayAccountCircuitMutationIdempotent, port.GatewayAccountCircuitMutationNotFound, port.GatewayAccountCircuitMutationStateMismatch, port.GatewayAccountCircuitMutationStaleGeneration, port.GatewayAccountCircuitMutationStaleDispatchRevision, port.GatewayAccountCircuitMutationLeaseMismatch, port.GatewayAccountCircuitMutationNotDue, port.GatewayAccountCircuitMutationCapacityExhausted:
	default:
		return port.GatewayAccountCircuitMutationResult{}, fmt.Errorf("account circuit runtime mutation status is invalid")
	}
	return port.GatewayAccountCircuitMutationResult{Status: status, State: state}, nil
}
...
func (s *AccountCircuitRuntimeStore) validateIdentity(accountID string, scope port.GatewayAccountCircuitScope) error {
	...
	return port.ValidateGatewayAccountCircuitScope(scope)
}

func (s *AccountCircuitRuntimeStore) validateMutationIdentity(accountID string, scope port.GatewayAccountCircuitScope, dispatchRevision int64, transitionID string) error {
...
func (s *AccountCircuitRuntimeStore) validateTransitionIdentity(identity port.GatewayAccountCircuitTransitionIdentity) error {
...
func (s *AccountCircuitRuntimeStore) validateLeaseIdentity(identity port.GatewayAccountCircuitTransitionIdentity, leaseID string, leaseUntil time.Time) error {
...
func runtimeScopeWire(scope port.GatewayAccountCircuitScope) accountCircuitRuntimeScopeWire {
...
func runtimeStateFromWire(value accountCircuitRuntimeStateWire) (port.GatewayAccountCircuitState, error) {
	...
	scope := port.GatewayAccountCircuitScope{Kind: port.GatewayAccountCircuitScopeKind(value.Scope.Kind), AccountRuntimeKey: value.Scope.AccountRuntimeKey, KeyFingerprint: value.Scope.KeyFingerprint, ProtocolProfile: value.Scope.ProtocolProfile, RequestLane: value.Scope.RequestLane, ModelBucket: value.Scope.ModelBucket}
	openedAt := runtimeWireTime(value.OpenedAtMS)
	retryAt := runtimeWireTime(value.RetryAtMS)
	var lease *port.GatewayAccountCircuitLease
	if value.Lease != nil {
		if value.Lease.LeaseUntilMS <= 0 {
			return port.GatewayAccountCircuitState{}, fmt.Errorf("account circuit runtime lease is invalid")
		}
		lease = &port.GatewayAccountCircuitLease{Kind: port.GatewayAccountCircuitLeaseKind(value.Lease.Kind), ID: value.Lease.LeaseID, Until: time.UnixMilli(value.Lease.LeaseUntilMS).UTC()}
	}
	state := port.GatewayAccountCircuitState{ScopeKey: value.ScopeKey, Scope: scope, Phase: port.GatewayAccountCircuitPhase(value.Phase), Generation: value.Generation, DispatchRevision: dispatchRevision, LedgerRevision: ledgerRevision, TransitionID: value.TransitionID, BackoffAttempt: value.BackoffAttempt, RecoverySuccessCount: value.RecoverySuccessCount, OpenedAt: openedAt, RetryAt: retryAt, FailureReason: value.FailureReason, Lease: lease, HalfOpenOrigin: port.GatewayAccountCircuitPhase(value.HalfOpenOrigin), IncidentID: value.IncidentID, ShadowedByIncidentID: value.ShadowedByIncidentID, ChildIncidentIDs: canonicalRuntimeStrings(value.ChildIncidentIDs), ChildScopeKeys: canonicalRuntimeStrings(value.ChildScopeKeys), RequiredRecoveryScopeKeys: canonicalRuntimeStrings(value.RequiredRecoveryScopeKeys), RecoveryEvidenceScopeKeys: canonicalRuntimeStrings(value.RecoveryEvidenceScopeKeys), UpdatedAt: time.UnixMilli(value.UpdatedAtMS).UTC()}
	if err := port.ValidateGatewayAccountCircuitState(state); err != nil {
		return port.GatewayAccountCircuitState{}, err
	}
	return state, nil
}

func runtimeStateToWire(state port.GatewayAccountCircuitState) (accountCircuitRuntimeStateWire, error) {
	if err := port.ValidateGatewayAccountCircuitState(state); err != nil {
...
func validRuntimeOutcome(value port.GatewayAccountCircuitCompletionOutcome) bool {
	return value == port.GatewayAccountCircuitCompletionFramingComplete || value == port.GatewayAccountCircuitCompletionTransportFailure || value == port.GatewayAccountCircuitCompletionUnknown
}

func mustGatewayAccountCircuitScopeKey(scope port.GatewayAccountCircuitScope) string {
	value, err := port.GatewayAccountCircuitScopeKey(scope)
	if err != nil {
		panic(err)
	}
	return value
...
var _ port.GatewayAccountCircuitStore = (*AccountCircuitRuntimeStore)(nil)
