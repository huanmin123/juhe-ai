...
const (
	...
	signatureDomain = "juhe-ai:model-catalog-snapshot-rebuild:v1\n"
)
...
func NewClient(rawBaseURL, secret string) (*Client, error) {
	return NewClientWithTimeout(rawBaseURL, secret, time.Minute)
}
...
func NewClientWithTimeout(rawBaseURL, secret string, timeout time.Duration) (*Client, error) {
	...
	return &Client{
		endpoint: baseURL.String(), secret: []byte(secret),
...
func (c *Client) Rebuild(ctx context.Context, scope, systemAccountID string) error {
	...
	req.Header.Set("X-Juhe-AI-Signature", signature(c.secret, raw))
...
func signature(secret, raw []byte) string {
