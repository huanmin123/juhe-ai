...
func TestDecideTerminalDisposition(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{name: "transport only remains conservative", input: TerminalDispositionInput{Commit: transportOnly, TerminalKind: TerminalKindReadFailure, Capability: CommittedFailureSignalProtocolEvent}, want: TerminalDisposition{TerminalKind: TerminalKindReadFailure, EmitControlledEvent: true, controlledEventAuthorized: true}},
		{name: "committed precise client event", input: TerminalDispositionInput{Commit: committed, TerminalKind: TerminalKindReadFailure, Capability: CommittedFailureSignalProtocolEvent}, want: TerminalDisposition{TerminalKind: TerminalKindReadFailure, EmitControlledEvent: true, controlledEventAuthorized: true}},
