...
func TestProviderAuthProtocolCatchUpMigrationUpgradesVersion57Databases(t *testing.T) {
	const migrationName = "000058_w2_provider_auth_protocol_schema_20260718.sql"
	source, err := os.ReadFile(migrationPath(migrationName))
	if err != nil {
		t.Fatalf("read %s: %v", migrationName, err)
	}
