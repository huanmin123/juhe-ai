...
- W2 管理端只读辅助接口第一块：`GET /__aisys__/api/proxies/options` 已提供 `juhe_business.proxy_profiles` / `system_sessions` migration、sqlc 查询、store port、PostgreSQL adapter、`managementauth` session 鉴权、`managementproxies` service、HTTP handler 和 router opt-in guard；默认生产 server 不注册该后台路由，只有显式 `JUHE_AI_MANAGEMENT_API_ENABLED=true` 后才允许带 session middleware 灰度挂载。
- 不接管任何现有 Node 业务接口，不删除 Node 旧实现。

W1a / W1b / W2 第一块仍是 Go 实现中，不是生产接管状态；W1b 生产 router 默认不注册 `/__aipublic__`，只有显式设置 `JUHE_AI_PUBLIC_API_ENABLED=true` 才会挂载，且这只表示可灰度验证。W2 `proxies/options` 生产 router 默认不注册，只有显式设置 `JUHE_AI_MANAGEMENT_API_ENABLED=true` 才会带 Go session middleware 挂载；该开关也只表示可灰度验证，不代表反向代理切流、生产流量或 Node 删除已完成。完整 16 个资源级全量契约验证、真实 Docker/testcontainers shell E2E、真实 `w1a-public-settings-smoke`、真实 PG/Redis/Asynq integration、W2 前端 smoke、反向代理切流和 Node 删除证据还未完成。
...
`JUHE_AI_MANAGEMENT_API_ENABLED=true` 只注册当前已迁移的 W2 `GET /__aisys__/api/proxies/options`，并要求请求携带 Node 兼容的 `juhe_ai_session` cookie；登录、登出、验证码、改密和系统账户管理仍未迁移。
