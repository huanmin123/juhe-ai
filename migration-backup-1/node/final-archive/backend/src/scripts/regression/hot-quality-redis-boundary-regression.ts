...
assert.match(snapshotSource, /qualityAttempts = add\(add\(counters\.completedResponses, counters\.localTransportFailures\), counters\.explicitPolicyFailures\)/, '完成率分母只能使用成功、传输失败和显式用户策略，opaque HTTP 只允许诊断')
assert.match(source, /outcome == 'upstream_response_failure' then\s+bucket\['upstreamResponseFailures'\] = increment\(bucket\['upstreamResponseFailures'\]\)\s+elseif/, 'opaque HTTP 只能增加独立诊断计数，不得更新时间或可靠性字段')
assert.match(source, /outcome ~= 'upstream_response_failure'[\s\S]*outcome ~= 'unknown'[\s\S]*outcome ~= 'client_cancellation'/, 'opaque HTTP 不得进入 Redis 首字速度样本')
