...
try {
  ...
  assert.equal(listedRouteStrategy.bindingCount, 1, '策略路由列表应同步返回绑定数量')
  assert.equal(listedRouteStrategy.apiKeyCount, 1, '策略路由列表应同步返回 API Key 数量')
  assert.equal(listedRouteStrategy.groupBindingPreview[0]?.groupId, group.id, '策略路由列表应同步返回分组预览')
