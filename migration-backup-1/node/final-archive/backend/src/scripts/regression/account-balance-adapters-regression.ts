...
assert.deepEqual(parseSub2ApiBalance({ mode: 'quota_limited', remaining: 7.31, unit: 'USD' }), {
  status: 'fresh', remainingUsd: '7.310000', rawRemaining: '7.31', rawUnit: 'usd', basis: 'api_key_quota', scope: 'key'
})
assert.deepEqual(parseSub2ApiBalance({ mode: 'quota_limited', remaining: 0, unit: 'USD' }), {
  status: 'fresh', remainingUsd: '0.000000', rawRemaining: '0', rawUnit: 'usd', basis: 'api_key_quota', scope: 'key'
...
assert.deepEqual(parseSub2ApiBalance({ mode: 'unrestricted', planName: '钱包余额', remaining: '12.5', unit: 'USD' }), {
  status: 'fresh', remainingUsd: '12.500000', rawRemaining: '12.5', rawUnit: 'usd', basis: 'wallet', scope: 'account'
})
assert.deepEqual(parseSub2ApiBalance({ mode: 'unrestricted', planName: '钱包余额', remaining: '-0.25003741', balance: '-0.25003741', unit: 'USD' }), {
  status: 'fresh', remainingUsd: '-0.250037', rawRemaining: '-0.25003741', rawUnit: 'usd', basis: 'wallet', scope: 'account'
}, 'Sub2API 风格钱包透支余额必须保留实际负值')
assert.deepEqual(parseSub2ApiBalance({ mode: 'unrestricted', balance: '3.5', unit: 'USD' }), {
  status: 'fresh', remainingUsd: '3.500000', rawRemaining: '3.5', rawUnit: 'usd', basis: 'wallet', scope: 'account'
}, '兼容仅返回 balance 的钱包响应')
...
assert.deepEqual(parseNewApiBalance({ data: { total_available: 3655000 } }, { quotaPerUnit: 500000 }), {
  status: 'fresh', remainingUsd: '7.310000', rawRemaining: '3655000', rawUnit: 'quota', basis: 'api_key_quota', scope: 'key'
})
assert.deepEqual(parseNewApiBalance({ data: { total_available: 0 } }, { quotaPerUnit: 500000 }), {
  status: 'fresh', remainingUsd: '0.000000', rawRemaining: '0', rawUnit: 'quota', basis: 'api_key_quota', scope: 'key'
}, 'New API 配额耗尽仍是可解析的零余额')
assert.deepEqual(parseNewApiBalance({ data: { total_available: -125000 } }, { quotaPerUnit: 500000 }), {
  status: 'fresh', remainingUsd: '-0.250000', rawRemaining: '-125000', rawUnit: 'quota', basis: 'api_key_quota', scope: 'key'
}, 'New API 透支额度必须保留实际负值')
assert.deepEqual(parseNewApiBalance({ data: { unlimited_quota: true } }, { quotaPerUnit: 500000 }), {
  status: 'unsupported', basis: 'api_key_quota', scope: 'unknown'
})
...
assert.deepEqual(parseOpenAiCompatibleBillingBalance(
  ...
  status: 'fresh', remainingUsd: '7.310000', rawRemaining: '7.31', rawUnit: 'usd', basis: 'api_key_quota', scope: 'unknown'
})
assert.deepEqual(parseOpenAiCompatibleBillingBalance(
  ...
  status: 'fresh', remainingUsd: '0.000000', rawRemaining: '0', rawUnit: 'usd', basis: 'api_key_quota', scope: 'unknown'
...
assert.deepEqual(parseOpenAiCompatibleBillingBalance(
  ...
  status: 'fresh', remainingUsd: '-0.250000', rawRemaining: '-0.25', rawUnit: 'usd', basis: 'api_key_quota', scope: 'unknown'
...
assert.deepEqual(parseOpenAiCompatibleBillingBalance(
  ...
  status: 'unsupported', basis: 'api_key_quota', scope: 'unknown', errorMessage: '上游 API Key 为无限额度，无法确认实际可用余额'
}, '无限额度占位值不能被显示为真实余额')
assert.deepEqual(parseOpenAiCompatibleBillingBalance(
  ...
  status: 'fresh', remainingUsd: '6.073000', rawRemaining: '45.5475', rawUnit: 'cny', basis: 'api_key_quota', scope: 'unknown'
...
assert.deepEqual(parseOpenAiCompatibleBillingStatus({ success: true, data: { quota_display_type: 'TOKENS' } }), {
  snapshot: {
    status: 'unsupported', basis: 'api_key_quota', scope: 'unknown', errorMessage: '上游余额展示单位为 TOKENS，无法安全换算为美元'
  }
})

assert.deepEqual(parseLiteLlmBalance({ info: { max_budget: '10', spend: '2.69' } }), {
  status: 'fresh', remainingUsd: '7.310000', rawRemaining: '7.31', rawUnit: 'usd', basis: 'budget', scope: 'account'
})
assert.deepEqual(parseLiteLlmBalance({ info: { max_budget: '10', spend: '10' } }), {
  status: 'fresh', remainingUsd: '0.000000', rawRemaining: '0', rawUnit: 'usd', basis: 'budget', scope: 'account'
}, 'LiteLLM 预算用尽仍是可解析的零余额')
assert.deepEqual(parseLiteLlmBalance({ info: { max_budget: '10', spend: '10.25' } }), {
  status: 'fresh', remainingUsd: '-0.250000', rawRemaining: '-0.25', rawUnit: 'usd', basis: 'budget', scope: 'account'
}, 'LiteLLM 超额预算必须保留实际负值')
assert.deepEqual(parseLiteLlmBalance({ info: { spend: 2.69 } }), {
  status: 'unsupported', basis: 'budget', scope: 'account'
})

assert.deepEqual(parseUserBalance({ balance: '7.31', is_active: true }), {
  status: 'fresh', remainingUsd: '7.310000', rawRemaining: '7.31', rawUnit: 'usd', basis: 'wallet', scope: 'account'
})
assert.deepEqual(parseUserBalance({ balance: 0, is_active: true }), {
  status: 'fresh', remainingUsd: '0.000000', rawRemaining: '0', rawUnit: 'usd', basis: 'wallet', scope: 'account'
}, '钱包余额耗尽仍是可解析的零余额')
assert.deepEqual(parseUserBalance({ balance: '-1' }), {
  status: 'fresh', remainingUsd: '-1.000000', rawRemaining: '-1', rawUnit: 'usd', basis: 'wallet', scope: 'account'
}, '通用钱包透支必须保留实际负值')
...
), { status: 'fresh', remainingUsd: '7.310000', rawRemaining: '731', rawUnit: 'usd', basis: 'custom', scope: 'unknown' })
assert.deepEqual(parseCustomBalance(
  { total: '10', used: '2.69' },
  { path: '/usage', totalPointer: '/total', usedPointer: '/used' }
), { status: 'fresh', remainingUsd: '7.310000', rawRemaining: '7.31', rawUnit: 'usd', basis: 'custom', scope: 'unknown' })
assert.deepEqual(parseCustomBalance(
  { balance: '-1' },
  { path: '/balance', remainingPointer: '/balance' }
), { status: 'fresh', remainingUsd: '-1.000000', rawRemaining: '-1', rawUnit: 'usd', basis: 'custom', scope: 'unknown' })
