...
import { resolveRouteOwner, validateOwnerManifest } from './validate-owner-manifest.mjs'
...
function resolvedOwners(ownerReferences, route, ownerManifest) {
  return ownerReferences.map(owner => owner === 'manifest'
    ? resolveRouteOwner(ownerManifest, {
        surface: route.ownerClass,
        method: route.method,
        path: route.path
      })
    : owner)
...
function assertOwnerCutoverMatrix(candidate, ownerManifest) {
  ...
  for (const route of candidate.routes) {
    ...
    assert.equal(
      resolveRouteOwner(ownerManifest, { surface: route.ownerClass, method: route.method, path: route.path }),
      candidate.baselineOwner,
      `${routeKey(route)} manifest owner must match fixture baselineOwner`
    )
...
const overriddenRoute = fixture.routes[0]
const routeOverrideManifest = validateOwnerManifest({
  ...manifest,
  routeAllowlist: [
    ...manifest.routeAllowlist,
    {
      surface: overriddenRoute.ownerClass,
      method: overriddenRoute.method,
      path: overriddenRoute.path,
      owner: 'go',
      rollbackOwner: 'node'
    }
  ]
...
assert.throws(
  () => assertOwnerCutoverMatrix(fixture, routeOverrideManifest),
...
for (const route of fixture.routes) {
  ...
  const baseline = resolveRouteOwner(manifest, {
    surface: route.ownerClass,
    method: route.method,
    path: route.path
  })
