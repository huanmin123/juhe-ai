...
if (!binaryPath || !backendRoot || !logPath) {
  throw new Error('Usage: node scripts/start-go-sidecar.mjs <binary-path> <backend-root> <log-path>')
}
...
const names = [
  ...
  'JUHE_AI_AUDIT_LOG_BUSINESS_SETTINGS_PATH',
  'JUHE_AI_AUDIT_LOG_BUSINESS_SETTINGS_URL',
  'JUHE_AI_AUDIT_LOG_POSTGRES_SCHEMA',
  'JUHE_AI_AUDIT_LOG_INSTANCE_ID',
  'JUHE_AI_AUDIT_LOG_OWNER_LEASE',
  'JUHE_AI_AUDIT_LOG_RETENTION_INTERVAL',
  'JUHE_AI_AUDIT_LOG_RETENTION_BATCH_SIZE',
...
try {
  ...
  if (!child.pid) throw new Error('Unable to start juhe-ai-go-sidecar.')
  child.unref()
