...
function parseCanonicalPath(routePath, label, { allowTemplates }) {
  ...
  if (routePath.includes('*') || routePath.includes(':')) fail(`${label} must not contain unsafe wildcards`)
...
function pathTemplatesOverlap(leftSegments, rightSegments) {
  ...
  return leftSegments.every((left, index) => {
    ...
    return isParameterSegment(left) || isParameterSegment(right) || left === right
  })
}
...
function requestPathMatchesTemplate(requestPath, templatePath) {
  ...
  return templateSegments.every((segment, index) => isParameterSegment(segment) || segment === requestSegments[index])
}
