# BUG-0011 非 LTS 内置 SQLite 缺少 FTS5
...
- 现象：其他电脑运行 `pnpm -r --parallel dev` 时，前端正常启动，后端在记录库建表阶段报 `Error: no such module: fts5`。
- 期望：后端启动前明确拒绝不符合要求的 Node.js 运行时，并提示使用官方 LTS 与完整 SQLite 能力。
- 实际：旧预检只验证 `node:sqlite` 能否导入，Node.js v23.11.0 可以通过导入检查，但内置 SQLite 缺少项目需要的 FTS5 能力。
- 影响范围：使用非 LTS Node.js 或自带 SQLite 编译能力不完整的环境，尤其是日志搜索表 `runtime_log_search` 所需的 FTS5 / trigram tokenizer。
...
3. 预检通过后，`applyRecordSchema` 创建 `runtime_log_search` 虚拟表时报 `no such module: fts5`。
...
- 是否稳定复现：在缺少 FTS5 的运行时稳定复现。

## 根因分析

- 表象：SQLite 建表时报 `no such module: fts5`。
- 真实根因：项目依赖 Node 内置 SQLite 的 FTS5 虚拟表和 trigram tokenizer，但旧运行时预检只检查 `node:sqlite` 导入能力，没有实际验证 FTS5 建表能力；文档和 `engines` 还允许了非 LTS 的 23.x。
...
- 修改点：后端预检使用内存 SQLite 真实创建 `fts5(... tokenize = 'trigram')` 表并执行一次查询，提前发现 FTS5 / tokenizer 缺失。
...
- 兼容影响：非 LTS Node.js 和缺少 FTS5 的 Node.js 会更早失败；当前支持 22.x LTS（>=22.13.0）和 24.x LTS（>=24.11.0）。
...
| --- | --- | --- | --- | --- | --- |
| 运行时检查 | 当前环境是 LTS，且支持 `node:sqlite`、FTS5 和 trigram tokenizer | `pnpm --filter juhe-ai-backend check:runtime` | 通过 | 通过 | 已通过 |
...
- 不要只用 `import 'node:sqlite'` 判断运行时是否可用；必须实际验证 FTS5 / trigram tokenizer。
