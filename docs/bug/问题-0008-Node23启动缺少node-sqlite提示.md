...
- 系统 / Node 版本：Node.js v23.0.0 稳定复现；后续在 BUG-0011 中已改为只推荐官方 Node.js LTS，并补充 SQLite 能力预检。
- 是否稳定复现：是。
...
- 行为影响：不改变业务运行逻辑；不支持 `node:sqlite` 的环境会更早失败，并给出明确升级提示。
- 发布异常处理：如预检误判，修正当前预检脚本和版本文档；不恢复已淘汰的 Node 版本要求。
