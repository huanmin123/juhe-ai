...
| 编号 | 标题 | 状态 | 严重程度 | 模块 | 文档 |
| --- | --- | --- | --- | --- | --- |
| BUG-0117 | 旁路数据写入放大导致事件循环与后台任务停滞 | 临时止血完成；长期候选已隔离验收，生产切换待确认 | P0 | 后端 / 网关 / ingest-worker / 审计 / 运行日志索引 / PostgreSQL | [问题-0117-旁路数据写入放大导致事件循环与后台任务停滞.md](问题-0117-旁路数据写入放大导致事件循环与后台任务停滞.md) |
