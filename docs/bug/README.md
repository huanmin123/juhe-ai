...
- [BUG-0134](问题-0134-模型检测基础连通性重复计分.md)：Responses 非流式基础连通预检被误当作模型能力指标计分，可信对照原始分数还会污染目标结论。
- [BUG-0133](问题-0133-AI账户余额未查询时只显示刷新图标.md)：余额尚无快照、首次查询或暂时失败时隐藏状态文字，只剩无上下文的刷新图标。
- [BUG-0132](问题-0132-AI账户人工隔离与停用操作图标重复.md)：人工隔离和停用账户使用相同暂停图标，颜色与排列也无法区分临时隔离和永久停用。
- [BUG-0131](问题-0131-CodexResponses响应防护编辑时不可见.md)：账户编辑错误重算客户端兼容模式，导致 Codex Responses 响应防护区域和已保存开关不可见。
- [BUG-0130](问题-0130-成本优先误用首字截止且默认值过短.md)：成本优先误用速度首字截止，且速度优先默认值仍为 10 秒。
- [BUG-0129](问题-0129-审计traceId搜索叠加缓存筛选导致记录不可见.md)：审计列表手工 traceId 被缓存的用户、账户、结果、路径等高级筛选遮蔽，真实记录无法直接定位。
- [BUG-0128](问题-0128-Codex压缩被网关多层时限误杀.md)：Codex compact 误用普通文本总墙钟、首字、首响应和未提交生命周期，Chat 摘要改写后还会丢失压缩身份。
...
| 编号 | 标题 | 状态 | 严重程度 | 模块 | 文档 |
| --- | --- | --- | --- | --- | --- |
| BUG-0134 | 模型检测基础连通性重复计分 | 已修复（定向回归通过，待完整隔离预演） | P1 | 后端 / 前端 / 模型检测 / 质量处罚 | [问题-0134-模型检测基础连通性重复计分.md](问题-0134-模型检测基础连通性重复计分.md) |
| BUG-0133 | AI 账户余额未查询时只显示刷新图标 | 已修复（待完整隔离预演） | P2 | 前端 / 我的 AI 账户 / 余额查询 | [问题-0133-AI账户余额未查询时只显示刷新图标.md](问题-0133-AI账户余额未查询时只显示刷新图标.md) |
| BUG-0132 | AI 账户人工隔离与停用操作图标重复 | 已修复（待完整隔离预演） | P2 | 前端 / AI 账户 / 操作列 | [问题-0132-AI账户人工隔离与停用操作图标重复.md](问题-0132-AI账户人工隔离与停用操作图标重复.md) |
| BUG-0131 | Codex Responses 响应防护编辑时不可见 | 已修复（待完整隔离预演） | P1 | 前端 / AI 账户 / Codex Responses | [问题-0131-CodexResponses响应防护编辑时不可见.md](问题-0131-CodexResponses响应防护编辑时不可见.md) |
| BUG-0130 | 成本优先误用首字截止且默认值过短 | 已修复（待完整隔离预演） | P1 | 后端 / 网关 / 策略路由 / 前端 | [问题-0130-成本优先误用首字截止且默认值过短.md](问题-0130-成本优先误用首字截止且默认值过短.md) |
| BUG-0129 | 审计 traceId 搜索叠加缓存筛选导致记录不可见 | 已修复（待完整隔离预演） | P1 | 前端 / 审计日志 / 筛选状态缓存 | [问题-0129-审计traceId搜索叠加缓存筛选导致记录不可见.md](问题-0129-审计traceId搜索叠加缓存筛选导致记录不可见.md) |
| BUG-0128 | Codex 压缩被网关多层时限误杀 | 已修复（定向回归通过，待完整隔离预演） | P1 | 后端 / 网关 / Codex Responses / 超时 / 跨协议 | [问题-0128-Codex压缩被网关多层时限误杀.md](问题-0128-Codex压缩被网关多层时限误杀.md) |
