...
- 注册表按精确 worker 副本和 Gateway/DB service 同 ID 配对核验完整性；官方 macOS 安装先逐个升级并确认 Gateway/DB service 注册，最后升级 control/worker 并确认完整拓扑。每个服务在 `bootout` 后从 Redis 读取 freshness fence，并把样本 PID 绑定到健康响应中的本次 main、DB service 和 worker PID；旧 lease 和旧进程在途写都不会提前放行。

## 防复发

- 回归测试模拟大量无关业务 key，覆盖默认 14 角色、合法最大 132 角色、新旧双版本 264 成员、600 次实例 churn 与当前拓扑恢复。
- 测试夹具在收到 `SCAN` 时直接失败，保证实现不会静默退回全键空间扫描。
- 授权测试 Redis smoke 真实执行 Lua，验证 20 秒样本 TTL、60 秒索引 TTL、偏差一小时的 payload 由 Redis 时间覆盖、过期成员清理、512 成员上限、旧 lease 拒绝、错误 PID 拒绝、新样本放行与清理零残留。
...
- [验证报告](../reports/performance进程指标活跃索引修复验证报告-2026-07-27.md)
