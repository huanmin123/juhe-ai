...
客户端策略是运行时附加层，不作为供应商或协议档案的用户侧选择项。`client_compatibility` / client profile 用于描述 Codex、Claude Code、普通 OpenAI SDK、普通 Anthropic SDK 等客户端期望的重试、请求体归一化和 SSE 失败事件语义；这些行为只能由入口、请求上下文、端点族和账户档案共同识别，不能写进通用协议层。
