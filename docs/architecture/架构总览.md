...
- 运行形态：默认单机部署下，Web/网关主进程、`worker` 控制 / fallback 进程、`metrics-worker`、`ingest-worker`、`stats-worker`、`snapshot-worker`、`probe-worker`、`maintenance-worker` 和本地 DB service 进程分离；使用记录、审计、操作日志、公开接口日志、运行日志索引、模型检测记录、记录维护和数据保留期 dataset / usage shard 清理由 `ingest-worker` 执行；系统采样、增量统计、重窗口快照、账号质量、表监控和统计保留期清理由 `stats-worker` 执行；`metrics-worker`、`snapshot-worker` 保留为常驻隔离角色但不能直接写统计库；正常账号健康检测、账号 / 代理 / OAuth 探测和手动 AI 账户测试必须在 `probe-worker` 内执行；业务库维护写回由 `maintenance-worker` 生产 typed command 后交给 DB service 执行，系统管理 API 与网关业务库读写由 DB service 承接。SQLite standalone 模式下，DB service 只拥有业务库写锁；在 DB service 内触发的数据集目录库或统计结果库写入必须转发给 ingest / dataset writer 或 stats writer，不能在管理路由中直接写非所属库。PostgreSQL performance 模式下，typed command、优先级、重试和背压语义保留，但消费端可以按连接池和同 key 顺序约束并发执行，默认最大消费并发为 100。分布式模式作为显式配置能力，新增控制面 leader、节点心跳、用户 home node 路由和节点内部转发；具体边界见 [分布式部署与用户分片设计](../functions/分布式部署与用户分片设计.md)。
...
- 模型目录与价格是网关调度和计费的前置事实：内置模型来自官方目录，所有系统账户可见；自定义模型只归属创建者自己的系统账户，管理员创建也不会变成全局模型。`/v1/models`、账号支持模型、模型检测候选、账号模型映射和成本估算都必须从调用方可见的本地模型目录读取。普通 OpenAI-compatible `/v1/models` 返回标准 `object/list + data`，Codex 模型刷新请求返回同一目录派生出的 `{ models: ModelInfo[] }`，这是客户端响应形态分支，不新增模型目录来源。普通跨供应商 API Key 请求带 `model` 时，先在该 Key 已授权账号池内按账号映射匹配 `sourceModel + sourceEndpointFamily`；映射未命中时，再用请求 `model` 在该 Key 已绑定供应商范围内做有界目标档案收窄。模型名唯一性按调用方可见目录判断，不能通过个人模型覆盖其他用户目录。账号模型限制在候选账号池内参与账号候选排序：直接支持请求模型的账号优先，其次是模型映射命中的账号，最后是未声明模型限制的账号；账号模型映射命中后把客户端模型和协议改写为上游模型和协议，并在使用记录中同时保留下游模型、上游模型和实际计价模型。
