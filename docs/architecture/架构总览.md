...
当客户端协议和上游真实能力不一致时，协议转换必须作为共享 adapter 落在 provider driver 可复用边界内，而不是写成某个供应商的私有分支。跨协议转换由混合供应商账户触发：该账户保存自己的真实上游凭据、Base URL、真实上游协议、允许的下游协议入口和下游模型到真实上游模型的映射；它不指向其他账户或分组。普通账号 `modelMappings` 只保存当前账号内同协议模型别名。长期设计按 [协议桥接框架设计](../functions/协议桥接框架设计.md) 收敛为“下游协议 -> 统一生成中间表示 -> 上游协议”，新增独立协议时默认补一次入站 adapter 和一次出站 adapter，不能继续扩散成 N×N 私有转换。
...
- 策略路由是 API Key 和分组之间的独立层。API Key 只绑定一条路由策略；路由策略再绑定一个或多个分组，并按普通路由、混合智能路由、权重调度、故障回退或轮询选择候选分组。普通路由只能绑定一个分组，且不开放复杂配置。每个系统账户必须有一条不可删除的默认策略路由，默认路由属于策略路由层，不属于 API Key 或分组字段。
