...
- 客户端模型目录不再提供匿名公共聚合：`/models`、`/v1/models` 与各原生协议模型目录都必须先完成本地 Gateway API Key 认证，再按该 Key 的有效路由分组聚合本地供应商目录。上游模型目录是账户编辑时的显式补全数据源，不是新的本地模型目录事实来源；只有用户点击同步才可追加“上游可见 ∩ 本地可见且协议兼容”的模型，并且必须保留用户手动选择。人工测试、激活、周期检查与恢复探针只执行真实协议请求，不做上游目录预检。
...
会话识别与客户端画像绑定，不提供任意客户端通吃的通用 Header：Codex 画像只认 `session-id`，Claude Code 画像只认 `x-claude-code-session-id`，且仅解析请求头。识别结果统一写入审计 `session_id` / `session_client_type`，内部亲和键按客户端 namespace、租户、API Key、路由、分组和供应商协议档案池隔离；没有已支持的官方会话 Header 时正常调度，但不承诺跨请求粘黏。详细契约见 [统一会话 Header 与审计串联设计](../functions/统一会话身份与审计串联设计.md)。
...
当前检查边界：`/models`、`/v1/models` 或任何供应商目录路径不可用、返回 `404` 或不开放时，不能阻断保存后的人工测试、草稿测试、激活、周期健康、运行态恢复、冷却恢复、Key 恢复或调度。所有这些自动检查都以账户配置的真实生成或图片 endpoint 的协议成功为准；目录受控转发只服务用户显式同步操作，客户端 `/models` 始终由本地目录响应。
...
人工测试默认只生成诊断任务、使用记录和审计，不作为健康检查、激活或配置保存凭证。唯一例外是已保存自有物理账户的列表测试：真实协议完成成功，且任务模型与精确请求形态分别等于该账户保存的 `healthCheckModel` 和 `healthCheckEndpointMode`，当前持久 `schedulable = false`、未停用、未质量隔离、未过期并满足时间计划时，才可在测试任务终态与账户 `configRevision` 同时匹配后恢复调度资格。草稿、授权实例、模型或请求形态不匹配的任务保持无状态；恢复只清理已有不可用运行态，不写入新的健康检查成功事实。
...
- 当前账户检查补充：无论文本还是图片模型，人工测试、激活、周期健康和恢复都执行实际 endpoint 并按协议成功判定；上游模型目录只服务用户显式同步。目录不可用、`404` 或响应不兼容不能改变账户测试、激活、恢复或调度结果。
...
- 分组和分组之间保持容器隔离。策略路由可以在真实派发前按模式选择或切换候选分组，但仍不做跨分组账号混排、跨分组借号、共享分组队列或共享分组等待状态；真实派发后，各类请求在 `semanticCommitted = false` 且共享墙钟 / attempt 预算允许时，都可因 opaque 非 `2xx`、本地 transport failure、timeout、正文中断或精确协议声明的失败结构按当前候选账号和后续分组继续接管。完整 `2xx` / 普通 payload 透明；真实协议事件、正文或可见输出已经交付后禁止拼接第二次结果。配置化状态动作只改变其声明的状态，不参与自动切号准入。
...
当前对外主入口是 OpenAI 兼容网关，支持根路径 `/*` 和 `/v1/*`；系统后台、管理 API、健康检查和静态资源统一在 `/__aisys__/*` 下。主链路固定为认证、额度与路由、候选能力过滤、上游请求、响应转发、使用记录和审计。通用 `generic_*` 客户端默认不解释错误对象、错误码、错误类型、完成状态或正文；所有请求的完整非 `2xx` 只按 `response.ok` 进入请求级 opaque 接管，同账户当前可执行 Key 按池顺序唯一尝试，再按候选、墙钟和整个请求最多 64 次真实上游 attempt 的安全上限切换账户 / 分组。图片、音频、文件、资源创建、后台任务和 hosted tool 与普通文本使用同一候选切换准入；transport、timeout 和正文中断在 `semanticCommitted = false` 时同样继续扫描候选。精确 Codex、Claude Code、Gemini CLI 客户端只解释各自声明的协议结构；普通事件里的 `data.error` 不等于结构失败。上游状态码、错误码、错误类型、正文和事件 payload 均不可信，普通业务失败不得据此触发账户或 Key 共享语义状态；只有用户显式策略或独立后台探针可以执行对应状态动作，详见 [AI 账户错误语义与状态变更边界](../functions/AI账户错误语义与状态变更边界.md)。统一候选切换采用有界 at-least-once，产品接受小概率重复生成或重复计费以换取可用性。`ServerRetryBudget` 默认 270 秒，只累计零可派发、FIFO / 并发槽和受控半开等待；文本整请求墙钟固定默认 270 秒，图片整请求墙钟由 `imageRequestWallTimeoutSeconds` 控制且默认 3600 秒，图片另用 600/120/3600 秒单次时限。普通路由 `cost_first / speed_first` 的文本首 token 截止、慢样本与 `latency_degraded` 不作用于图片。客户端重试是新请求，必须按重试时的优先级、质量、速度、可用状态和轮换游标重新选号，不延续上一次候选游标。

GPT / OpenAI-compatible API Key 账户可以在同一个账户凭据内保存多个上游 API Key。账户内 Key 策略只在系统已经选中该账户后执行，默认轮询，也可按 Key 权重做平滑加权轮询；它不参与分组选号、账户候选排序、授权判断或账户并发边界，因此不会把每个上游 Key 扩展成独立 AI 账户。对允许有界重放的文本推理请求，当前请求按账户内池顺序唯一尝试所有当前可执行 Key，不以固定两个 Key 截断；跨账户重复的同一物理凭据在本请求只发送一次，并受整个请求 64 次真实 attempt、墙钟和最终响应保留时间共同限制。未知完整 HTTP 失败只把 Key 加入本请求排除集合，不写全局 Key 状态；本地解密/装配失败或用户显式 Key 动作才可进入 Key 级运行态。新客户端请求清空请求级 Key 游标并按当时池状态重新选择。账户整体是否可承接由账户事实和账户内已确认可用 Key 数量共同派生，具体设计见 [账户内 API Key 故障隔离设计](../functions/账户内APIKey故障隔离设计.md)。
...
网关流式转发必须增量处理并尊重 backpressure。下游状态拆为 `transportCommitted` 和 `semanticCommitted`：零可派发账号等待期间可发送 SSE 注释心跳，真实协议事件、正文或可见输出写出前，各类请求都允许在预算内从失败 attempt 切换到下一候选；语义提交后禁止拼接第二条流。通用客户端原样转发上游 SSE 事件和 EOF，不执行响应语义检查；精确客户端只把协议声明的失败事件结构解释为本次 attempt 失败，普通事件中的 `data.error` 保持 payload 中性。文本 / 图像单次 attempt 超时默认分别为 `120/30/1800` 与 `600/120/3600` 秒；图片继续使用 600 秒首响应、120 秒 raw chunk idle 和 3600 秒未提交寿命，配置可覆盖 10–15 分钟生成场景。图片 attempt 超时、断尾或正文读取失败且尚未语义提交时，按统一规则切换 Key、账户或分组；已经写出真实内容时只能结束或断流，不能改写既有响应。活跃且已提交语义的 SSE 不受未提交 attempt 寿命机械中断。
