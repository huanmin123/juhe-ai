...
- 登录页标题由系统名称派生为“系统名称 + 管理平台”；登录页角标、副标题和视觉样式属于前端设计，详见 [产品与品牌边界](frontend/产品与品牌边界.md)。
...
- 系统名称和系统图标路径由全局设置控制，只有管理员能改；登录页文案和样式写在 [产品与品牌边界](frontend/产品与品牌边界.md) 中。
...
账户列表第一期展示：账户名称、账户类型、供应商、并发数、账户到期时间、状态、用量情况、优先级、最近使用时间、操作。管理员视角额外展示 `系统账户` 列；用户视角自有账户不展示归属列，授权账户显示“来自授权”和所有者摘要。账号优先级按数字从小到大生效，`0` 优先于 `10`；同分组调度时先尝试优先级更高的账号，当前账号失败或进入冷却后再切换到下一个可用账号。`Access/API Key` 与 `Refresh Token` 不在列表展示；只有账户所有者可在编辑弹窗里查看和修改，授权账户永不返回完整敏感凭据。状态语义统一为：正常、停用、错误、限流中、临时不可调用。

账户套餐到期规则：`account_expires_at` 为空表示不设置本地套餐到期；一旦到期，后端把账户改为 `disabled`、`schedulable = 0`、清空冷却时间并写入到期原因。网关调度、后台 OAuth 额度探测、成功/失败恢复逻辑都不能重新启用已到期账户，只有用户清空或延后 `account_expires_at` 后才允许再次启用。
...
- `global_settings` 只保存系统名称和系统图标路径；登录页角标、副标题和视觉样式按 [产品与品牌边界](frontend/产品与品牌边界.md) 固定。
...
透传在产品上只提供一个账号级开关：关闭时走常规 OpenAI 兼容中转，开启时尽量原样转发客户端请求和请求头，不做 body 填充、默认内容追加或无关改写。

后端内部仍保留必要的网关职责：校验本地 API Key、按分组调度账号、替换上游认证、剔除协议安全头和明显代理链路头、隔离会话标识、稳定转发 SSE 流、记录使用情况并按错误策略兜底切换。这些是服务端实现细节，不在 UI 暴露成多套选项。

更完整的源码对照和透传定位见 `docs/functions/中转透传机制调研与定位修正.md`。
