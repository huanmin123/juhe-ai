...
- 对外中转协议：客户端入口按 OpenAI-compatible 形态暴露，支持根路径和 `/v1`；`openai/v1` 是协议层语义，`provider_code = openai` 是通用 OpenAI-compatible 供应商语义，二者通过字段所在层级区分。当前内置供应商为 `openai`、`gpt`、`anthropic` 和 `deepseek`；`anthropic` 官方直连供应商使用原生 `anthropic/v1` Messages 协议，不能和第三方 Anthropic-compatible 入口混在一个档案里；目标新增 `glm` 智谱供应商，通用 GLM API 与 GLM Coding Plan 都按 OpenAI Chat Completions 兼容形态接入，但必须拆成不同供应商协议档案；`deepseek` 供应商默认接入 OpenAI-compatible Chat Completions 档案，并目标新增 DeepSeek Anthropic-compatible Messages 档案，二者按不同 `provider_protocol_profile_id` 和账户接入类型区分。`gpt.parent_code = openai`，继承通用 OpenAI v1 能力并叠加 GPT / Codex 专属能力。通用 OpenAI 兼容 API Key 账号、GPT API Key 账号、Anthropic 官方 API Key 账号、目标 GLM API Key 账号和 DeepSeek API Key 账号按各自上游入口透传，GPT OAuth 账号只支持 Codex adapter 明确列出的 Responses 路径；Anthropic OAuth / Setup Token / Claude Code token 账号链路不纳入当前目标，Claude Code 只作为下游客户端画像，可在官方 Anthropic API Key 或 DeepSeek Anthropic-compatible API Key 的 `anthropic/v1` 档案内生效，但不能让两个供应商共用账号池、默认分组、模型目录或价格目录；模型目录上，`openai` 作为通用 OpenAI-compatible 聚合目录，仅聚合 GPT 等显式纳入的 OpenAI-compatible 子供应商和自身模型；`gpt`、`anthropic`、`glm` 和 `deepseek` 都只看各自供应商模型，DeepSeek 不进入 `openai` 聚合目录。
...
| 供应商 | `provider_code` | 表示账号池、模型目录和价格目录归属；当前内置 `openai` 通用 OpenAI-compatible 供应商、`gpt` 子供应商、`anthropic` 官方 Claude 供应商和 `deepseek` 供应商，目标新增 `glm` 智谱供应商。`openai` 同时可以是协议代码和供应商代码，但必须通过 `protocol_code` / `provider_code` 字段区分。 |
| 供应商协议档案 | `provider_protocol_profile_id` | 把供应商和协议版本绑定成一个可调度能力档案，例如 `profile_gpt_openai_v1`。账户、分组和账号测试都必须落到该档案。 |

客户端策略是运行时附加层，不作为供应商或协议档案本身的替代项。`client_compatibility` 是账户级能力配置，用于明确该账号可承接哪些客户端请求形态；client profile 和请求侧 `requestClientCompatibility` 用于描述 Codex、Claude Code、普通 OpenAI SDK、普通 Anthropic SDK 等客户端期望的重试、请求体归一化和 SSE 失败事件语义。候选账号筛选和上游请求准备同时看账户兼容能力与请求侧兼容能力，这些行为只能由入口、请求上下文、端点族和账户档案共同识别，不能写进通用协议层。GLM Coding 这类需要协议转换的账号必须通过页面或导入协议显式选择 `codex_responses`，选择 `openai_standard` 时即使 profile 支持桥接也不承接 Codex。

当客户端协议和上游真实能力不一致时，协议转换必须作为共享 adapter 落在 provider driver 可复用边界内，而不是写成某个供应商的私有分支。例如 Codex 客户端只发 OpenAI Responses，但 GLM Coding、DeepSeek 等上游可能只支持 OpenAI-compatible Chat Completions；这类场景由通用 “Codex Responses -> Chat Completions” adapter 承担字段和 SSE 事件转换，具体供应商只配置可选启用条件、默认模型、Base URL 拼接、模型映射和供应商局部差异。账户 `client_compatibility` 决定是否允许该账号进入桥接，账户 `supported_endpoint_modes` 仍保存真实上游能力，例如 `chat_sse`，不要为了桥接把账号伪装成原生 `responses_sse`。

当前已落地三个 OpenAI v1 档案和一个官方 Anthropic v1 档案，目标新增一个 DeepSeek Anthropic-compatible v1 档案：`profile_openai_openai_v1` 表示通用 `openai` 供应商使用 OpenAI v1 协议，承载 API Key 透传、OpenAI v1 聚合模型目录和通用协议策略；`profile_gpt_openai_v1` 表示 `gpt` 子供应商使用 OpenAI v1 协议，继承通用 OpenAI-compatible 能力，并叠加 GPT OAuth、Codex Responses、Codex 客户端策略和 GPT 自身模型目录优化；`profile_deepseek_openai_v1` 表示 DeepSeek OpenAI-compatible Chat Completions 档案，默认 Base URL 为 `https://api.deepseek.com`；`profile_deepseek_anthropic_v1` 表示 DeepSeek Anthropic-compatible Messages 档案，默认 Base URL 为 `https://api.deepseek.com/anthropic`，使用 `anthropic/v1` 协议语义但仍归属 `provider_code = deepseek`；`profile_anthropic_anthropic_v1` 表示官方 Anthropic Claude 原生 `anthropic/v1` Messages 档案，当前只允许 `api_key` 账号类型，Anthropic OAuth / Setup Token / Claude Code token 账号链路不纳入当前目标，Claude Code 下游客户端画像通过显式本地 header 识别。目标新增 GLM 时，`profile_glm_general_openai_v1` 表示智谱通用 GLM API 的 OpenAI Chat Completions 档案，`profile_glm_coding_openai_v1` 表示 GLM Coding Plan 的 OpenAI Chat Completions 档案；二者都属于 `provider_code = glm`，但默认 Base URL、分组、额度解释和账号测试边界不同。后续即使新增另一个同样使用 OpenAI v1 或 Anthropic v1 的供应商，也必须新增自己的 `provider_code` 和 `provider_protocol_profile_id`，不能和 GPT、Anthropic、GLM 或 DeepSeek 共用账号池、分组或价格目录；是否进入 `openai` 聚合目录必须由模型目录方案显式声明，DeepSeek 当前不进入 `openai` 聚合目录。官方 Anthropic 原生档案只进入自身供应商目录，DeepSeek Anthropic-compatible 只进入 DeepSeek 目录，二者都不进入 `openai` 聚合目录。
...
- DB service 不改变 SQLite 单写者边界；业务库写入由 DB service 同步执行并依赖 WAL、短事务和 `busy_timeout` 控制短写锁等待。统计数据集域高频写入从请求链路剥离到 background writer，其中 usage 写入 usage shard，非 usage 明细写入数据集目录库；统计结果库写入由 stats-worker typed operation 串行提交，避免 `metrics-worker`、`snapshot-worker` 和 `maintenance-worker` 抢同一个 stats 文件。usage shard 只能由 ingest 写入；统计聚合只读取 shard，不再把派生估算字段回写原始 shard，同一个 shard 文件不得拥有多个并发 writer。
