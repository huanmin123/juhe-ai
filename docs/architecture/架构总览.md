...
- 静态模型发布目录不属于账户调度运行态。API Key 不绑定或复制模型列表，只在认证后定位系统账户；模型目录页面的发布开关在写时生成 `system_account + protocol + variant` 持久化最终响应快照，并镜像到 Redis。动态账户、余额、探针、OAuth、Key 和代理状态变化不得重建或清理该快照；主进程请求路径只读单个快照，不扫描模型事实表、不按供应商 fan-out，也不承担快照重建。
