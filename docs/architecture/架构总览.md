...
- 运行形态：默认单机部署下，Web/网关主进程、`ingest-worker`、`stats-worker`、`ops-worker`、独立 Go `juhe-ai-runtime-log-indexer`、独立 Go `juhe-ai-table-monitor` 和本地 DB service 进程分离；默认 `worker` 不再作为 supervisor 常驻业务角色，仅保留统一进程角色命名和控制入口语义。`ingest-worker` 承载使用记录、审计、操作日志、公开接口日志、记录维护和 dataset / usage shard 保留期清理；Go F1 indexer 独占运行日志文件导入、索引、cursor、facet 和保留清理；Go F2 table monitor 独占表空间采样、快照写入和 F2 retention；`stats-worker` 承载系统采样、增量统计、重窗口和账号质量，但不再写 F2 快照；`ops-worker` 承载账号测试、正常账号健康检测、冷却复测、代理检测、OAuth token 保活、可用性时间计划同步、授权到期扫描和过期删除账号清理协调。SQLite standalone 模式下，DB service 只拥有业务库写锁；数据集、运行日志索引、使用记录目录库、usage shard、统计结果库和 F2 专用监控库分别由唯一 writer 持有，Node 表监控查询固定 query-only。确认安全的管理端和兼容接口纯读进入 DB service 下的 query-only SQLite read worker 子进程，写入、维护和有 owner 语义的 runtime/cache/quota/health 路径不进入该读通道。PostgreSQL performance 模式下，typed command、优先级、重试和背压语义保留，但 F2 Go 进程直接按连接池和同 key 顺序约束异步并发写 `juhe_stats`，不引入新的 Node 队列。分布式模式作为显式配置能力，新增控制面 leader、节点心跳、用户 home node 路由和节点内部转发；具体边界见 [分布式部署与用户分片设计](../functions/分布式部署与用户分片设计.md)。
...
- 普通运行日志的业务热路径只能把完整 JSONL 顺序追加到角色日志文件；禁止在 server、DB service、网关回调或 logger 写入路径执行 `JSON.parse`、正则扫描、敏感字段脱敏、SHA-256、索引 DTO 构造、数据库写入或 Redis XADD。只有独立 Go F1 indexer 可以按文件 cursor 在有界窗口内直接解析、生成稳定 ID 和批量写入运行日志专用库；文件是该链路唯一耐久 spool，不得通过抽样、截断整行或丢弃低级别日志伪造吞吐。
