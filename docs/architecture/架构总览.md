...
当前对外主入口是 OpenAI 兼容网关，支持根路径 `/*` 和 `/v1/*`；系统后台、管理 API、健康检查和静态资源统一在 `/__aisys__/*` 下。主链路固定为认证、额度与路由、候选能力过滤、上游请求、响应转发、使用记录和审计。通用 `generic_*` 客户端不解释上游状态、错误对象、错误码、错误类型、完成状态或正文；完整响应原样转发。transport / timeout 失败在 `semanticCommitted = false` 时可继续扫描账号和后备分组。精确 Codex、Claude Code、Gemini CLI 客户端才允许按自身协议语义和响应检查策略救回。`recovery_wait`、`precheck_pending`、受控半开和 FIFO 继续保护账号池，但 270 秒只统计零可派发账号阶段。混合智能路由仍只负责策略、分组和目标模型，跨协议转换由真实混合供应商账户处理。
...
高并发分组不把上游状态码作为默认健康事实，而是使用当前并发、分组排队、首段等待、请求耗时、近期 transport 超时、质量缓存和账号软并发占用。通用客户端的完整非成功响应也不影响当前请求调度或账号运行态；精确客户端显式策略、后台事前确认、后台复测或人工操作才允许执行对应状态动作。
