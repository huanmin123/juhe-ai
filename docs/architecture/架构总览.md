...
用户侧账户创建流程按“供应商 -> 账户类型 -> 凭据与调度配置”展开；页面不直接展示内部供应商协议档案 ID，但可以用“接入类型 / 客户端兼容”文案承接不同档案选择。内部流程由后端根据供应商和接入类型解析 `provider_protocol_profile_id`，并据此决定端点族、账户类型、测试策略和调度边界。分组同样绑定一个供应商协议档案；账户只能加入相同 `provider_protocol_profile_id` 的分组。API Key 可以绑定多个供应商协议档案分组，但运行时只在当前 Key 已绑定的有界分组集合内调度：普通路由请求带 `model` 时先用模型目录在当前 Key 绑定供应商集合内唯一命中目标供应商，再只在该供应商档案的绑定分组内执行 API Key 路由策略；无法明确定位时返回本地错误，不猜测全局账号池，也不继续落到默认分组。
...
- API Key 可以绑定调用方自己的一个或多个分组，也可以绑定有效授权给调用方的分组，绑定分组可以来自不同供应商协议档案。当前请求进入后，普通路由先按请求 `model` 尝试筛出目标供应商协议档案分组，再在目标分组集合内按主备、轮询或权重策略选号池；请求带 `model` 但无法明确定位目标供应商时返回本地错误，不继续使用默认分组。授权暂停、过期、回收、归还或额度不可用时，授权分组绑定配置保留但运行态不可用。
