...
- 管理页高频列表采用数据域 revision / confirm 与 IndexedDB cache-first：缓存 key 必须包含登录主体、管理目标、路由、筛选、排序、分页和协议版本；账户列表最大陈旧 30 秒，使用记录 15 秒。confirm 只读 memory / Redis 修订令牌，禁止回源业务明细库。写操作提交后发布 owner / allScopes 变更；发布或 Redis 故障通过 db-service 中按 domain generation 持久的 dirty outbox 保底，恢复后发布全域 reset 并用 generation CAS 清理。db-service 启动时一次加载 dirty 表到内存，confirm 热路径只读内存 Map；重放使用单 timer、有界指数退避和 singleflight，Redis 未恢复不能阻止 db-service 启动。
