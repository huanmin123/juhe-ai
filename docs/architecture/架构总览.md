...
- 对外中转协议：客户端入口按 OpenAI-compatible 形态暴露，支持根路径和 `/v1`；`openai/v1` 是协议层语义，`provider_code = openai` 是通用 OpenAI-compatible 供应商语义，二者通过字段所在层级区分。当前内置供应商为 `openai`、`gpt`、`anthropic`、`deepseek`、`glm` 和 `gemini`；`anthropic` 官方直连供应商使用原生 `anthropic/v1` Messages 协议，不能和第三方 Anthropic-compatible 入口混在一个档案里；`gemini` 官方直连供应商默认使用原生 `gemini/v1beta` GenerateContent / StreamGenerateContent 协议，同时提供 `profile_gemini_openai_chat_v1beta` 作为 Gemini 官方 OpenAI Chat compatibility 直连档案，两者按不同 `provider_protocol_profile_id` 隔离，不能混在一个账户或分组档案里；`glm` 智谱供应商同时包含通用 OpenAI Chat Completions 档案、GLM Coding OpenAI Chat Completions 档案和 GLM Coding Anthropic Messages 档案，必须按不同供应商协议档案隔离；`deepseek` 供应商同时包含 OpenAI-compatible Chat Completions 档案和 DeepSeek Anthropic-compatible Messages 档案，二者按不同 `provider_protocol_profile_id` 和账户接入类型区分。`gpt.parent_code = openai`，继承通用 OpenAI v1 能力并叠加 GPT / Codex 专属能力。通用 OpenAI 兼容 API Key 账号、GPT API Key 账号、Anthropic 官方 API Key 账号、GLM API Key 账号、DeepSeek API Key 账号和 Gemini API Key 账号按各自上游入口透传，GPT OAuth 账号只支持 Codex adapter 明确列出的 Responses 路径；Anthropic OAuth / Setup Token / Claude Code token 账号链路和 Gemini Google 登录 / Code Assist / Vertex AI 账号链路不纳入当前目标，Claude Code 只作为下游客户端画像，可在官方 Anthropic API Key、DeepSeek Anthropic-compatible API Key 或 GLM Coding Anthropic-compatible API Key 的 `anthropic/v1` 档案内生效，`gemini-cli` 只作为 Gemini native 下游客户端画像通过 `GOOGLE_GEMINI_BASE_URL` 验证；Codex 使用 Gemini OpenAI Chat、Gemini native 或 Anthropic Messages 时必须通过 API Key 显式混合路由声明 `responses -> chat_completions|generate_content|messages` 目标，不在账号模型别名里表达跨协议桥接；模型目录上，`openai` 作为通用 OpenAI-compatible 聚合目录，会聚合支持 OpenAI 协议的 GPT、DeepSeek、GLM、Gemini OpenAI Chat 和自身模型，用于未深度接入但兼容 OpenAI 协议的临时接入；`gpt`、`anthropic`、`glm`、`deepseek` 和 `gemini` 专用供应商目录仍只看各自供应商模型，Anthropic / Messages 模型和 Gemini native 端点能力不进入 `openai` 聚合目录。
- 存储：默认 `standalone` 模式使用 SQLite，当前按业务库、数据集目录库、使用记录目录库、统计结果库和分片文件运行；业务库保存可恢复核心业务数据，必须在迁移和恢复时保留；数据集目录库保存审计、操作日志、公开接口日志、运行日志索引、模型检测和清理目标；使用记录目录库保存 usage shard 注册表、列表筛选目录和账号 / API Key scope catalog；新写入的 `usage_records` 已按日期与稳定 hash 拆到多个本地 SQLite shard 文件；统计结果库保存统计缓存、额度窗口、账号质量、系统监控、表监控历史和 `stats_job_state`。数据集目录库、使用记录目录库、usage shard 和统计结果库都可以按需要丢弃、清空或重建，具体见 [数据集库分片写入设计](../functions/数据集库分片写入设计.md)。显式 `performance` 模式使用 PostgreSQL 承接当前事实域和统计域，使用 Redis 承接跨进程短 TTL 缓存、调度运行态和原子计数，边界见 [PostgreSQL 与 Redis 高性能模式设计](../functions/PostgreSQL与Redis高性能模式设计.md)。
...
- 运行形态：默认单机部署下，Web/网关主进程、`ingest-worker`、`stats-worker`、`ops-worker` 和本地 DB service 进程分离；默认 `worker` 只保留历史 fallback / 控制语义，不再作为 supervisor 常驻角色。`ingest-worker` 承载使用记录、审计、操作日志、公开接口日志、运行日志索引、运行日志文件导入、记录维护和 dataset / usage shard 保留期清理；`stats-worker` 承载系统采样、增量统计、重窗口、账号质量、表监控和统计保留期清理；`ops-worker` 承载账号测试、正常账号健康检测、冷却复测、代理检测、OAuth token 保活、可用性时间计划同步、授权到期扫描和过期删除账号清理协调。SQLite standalone 模式下，DB service 只拥有业务库写锁；数据集、使用记录目录库、usage shard 和统计结果库写入必须转发给对应 worker / writer，不能在管理路由中直接写非所属库。PostgreSQL performance 模式下，typed command、优先级、重试和背压语义保留，但消费端可以按连接池和同 key 顺序约束并发执行，默认最大消费并发为 100。分布式模式作为显式配置能力，新增控制面 leader、节点心跳、用户 home node 路由和节点内部转发；具体边界见 [分布式部署与用户分片设计](../functions/分布式部署与用户分片设计.md)。
...
更具体的运行、存储和敏感字段规则见 [SQLite 存储说明](../functions/SQLite存储说明.md)、[PostgreSQL 与 Redis 高性能模式设计](../functions/PostgreSQL与Redis高性能模式设计.md) 与 [核心功能设计](../functions/核心功能设计.md)。
...
- `degraded`、`precheck_pending`、`local_suppressed`、`half_open` 和 `precheck_failed` 只属于网关 / 探测运行态，不新增 `accounts.status` 枚举。手动账户测试和批量账户测试失败不能直接把正常账户写成 `temporary_unavailable`；应先记录测试失败结果并进入事前确认运行态，确认探针仍失败后才写持久临时不可调用并进入冷却复测。待测试账户失败继续保持 `pending_test`，停用账户失败只保留诊断，授权实例只修改被授权侧本地实例状态，不回写授权方原账户。
...
| 模型目录与价格 | 汇总内置模型和当前系统账户个人自定义模型，支撑 `/v1/models`、价格估算、账号支持模型和账号模型映射 | [自定义模型与模型映射设计](../functions/自定义模型与模型映射设计.md)、[厂商模型目录更新与清洗指南](../functions/厂商模型目录更新与清洗指南.md)、[模型价格与用量统计口径](../functions/模型价格与用量统计口径.md) |
...
- 模型目录与价格是网关调度和计费的前置事实：内置模型来自官方目录，所有系统账户可见；自定义模型只归属创建者自己的系统账户，管理员创建也不会变成全局模型。`/v1/models`、账号支持模型、模型检测候选、账号模型别名和成本估算都必须从调用方可见的本地模型目录读取。普通 OpenAI-compatible `/v1/models` 返回标准 `object/list + data`，Codex 模型刷新请求返回同一目录派生出的 `{ models: ModelInfo[] }`，这是客户端响应形态分支，不新增模型目录来源。普通跨供应商 API Key 请求带 `model` 时，先尝试当前 Key 的显式混合路由规则；未命中时，再用请求 `model` 在该 Key 已绑定供应商范围内做有界目标档案收窄。模型名唯一性按调用方可见目录判断，不能通过个人模型覆盖其他用户目录。账号模型限制在候选账号池内参与账号候选排序：直接支持请求模型的账号优先，其次是同协议模型别名命中的账号，最后是未声明模型限制的账号；账号模型别名命中后只改写上游模型名和同协议端点族，并在使用记录中同时保留下游模型、上游模型和实际计价模型。
...
- API Key 可以绑定调用方自己的一个或多个分组，也可以绑定有效授权给调用方的分组，绑定分组可以来自不同供应商协议档案。当前请求进入后，显式混合路由规则先按下游协议、客户端画像和模型精确匹配目标分组；未命中显式规则时，普通路由按请求 `model` 尝试筛出目标供应商协议档案分组，并在目标分组集合内按主备、轮询或权重策略选号池。DeepSeek / GLM / Gemini OpenAI Chat 等 Chat-only 上游承接 Codex Responses、Anthropic Messages 或 Gemini native 来源时，必须配置 API Key 显式混合路由。请求带 `model` 但无法通过显式规则或目录明确定位目标时返回本地错误，不继续使用默认分组。授权暂停、过期、回收、归还或额度不可用时，授权分组绑定配置保留但运行态不可用。
...
- 默认 standalone 版本不引入 Redis、Kafka 或独立分布式队列。显式 performance 模式引入 Redis 作为跨进程短 TTL 缓存和运行态 state store，但不作为事实源；显式分布式模式下，用户本地任务由各 data node worker 只处理本机 home users；leader 只持有集群级 worker lease，负责节点摘要合并、分片分配和控制面维护，不能扫描其他节点的使用记录明细。
...
- 易失运行态只用于性能、体验和临时调度辅助，不能作为长期账务、权限、授权来源、账号健康或审计事实。服务重启、子进程重启、缓存淘汰或 IPC 队列溢出后，允许这些状态丢失或回到保守默认；请求路径必须重新从 SQLite 事实和预聚合缓存恢复判断。运行态调度降级只把近期失败账号排到普通候选之后，普通候选不足时仍可兜底尝试，完整成功后清理，不写账号冷却或异常状态。IP 级账号回避状态也属于这类短 TTL 调度态，只影响同一客户端 IP、API Key 和账号组合的候选排序，不按分组切碎；分组只是当前 API Key 的候选容器，不写账号冷却状态。认证前入口保护和认证后 IP 级错误熔断也属于短 TTL 易失运行态，只短路当前窄作用域或当前来源的高置信错误风暴，不写账号状态、不作为审计事实和长期处罚依据。
