...
- 策略路由是 API Key 和分组之间的独立层。API Key 只绑定一条路由策略；路由策略再绑定一个或多个分组，并按普通路由、混合智能路由、权重调度、故障回退或轮询选择候选分组。普通路由只能绑定一个分组，且不开放复杂配置。系统只自动创建内置默认分组，不自动创建策略路由或 API Key；普通路由、混合智能路由、权重调度、故障回退和轮询路由都由用户在策略路由页面显式创建，混合供应商路由属于高级路由，不作为默认入口。
