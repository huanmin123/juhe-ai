...
- `degraded`、`precheck_pending`、`local_suppressed`、`half_open` 和 `precheck_failed` 只属于网关 / 探测运行态，不新增 `accounts.status` 枚举。用户业务请求没有账户级状态写权限：失败只允许当前请求换号、按 API Key / IP / 会话局部回避并原子首次投递后台核实信号，不能合并、改写、续期或确认已有账户级事件，也不能写 `temporary_unavailable`；用户请求成功同样不能清理账户级状态。Redis `recovery_wait` 只是内部后台任务，不能显示成账户运行态或从全局候选池移除账户；后台标准探针不按状态码、错误码或错误文案白名单判断，只按完整成功、已形成真实上游失败、未形成有效上游尝试三类事实推进。前端人工单账户测试只产出诊断结果、使用记录与审计，产品不提供多账户批量测试；无论成功或失败都不能改写账户或授权实例的 `status`、`schedulable`、冷却、最近错误和 API Key 运行态，也不能推进账户内 Key 轮换计数，或写入、确认、清理生产上游桶、客户端 IP 熔断、客户端 IP 账号回避和 Codex turn 账号回避运行态；待检查、停用、异常、限流和临时不可调用账户在测试前后保持原状态。账户级临时不可调度只允许由跨时间连续失败的后台标准探针最终确认；后台恢复探针或主动健康检查完整成功立即清理未完成判断。
