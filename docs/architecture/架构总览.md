...
| AI 问答（规划） | 登录用户使用自己 API Key 的平台内置 OpenAI Chat 客户端；维护会话、最近 7 天完整轮次、站内 SSE 和富文本展示，聊天正文使用独立 SQLite / `juhe_chat` schema，所有模型请求仍按普通客户端进入现有网关 | [AI 问答设计](../functions/AI问答设计.md) |
...
- AI 问答规划为登录用户会话层和平台内置 OpenAI Chat 客户端，不是新的模型调度层。会话固定绑定当前用户自己的一个 API Key，模型允许在该 Key 当前可用范围内逐轮切换；Node Chat 模块只负责会话、消息、最近 7 天完整成功轮次和站内 SSE，聊天正文不进入 `juhe_business`，standalone 使用独立 `juhe-ai-chat.sqlite3`，performance 使用独立 `juhe_chat` schema 与按日分区的消息表。模型请求必须使用真实 Bearer Key 重新进入 `/v1/chat/completions`，由现有网关继续按普通客户端处理认证、限流、路由策略、分组、账户、代理、额度、计费、使用记录和原始审计。聊天数据生命周期与审计生命周期相互独立。MVP 不包含图片上传、Agent、MCP、Skill 或工具执行。
