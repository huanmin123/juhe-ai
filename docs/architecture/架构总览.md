...
当客户端协议和上游真实能力不一致时，协议转换必须作为共享 adapter 落在 provider driver 可复用边界内，而不是写成某个供应商的私有分支。OpenAI 协议档案下的 `responses -> chat_completions` 只能由账号模型映射显式触发，映射保存 `sourceModel + sourceEndpointFamily -> upstreamModel + upstreamEndpointFamily`；当前支持的白名单桥接包括 OpenAI 协议档案内的 `responses -> chat_completions`，以及 PLAN-0058 定义的 `chat_completions|responses -> messages`。仍不支持 `chat_completions -> responses` 或 Anthropic Messages 到 OpenAI Chat / Responses 的反向转换。账户 `supported_endpoint_modes` 仍保存真实上游能力，例如 `chat_sse` 或 `messages_sse`，不要为了桥接把账号伪装成原生 `responses_sse`。
