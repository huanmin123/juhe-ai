...
- 对外中转协议：客户端入口按 OpenAI-compatible 形态暴露，支持根路径和 `/v1`；`openai/v1` 是协议层语义，`provider_code = openai` 是通用 OpenAI-compatible 供应商语义，二者通过字段所在层级区分。当前内置真实供应商为 `openai`、`gpt`、`anthropic`、`deepseek`、`glm`、`gemini` 和 `hybrid`；`hybrid` 混合供应商用于创建真实 AI 账户，不是 API Key 规则层，也不是再指向其他账户或分组的虚拟路由节点。普通供应商账户只表达自身真实上游协议和模型能力；跨协议入口适配、下游模型到真实上游模型映射和不可保真能力处理，统一进入混合供应商账户。客户端画像只作为网关内部运行时策略，不在 API Key 或普通账户页面暴露。模型目录上，`openai` 作为通用 OpenAI-compatible 聚合目录，会聚合支持 OpenAI 协议的 GPT、DeepSeek、GLM、Gemini OpenAI Chat 和自身模型，用于未深度接入但兼容 OpenAI 协议的临时接入；`hybrid` 混合供应商面向跨协议上游账户，前端模型选择可查看全局模型集合且允许手动输入真实模型名；`gpt`、`anthropic`、`glm`、`deepseek` 和 `gemini` 专用供应商目录按各自供应商 / 上游协议边界维护，不能把非 OpenAI 协议能力混进通用 `openai` 目录。
...
当前目标设计固定为：API Key 是用户入口并选择路由策略；路由策略保存路由模式、分组绑定、权重、故障回退、轮询和混合智能路由规则；分组协调 AI 账户；供应商对 AI 账户分类并提供协议、模型目录、价格和专项优化；AI 账户管理真实上游账户和限制配置。账号表单只展示真实上游接入契约和 endpoint modes，不再把泛化 `client_compatibility` 作为普通账号配置。普通账号模型映射收敛为当前供应商内同协议模型别名；跨供应商、跨协议和“下游模型名打到另一类上游协议”的场景进入混合供应商账户，详见 [客户端画像与混合供应商边界设计](../functions/客户端画像与混合路由设计.md)。

当客户端协议和上游真实能力不一致时，协议转换必须作为共享 adapter 落在 provider driver 可复用边界内，而不是写成某个供应商的私有分支。跨协议转换由混合供应商账户触发：该账户保存自己的真实上游凭据、Base URL、支持的 endpoint modes，以及下游模型到真实上游模型 / 目标协议的映射；它不指向其他账户或分组。普通账号 `modelMappings` 只保存当前账号内同协议模型别名。长期设计按 [协议桥接框架设计](../functions/协议桥接框架设计.md) 收敛为“下游协议 -> 统一生成中间表示 -> 上游协议”，新增独立协议时默认补一次入站 adapter 和一次出站 adapter，不能继续扩散成 N×N 私有转换。
...
用户侧账户创建流程按“供应商 -> 账户类型 -> 凭据与账户配置”展开；页面不直接展示内部供应商协议档案 ID，但可以用“接入类型”文案承接不同档案选择。内部流程由后端根据供应商和接入类型解析 `provider_protocol_profile_id`，并据此决定端点族、账户类型、测试策略和账户能力边界。分组只负责协调 AI 账户，不负责定义供应商、模型或协议语义。API Key 只负责入口、鉴权、额度和路由策略选择；普通路由、混合智能路由、权重调度、故障回退和轮询都属于策略路由层，分组统一绑定到路由策略。跨协议转换和跨供应商模型映射不在 API Key 或路由策略中声明，由最终命中的混合供应商账户按自身真实上游配置处理。
...
| 分组 | 调度资源集合和 API Key 可访问号池边界；分组负责协调 AI 账户，不承载供应商、模型或协议语义；按类型区分个人轻量调度和高并发协同调度 | [核心功能设计](../functions/核心功能设计.md)、[高并发分组调度设计](../functions/高并发分组调度设计.md) |
...
- 模型目录与价格是网关调度和计费的前置事实：内置模型来自官方目录，所有系统账户可见；自定义模型只归属创建者自己的系统账户，管理员创建也不会变成全局模型。`/v1/models`、账号支持模型、模型检测候选、账号测试默认模型、账号模型别名和成本估算都必须从调用方可见的本地模型目录读取。供应商协议档案的 `default_test_model` 只是系统兜底，用户可在自己的可见模型目录中按 `system_account_id + provider_code` 设置默认测试模型偏好，不能影响其他用户或全局供应商档案；供应商 `defaultSupportedModels` 是新建 AI 账户的支持模型默认回填值，账户保存后的 `supportedModels` 必须非空。普通 OpenAI-compatible `/v1/models` 返回标准 `object/list + data`，Codex 模型刷新请求返回同一目录派生出的 `{ models: ModelInfo[] }`，这是客户端响应形态分支，不新增模型目录来源。模型名唯一性按调用方可见目录判断，不能通过个人模型覆盖其他用户目录。账号模型限制在候选账号池内参与账号候选排序：直接支持请求模型的账号优先，其次是同协议模型别名命中且上游模型命中账号支持模型的账号；未声明支持模型的账号不作为兜底候选。账号模型别名命中后只改写上游模型名和同协议端点族。跨协议或跨供应商模型映射只允许混合供应商账户保存，并在使用记录中同时保留下游模型、上游模型和实际计价模型。
...
- 策略路由是 API Key 和分组之间的独立层。API Key 只绑定一条路由策略；路由策略再绑定一个或多个分组，并按普通路由、混合智能路由、权重调度、故障回退或轮询选择候选分组。普通路由只能绑定一个分组，且不开放复杂配置。每个系统账户的每个内置默认分组都必须有一条不可删除的默认普通路由，默认路由属于策略路由层，不属于 API Key 或分组字段；系统同步为每条默认路由创建一个不可删除但可编辑、可刷新密钥的默认 API Key。
- 分组和分组之间保持容器隔离。策略路由可以在真实派发前按模式选择或切换候选分组，但仍不做跨分组账号混排、跨分组借号、共享分组队列或共享分组等待状态；普通上游失败和已写下游输出后的失败不做服务端透明重放，只有配置化响应检查在写下游前命中、允许服务端重试且当前号池账号耗尽时，才可按本次策略候选顺序尝试后续分组。新版本响应检查同时覆盖 OpenAI v1 Chat / Responses 的 JSON 和 SSE 场景；旧 `200 + SSE` 流式拦截不作为新版本兼容边界。
...
- 路由策略是调控维度，账户是资源维度；路由策略只负责号池分配、路由顺序和容器级候选规则，不能改变账户能力、凭据、代理、状态、冷却、并发、质量、模型能力、供应商协议、账户错误处理策略或亲和语义。
