...
- 分组和分组之间保持容器隔离。策略路由可以在真实派发前按模式选择或切换候选分组，但仍不做跨分组账号混排、跨分组借号、共享分组队列或共享分组等待状态；真实派发后只有可安全重放文本在 `semanticCommitted = false` 且共享墙钟 / attempt 预算允许时，才可因 opaque 非 `2xx`、本地 transport failure 或精确协议声明的失败结构按当前候选账号和后续分组继续接管。完整 `2xx` / 普通 payload 透明，副作用请求派发后和已提交响应都禁止重放；配置化状态动作不能绕过该门禁。
...
当前对外主入口是 OpenAI 兼容网关，支持根路径 `/*` 和 `/v1/*`；系统后台、管理 API、健康检查和静态资源统一在 `/__aisys__/*` 下。主链路固定为认证、额度与路由、候选能力过滤、上游请求、响应转发、使用记录和审计。通用 `generic_*` 客户端默认不解释错误对象、错误码、错误类型、完成状态或正文；安全文本推理端点的完整非 `2xx` 只按 `response.ok` 进入请求级 opaque 接管，同账户当前可执行 Key 按池顺序唯一尝试，再按候选、墙钟和整个请求最多 64 次真实上游 attempt 的安全上限切换账户 / 分组。图片、音频、文件、资源创建、后台任务、hosted tool 和其他副作用 POST 一旦派发后不得隐式重放。transport / timeout 失败只在请求语义允许重放且 `semanticCommitted = false` 时继续扫描候选。精确 Codex、Claude Code、Gemini CLI 客户端只解释各自声明的协议结构；普通事件里的 `data.error` 不等于结构失败。上游状态码、错误码、错误类型、正文和事件 payload 均不可信，普通业务失败不得据此触发账户或 Key 共享语义状态；只有用户显式策略或独立后台探针可以执行对应状态动作，详见 [AI 账户错误语义与状态变更边界](../functions/AI账户错误语义与状态变更边界.md)。`ServerRetryBudget` 默认 270 秒，只累计零可派发、FIFO / 并发槽和受控半开等待；可重放文本另有独立、固定默认 270 秒的 `GatewayRequestWallBudget`，从请求接收贯穿 Key、账户、分组、排队、attempt 和后续重放决策，两者不能互相派生或替代。客户端重试是新请求，必须按重试时的优先级、质量、速度、可用状态和轮换游标重新选号，不延续上一次候选游标。
...
网关流式转发必须增量处理并尊重 backpressure。下游状态拆为 `transportCommitted` 和 `semanticCommitted`：零可派发账号等待期间可发送 SSE 注释心跳，仍允许语义提交前且请求语义安全的 transport 救回；真实协议事件、正文或可见输出写出后禁止拼接第二条流。通用客户端原样转发上游 SSE 事件和 EOF，不执行响应语义检查；精确客户端只把协议声明的失败事件结构解释为本次 attempt 失败，普通事件中的 `data.error` 保持 payload 中性。文本 / 图像超时默认分别为 `120/30/1800` 与 `600/120/3600` 秒；图片派发后脱离可重放文本的 270 秒墙钟，只受 600 秒首响应、120 秒 raw chunk idle 和 3600 秒未提交寿命约束，配置可覆盖 10–15 分钟生成场景。该图片请求始终只有唯一 attempt，超时或断尾都不能切 Key、账户或分组：下游未提交时返回 `503/upstream_outcome_unknown`，已经写出时只能结束或断流，不能改写既有响应。活跃且已提交语义的 SSE 不受未提交 attempt 寿命机械中断。
...
- 页面性能优化必须同步对应业务功能、前后端架构和验证文档；不得恢复已由 PLAN-0156-20260722T123439000Z 退场的通用 page-data confirm/revision 机制。
