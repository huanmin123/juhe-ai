...
- `degraded`、`precheck_pending`、`local_suppressed`、`half_open` 和 `precheck_failed` 只属于网关 / 探测运行态，不新增 `accounts.status` 枚举。前端人工单账户测试只产出诊断结果、使用记录与审计，产品不提供多账户批量测试；无论成功或失败都不能改写账户或授权实例的 `status`、`schedulable`、冷却、最近错误和 API Key 运行态，也不能推进账户内 Key 轮换计数，或写入、确认、清理生产上游桶、客户端 IP 熔断、客户端 IP 账号回避和 Codex turn 账号回避运行态；待测试、停用、异常、限流和临时不可调用账户在测试前后保持原状态。真实网关请求、后台激活检查、周期健康检查、故障确认和冷却复测继续按各自职责维护运行态。
