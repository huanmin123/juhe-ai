...
> **F1 当前事实（2026-08-09）。** “运行日志索引与保留”已由独立 Go 进程直接完整接管：Node 仅生成 JSONL 和只读查询索引；Node importer、cursor/writer、scheduler、retention、schema 初始化和 Node/Go 切换开关均已退出活跃路径并归档。本文其余将其描述为 Node ingest-worker 职责或可关闭 Node 索引的旧文字，均为该功能接管前的历史说明，不得据此恢复 Node 路径。
...
- 客户端模型目录不再提供匿名公共聚合：`/models`、`/v1/models` 与各原生协议模型目录都必须先完成本地 Gateway API Key 认证，再按该 Key 的有效路由分组聚合本地供应商目录。上游模型目录是账户编辑时的补全数据源，不是新的本地模型目录事实来源；新增 API Key 账户凭据和 Base URL 完整且支持模型为空时自动同步一次，仍可由用户手动刷新，追加内容限定为“上游可见 ∩ 本地可见且协议兼容”的模型，并且必须保留用户手动选择。人工测试、激活、周期检查与恢复探针只执行真实协议请求，不做上游目录预检。
...
- 运行形态：默认单机部署下，Web/网关主进程、`ingest-worker`、`stats-worker`、`ops-worker`、独立 Go F1/F2 进程和本地 DB service 进程分离；默认 `worker` 不再作为 supervisor 常驻业务角色，仅保留统一进程角色命名和控制入口语义。`ingest-worker` 承载使用记录、审计、操作日志、公开接口日志、运行日志文件导入、记录维护和 dataset / usage shard 保留期清理；独立 Go F1 indexer 负责运行日志索引与保留，独立 Go F2 `juhe-ai-table-monitor` 负责表存储采样、快照写入与 retention；`stats-worker` 继续承载尚未迁移的系统采样、增量统计、重窗口和账号质量；`ops-worker` 承载账号测试、正常账号健康检测、冷却复测、代理检测、OAuth token 保活、可用性时间计划同步、授权到期扫描和过期删除账号清理协调。SQLite standalone 模式下，DB service 只拥有业务库写锁；数据集、使用记录目录库、usage shard 和非 F2 统计结果库写入必须转发给对应 worker / writer，F2 专用 SQLite 输出只由 Go owner 写入，不能在管理路由中直接写非所属库。确认安全的管理端和兼容接口纯读进入 DB service 下的 query-only SQLite read worker 子进程，写入、维护和有 owner 语义的 runtime/cache/quota/health 路径不进入该读通道。PostgreSQL performance 模式下，typed command、优先级、重试和背压语义保留，但 F2 直接写 `juhe_stats` 并受 owner lease fencing，不经过 Node queue；其他消费端可以按连接池和同 key 顺序约束并发执行，默认最大消费并发为 100，不套 SQLite 单机读写队列。分布式模式作为显式配置能力，新增控制面 leader、节点心跳、用户 home node 路由和节点内部转发；具体边界见 [分布式部署与用户分片设计](../functions/分布式部署与用户分片设计.md)。
...
- 普通运行日志的业务热路径只能把完整 JSONL 顺序追加到角色日志文件；禁止在 server、DB service、网关回调或 logger 写入路径执行 `JSON.parse`、正则扫描、敏感字段脱敏、SHA-256、索引 DTO 构造、数据库写入或 Redis XADD。只有独立 Go F1 indexer 可以按文件 cursor 直接并发解析、生成稳定 ID 和批量写入运行日志专用库；文件是该链路唯一耐久 spool，不得通过抽样、截断整行或丢弃低级别日志伪造吞吐。
...
当前检查边界：`/models`、`/v1/models` 或任何供应商目录路径不可用、返回 `404` 或不开放时，不能阻断保存后的人工测试、草稿测试、激活、周期健康、运行态恢复、冷却恢复、Key 恢复或调度。所有这些自动检查都以账户配置的真实生成或图片 endpoint 的协议成功为准；目录受控转发只服务新增 API Key 账户草稿完整后的自动同步和用户手动刷新，客户端 `/models` 始终由本地目录响应。
...
- 全局设置属于平台公开展示资源；系统设置属于平台运行策略，影响网关、后台任务、操作日志和数据保留。原始审计默认启用，但允许通过部署环境变量 `JUHE_AI_AUDIT_LOG_ENABLED=false` 临时整体关闭；F1 运行日志索引不再有 Node 侧 `JUHE_AI_RUNTIME_LOG_INDEX_ENABLED` 开关，Go indexer 是唯一 owner。文件日志和 grep 独立于索引运行，审计设置不影响使用记录。
...
- 当前账户检查补充：无论文本还是图片模型，人工测试、激活、周期健康和恢复都执行实际 endpoint 并按协议成功判定；上游模型目录只服务新增 API Key 账户草稿完整后的自动同步和用户手动刷新。目录不可用、`404` 或响应不兼容不能改变账户测试、激活、恢复或调度结果。
...
- 业务库、数据集目录库、使用记录目录库、usage shard、F2 表监控输出和统计结果库是当前明确边界：系统账户、账户、分组、API Key、授权、公告、设置等可恢复业务数据在业务库；新写入的使用记录进入 usage shard 文件，usage shard 注册表和筛选目录进入使用记录目录库；审计、日志、操作日志、模型检测明细和清理目标进入统计数据集目录库；非 F2 统计桶、额度窗口、范围窗口、排行快照、账号质量结果、系统监控采样和系统监控趋势进入统计结果库；F2 standalone 快照进入 `JUHE_AI_TABLE_MONITOR_DATABASE_PATH` 专用 SQLite，F2 PostgreSQL 快照与 retention 进入 `juhe_stats`。默认备份只覆盖业务库和 `backend/.env`；数据集目录库、使用记录目录库、usage shard、F2 表监控输出和统计结果库属于可丢弃 / 可重建数据，只有离线取证时才额外停机备份。请求链路和 DB service 不直接写高频数据集明细，使用记录、审计和日志索引等高频写入应投递 background worker 队列；F2 由独立 Go 进程直接异步采样、写入和 retention，不进入 Node 队列。为解决单个数据集目录库文件的 SQLite 写锁上限，`usage_records` 分片写入设计已把统计数据集域扩展为数据集目录库、使用记录目录库和多个 usage shard 文件，但不改变业务库和 F2 输出职责。
...
- 后台任务按数据域和任务类型隔离：`ingest-worker` 承载使用记录、操作日志、公开接口日志、运行日志文件增量导入与索引、审计日志批量落库、记录维护队列和 dataset / usage shard 保留期清理；`stats-worker` 继续承载尚未迁移的系统指标采样、用量聚合、IP 聚合、分组账户统计缓存、TopN、概览、范围窗口、授权窗口、系统趋势窗口和账号质量刷新；`ops-worker` 承载 OpenAI OAuth Access Token 保活、手动 AI 账户诊断、账户激活检查、正常账号周期健康检测、代理检测、账号级 / Key 级冷却复测、可用性时间计划同步、授权到期扫描和过期删除账号清理协调。F2 表存储监控采样、快照写入和表监控历史保留已由独立 Go `juhe-ai-table-monitor` 完整接管，Node 不再注册 scheduler、stats writer 或 retention writer，只保留 HTTP 读接口。周期健康检查是基础机制，不提供账户级关闭开关。`metrics-worker`、`snapshot-worker`、`probe-worker`、`maintenance-worker` 不再作为当前常驻拓扑；需要拆分时必须重新立项并同步 registry、API 契约、前端展示和验证脚本。SQLite standalone 模式继续按 writer owner 串行提交同一 SQLite 文件写入；PostgreSQL performance 模式不再受 SQLite 文件级写锁约束，但仍必须通过 typed command、队列优先级、连接池、同 key 顺序和背压控制并发。OpenAI OAuth 额度快照由真实网关响应头被动写入，不再作为后台主动刷新任务。
...
- worker 角色隔离不等于无限并发写入许可。SQLite standalone 模式下，同一个 SQLite 文件必须只有一个写 owner / writer 队列：业务库写入归 DB service，数据集目录库和使用记录目录库写入归 ingest / log writer，统计结果库中非 F2 统计写入归 stats writer，F2 专用表监控 SQLite 由 Go `juhe-ai-table-monitor` 唯一写入，usage shard 按 shard 文件串行写；具体边界见 [SQLite 单写者写队列治理设计](../functions/SQLite单写者写队列治理设计.md) 与 [F2 表存储监控采样与保留功能冻结](../migration/F2-表存储监控采样与保留功能冻结.md)。PostgreSQL performance 模式下，同类 typed command 可以并发消费，F2 输出事务仍受 Go owner lease 和连接池限制，不使用队列或 fallback。
...
- SQLite 仍是默认事实存储，不配置高性能模式时不引入 PostgreSQL、Redis 或其他第三方数据库依赖；当前 standalone 部署使用业务库 `JUHE_AI_DATABASE_PATH`、统计数据集目录库 `JUHE_AI_DATASET_DATABASE_PATH`、使用记录目录库 `JUHE_AI_USAGE_CATALOG_DATABASE_PATH`、统计结果库 `JUHE_AI_STATS_DATABASE_PATH`、F2 表监控专用输出 `JUHE_AI_TABLE_MONITOR_DATABASE_PATH` 和 usage shard 根目录 `JUHE_AI_USAGE_SHARD_ROOT`。显式 performance 部署使用 `JUHE_AI_POSTGRES_URL`、`JUHE_AI_REDIS_CACHE_URL`、`JUHE_AI_REDIS_STATE_URL` 和 `JUHE_AI_REDIS_QUEUE_URL`，并按 [PostgreSQL 与 Redis 高性能模式设计](../functions/PostgreSQL与Redis高性能模式设计.md) 映射数据域；F2 直接把快照与 retention 写入 `juhe_stats`。需要处理当前 schema 之外的数据时，只能停机离线处理，不进入运行路径。`JUHE_AI_DATASET_DATABASE_PATH` 继续作为 standalone 模式的非 usage 数据集目录库，usage shard 文件和 usage catalog 位于独立使用记录数据域。
...
- DB service 在 standalone 模式下不改变 SQLite 单写者边界；业务库写入由 DB service 同步执行并依赖 WAL、短事务和 `busy_timeout` 控制短写锁等待。统计数据集域高频写入从请求链路剥离到 background writer，其中 usage 明细写入 usage shard，usage 定位目录写入使用记录目录库，非 usage 明细写入数据集目录库；非 F2 统计结果库写入由 `stats-worker` typed operation 串行提交，F2 表监控快照与 retention 由 Go `juhe-ai-table-monitor` 直接异步写入专用 SQLite 或 `juhe_stats`，受 owner lease fencing 保护。usage shard 只能由 ingest 写入；统计聚合只读取 shard，不再把派生估算字段回写原始 shard，同一个 shard 文件不得拥有多个并发 writer。DB service 在 performance 模式下继续作为系统 API、网关关键读写和连接池隔离边界；后台 writer 可以对 PostgreSQL 并发消费 typed command，但不能绕过队列背压、优先级和事务边界。
