...
- 多模型账户的完整目标架构在账户持久状态与传输电路之间增加稀疏模型能力健康层，作用域使用映射后的真实上游模型、协议档案、精确 endpoint mode 和 request lane。单模型独立探针失败只阻断该能力，账户仍可显示“可调度 · 部分模型不可用”；只有本地可验证整号事实、用户显式账户策略，或全部可路由能力完成独立全覆盖仲裁且均不可用时，才允许进入账户级不可调度。请求失败按精确能力触发跨实例单飞探针，不按 HTTP 状态码、错误码或正文推断具体错误类型，详见 [AI 账户多模型能力健康与精确隔离设计](../functions/AI账户多模型能力健康与精确隔离设计.md)。
