...
- 客户端模型目录不再提供匿名公共聚合：`/models`、`/v1/models` 与各原生协议模型目录都必须先完成本地 Gateway API Key 认证，再按该 Key 的有效路由分组聚合本地供应商目录。上游模型目录是账户编辑时的补全数据源，不是新的本地模型目录事实来源；同步只能追加“上游可见 ∩ 本地可见且协议兼容”的模型，必须保留用户手动选择。人工测试、周期检查与恢复探针可先进行短时缓存的上游目录预检，但仍以真实检查模型调用作为健康结论。
...
AI 账户的生产可用性和人工诊断必须分离：每个物理账户保存自己的必填检查模型 `health_check_model` 和不可空精确请求形态 `health_check_endpoint_mode`。允许值是 Chat Completions、Responses、Messages、GenerateContent 的 JSON / Streaming mode；GPT 新账户默认 `responses_sse`，其他协议档案默认对应 JSON mode。账户创建、导入后的激活检查、周期健康检测、运行态恢复、账号冷却恢复和账户内 Key 恢复都由后台系统检查负责。普通生成模型直接使用账户保存的检查模型与请求形态；OpenAI v1 纯图像模型的自动、周期和恢复检查只使用受控上游 `GET /v1/models` 目录探针精确确认模型 ID，不误用文本 `/responses`，也不通过后台生图消耗额度。只有用户明确发起并接受成本的人工诊断或独立真实 E2E，才允许一次 at-most-once 的真实生图。供应商目录的个人默认、管理员系统默认和协议档案默认只按“个人 > 管理员 > 协议档案”初始化新账户，不在运行时改写已有账户。人工测试只生成诊断任务、使用记录和审计，不修改账户、授权实例、Key、健康事实或调度运行态；列表测试所需模型目录按需加载，不扩充账户列表摘要，也不再提供多账户批量测试。账户所有者可在统一的“恢复正常”入口对自有 `pending_test` 账户执行强确认放行，用于明确接受上游可用性风险后立即恢复；该操作不等同于人工测试，不适用于授权实例或已过期账户，并继续服从账户时间计划。“重新检查”仍只重置检查窗口并交给后台确认，不直接放行。
...
- 多模型账户的完整目标架构在账户持久状态与传输电路之间增加稀疏模型执行能力层，作用域使用最终真实上游模型、协议档案、精确 endpoint mode、request lane、转换路径和选中凭据作用域。单个能力失败只阻断该精确作用域；只要同一 Route 仍有可执行 Key，或账户仍有其他可执行 Route，就继续调度。模型能力层永不写入或清理 `accounts.status`；全部配置能力阻断也只形成 `instance_all_capabilities_unavailable` 派生门禁，并在任一精确能力恢复后自动解除。请求失败按精确能力触发跨实例单飞探针，不按 HTTP 状态码、错误码或正文推断具体错误类型；现有 `protocol_model` 子电路也不得再以任意子 scope 计数打开父账户电路，详见 [AI 账户多模型能力健康与精确隔离设计](../functions/AI账户多模型能力健康与精确隔离设计.md)。
