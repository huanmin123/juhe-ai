...
- 模型目录与价格是网关调度和计费的前置事实：内置模型来自官方目录，管理员全局模型与内置模型一样对所有系统账户可见，个人自定义模型只对归属系统账户可见；管理员可在供应商模型目录中切换目标用户维护该用户的个人模型，也可创建全局模型。`/v1/models`、账号支持模型、模型检测候选、账号测试默认模型、账号模型别名和成本估算都必须从调用方可见的本地模型目录读取。供应商协议档案的 `default_test_model` 是系统兜底，用户可在自己的可见模型目录中按 `system_account_id + provider_code` 设置供应商级个人默认，不能影响其他用户或全局供应商档案；每个 AI 账户还可按账户 ID 保存独立 `accounts.default_test_model`，账户 A 与账户 B 不共享。测试模型统一按本次显式模型、账户独立默认、当前执行用户个人默认、账户绑定协议档案系统默认、账户支持模型首项解析；前三个持久化默认候选必须受该账户 `supportedModels` 约束。供应商 `defaultSupportedModels` 是新建 AI 账户的支持模型默认回填值，账户保存后的 `supportedModels` 必须非空。普通 OpenAI-compatible `/v1/models` 返回标准 `object/list + data`，Codex 模型刷新请求返回同一目录派生出的 `{ models: ModelInfo[] }`，这是客户端响应形态分支，不新增模型目录来源。模型 ID 保留上游大小写语义，唯一性按调用方可见目录内的原始字符串判断，不能通过个人模型覆盖其他用户目录。账号模型限制在候选账号池内参与账号候选排序：直接支持请求模型的账号优先，其次是同协议模型别名或 OpenAI v1 `responses -> chat_completions` 显式 bridge 命中且上游模型命中账号支持模型的账号；未声明支持模型的账号视为不限模型，排在直接命中和映射命中之后作为兜底候选。账号模型别名命中后只改写上游模型名和允许的端点族。除 OpenAI v1 Responses 到 Chat Completions bridge 外，跨协议或跨供应商模型映射只允许混合供应商账户保存，并在使用记录中同时保留下游模型、上游模型和实际计价模型。
