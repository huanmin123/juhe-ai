...
AI 账户的生产可用性和人工诊断必须分离：每个物理账户保存自己的必填检查模型 `health_check_model` 和不可空精确请求形态 `health_check_endpoint_mode`。允许值是 Chat Completions、Responses、Messages、GenerateContent 的 JSON / Streaming mode；GPT 新账户默认 `responses_sse`，其他协议档案默认对应 JSON mode。账户创建、导入后的激活检查、周期健康检测、运行态恢复、账号冷却恢复和账户内 Key 恢复都由后台系统检查负责，并直接使用账户保存的检查模型与请求形态，不再从协议族推导 JSON / SSE。供应商目录的个人默认、管理员系统默认和协议档案默认只按“个人 > 管理员 > 协议档案”初始化新账户，不在运行时改写已有账户。人工测试只生成诊断任务、使用记录和审计，不修改账户、授权实例、Key、健康事实或调度运行态；列表测试所需模型目录按需加载，不扩充账户列表摘要，也不再提供多账户批量测试。账户所有者可在统一的“恢复正常”入口对自有 `pending_test` 账户执行强确认放行，用于明确接受上游可用性风险后立即恢复；该操作不等同于人工测试，不适用于授权实例或已过期账户，并继续服从账户时间计划。“重新检查”仍只重置检查窗口并交给后台确认，不直接放行。
...
| AI 问答 | 登录用户使用自己 API Key 的平台内置 OpenAI 客户端；维护可配置保留期（默认 3 天）的完整轮次、站内 SSE、原生工具投影和富文本展示，聊天正文使用独立 SQLite / `juhe_chat` schema，所有模型请求仍按普通客户端进入现有网关 | [AI 问答设计](../functions/AI问答设计.md) |
...
- AI 问答是登录用户会话层和平台内置 OpenAI 客户端，不是新的模型调度层。会话固定绑定当前用户自己的一个 API Key，模型允许在该 Key 当前可用范围内逐轮切换；Node Chat 模块只负责会话、消息、可配置保留期（默认 3 天）的完整成功轮次和站内 SSE，聊天正文不进入 `juhe_business`，standalone 使用独立 `juhe-ai-chat.sqlite3`，performance 使用独立 `juhe_chat` schema 与按日分区的消息表。模型请求必须使用真实 Bearer Key 按实际账号与模型能力重新进入 `/v1/chat/completions` 或 `/v1/responses`，由现有网关继续按普通客户端处理认证、限流、路由策略、分组、账户、代理、额度、计费、使用记录和原始审计。Responses 可透传上游原生工具，本站不执行未注册的本地工具；聊天数据生命周期与审计生命周期相互独立。MVP 不包含 Agent、MCP、Skill 或站内工具执行。
