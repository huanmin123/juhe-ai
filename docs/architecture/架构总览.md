...
- `degraded`、`precheck_pending`、`local_suppressed`、`half_open` 和 `precheck_failed` 只属于网关 / 探测运行态，不新增 `accounts.status` 枚举。普通业务请求事实不能被系统自动解释为账户级状态变更：未命中用户显式策略的失败只允许当前请求换号、按 API Key / IP / 会话局部回避并原子首次投递后台核实信号，不能合并、改写、续期或确认已有账户级事件，也不能写 `temporary_unavailable`；普通请求成功同样不能清理账户级状态。账户所有者主动配置的 `credentials.error_handling_rules` 和响应拦截策略属于明确管理意图，实际命中后允许按规则直接执行账户硬状态或 TTL 运行态动作，不能被“后台探针唯一自动确认”边界拦截。Redis `recovery_wait` 只是内部后台任务，保持普通可调度且不展示账户异常；首次有效独立后台探针确认失败后才进入 `precheck_pending` 共享软阻断，普通调度停止命中，但后台探针、主动健康检查和持有 generation 租约的受控半开仍可访问。健康候选和后备分组优先；全池软阻断时同账户、同分组都只允许一个半开请求，其余请求进入有上限的 FIFO 等待，后备、重试、换号、fetch 和响应读取共用从原始 `startedAt` 计算的最多 270 秒绝对 deadline。后台标准探针不按状态码、错误码或错误文案白名单判断，只按完整成功、已形成真实上游失败、未形成有效上游尝试三类事实推进。前端人工单账户测试只产出诊断结果、使用记录与审计，产品不提供多账户批量测试；无论成功或失败都不能改写账户或授权实例的 `status`、`schedulable`、冷却、最近错误和 API Key 运行态，也不能推进账户内 Key 轮换计数，或写入、确认、清理生产上游桶、客户端 IP 熔断、客户端 IP 账号回避和 Codex turn 账号回避运行态；待检查、停用、异常、限流和临时不可调用账户在测试前后保持原状态。系统自动账户级临时不可调度至少需要 5 分钟、3 个独立后台轮次且轮次间隔不少于 2 分钟；后台恢复探针或主动健康检查完整成功立即清理未完成判断。
...
当前对外主入口是 OpenAI 兼容网关，支持根路径 `/*` 和 `/v1/*`；系统后台、管理 API、健康检查和静态资源统一在 `/__aisys__/*` 下，OpenAI `/v1` 路径归一化和 Base URL 拼接规则见 [客户端网关入口路径设计](../functions/客户端网关入口路径设计.md)。架构层只固定主链路边界：校验本地 API Key、读取额度与路由策略、选择可承接分组、按账户真实能力和模型限制过滤候选、适配上游认证与请求、处理响应、记录使用情况并捕获审计链路。普通请求失败时只在当前请求内重试 / 换号，建立 API Key / IP / 会话局部回避，并在不存在后台事件时原子首次投递 `recovery_wait`；普通成功同样不能清理账户级运行态。`recovery_wait` 保持普通可调度且不展示，只有独立后台探针真实到达上游并确认失败后，才按 generation 在 standalone memory 或 performance Redis runtime state 中建立 `precheck_pending` 软阻断。`precheck_pending` 从普通候选中过滤，但后台探针、主动健康检查和匹配 generation 的受控半开可访问；健康候选和路由策略允许的后备分组优先，全池软阻断时同账户 / 同分组单飞半开，其余请求进入 FIFO，有界等待、重试、换号、后备、fetch 和响应读取共用从原始 `startedAt` 起算的最多 270 秒绝对 deadline。账户所有者显式配置的账户错误策略和响应拦截策略属于主动管理意图，实际命中后可按规则直接执行状态或 TTL 运行态，不被自动探针边界拦截。模型和协议能力判断只在当前路由策略绑定的有限分组和缓存账号快照内完成；混合智能路由仍只负责策略、分组和目标模型，跨协议转换由真实混合供应商账户处理。高并发分组仍在本地维护队列和并发槽，账户探针运行态按 standalone memory / performance Redis 保存；授权实例使用独立运行态键，不能误伤授权方原账户。上游桶和 IP 回避只影响来源或候选排序，不写账户状态；未命中显式策略的普通请求不能直接写 `temporary_unavailable`、`rate_limited` 或 `error`。
...
高并发分组不把上游状态码作为默认健康事实，而是优先使用当前并发、分组排队、首段等待、请求耗时、近期超时、质量缓存和账号软并发占用。普通上游请求异常和非成功响应只影响当前请求换号、局部回避并首次投递后台核实，自动易失探针阶段不屏蔽账户；状态码和错误体只作为用户显式账户错误策略、响应拦截、审计和排障输入。只有命中用户显式策略、后台事前确认完成、后台复测或人工操作时，才写入配置指定的运行态或持久状态。
...
网关流式转发必须按增量方式解析 SSE 状态、usage 和失败事件；成功流不应为了常规解析反复拼接完整响应体。`200 + text/event-stream` 内的协议级失败按“下游是否已经提交 / 是否已有可见输出”统一处理：下游尚未提交时可在当前请求共享预算内重试并切换后续账号 / 分组；预算耗尽后只在当前请求内排除失败账号并首次投递 `recovery_wait`，不直接建立账户级运行态。服务端承接能力耗尽后的最终失败由客户端画像和下游协议决定，`upstream_retryable_error` 只表达网关建议客户端重试，不继承任何上游错误分类。普通路由 `speed_first` 的慢样本只由后台探针确认后建立 `latency_degraded`，普通请求不能直接改变账户级运行态。只有独立后台探针真实失败才建立 `precheck_pending`；performance 使用 Redis runtime state 的 generation / lease CAS，健康和后备优先，全池软阻断时同账户 / 同分组单飞半开，其余 FIFO 等待；从原始 `startedAt` 起，排队、重试、换号、后备、fetch 和响应读取共用最多 270 秒绝对 deadline。可见输出后不静默拼接第二条流。非 `2xx` 错误不按状态码、错误码或文案内置特征规则短路；显式账户错误 / 响应拦截策略命中后可直接执行用户配置动作，未命中时只首次投递后台核实。慢客户端读取必须尊重 backpressure，客户端断开时关闭上游并释放并发槽。`/v1` raw body 入口硬上限、文本 / 图像 lane 限额、worker 元数据扫描和原样透传边界保持不变；上传中断返回 HTTP `408/request_timeout` 并归因 `client_lifecycle`，完整但非法 JSON 返回 HTTP `400`，两者都不执行账户状态副作用。
