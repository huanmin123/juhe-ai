...
- 策略路由是 API Key 和分组之间的独立层。API Key 只绑定一条路由策略；路由策略再绑定一个或多个分组，并按普通路由、混合智能路由、权重调度、故障回退或轮询选择候选分组。普通路由只能绑定一个分组，且不开放复杂配置。系统会自动创建内置默认分组，并为除“默认混合供应商分组”以外的每个默认分组创建默认普通路由和默认 API Key；混合供应商路由属于高级路由，不作为默认入口。
