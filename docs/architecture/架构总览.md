...
下一版目标设计将进一步拆分“下游客户端画像”和“上游接入契约”：账号表单只展示真实上游接入契约和 endpoint modes，不再把泛化 `client_compatibility` 作为普通账号配置；API Key 或混合路由规则可指定下游客户端画像，默认自动识别。普通账号模型映射收敛为当前供应商内模型别名，跨供应商、跨协议和 Codex/GPT 模型名打到 Anthropic / Gemini / DeepSeek 等高级场景进入独立混合路由，详见 [客户端画像与混合路由设计](../functions/客户端画像与混合路由设计.md)。

当客户端协议和上游真实能力不一致时，协议转换必须作为共享 adapter 落在 provider driver 可复用边界内，而不是写成某个供应商的私有分支。协议转换只能由账号模型映射显式触发，映射保存 `sourceModel + sourceEndpointFamily -> upstreamModel + upstreamEndpointFamily`。长期设计按 [协议桥接框架设计](../functions/协议桥接框架设计.md) 收敛为“下游协议 -> 统一生成中间表示 -> 上游协议”，新增独立协议时默认补一次入站 adapter 和一次出站 adapter，不能继续扩散成 N×N 私有转换。
