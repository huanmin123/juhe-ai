...
当客户端协议和上游真实能力不一致时，协议转换必须作为共享 adapter 落在 provider driver 可复用边界内，而不是写成某个供应商的私有分支。OpenAI 协议档案下的协议转换只能由账号模型映射显式触发，映射保存 `sourceModel + sourceEndpointFamily -> upstreamModel + upstreamEndpointFamily`；当前支持的白名单桥接包括 OpenAI 协议档案内的 `responses -> chat_completions`、真实 Responses 上游的 `chat_completions -> responses`，以及 PLAN-0058 定义的 `chat_completions|responses -> messages`。`chat_completions -> responses` 只允许账号真实支持 `responses_json` 或 `responses_sse` 的 OpenAI/GPT 原生上游，仍不支持 Anthropic Messages 到 OpenAI Chat / Responses 的反向转换。账户 `supported_endpoint_modes` 仍保存真实上游能力，例如 `chat_sse`、`responses_sse` 或 `messages_sse`，不要为了桥接把账号伪装成原生 `responses_sse`。
