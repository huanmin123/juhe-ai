...
- `degraded`、`precheck_pending`、`local_suppressed`、`half_open` 和 `precheck_failed` 只属于网关 / 探测运行态，不新增 `accounts.status` 枚举。普通业务请求事实不能被系统自动解释为账户级状态变更：未命中用户显式策略的失败只允许当前请求换号、按 API Key / IP / 会话局部回避并原子首次投递后台核实信号，不能合并、改写、续期或确认已有账户级事件，也不能写 `temporary_unavailable`；普通请求成功同样不能清理账户级状态。账户所有者主动配置的 `credentials.error_handling_rules` 和响应拦截策略属于明确管理意图，实际命中后允许按规则直接执行账户硬状态或 TTL 运行态动作，不能被“后台探针唯一自动确认”边界拦截。Redis `recovery_wait` 只是内部后台任务；全部系统自动易失探针阶段只能观察和展示，不能从候选池移除账户。后台标准探针不按状态码、错误码或错误文案白名单判断，只按完整成功、已形成真实上游失败、未形成有效上游尝试三类事实推进。前端人工单账户测试只产出诊断结果、使用记录与审计，产品不提供多账户批量测试；无论成功或失败都不能改写账户或授权实例的 `status`、`schedulable`、冷却、最近错误和 API Key 运行态，也不能推进账户内 Key 轮换计数，或写入、确认、清理生产上游桶、客户端 IP 熔断、客户端 IP 账号回避和 Codex turn 账号回避运行态；待检查、停用、异常、限流和临时不可调用账户在测试前后保持原状态。系统自动账户级临时不可调度至少需要 5 分钟、3 个独立后台轮次且轮次间隔不少于 2 分钟；后台恢复探针或主动健康检查完整成功立即清理未完成判断。
