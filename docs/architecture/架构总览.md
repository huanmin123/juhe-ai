...
- 页面数据读取：`PLAN-0115` 目标链路是“IndexedDB 先显示、30 秒轻量变更确认、领域字段 / cursor / 统计区块增量更新、服务端 Redis / 内存投影、数据库有界保底”，当前尚未实现统一 confirm 和全页面缓存。计划在 Node standalone / performance 暴露同一 HTTP 契约，分别由 SQLite outbox + memory / IPC 和 PostgreSQL outbox + Redis 承接；Node / Go 共存期共享 revision 和事件协议，Go 长期只保留 PostgreSQL + Redis。具体边界见 [页面数据缓存与增量更新设计](../functions/页面数据缓存与增量更新设计.md)。
...
- 管理页高频列表采用数据域 revision / confirm 与 IndexedDB cache-first：缓存 key 必须包含登录主体、管理目标、路由、筛选、排序、分页和协议版本；账户列表最大陈旧 30 秒，使用记录 15 秒。KeepAlive 页面停用后必须停止确认与状态轮询，恢复时立即同步；有效 token 的人工强刷采用网络快照后单次确认，同作用域确认复用在途请求并按微任务跨域合并。confirm 只读 memory / Redis 修订令牌，禁止回源业务明细库；System API 按 read 请求处理，不 touch session。performance 模式的多域确认由单个 Redis Lua 原子读取 epoch、域序列、全域 reset 序列和必要日志，unchanged 热路径每次只发生一次 `EVAL`。写操作提交后发布 owner / allScopes 变更；发布或 Redis 故障通过 db-service 中按 domain generation 持久的 dirty outbox 保底，恢复后发布全域 reset 并用 generation CAS 清理。db-service 启动时一次加载 dirty 表到内存，confirm 热路径只读内存 Map；重放使用单 timer、有界指数退避和 singleflight，Redis 未恢复不能阻止 db-service 启动。
...
- 页面接口缓存、变更确认、增量更新、服务端页面投影和预热边界变化时，同步更新 [页面数据缓存与增量更新设计](../functions/页面数据缓存与增量更新设计.md) 以及对应存储、迁移和验证文档。
