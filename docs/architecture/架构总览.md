...
- 对外中转协议：客户端入口按 OpenAI-compatible 形态暴露，支持根路径和 `/v1`；`openai/v1` 是协议层语义，`provider_code = openai` 是通用 OpenAI-compatible 供应商语义，二者通过字段所在层级区分。当前内置供应商为 `openai`、`gpt` 和 `anthropic`；`anthropic` 官方直连供应商使用原生 `anthropic/v1` Messages 协议，不能和第三方 Anthropic-compatible 入口混在一个档案里；目标新增 `glm` 智谱供应商，通用 GLM API 与 GLM Coding Plan 都按 OpenAI Chat Completions 兼容形态接入，但必须拆成不同供应商协议档案；目标新增 `deepseek` 供应商，只接入 OpenAI-compatible Chat Completions 档案，不接入 DeepSeek Anthropic-compatible surface。`gpt.parent_code = openai`，继承通用 OpenAI v1 能力并叠加 GPT / Codex 专属能力。通用 OpenAI 兼容 API Key 账号、GPT API Key 账号、Anthropic 官方 API Key 账号、目标 GLM API Key 账号和目标 DeepSeek API Key 账号按各自上游入口透传，GPT OAuth 账号只支持 Codex adapter 明确列出的 Responses 路径；Anthropic OAuth / Setup Token / Claude Code token 账号链路不纳入当前目标，Claude Code 仅作为下游客户端画像接入 Anthropic native API Key 路径；模型目录上，`openai` 作为 OpenAI v1 协议聚合目录，可以看到所有启用 OpenAI v1 供应商的模型，`gpt`、`anthropic`、`glm` 和 `deepseek` 都只看各自供应商模型。
...
| 供应商 | `provider_code` | 表示账号池、模型目录和价格目录归属；当前内置 `openai` 通用 OpenAI-compatible 供应商、`gpt` 子供应商和 `anthropic` 官方 Claude 供应商，目标新增 `glm` 智谱供应商和 `deepseek` 供应商。`openai` 同时可以是协议代码和供应商代码，但必须通过 `protocol_code` / `provider_code` 字段区分。 |
| 供应商协议档案 | `provider_protocol_profile_id` | 把供应商和协议版本绑定成一个可调度能力档案，例如 `profile_gpt_openai_v1`。账户、分组和账号测试都必须落到该档案。 |

客户端策略是运行时附加层，不作为供应商或协议档案的用户侧选择项。`client_compatibility` 只作为账号能力派生输入，client profile 和请求侧 `requestClientCompatibility` 用于描述 Codex、Claude Code、普通 OpenAI SDK、普通 Anthropic SDK 等客户端期望的重试、请求体归一化和 SSE 失败事件语义；候选账号筛选和上游请求准备都以请求侧兼容能力驱动，这些行为只能由入口、请求上下文、端点族和账户档案共同识别，不能写进通用协议层。

当前启用两个 OpenAI v1 档案和一个 Anthropic v1 档案：`profile_openai_openai_v1` 表示通用 `openai` 供应商使用 OpenAI v1 协议，承载 API Key 透传、OpenAI v1 聚合模型目录和通用协议策略；`profile_gpt_openai_v1` 表示 `gpt` 子供应商使用 OpenAI v1 协议，继承通用 OpenAI-compatible 能力，并叠加 GPT OAuth、Codex Responses、Codex 客户端策略和 GPT 自身模型目录优化；`profile_anthropic_anthropic_v1` 表示官方 Anthropic Claude 原生 `anthropic/v1` Messages 档案，当前只允许 `api_key` 账号类型，Anthropic OAuth / Setup Token / Claude Code token 账号链路不纳入当前目标，Claude Code 下游客户端画像通过显式本地 header 识别。目标新增 GLM 时，`profile_glm_general_openai_v1` 表示智谱通用 GLM API 的 OpenAI Chat Completions 档案，`profile_glm_coding_openai_v1` 表示 GLM Coding Plan 的 OpenAI Chat Completions 档案；二者都属于 `provider_code = glm`，但默认 Base URL、分组、额度解释和账号测试边界不同。目标新增 DeepSeek 时，`profile_deepseek_openai_v1` 表示 DeepSeek OpenAI-compatible Chat Completions 档案，当前不新增 DeepSeek Anthropic-compatible 档案。后续即使新增另一个同样使用 OpenAI v1 的供应商，也必须新增自己的 `provider_code` 和 `provider_protocol_profile_id`，不能和 GPT、Anthropic、GLM 或 DeepSeek 共用账号池、分组或价格目录；这类供应商的模型会进入 `openai` 聚合目录。Anthropic 原生档案只进入自身供应商目录，不进入 `openai` 聚合目录。
