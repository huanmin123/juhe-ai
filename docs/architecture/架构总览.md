...
- 客户端模型目录不属于账户调度运行态，也不复制到 API Key。公开 `/models`、`/v1/models` 聚合全部已启用的真实供应商目录；携带有效 API Key 时，从该 Key 路由策略的全部 active 分组绑定收集供应商并聚合，不只取当前选中分组，空绑定也不回退公开全量。AI 问答按会话 API Key 的相同路由供应商合集直接复用该动态目录，新会话默认使用列表第一项。聚合直接复用供应商级进程内 / Redis 模型目录缓存，只返回启用、可见、可计价模型，不按发布时间过滤。网关和 AI 问答都不再读取 `default/codex`、`chat_list:*` 或 `chat_model:*` 最终响应快照；`gateway_model_catalog_snapshots` 仅作为尚未清理的历史基础设施保留，不是当前模型目录事实来源。
...
GPT 模型目录保留无独立官方价格的 `codex-auto-review`，仅声明 Responses。它进入 GPT 专属 `DEFAULT_GPT_SUPPORTED_MODELS`，新建 GPT API Key / OAuth 账户默认勾选并可参与调度；它不进入通用 `DEFAULT_OPENAI_SUPPORTED_MODELS`、默认检查模型、公开已计价目录或授权配置，已有账户的支持模型也不被自动改写。
