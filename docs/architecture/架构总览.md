...
用户侧账户创建流程按“供应商 -> 账户类型 -> 凭据与调度配置”展开；页面不直接展示内部供应商协议档案 ID，但可以用“接入类型 / 客户端兼容”文案承接不同档案选择。内部流程由后端根据供应商和接入类型解析 `provider_protocol_profile_id`，并据此决定端点族、账户类型、测试策略和调度边界。分组同样绑定一个供应商协议档案；账户只能加入相同 `provider_protocol_profile_id` 的分组。API Key 可以绑定多个供应商协议档案分组，但运行时只在当前 Key 已绑定的有界分组集合内调度：普通路由请求带 `model` 时先用模型目录唯一供应商命中或账号支持模型 / 模型映射显式命中收窄目标档案，再只在该档案的绑定分组内执行 API Key 路由策略；无法明确定位时不猜测全局账号池，继续按本 Key 候选顺序做请求能力、模型过滤和后备分组处理。
...
- 模型目录与价格是网关调度和计费的前置事实：内置模型来自官方目录，管理员可维护全局自定义模型，用户可维护个人自定义模型；`/v1/models`、账号支持模型、模型检测候选、账号模型映射和成本估算都必须从调用方可见的本地模型目录读取。普通 OpenAI-compatible `/v1/models` 返回标准 `object/list + data`，Codex 模型刷新请求返回同一目录派生出的 `{ models: ModelInfo[] }`，这是客户端响应形态分支，不新增模型目录来源。当前普通跨供应商 API Key 先用请求 `model` 在该 Key 已绑定供应商范围内做有界目标档案收窄；网关 runtime cache 已为当前 Key 的绑定供应商集合构建 `modelKey -> providerCode[]` 索引，同一供应商集合热命中按 `model` 做 O(1) 查询。PLAN-0044 继续推进客户端模型名全系统唯一和全局模型路由事实，避免通过个人 / 全局 / 内置目录覆盖式去重导致路由不确定。账号模型限制使用下游请求模型过滤候选账号；账号模型映射只在选中具体账号后把下游模型改写成上游模型，并在使用记录中同时保留下游模型、上游模型和实际计价模型。
...
- API Key 可以绑定调用方自己的一个或多个分组，也可以绑定有效授权给调用方的分组，绑定分组可以来自不同供应商协议档案。当前请求进入后，普通路由先按请求 `model` 尝试筛出目标供应商协议档案分组，再在目标分组集合内按主备、轮询或权重策略选号池；未带模型或无法明确定位时继续使用 API Key 候选顺序和分组内账号过滤。授权暂停、过期、回收、归还或额度不可用时，授权分组绑定配置保留但运行态不可用。
