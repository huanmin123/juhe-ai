...
- 后台定时任务必须在独立 worker 子进程内注册和执行；当前主进程会固定拉起默认 `worker`、`metrics-worker` 和 `ingest-worker` 三类 worker。
...
- 后续多 worker 拆分时，任务必须同时登记角色和生命周期；系统采样、写入、日志、统计、探测属于持久 worker，表监控 `non_business_data_cleanup`、手动清理、历史重建和一次性修复优先归临时 worker，跑完退出。当前 `usage_records_cleanup` 和 `non_business_data_cleanup` 已由默认 worker 投递到 `temporary-maintenance-worker` 执行。
...
flowchart LR
  ...
  Scheduler --> Jobs["统计 / 维护 / 探测任务"]
  Worker --> SQLite["SQLite 聚合 / 清理 / 状态维护"]
...
| `backend/src/worker.ts` | worker 入口，根据 `JUHE_AI_WORKER_ROLE` 启动默认 `worker`、`metrics-worker` 或 `ingest-worker`；默认 worker 安装统计 / 维护 / 探测任务和账号测试队列，metrics-worker 只安装系统采样和控制消息，ingest-worker 安装 append-only 写队列和运行日志导入 |
| `backend/src/modules/background/background-worker-supervisor.ts` | 主进程内的默认 worker / metrics-worker / ingest-worker 启动、退出看护和退避重启 |
| `backend/src/modules/background/background-ipc.ts` | 主进程到默认 worker 与 ingest-worker 的有界 IPC 消息队列，同时维护 metrics-worker / ingest-worker 运行态、快照和事件循环采样控制 |
| `backend/src/modules/background/worker-scheduler.ts` | worker 内轻量调度器，支持不可重入和错误隔离 |
| `backend/src/modules/background/background-jobs.ts` | 后台业务任务注册和任务函数；按 worker role 过滤 `system-metrics-sample` 与普通业务任务 |
| `backend/src/modules/background/data-retention-cleanup.service.ts` | 统一表数据保留期清理任务 |

当前注册任务还包括：

- `system-metrics-sample`：固定由 `metrics-worker` 执行，负责系统指标采样、小时桶写入和进程事件循环采样协调。默认 worker 即使被重统计任务占用，也不能阻断该任务持续采样。
...
6. 补充验证记录。
   ...
   - 运行态确认 server、默认 worker、metrics-worker、ingest-worker 和 DB service 的 PID 不同，并确认统计 / 维护 / 探测任务日志出现在默认 worker 侧、系统采样任务出现在 metrics-worker 侧、append-only 写入和运行日志导入出现在 ingest-worker 侧。
...
| 数据集 / 统计结果维护 | `enqueueRecordMaintenanceJob()` | 默认 worker 调用 `enqueueRecordMaintenanceJobsLocal()`；快照和轻量关联清理本地小批执行，`usage_records_cleanup` / `non_business_data_cleanup` 创建 `background_task_runs` 后交给 `temporary-maintenance-worker` |
...
- 运行态 server、默认 worker、metrics-worker、ingest-worker 和 DB service 的 PID 不同。
