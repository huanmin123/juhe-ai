...
- 日志搜索不维护额外回填任务；运行日志 keyword 只查 `runtime_logs.message`，操作日志 keyword 读取随操作日志写入同步生成的 `operation_log_search_terms` 倒排词项，不能在列表请求里扫描 `operation_logs` 主表字段。
