...
- 第一阶段不引入 Redis、Kafka、BullMQ、Agenda 等外部队列；默认使用本地有界 IPC 和 worker 内存队列。
...
- `proxy-latency-refresh`：默认每 60 秒在 worker 内批量检测启用代理，检测目标来自已启用供应商默认地址，并更新代理最近检测状态和延迟。
...
| `audit_logs`、`audit_log_attempts`、`audit_log_payloads` | 7 天 | 7 天 | 原始审计日志，依赖外键级联清理子表 |
...
- 多实例部署前不能假设只有一个 worker；当前第一阶段默认单实例，多实例需要共享锁或显式任务归属。
...
主进程不应保留统计聚合、日志索引批量写入、审计批量落库、清理、OAuth 快照刷新、账号复测等业务后台 timer。
