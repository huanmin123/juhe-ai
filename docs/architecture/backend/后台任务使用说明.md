...
| `usage_records` | 7 天 | 7 天 | 自动保留任务只清理已被统计游标处理过的事实记录；管理员手动清理另按最近 1 天保护执行 |
...
管理员在表监控页面手动清理 `usage_records` 时，产品口径是“只强制保护最近 1 天”：只要截止时间早于当前时间 24 小时，后端就按 `created_at < cutoffAt` 做有限预检查并投递 worker 分批删除，不再按统计游标阻塞；没有可清理记录或 worker 投递不可用时返回 `queued = false` 和原因。
