...
- `table-storage-monitor`：默认每 10 分钟采样业务库和记录库的文件大小、页数、空闲页和表 / 索引数量；表大小和增长趋势按游标轮转采样，默认每轮每个库最多刷新 4 张表，采样历史默认保留最近一月。后台任务默认不做精确 `COUNT(*)`；FTS5 影子表始终跳过行数统计，避免对日志全文索引产生额外扫描。
...
- `cooldown-account-retest`：默认每 60 秒检查一次冷却到期账号，只复测 `temporary_unavailable`、仍可调度、已绑定分组且未过期的 OpenAI 账号；`rate_limited`、`error`、`disabled` 等其他状态不进入后台复测，不做自愈恢复。复测会优先学习最近真实请求的 `endpoint/model/stream` 元信息生成最小探活请求，不读取或重放用户 prompt、工具参数和完整请求快照；单次复测复用真实网关链路，内部请求时长和同账号短重试沿用网关默认调度配置。复测连续失败前 3 次仍进入统一异步重试队列，按 3 秒、10 秒、30 秒确认；第 4 次失败开始按 `defaultTemporaryUnschedulableMinutes` 起步做指数退避，每次失败后翻倍更新 `cooldown_until`。下一次退避超过系统设置 `cooldownAccountRetestMaxBackoffHours` 时，账号转为 `status = error`、退出调度，并保留最近测试 HTTP 状态、错误码、错误摘要和连续失败次数。worker 运行态会暴露该复活队列的待执行数、运行中数和下次唤醒时间，统计页后台任务表在该任务行内展示摘要。
- `log-search-index-backfill`：默认每 1 小时按 `stats_job_state` 游标分批补齐 `operation_log_search` 和 `runtime_log_search`，用于升级后已有日志主表但 FTS 行缺失的历史数据；单轮只处理有限批次，先删除同批搜索行再插入，避免重复命中。
...
| 记录库维护 | `enqueueRecordMaintenanceJob()` | `enqueueRecordMaintenanceJobsLocal()` 后执行 API Key 关联记录清理、使用记录手动清理、账号用量快照写入等动作 |
...
使用记录、操作日志和记录库维护队列即使在 drain 模式下也可以限制单次 flush 批数；后台聚合前只做有限批次落库或维护，剩余积压交给后续 timer tick 继续处理，避免 worker 在高峰后一次性同步写库太久。
...
| `usage_records` | 7 天 | 7 天 | 自动保留任务和管理员手动清理都只清理已被统计安全游标确认处理过的事实记录；手动清理的最近 1 天只是最小年龄门槛 |
...
管理员在表监控页面手动清理 `usage_records` 时，产品口径是“先强制保护最近 1 天，再按统计安全游标清理”：只有截止时间早于当前时间 24 小时，后端才会继续按 `created_at < cutoffAt` 做有限预检查；首批可清理数量只统计已被主统计聚合游标和必要回填 / 迁移游标覆盖的明细。游标未就绪、未追平、没有可清理记录或 worker 投递不可用时返回 `queued = false`、`eligibleRows = 0` 和原因，等待 worker 追平后再重试。
...
- 日志搜索索引回填只能按游标小批次推进，不能在启动时一次性 `INSERT INTO ... SELECT` 重建全量 FTS，也不能在查询请求里临时补索引。
