...
> **迁移现行边界（2026-08-08）。** 本文描述当前 Node owner；Go 必须同时支持 SQLite 与 PostgreSQL/Redis，但不按 task、queue 或局部链路接管。首个 F1 候选是“运行日志索引与保留”完整功能：先将 Node 的专属启动、服务、测试和配置整理成完整文件集，再由 Go 完整实现、成为唯一 owner，并将 Node 专属文件归档。SQLite 的 owner bridge 只能用于 B0 兼容验证或未迁 Node 功能，F3 后不得作为 Go 功能依赖。账户健康检查、冷却复测、余额、OAuth、代理、模型质量、账户电路和网关 quota 发布均不在首期正式接管范围。完整规则见 [完整功能接管与 Node 归档迁移规则](../../migration/完整功能接管与Node归档迁移规则.md)。
...
| `runtime-log-file-import` | 运行日志只从角色 JSONL 文件按 offset / cursor 有界增量写入；批次提交成功后推进 cursor，禁止整文件读入内存。`JUHE_AI_RUNTIME_LOG_INDEX_ENABLED=false` 时停止扫描和索引写入，但文件输出、普通轮转清理与 grep 保留 |
...
| `audit-hot-retention-cleanup` / `runtime-log-index-maintenance` / `data-retention-cleanup` dataset 部分 | 小批多轮，不能压住高优先级写入；运行日志索引开关关闭时不调度索引维护，也不删除历史 runtime 索引/cursor/facets；PostgreSQL performance 下 `data-retention-cleanup` 只按系统设置投递 record-maintenance 任务 |
...
- `background-ipc` 中使用记录、审计、操作日志、公开接口日志和 record maintenance 路由到 `ingest-worker`；普通运行日志不得进入 IPC 或 Redis Stream，只能由 `runtime-log-file-import` 消费角色日志文件；账号测试和取消路由到 `ops-worker`；stats write request 路由到 `stats-worker`。
