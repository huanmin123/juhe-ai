...
- `account-quality-refresh`：默认每 10 分钟刷新账号质量缓存；先汇总近 10 分钟真实网关使用记录，再只对近 24 小时有真实网关请求、同分组、同超级优先状态、同优先级且候选数 `>= 2` 的 OpenAI 账号复用“测试账号连接”主动探测，prompt 固定 `hi`，每账号 3 次取首 token 最大值。超过 24 小时无真实网关请求的分组暂停主动探测，避免长期空闲客户持续消耗上游 token。
