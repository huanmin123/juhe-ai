...
| Go F2 `juhe-ai-table-monitor` | 每分钟直接异步并发采样 SQLite 源库或 PostgreSQL schema，写入专用 F2 SQLite 输出库或 `juhe_stats`，由 Go 负责 owner lease 与 retention；Node 只读 HTTP 查询，不注册定时器、writer 或清理任务。 |
