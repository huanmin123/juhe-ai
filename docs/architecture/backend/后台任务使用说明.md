...
- `table-storage-monitor`：默认每 10 分钟采样业务库、统计数据集库和统计结果库的文件大小、页数、空闲页和表 / 索引数量；表大小、可推导行数和增长趋势按游标轮转采样，默认每轮每个库最多刷新 4 张表，采样历史默认保留最近一月。后台任务通过 `dbstat` 叶子页 cell 数滚动写入行数，不提供精确 `COUNT(*)` 采样分支。
...
- `cooldown-account-retest`：默认每 3 秒检查一次冷却到期账号，只复测 `temporary_unavailable`、仍可调度、已绑定分组且未过期的 OpenAI 账号；`rate_limited`、`error`、`disabled` 等其他状态不进入后台复测，不做自愈恢复。复测会优先学习最近真实请求的 `endpoint/model/stream` 元信息生成最小探活请求，不读取或重放用户 prompt、工具参数和完整请求快照；单次复测复用真实网关链路，但候选只包含当前复测账号，失败退避由复测任务自身控制。账号进入 `temporary_unavailable` 后先按 3 秒进入快速恢复通道，连续失败按 `3s -> 6s -> 12s -> 24s -> ...` 翻倍更新 `cooldown_until`；超过快速阈值后退化到慢速恢复，单次等待不超过 `defaultTemporaryUnschedulableMinutes` 表达的最大暂停时间。账号进入 `temporary_unavailable` 时记录本轮自动恢复观察起点，进入慢速恢复后若失败发生在 `cooldownAccountRetestMaxBackoffHours` 表达的最长自动恢复观察之后，账号转为 `status = error`、退出调度，并保留最近测试 HTTP 状态、错误码、错误摘要、观察起点和连续失败次数。恢复探活的使用记录和审计均标记 `traffic_source = cooldown_retest`，快速通道失败只保留低噪声诊断。worker 运行态会暴露该复活队列的待执行数、运行中数和下次唤醒时间，统计页后台任务表在该任务行内展示摘要。
## 3. 新增定时任务步骤
...
使用记录、操作日志和数据集 / 统计结果维护队列即使在 drain 模式下也可以限制单次 flush 批数；后台聚合前只做有限批次落库或维护，剩余积压交给后续 timer tick 继续处理，避免 worker 在高峰后一次性同步写库太久。
...
管理员在表监控页面手动清理 `usage_records` 时，产品口径是“先强制保护最近 1 天，再按统计安全游标清理”：只有截止时间早于当前时间 24 小时，后端才会继续按 `created_at < cutoffAt` 做有限预检查；首批可清理数量只统计已被当前主统计聚合游标和必要维护游标覆盖的明细。游标未就绪、未追平、没有可清理记录或 worker 投递不可用时返回 `queued = false`、`eligibleRows = 0` 和原因，等待 worker 追平后再重试。
...
- 日志搜索不维护额外搜索影子表回填任务；运行日志 keyword 只查 `runtime_logs.message`，操作日志 keyword 只查 `operation_logs` 主表字段。
