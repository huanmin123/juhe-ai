...
| `cooldown-account-retest` / `account-api-key-cooldown-retest` | 完整诊断统一最多 3 路并发，启动首轮分别延迟 60 / 65 秒，失败退避；写使用记录仍投递 ingest |
...
- `ops-worker` 账号健康检测、账号级冷却复测、Key 级冷却复测、账号质量 full diagnostic 失败预检和速度优先恢复探针按 batch size 派生并发；同一 worker 进程内的完整诊断队列共享最多 3 路门禁，批量设置和多个队列同时到期都不能放大该进程访问上游和 DB service 的总并发。server 运行态恢复探针、升级后的 precheck 和 Redis 运行态探针共享最多 3 路门禁；不同 worker 不新增 Redis 全局诊断锁。
