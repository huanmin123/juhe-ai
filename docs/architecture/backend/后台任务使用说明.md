...
- `ingest-worker` 承载使用记录、审计、操作日志和运行日志索引四类 append-only 写入，以及运行日志文件增量导入；默认 `worker` 不能直接执行这些本地写队列。
...
flowchart LR
  ...
  IngestWorker --> IngestQueues["usage / audit / operation / runtime log 队列"]
