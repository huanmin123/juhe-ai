...
- `/__aisys__/api/stats/system-metrics` 只读预聚合趋势和进程采样，不请求 server runtime 或 Redis Stream 队列；进程事件循环角色固定为 `server`、`ingest-worker`、`stats-worker`、`ops-worker`、`db-service`。
