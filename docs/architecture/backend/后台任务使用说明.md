...
- `table-storage-monitor`：默认每 10 分钟采样业务库、统计数据集库和统计结果库的文件大小、页数、空闲页和表 / 索引数量；表大小和增长趋势按游标轮转采样，默认每轮每个库最多刷新 4 张表，采样历史默认保留最近一月。后台任务默认不做精确 `COUNT(*)`，只在显式诊断场景统计本轮选中的普通表行数。
