...
- `table-storage-monitor`：默认每 10 分钟采样业务库、数据集目录库、usage shard 和统计结果库的文件大小、页数、空闲页和表 / 索引数量；表大小、可推导行数和增长趋势按游标轮转采样，默认每轮每个库最多刷新 4 张表，采样历史默认保留最近一月。后台任务通过 `dbstat` 叶子页 cell 数滚动写入行数，不提供精确 `COUNT(*)` 采样分支。
