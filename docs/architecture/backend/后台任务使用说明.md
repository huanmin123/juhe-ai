...
- 后续多 worker 拆分时，任务必须同时登记角色和生命周期；系统采样、写入、日志、统计、探测属于持久 worker，表监控 `non_business_data_cleanup`、手动清理、历史重建和一次性修复优先归临时 worker，跑完退出。
...
在多 worker 方案落地后，`non_business_data_cleanup` 不应长期占用常驻 `maintenance-worker`：常驻 worker 负责投递和状态记录，实际硬清理由临时维护 worker 按本轮批次执行，完成 / 失败 / 超时后退出。
