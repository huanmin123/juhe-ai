...
- `table-storage-monitor`：默认每 10 分钟采样业务库和记录库的文件大小、页数、空闲页和表 / 索引数量；表大小、行数和增长趋势按游标轮转采样，默认每轮每个库最多刷新 4 张普通表，采样历史默认保留最近一月。后台任务不对所有表做全量 `COUNT(*)`；FTS5 影子表跳过行数统计，避免对日志全文索引产生额外扫描。
