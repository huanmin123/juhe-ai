...
- 页面数据读取：Node `PageDataChangeStore`、`data-changes/confirm`、page-data read-through cache 和相关 IPC 已退役。页面和轻量 options 通过各自的有界 API 直读事实库或已有预聚合表；需要短时缓存的特定资源由各领域显式定义容量、TTL 和失效边界，不再接入通用页面变更确认协议。
