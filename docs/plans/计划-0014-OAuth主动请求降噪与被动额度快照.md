...
- 源码和本地 SQLite 不再保留主动探测遗留字段 `last_probe_at`。
...
- 冷却复测仍会产生恢复性测试请求；当前已避免固定 `hi` 和固定 stream 形态，并在 BUG-0013 中增加退避上限。没有真实样本时仍会回退到最小 Responses 探活，后续如风控仍敏感，应继续增加更严格的批量预算和审计来源。
