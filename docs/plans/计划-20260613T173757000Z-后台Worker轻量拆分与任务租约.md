# PLAN-20260613T173757000Z 后台 Worker 轻量拆分与任务租约

> **历史 Node worker 规划。** 文中提及的 J3a 代理延迟刷新不再属于 `ops-worker`；J3a 已由 Go `juhe-ai-jobs/proxylatency` 独占，Node 不注册 scheduler、executor、projector 或 business writer。

## 基本信息

- 计划 ID：PLAN-20260613T173757000Z
- 状态：进行中
- 创建时间：2026-06-13
- 更新时间：2026-06-14
- 需求来源：用户对话 / 生产排障
...
| 文档 | 关系 |
| --- | --- |
| `docs/plans/计划-20260505T163917000Z-后台任务进程隔离.md` | 已完成的单 background worker 进程隔离基础 |
...
- 2026-06-14 完成阶段 3 首批 append-only ingest-worker 代码落地，已执行：
  ...
  - `pnpm --filter juhe-ai-backend test:usage-record-snapshot-boundary`
...
- 主要改动位置：`backend/src/modules/background/`、`backend/src/worker.ts`、`backend/src/config/runtime.ts`、`backend/src/shared/process-event-loop-monitor.ts`、`backend/src/storage/`、`backend/src/modules/db-service/`、`backend/src/modules/stats/stats.routes.ts`、`backend/src/scripts/regression/`、`backend/src/scripts/maintenance/`、`frontend/src/views/stats/`、`frontend/src/types/domain/`、`docs/plans/计划-20260613T173757000Z-后台Worker轻量拆分与任务租约.md`、`docs/architecture/架构总览.md`、`docs/architecture/backend/后台Worker多角色拆分设计.md`、`docs/architecture/backend/后台任务使用说明.md`、`docs/develop/运行说明.md`、`docs/deploy/部署指南.md`、`docs/functions/`
