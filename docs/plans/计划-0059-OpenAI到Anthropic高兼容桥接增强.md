...
- Mock 回归：2026-06-25 已复跑并通过 `pnpm --dir backend test:openai-anthropic-bridge-mock`，覆盖四入口、采样 / 输出控制本地拒绝、工具调用、reasoning、图片 / 文件输入、structured output、thinking、hosted tool guidance、code_interpreter mock、本地图像 provider、file_id 边界、compact、跨 API Key snapshot 和 Codex `previous_response_id`。MCP 当前只认 guidance；历史 MCP mock 覆盖不再作为当前能力。
