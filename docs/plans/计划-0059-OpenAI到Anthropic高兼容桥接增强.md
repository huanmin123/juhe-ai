...
- [x] 补齐 Responses 非等价输出扩展 include 边界：`web_search_call.action.sources`、`computer_call_output.output.image_url`、`message.input_image.image_url` 本地拒绝，`file_search_call.results` 保持允许；`code_interpreter_call.outputs` 仅在 PLAN-0063 的 `code_interpreter=mock` runtime 下返回固定 logs。
...
| Mock 回归 | 基础四入口回归 | `pnpm --dir backend test:protocol-boundary-openai-anthropic` | Chat / Responses JSON / SSE 保持通过 | 已通过 | 已通过 |
| Mock 回归 | 工具历史闭合 | `pnpm --dir backend test:protocol-boundary-openai-anthropic` | Chat / Responses orphan 与 duplicate 工具结果返回本地 OpenAI 错误，且不命中 Anthropic | 已通过 | 2026-06-24 复跑通过，Chat / Responses orphan / duplicate 工具结果均本地 400 且不请求 Anthropic |
| Mock 回归 | Function tool_choice | `pnpm --dir backend test:protocol-boundary-openai-anthropic` | Responses `allowed_tools` function 子集只发送允许工具；reasoning + 强制工具调用本地 400 且不请求 Anthropic | 已通过 | 2026-06-24 复跑通过，Anthropic 上游只收到 allowed function tool；thinking + required tool_choice 本地拒绝且不上游 |
| Mock 回归 | Function 并行工具约束 | `pnpm --dir backend test:protocol-boundary-openai-anthropic` | Chat / Responses `parallel_tool_calls=false` 映射为 Anthropic `tool_choice.disable_parallel_tool_use=true`；`tool_choice=none` 不附加该字段 | 已通过 | 2026-06-24 复跑通过，Chat auto 映射 `{type:auto, disable_parallel_tool_use:true}`，Responses required 映射 `{type:any, disable_parallel_tool_use:true}`，Chat none 保持 `{type:none}` |
| Mock 回归 | Function 工具调用次数边界 | `pnpm --dir backend test:protocol-boundary-openai-anthropic` | Responses `max_tool_calls` 存在且非 `null` 时本地 400，且不请求 Anthropic | 已通过 | 2026-06-25 复跑通过，返回 `openai_anthropic_bridge_max_tool_calls_unsupported` 且不命中 Anthropic |
| Mock 回归 | Function arguments 保留 | `pnpm --dir backend test:protocol-boundary-openai-anthropic` | Chat / Responses 历史 function call 的 JSON 非对象和非法 JSON arguments 不被吞成 `{}` | 已通过 | 2026-06-24 复跑通过，Chat JSON 数组 arguments 保留为 `openai_arguments`，Responses 非法 JSON arguments 保留为 `openai_arguments_text` |
| Mock 回归 | Tool use 流式反渲染 | `pnpm --dir backend test:protocol-boundary-openai-anthropic` | Anthropic 多 tool_use / input_json_delta 转 Chat SSE 和 Responses SSE 时工具顺序、index 与 arguments 正确 | 已通过 | 2026-06-24 复跑通过；修复空 `input:{}` 与 `input_json_delta` 拼接成 `{}{...}` 的问题，Chat SSE 工具 index 按 0/1 连续递增 |
| Mock 回归 | Encrypted reasoning include | `pnpm --dir backend test:protocol-boundary-openai-anthropic` | Responses `include=reasoning.encrypted_content` 本地 400，且不请求 Anthropic | 已通过 | 2026-06-24 复跑通过，返回 `openai_anthropic_bridge_encrypted_reasoning_unsupported` 且不命中 Anthropic |
| Mock 回归 | Encrypted reasoning input item | `pnpm --dir backend test:protocol-boundary-openai-anthropic` | Responses 历史 `type=reasoning` item 只带 `encrypted_content` 时本地 400，且不请求 Anthropic；`compaction` 路径不回归 | 已通过 | 2026-06-25 复跑通过，返回 `openai_anthropic_bridge_encrypted_reasoning_input_unsupported` 且不命中 Anthropic；既有 `compaction` / `compaction_summary` 恢复用例继续通过 |
| Mock 回归 | Reasoning 请求枚举边界 | `pnpm --dir backend test:protocol-boundary-openai-anthropic` | `none` 不启用 Anthropic thinking，`xhigh` / 未知 effort 本地拒绝，`summary=none` 抑制 reasoning item，未知 summary 本地拒绝 | 已通过 | 2026-06-25 复跑通过，覆盖 Responses / Chat `none` 不发送 Anthropic `thinking`、Responses / Chat `xhigh` 本地 400、Responses `summary=none` JSON / SSE 抑制 reasoning item、未知 summary 本地 400 |
| Mock 回归 | Responses 非等价输出扩展 include | `pnpm --dir backend test:protocol-boundary-openai-anthropic` | `file_search_call.results` 保持允许；`web_search_call.action.sources`、`computer_call_output.output.image_url`、`message.input_image.image_url` 本地 400 且不请求 Anthropic；`code_interpreter_call.outputs` 仅在 `code_interpreter=mock` 时允许 | 已通过 | 2026-06-25 复跑通过，`file_search_call.results` 既有成功路径继续通过；三类非等价 include 返回 `openai_anthropic_bridge_include_unsupported` 且不命中 Anthropic；`code_interpreter=mock` 时输出固定 logs |
| Mock 回归 | 输出形态边界 | `pnpm --dir backend test:protocol-boundary-openai-anthropic` | Chat `n>1`、Chat `logprobs=true` / `top_logprobs>0`、Responses `include=message.output_text.logprobs` / `top_logprobs>0` 本地 400，且不请求 Anthropic | 已通过 | 2026-06-24 复跑通过，返回 `openai_anthropic_bridge_multiple_choices_unsupported` / `openai_anthropic_bridge_logprobs_unsupported` 且不命中 Anthropic |
| Mock 回归 | 输出模态和内容块边界 | `pnpm --dir backend test:protocol-boundary-openai-anthropic` | Chat audio output、Chat / Responses `input_audio` 和未知 content block 本地 400，且不请求 Anthropic | 已通过 | 2026-06-24 复跑通过，返回 `openai_anthropic_bridge_output_modality_unsupported` / `openai_anthropic_bridge_audio_input_unsupported` / `openai_anthropic_bridge_unsupported_content_part` 且不命中 Anthropic |
| Mock 回归 | Chat SSE usage 形态 | `pnpm --dir backend test:protocol-boundary-openai-anthropic` | 默认 Chat SSE 不输出真实 usage；`stream_options.include_usage=true` 时在 `[DONE]` 前输出 `choices=[]` usage chunk | 已通过 | 2026-06-24 复跑通过，默认流式无真实 usage chunk；include_usage 流式输出 `prompt_tokens` / `completion_tokens` / `total_tokens` |
| Mock 回归 | 采样 / 预测输出控制边界 | `pnpm --dir backend test:protocol-boundary-openai-anthropic` | `temperature>1`、非默认 penalty、非空 `logit_bias`、`seed`、`prediction`、`verbosity` 本地 400，且不请求 Anthropic | 已通过 | 2026-06-25 复跑通过，返回 `openai_anthropic_bridge_sampling_control_unsupported` / `openai_anthropic_bridge_prediction_unsupported` / `openai_anthropic_bridge_verbosity_unsupported` 且不命中 Anthropic |
| Mock 回归 | 请求级控制边界 | `pnpm --dir backend test:protocol-boundary-openai-anthropic` | `safety_identifier` 映射 Anthropic metadata，`prompt_cache_key` 不透传；`service_tier=priority/flex`、`prompt_cache_retention`、Responses `prompt`、顶层 `moderation` 本地 400，且不请求 Anthropic | 已通过 | 2026-06-25 复跑通过，`safety_identifier` 映射 `metadata.user_id`，`prompt_cache_key` / `service_tier=default` 不透传；`priority` / `flex` 服务档位、`prompt_cache_retention`、Responses `prompt`、顶层 `moderation` 本地 400 且不上游 |
| Mock 回归 | Hosted tool 策略 | 扩展 bridge mock 脚本 | web_search / code_interpreter / computer / MCP / tool_search namespace / image_generation / file_search 等按矩阵映射、guidance 或受控失败 | 已通过 | 当前口径覆盖 Responses / Chat `web_search` guidance 且不请求 Anthropic、Responses `computer` / `code_interpreter` / `mcp` guidance 且不请求 Anthropic、Chat `code_interpreter` guidance 且不请求 Anthropic、Responses `code_interpreter=mock` JSON / SSE 本地 `code_interpreter_call` 且不执行代码不上游、Responses `tool_search` + `namespace` JSON / SSE 本地展开、`image_generation` 无 provider guidance、provider JSON / completed-only SSE / partial SSE 成功路径、provider `moderation_blocked` failed response、edit / mask / 历史复用 guidance、provider 非 JSON / 超大响应体 / timeout failed response、`file_search` 本地运行时成功和边界失败。MCP mock/proxy 旧分支已由 PLAN-0063 决策撤销，当前只保留 guidance 口径 |
...
| Mock 回归 | 图片 data URL 校验 | `pnpm --dir backend test:protocol-boundary-openai-anthropic` | Chat / Responses 图片 data URL 非图片 MIME 或非法 base64 本地 400，且不请求 Anthropic | 已通过 | 2026-06-24 复跑通过，非图片 MIME 返回 `openai_anthropic_bridge_unsupported_image_media_type`，非法 base64 返回 `openai_anthropic_bridge_invalid_image_base64` |
...
| Mock 回归 | Responses 状态 / 上下文控制边界 | `pnpm --dir backend test:protocol-boundary-openai-anthropic` | `conversation`、`background=true`、`truncation=auto`、`context_management` 本地 400，且不请求 Anthropic；`background=false` 和 `truncation=disabled` 正常桥接 | 已通过 | 2026-06-25 复跑通过，覆盖 `background=false` / `truncation=disabled` 正常桥接，`conversation` / `background=true` / `truncation=auto` / `context_management` 本地 400 且不命中 Anthropic |
| Mock 回归 | Chat / Responses `store=true` 存储边界 | `pnpm --dir backend test:protocol-boundary-openai-anthropic` | `store=false` 正常无状态桥接；Chat / Responses `store=true` 本地 400，且不请求 Anthropic | 已通过 | 2026-06-25 复跑通过，Responses `store=false` 不透传 Anthropic 且正常桥接，Chat / Responses `store=true` 返回 `openai_anthropic_bridge_store_unsupported` 且不命中上游 |
...
| 2026-06-24 | 进行中 | AI | 已将 computer / code execution / Codex 本地工具真实运行时拆分到 `PLAN-0063`，当前未配置时保持 guidance；MCP 后续不在服务端实现 runtime/proxy，固定 guidance。 |
...
| 2026-06-25 | 进行中 | AI | 已明确 Responses 非等价输出扩展 include 边界：除已承接的 `file_search_call.results` 外，web search sources、computer output image URL 和 input image URL 回显都不能在当前 Anthropic bridge 中静默忽略；code interpreter outputs 必须等本地 runtime 承接。 |
| 2026-06-25 | 进行中 | AI | 已补 Responses 非等价输出扩展 include 本地拒绝和 mock：`web_search_call.action.sources`、`computer_call_output.output.image_url`、`message.input_image.image_url` 返回统一错误码且不上游；`file_search_call.results` 成功路径保持通过；`code_interpreter_call.outputs` 后续由 PLAN-0063 mock runtime 放行固定 logs。 |
...
| 2026-06-25 | 进行中 | AI | 已补 PLAN-0063 Responses `code_interpreter` mock runtime：`codeInterpreter=mock` 时 JSON / SSE 返回 OpenAI `code_interpreter_call` 和 assistant message，`include=code_interpreter_call.outputs` 返回固定 logs，不执行代码、不请求 Anthropic、不把用户 prompt 写入工具输出。 |
| 2026-06-25 | 进行中 | AI | 历史记录：当日曾补 Responses `mcp` mock proxy；该方向已被 PLAN-0063 撤销，当前 MCP 固定 guidance，不连接远程 MCP、不请求 Anthropic、不伪造工具结果。 |
...
| 2026-06-25 | Responses 非等价输出扩展 include 先本地拒绝，已有 runtime 时单项放行 | 官方 include 表示客户端要求响应体带额外输出数据；忽略会产生“请求成功但缺字段”的假成功 | 允许 `file_search_call.results` 这类已由本地运行时填充的 include；`code_interpreter_call.outputs` 仅在 PLAN-0063 mock runtime 下放行固定 logs；logprobs / encrypted reasoning 走专用错误码，其余非等价 include 统一本地拒绝 |
...
| 2026-06-25 | code_interpreter mock 只固定协议外形 | 真实 code interpreter 需要隔离 container / worker；当前不能宣称执行 Python、shell、文件产物或网络访问 | 仅 `codeInterpreter=mock` 放行 Responses `code_interpreter_call.outputs` 并返回固定 logs；默认 guidance 和真实执行仍按 PLAN-0063 推进 |
| 2026-06-25 | MCP mock / proxy 方向撤销 | 真实 MCP 会访问第三方 server 和授权数据；中转服务端不承接 MCP runtime / proxy / approval | 当前 MCP 固定 guidance；历史 MCP mock 结果不再作为当前能力或待办 |
...
- Mock 回归：2026-06-25 已复跑并通过 `pnpm --dir backend test:protocol-boundary-openai-anthropic`，覆盖四入口、采样 / 输出控制本地拒绝、工具调用、reasoning、图片 / 文件输入、structured output、thinking、hosted tool guidance、code_interpreter mock、本地图像 provider、file_id 边界、compact、跨 API Key snapshot 和 Codex `previous_response_id`。MCP 当前只认 guidance；历史 MCP mock 覆盖不再作为当前能力。
