...
- [ ] 在网关运行时构建按模型名 O(1) 查询的路由索引，禁止请求链路扫描全部供应商目录。
- [ ] API Key 允许绑定多个供应商协议档案的分组，但请求调度必须先通过模型路由筛出目标档案。
- [ ] 更新 API Key 前端绑定号池体验，不再按第一个分组的协议档案隐藏其他供应商分组。
...
- [ ] 调整模型名唯一性校验和必要索引。
- [ ] 新增或扩展模型路由 runtime index。
- [ ] 修改 API Key 绑定校验，允许跨供应商协议档案分组。
- [ ] 修改网关分组选路顺序：先模型路由，再分组策略。
- [ ] 修改前端 API Key 绑定号池筛选和提示。
...
| --- | --- | --- | --- | --- | --- |
| 命令类验证 | 类型检查 | `pnpm typecheck` | 前后端类型检查通过 | 未执行 | 待实现后执行 |
| 命令类验证 | 构建验证 | `pnpm build` | 前后端构建通过 | 未执行 | 待实现后执行 |
| 功能主流程 | 跨供应商 API Key 按模型路由 | 绑定 GPT 与 GLM 分组，请求不同模型 | 请求命中模型所属供应商分组 | 未执行 | 待实现 |
...
| 回归场景 | 单供应商 API Key | 只绑定 GPT 分组并请求 GPT 模型 | 行为保持现有语义 | 未执行 | 待实现 |
| 前端体验 | API Key 绑定多个供应商分组 | 手动打开 API Key 表单 | 分组选择、错误提示和展示文案均为中文 | 未执行 | 待实现 |
...
- [ ] 网关按请求 `model` O(1) 解析目标供应商协议档案。
- [ ] API Key 能绑定多个供应商分组，并按模型只调度目标供应商分组。
...
- 单项验证：未执行，说明：当前仅完成方案文档草稿。
- 手动验证：未执行，说明：当前未改运行代码。
- 测试项结果：未执行，说明：待实现后补充。
- 未验证项：模型路由 runtime index、跨供应商 API Key 调度、前端表单体验。
