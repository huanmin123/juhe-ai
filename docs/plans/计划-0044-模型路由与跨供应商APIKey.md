...
API Key 绑定分组时不再要求所有分组 `provider_protocol_profile_id` 一致。当前运行时先使用有界模型路由：在该 Key 已绑定供应商范围内用供应商集合索引唯一命中目标档案；无法唯一命中时返回本地错误。账号支持模型和账号模型映射只在目标档案确定后用于账号过滤、候选账号分层排序和上游模型改写，不能反向决定供应商路由；同一目标分组内按直接支持、映射命中、未限制模型三层排序，层内继续复用原调度和错误切号规则。后续再把供应商定位这一步替换为全系统模型路由索引。
