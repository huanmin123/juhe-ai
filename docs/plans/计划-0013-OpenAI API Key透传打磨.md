...
- 目标：保持 API Key raw body 真透传优势，同时把 Header 透传收敛到账号池适合的边界：过滤危险头和噪声头，从账号凭据写入真正属于上游账号的 OpenAI 组织、项目和 Beta 配置。
- 交付物：细节统计比较文档、计划文档、后端 Header 策略、前端 API Key 账号可选字段、回归脚本和验证记录。
...
- [x] 后端不再默认相信客户端 `OpenAI-Organization` / `OpenAI-Project`，改由账号凭据写入。
- [x] 保留客户端 `OpenAI-Beta` 功能语义，并允许账号凭据 `openai_beta` 覆盖。
- [x] 前端 API Key 账户表单补充可选 `OpenAI 组织`、`OpenAI 项目`、`OpenAI Beta`。
...
- 不新增数据库 schema 字段；三个 OpenAI Header 配置继续保存在加密 `credentials` JSON 中。
...
- 数据变化：不改 schema；API Key 账号凭据新增可选 JSON 键 `openai_organization`、`openai_project`、`openai_beta`。
- 接口变化：管理 / 用户账户创建和编辑接口仍使用原 `credentials` 字段；不新增 API。
- 前端变化：OpenAI API Key 配置区增加三个可选输入项，文案说明客户端同名组织 / 项目头不会透传。
- 后端变化：`buildUpstreamHeaders()` 增加 Header 过滤函数和账号级 OpenAI Header 覆盖逻辑。
- 验证变化：新增 `test:openai-api-key-passthrough` 脚本，覆盖 raw body、Header 过滤和账号级覆盖。
...
- [x] 实现后端 Header 过滤和账号级 OpenAI Header 覆盖。
- [x] 实现前端 API Key 账户可选 OpenAI Header 配置。
...
| --- | --- | --- | --- | --- | --- |
| 命令类验证 | API Key passthrough 回归 | `pnpm --filter juhe-ai-backend test:openai-api-key-passthrough` | raw body、Header 过滤、账号级覆盖均通过断言 | 已通过 | 2026-05-08 通过 |
...
| 功能主流程 | 账号级 OpenAI Header | 账号配置组织、项目、Beta，同时客户端传同名头 | 组织 / 项目 / Beta 使用账号配置；客户端组织 / 项目不透传 | 已通过 | 回归脚本覆盖 |
...
| 前端体验 | API Key 账户表单 | 打开创建 / 编辑 API Key 账户弹窗 | 新字段中文文案清晰，非必填，不影响 OAuth 表单 | 已通过 | `pnpm typecheck` 覆盖字段绑定和类型；未做浏览器截图 |
...
| 2026-05-08 | 默认不透传客户端 `OpenAI-Organization` / `OpenAI-Project` | 这两个头属于上游账号归属上下文，不应由本地 API Key 调用方随意指定 | 改由账号凭据统一配置 |
| 2026-05-08 | `OpenAI-Beta` 客户端可透传但账号可覆盖 | Beta 头常是公开 API 功能语义，完全过滤会破坏兼容；账号配置需要更高优先级 | 兼顾客户端语义与账号固定策略 |
...
- [x] `OpenAI-Organization` / `OpenAI-Project` 从账号配置写入，不默认透传客户端值。
- [x] API Key 账户表单可配置 OpenAI 组织、项目和 Beta。
...
- 客户端如果依赖直接传 `OpenAI-Organization` / `OpenAI-Project` 临时切换组织或项目，将不再生效；应改为拆分不同上游账号配置。
...
- 完成时间：2026-05-08
- 实际完成内容：完成 API Key 透传细节比较文档；保留 raw body 透传；收敛 Header 过滤策略；新增账号级 OpenAI 组织、项目和 Beta 配置；补充前端表单和回归脚本。
- 主要改动位置：`backend/src/modules/gateway/openai-gateway-upstream.ts`、`backend/src/scripts/openai-api-key-passthrough-regression.ts`、`frontend/src/views/accounts/AccountApiKeySection.vue`、`docs/functions/OpenAI API Key透传细节统计与比较.md`。
