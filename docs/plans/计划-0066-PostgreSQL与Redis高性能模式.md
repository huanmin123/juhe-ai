...
- [ ] 补充自动化回归、压测、Redis / PostgreSQL 故障演练和备份恢复验证。已完成 System API 100 并发 PG/Redis 压测，故障演练和备份恢复仍待执行。
...
| PG repository | API Key 管理读写与网关 Key 校验双 driver | `pnpm --filter juhe-ai-backend test:api-key-management-driver` | API Key 分页 / secret / 创建 / 分组绑定 / 更新 / 刷新密钥 / 删除 / 唯一性校验 / 网关 Key 校验 / `read_gateway_runtime` Key、分组和最小账号候选读取入口一致 | 已通过 | 本地 SQLite 通过；`192.168.1.203` 远端 PostgreSQL / Redis URL 下通过；`/__aisys__/api/api-keys` 列表、secret、创建、更新、刷新密钥和删除已切到 async repository；回归通过 `createAccountAsync` 创建带支持模型和模型映射的最小候选账号并绑定分组；PG 模式下 `read_gateway_runtime` 已能读取有效 Key、绑定、分组访问元数据、默认响应检查策略、最小可调度账号候选、账号支持模型和模型映射；混合路由模型目录 HTTP 校验已由 `test:performance-system-api-smoke` 覆盖；删除后的 dataset/stats 清理目标登记仍待清理仓储迁移 |
...
| System API | performance 最小 HTTP 烟测 | `pnpm --filter juhe-ai-backend test:performance-system-api-smoke` | public settings、captcha、login、me、settings GET/PATCH、system-accounts GET/POST/PATCH、providers、provider options、provider models GET/POST/PATCH/DELETE、groups GET/POST/PATCH/DELETE、authorization-options GET、authorizations GET、my-authorizations GET、authorizations `POST` / `GET /:id/usage` / `PATCH /:id` / `PATCH /:id/expire` / `DELETE /:id`、accounts POST/GET 列表/详情/options/PATCH/clearFailureState/DELETE、授权实例 `/authorized-dispatch` clearFailureState、api-keys GET/POST/PATCH/secret/refresh-key/DELETE、混合路由 API Key 创建 / 更新 / 删除、logout 链路在 SQLite 和 PG/Redis 下可用 | 已通过 | 本地 SQLite 通过；`192.168.1.203` 远端 PostgreSQL / Redis URL 下通过；只覆盖登录前后、设置管理、系统账户管理、分组管理、自定义模型管理、授权列表 / options / usage 详情、授权创建、现有授权更新 / 有效期和额度更新 / 回收、AI 账户创建 / 普通 owner 列表 / 详情 / options / 常规更新 / 异常状态恢复 / 删除、授权实例详情读取 / 异常状态恢复、API Key 管理和混合路由模型目录校验最小链路，不代表完整管理端 PG-ready |
| PG repository | 授权个人归还双 driver | `pnpm --filter juhe-ai-backend test:authorization-return-driver` | 账户授权实例归还、授权列表个人归还和分组个人归还后，grant 为 `returned`，manual source 撤销，runtime authorization 为 `returned`，重复归还返回空结果 | 已通过 | 本地 SQLite 通过；`192.168.1.203` 远端 PostgreSQL / Redis URL 下通过；只覆盖个人归还仓储，团队授权归还仍待完整场景回归 |
...
| Redis state | 原子计数 / 锁 / 并发占用 / 缓存失效版本 | `pnpm --filter juhe-ai-backend test:runtime-state-cache-driver`、`test:account-concurrency-lane-index`、`test:gateway-concurrency-preparation`、`test:gateway-cache-invalidation-runtime-state` | 不突破上限，不误释放锁，跨进程失效版本能触发本地 handler | 部分通过 | memory / 默认链路通过；网关缓存失效 runtime state 源码边界通过，`192.168.1.203` redis-state 已通过跨进程版本同步烟测；Redis Lua 路径待连通压测 |
...
| System API 压测 | PG/Redis 管理链路 100 并发 | `pnpm --filter juhe-ai-backend test:performance-system-api-load` | HTTP 0 错误，deadlocks delta 0，无大事务 / 长阻塞，压测夹具清理干净 | 已通过 | `192.168.1.203` 远端 PG/Redis 下 100 并发、25 个账号分摊写入、15% 写请求通过：最终 20373 请求全 200，端到端脚本 RPS 455.09，总体 p95 335.60ms，PATCH p95 451.17ms，deadlocks delta 0，最大事务 0.38s，最大 active query 0.35s；详见 `docs/deploy/高性能模式压测报告.md` |
...
| 2026-06-26 | System API DB 在途请求上限改为可配置并按模式默认 | 64 的管理端 DB 在途请求保护会在 100 并发高性能压测中提前返回 `system_api_busy` 503；但普通 standalone 节点不应默认继承高性能阈值 | 新增 `JUHE_AI_SYSTEM_API_DB_SERVICE_MAX_IN_FLIGHT`，standalone 默认 64、performance 默认 256；`192.168.1.203` 100 并发压测使用 256 通过 |
...
- 类型检查：本轮缓存失效 reason 优化和回归补充后，`pnpm --filter juhe-ai-backend typecheck` 已通过。
...
- 未验证项：其他 PostgreSQL repository adapter、应用容器 performance 模式、Redis Streams 远端端到端积压恢复、迁移脚本、故障演练和备份恢复。
