...
- [~] 迁移后台管理只读接口：`GET /__aisys__/api/proxies/options`、`GET /__aisys__/api/providers/options`、`GET /__aisys__/api/providers/models/options`、`GET /__aisys__/api/providers/{code}/models`、`GET /__aisys__/api/route-strategies/options` 和 `GET /__aisys__/api/my-route-strategies/options` 已补 Go PG schema / 既有表复用、store、service、handler、`requireAuth` 会话鉴权基线、admin / self 作用域和默认不接管的 `JUHE_AI_MANAGEMENT_API_ENABLED` router opt-in guard；其余 W2 接口、前端 smoke、生产切流和 Node 删除仍未完成。
- [ ] 迁移后台管理写接口和权限链路。
