...
- [~] 迁移后台管理只读接口：第一块 `GET /__aisys__/api/proxies/options` 已补 Go PG schema、store、service、handler 和默认不接管的 router 注入 guard；Go 管理端会话鉴权、其余 W2 接口、前端 smoke、生产切流和 Node 删除仍未完成。
- [ ] 迁移后台管理写接口和权限链路。
...
| W2 管理只读 | `GET /__aisys__/api/proxies/options` 第一块 | `go test ./internal/modules/managementproxies ./internal/httpapi ./internal/store/postgres`、`go test -tags=integration ./internal/testkit/integration -run TestW2ManagementProxyOptionsPostgresSmoke -count=1` | 只返回启用代理 `id/name/type/enabled`，`keyword` 前缀搜索，`limit` clamp 到 1-50，错误脱敏，默认生产 router 未注入鉴权时不注册后台路由 | Go 实现中 | 单元 / handler / store 快速测试已通过；integration smoke 代码已补，当前 Docker/testcontainers 真实执行待复跑 |
