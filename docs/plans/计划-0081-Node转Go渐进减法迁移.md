...
- [~] 迁移后台管理只读接口：当前三块 `GET /__aisys__/api/proxies/options`、`GET /__aisys__/api/providers/options`、`GET /__aisys__/api/route-strategies/options` 和 `GET /__aisys__/api/my-route-strategies/options` 已补 Go PG schema / 既有表复用、store、service、handler、`requireAuth` 会话鉴权基线、admin / self 作用域和默认不接管的 `JUHE_AI_MANAGEMENT_API_ENABLED` router opt-in guard；其余 W2 接口、前端 smoke、生产切流和 Node 删除仍未完成。
- [ ] 迁移后台管理写接口和权限链路。
...
| W2 管理只读 | `GET /__aisys__/api/proxies/options`、`GET /__aisys__/api/providers/options`、`GET /__aisys__/api/route-strategies/options` 和 `GET /__aisys__/api/my-route-strategies/options` 当前三块 | `go test ./internal/modules/managementauth ./internal/modules/managementproxies ./internal/modules/managementproviders ./internal/modules/managementroutestrategies ./internal/httpapi ./internal/store/postgres ./internal/config ./internal/app`、`go test -tags=integration ./internal/testkit/integration -run TestW2Management -count=1` | `proxies/options` 只返回启用代理 `id/name/type/enabled`，`keyword` 前缀搜索，`limit` clamp 到 1-50，错误脱敏；`providers/options` 返回启用供应商的完整 `ProviderDefinition[]`、协议画像、endpoint families 和系统账户默认测试模型偏好，普通用户不能通过 query 指定其他 `systemAccountId`；`route-strategies/options` admin-only，支持 `systemAccountId`、`ids`、`keyword`、`limit=1..100`、`activeOnly`，返回 owner 字段；`my-route-strategies/options` 强制当前用户作用域、忽略 query `systemAccountId` 且不返回 owner 字段；`juhe_ai_session` 会话 cookie、SHA-256 `token_hash`、`system_sessions` join `system_accounts`、过期 / 停用拒绝和普通用户初始密码拦截符合 Node 当前语义；默认生产 router 不注册，显式 `JUHE_AI_MANAGEMENT_API_ENABLED=true` 才允许灰度挂载 | Go 实现中 | 单元 / handler / store / auth / config / app 快速测试已通过；integration smoke 代码已补，当前 Docker/testcontainers 真实执行输出 `SKIP`，待健康环境复跑 |
...
- Go W2 管理只读当前三块：已新增 `backend-go/db/migrations/000006_w2_management_proxy_options.sql`、`backend-go/db/migrations/000007_w2_management_session_auth.sql`、`backend-go/db/migrations/000008_w2_management_provider_options.sql`、`backend-go/internal/store/port/management.go`、`backend-go/internal/store/postgres/queries/w2_management_proxy_options.sql`、`backend-go/internal/store/postgres/queries/w2_management_auth.sql`、`backend-go/internal/store/postgres/queries/w2_management_provider_options.sql`、`backend-go/internal/store/postgres/queries/w2_management_route_strategy_options.sql`、`backend-go/internal/store/postgres/managementproxies.go`、`backend-go/internal/store/postgres/managementauth.go`、`backend-go/internal/store/postgres/managementproviders.go`、`backend-go/internal/store/postgres/managementroutestrategies.go`、`backend-go/internal/modules/managementproxies`、`backend-go/internal/modules/managementauth`、`backend-go/internal/modules/managementproviders`、`backend-go/internal/modules/managementroutestrategies`、`backend-go/internal/httpapi/management_proxies.go`、`backend-go/internal/httpapi/management_auth.go`、`backend-go/internal/httpapi/management_providers.go`、`backend-go/internal/httpapi/management_route_strategies.go`、`backend-go/internal/testkit/integration/w2_management_proxy_options_smoke_test.go`、`backend-go/internal/testkit/integration/w2_management_auth_smoke_test.go`、`backend-go/internal/testkit/integration/w2_management_provider_options_smoke_test.go` 和 `backend-go/internal/testkit/integration/w2_management_route_strategy_options_smoke_test.go`；覆盖 `GET /__aisys__/api/proxies/options` 的启用代理过滤、`keyword` 名称前缀搜索、`limit` clamp、响应白名单、错误脱敏，覆盖 `GET /__aisys__/api/providers/options` 的完整 `ProviderDefinition[]`、协议画像、endpoint families、启用供应商、默认 profile、系统账户默认测试模型偏好和普通用户 `systemAccountId` 作用域隔离，覆盖 `GET /__aisys__/api/route-strategies/options` 的 admin-only、全局 / 指定用户作用域、`ids`、`keyword`、`limit`、`activeOnly` 和 owner 字段，覆盖 `GET /__aisys__/api/my-route-strategies/options` 的 self scope、忽略 `systemAccountId` 和 owner 字段隐藏，并覆盖 `juhe_ai_session` cookie、SHA-256 `token_hash`、`system_sessions` join `system_accounts`、过期 / 停用会话 401、普通用户初始密码 403、admin 初始密码有效值归零、`JUHE_AI_MANAGEMENT_API_ENABLED=false` 默认不注册和显式开启时必须注入管理端会话鉴权 middleware。已通过 `sqlc generate`、`go test ./internal/modules/managementauth ./internal/modules/managementproxies ./internal/modules/managementproviders ./internal/modules/managementroutestrategies ./internal/httpapi ./internal/store/postgres ./internal/config ./internal/app`、`go test ./...` 和 integration 包编译；真实 Docker/testcontainers integration、其余 W2 接口、生产路由切流、前端 smoke 和 Node 删除仍待完成。
...
- 风险 6：W2 管理只读接口如果绕过 Node 当前 `requireAuth` / `requireAdmin` / `my-*` 作用域，会把后台数据裸露出来；因此当前 `proxies/options`、`providers/options`、`route-strategies/options` 和 `my-route-strategies/options` 虽已补 Go `requireAuth` 会话鉴权基线、admin-only 管理侧策略下拉和 self-only 用户侧策略下拉，也仍必须由 `JUHE_AI_MANAGEMENT_API_ENABLED=false` 默认关闭。显式开启只允许灰度验证已迁移路径，不能替代其余 W2 接口、前端 smoke、单 owner 切流和 Node 删除证据。
...
- 完成时间：待后续迁移全部完成后填写。
- 实际完成内容：当前完成 M0 文档规划，W0 Go 常规工程基线已落地，W1a 公开设置读接口已进入 Go 实现中，W1b 已补 catalog/auth、PG auth/log adapter、Redis penalty-window、公开接口日志 snapshot/log builder、Asynq payload/enqueue/handler、public API log 单任务 worker runtime、HTTP shell / capture 契约、499 客户端提前断开捕获、ResponseWriter 可选接口透传、public group CRUD、public route strategy CRUD、public API Key CRUD、public account CRUD 四条真实资源纵切面、默认关闭的生产 router opt-in guard，以及 `w1b-public-api-smoke` 独立 maintenance smoke；W2 当前三块 `proxies/options`、`providers/options`、`route-strategies/options` 和 `my-route-strategies/options` 已补 Go schema / 既有表复用 / store / service / handler / `requireAuth` 会话鉴权 / admin / self 作用域 / `JUHE_AI_MANAGEMENT_API_ENABLED` router opt-in guard；Go 系统指标与观测迁移规划已补齐；业务接口尚未正式生产接管。
- 主要改动位置：`docs/migration/`、`docs/plans/计划-0081-Node转Go渐进减法迁移.md`、`backend-go/`。
- 验证结果：W0 常规 Go 验证矩阵、race、lint、漏洞扫描、命令入口、health smoke、public group / public route strategy / public API Key / public account handler / service 非容器专项、W2 `managementauth` / `managementproxies` / `managementproviders` / `managementroutestrategies` / `httpapi` / `store/postgres` / `config` / `app` 快速测试和跨平台构建已记录；本轮 `sqlc generate`、`go test ./internal/modules/managementauth ./internal/modules/managementproxies ./internal/modules/managementproviders ./internal/modules/managementroutestrategies ./internal/httpapi ./internal/store/postgres ./internal/config ./internal/app`、`go test ./...`、`go test -tags=integration ./internal/testkit/integration -run '^$'` 已通过；`go test -v -tags=integration ./internal/testkit/integration -run TestW2Management -count=1` 因 Docker/testcontainers 不可用输出 `SKIP`，覆盖 W2 proxy options、provider options、route strategy options 和 management auth 四个 smoke，真实 PG/Redis/Asynq integration 仍需 Docker 健康环境复跑。Node 对照中 route strategy query guard、API Key route validation 和 system API read burst 已通过；`test:scope` 仍失败在 Node 账户详情 Base URL 断言，需后续单独处理。
