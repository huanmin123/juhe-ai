...
- [~] 迁移后台管理只读接口：`GET /__aisys__/api/proxies/options`、`GET /__aisys__/api/providers/options`、`GET /__aisys__/api/providers/models/options`、`GET /__aisys__/api/providers/{code}/models`、`GET /__aisys__/api/route-strategies/options`、`GET /__aisys__/api/my-route-strategies/options`、`GET /__aisys__/api/groups/options`、`GET /__aisys__/api/my-groups/options`、`GET /__aisys__/api/accounts/options` 和 `GET /__aisys__/api/my-accounts/options` owner-only 阶段已补 Go PG schema / 既有表复用、store、service、handler、`requireAuth` 会话鉴权基线、admin / self 作用域和默认不接管的 `JUHE_AI_MANAGEMENT_API_ENABLED` router opt-in guard；授权组 union、授权账户 union、账号标签、group account-options、前端 smoke、生产切流和 Node 删除仍未完成。
- [ ] 迁移后台管理写接口和权限链路。
...
| W2 管理只读 | `GET /__aisys__/api/proxies/options`、`GET /__aisys__/api/providers/options`、`GET /__aisys__/api/providers/models/options`、`GET /__aisys__/api/providers/{code}/models`、`GET /__aisys__/api/route-strategies/options`、`GET /__aisys__/api/my-route-strategies/options`、`GET /__aisys__/api/groups/options`、`GET /__aisys__/api/my-groups/options`、`GET /__aisys__/api/accounts/options` 和 `GET /__aisys__/api/my-accounts/options` owner-only 阶段 | `go test ./internal/modules/managementauth ./internal/modules/managementproxies ./internal/modules/managementproviders ./internal/modules/managementprovidermodels ./internal/modules/managementroutestrategies ./internal/modules/managementgroups ./internal/modules/managementaccounts ./internal/httpapi ./internal/store/postgres ./internal/config ./internal/app`、`go test -tags=integration ./internal/testkit/integration -run TestW2Management -count=1` | `proxies/options` 只返回启用代理 `id/name/type/enabled`，`keyword` 前缀搜索，`limit` clamp 到 1-50，错误脱敏；`providers/options` 返回启用供应商的完整 `ProviderDefinition[]`、协议画像、endpoint families 和系统账户默认测试模型偏好，普通用户不能通过 query 指定其他 `systemAccountId`；`providers/models/options` / `providers/{code}/models` 保留模型目录聚合和过滤规则；`route-strategies/options` admin-only，支持 `systemAccountId`、`ids`、`keyword`、`limit=1..100`、`activeOnly`，返回 owner 字段；`my-route-strategies/options` 强制当前用户作用域、忽略 query `systemAccountId` 且不返回 owner 字段；`groups/options` admin-only，支持 `systemAccountId`、`ids`、`keyword`、`providerCode`、`limit=1..50`、`manageableOnly`、`preferDefault`，返回 owner 字段、`accessType=owner` 和 owner permissions；`my-groups/options` 强制当前用户 owner-only 作用域、忽略 query `systemAccountId` 且不返回管理侧系统账户字段；`accounts/options` admin-only，支持 `systemAccountId`、`ids`、`page`、`keyword`、`providerCode`、`groupId`、`type`、`status`、`schedulable`、`limit=1..50`，返回 owner 字段、`accessType=owner`、owner permissions 和轻量账号字段；`my-accounts/options` 强制当前用户 owner-only 作用域、忽略 query `systemAccountId` 且不返回管理侧系统账户字段；当前分组 options 不覆盖 Node 授权组 union，账户 options 不覆盖 Node 授权账户 union、账号标签和名称包含搜索；`juhe_ai_session` 会话 cookie、SHA-256 `token_hash`、`system_sessions` join `system_accounts`、过期 / 停用拒绝和普通用户初始密码拦截符合 Node 当前语义；默认生产 router 不注册，显式 `JUHE_AI_MANAGEMENT_API_ENABLED=true` 才允许灰度挂载 | Go 实现中 | 单元 / handler / store / auth / config / app 快速测试已通过；integration smoke 代码已补，当前 Docker/testcontainers 真实执行输出 `SKIP`，待健康环境复跑 |
...
- Go W2 管理只读当前五类：已新增 `backend-go/db/migrations/000006_w2_management_proxy_options.sql`、`backend-go/db/migrations/000007_w2_management_session_auth.sql`、`backend-go/db/migrations/000008_w2_management_provider_options.sql`、`backend-go/db/migrations/000009_w2_management_provider_model_catalog.sql`、`backend-go/internal/store/port/management.go`、`backend-go/internal/store/postgres/queries/w2_management_proxy_options.sql`、`backend-go/internal/store/postgres/queries/w2_management_auth.sql`、`backend-go/internal/store/postgres/queries/w2_management_provider_options.sql`、`backend-go/internal/store/postgres/queries/w2_management_provider_models.sql`、`backend-go/internal/store/postgres/queries/w2_management_route_strategy_options.sql`、`backend-go/internal/store/postgres/queries/w2_management_group_options.sql`、`backend-go/internal/store/postgres/queries/w2_management_account_options.sql`、`backend-go/internal/store/postgres/managementproxies.go`、`backend-go/internal/store/postgres/managementauth.go`、`backend-go/internal/store/postgres/managementproviders.go`、`backend-go/internal/store/postgres/managementprovidermodels.go`、`backend-go/internal/store/postgres/managementroutestrategies.go`、`backend-go/internal/store/postgres/managementgroups.go`、`backend-go/internal/store/postgres/managementaccounts.go`、`backend-go/internal/modules/managementproxies`、`backend-go/internal/modules/managementauth`、`backend-go/internal/modules/managementproviders`、`backend-go/internal/modules/managementprovidermodels`、`backend-go/internal/modules/managementroutestrategies`、`backend-go/internal/modules/managementgroups`、`backend-go/internal/modules/managementaccounts`、`backend-go/internal/httpapi/management_proxies.go`、`backend-go/internal/httpapi/management_auth.go`、`backend-go/internal/httpapi/management_providers.go`、`backend-go/internal/httpapi/management_provider_models.go`、`backend-go/internal/httpapi/management_route_strategies.go`、`backend-go/internal/httpapi/management_groups.go`、`backend-go/internal/httpapi/management_accounts.go`、`backend-go/internal/testkit/integration/w2_management_proxy_options_smoke_test.go`、`backend-go/internal/testkit/integration/w2_management_auth_smoke_test.go`、`backend-go/internal/testkit/integration/w2_management_provider_options_smoke_test.go`、`backend-go/internal/testkit/integration/w2_management_provider_models_smoke_test.go`、`backend-go/internal/testkit/integration/w2_management_route_strategy_options_smoke_test.go`、`backend-go/internal/testkit/integration/w2_management_group_options_smoke_test.go` 和 `backend-go/internal/testkit/integration/w2_management_account_options_smoke_test.go`；覆盖 `GET /__aisys__/api/proxies/options` 的启用代理过滤、`keyword` 名称前缀搜索、`limit` clamp、响应白名单、错误脱敏，覆盖 `GET /__aisys__/api/providers/options` 的完整 `ProviderDefinition[]`、协议画像、endpoint families、启用供应商、默认 profile、系统账户默认测试模型偏好和普通用户 `systemAccountId` 作用域隔离，覆盖 provider model options / catalog 的聚合与过滤规则，覆盖 `GET /__aisys__/api/route-strategies/options` 的 admin-only、全局 / 指定用户作用域、`ids`、`keyword`、`limit`、`activeOnly` 和 owner 字段，覆盖 `GET /__aisys__/api/my-route-strategies/options` 的 self scope、忽略 `systemAccountId` 和 owner 字段隐藏，覆盖 `GET /__aisys__/api/groups/options` 的 admin-only、全局 / 指定用户作用域、`ids`、`keyword`、`providerCode`、`limit=1..50`、`manageableOnly`、`preferDefault`、owner 字段、`accessType=owner` 和 owner permissions，覆盖 `GET /__aisys__/api/my-groups/options` 的 self owner-only scope、忽略 `systemAccountId` 和管理侧系统账户字段隐藏，覆盖 `GET /__aisys__/api/accounts/options` 的 admin-only、全局 / 指定用户 owner 作用域、`ids`、`page`、`keyword`、`providerCode`、`groupId`、`type`、有效 `status`、`schedulable`、`limit=1..50`、owner 字段、`accessType=owner` 和 owner permissions，覆盖 `GET /__aisys__/api/my-accounts/options` 的 self owner-only scope、忽略 `systemAccountId` 和管理侧系统账户字段隐藏，并覆盖 `juhe_ai_session` cookie、SHA-256 `token_hash`、`system_sessions` join `system_accounts`、过期 / 停用会话 401、普通用户初始密码 403、admin 初始密码有效值归零、`JUHE_AI_MANAGEMENT_API_ENABLED=false` 默认不注册和显式开启时必须注入管理端会话鉴权 middleware。已通过 `sqlc generate`、`go test ./internal/modules/managementauth ./internal/modules/managementproxies ./internal/modules/managementproviders ./internal/modules/managementprovidermodels ./internal/modules/managementroutestrategies ./internal/modules/managementgroups ./internal/modules/managementaccounts ./internal/httpapi ./internal/store/postgres ./internal/config ./internal/app`、`go test ./...` 和 integration 包编译；真实 Docker/testcontainers integration、授权组 union、授权账户 union、账号标签、group account-options、生产路由切流、前端 smoke 和 Node 删除仍待完成。
- W2 Node 对照：已通过 `pnpm --filter juhe-ai-backend test:route-strategy-list-query-guard`、`pnpm --filter juhe-ai-backend test:api-key-route-validation` 和 `pnpm --filter juhe-ai-backend test:system-api-read-burst`；`pnpm --filter juhe-ai-backend test:scope` 当前失败，失败点为“用户 A 应能打开自有账户详情并读取非敏感 Base URL”，属于 Node 旧链路账户详情断言，未作为本轮 Go W2 route strategy options 的通过证据。
...
- 文档验证：已执行覆盖本次新增 W1b / W2 文档、迁移入口、迁移清单、测试策略和 PLAN-0081 的相对链接检查，结果为 `All checked markdown links resolve.`；已执行 `git diff --check`，仅有既有 LF/CRLF 提示，无 whitespace error。本轮 W2 `route-strategies/options` 文档同步后已再次执行本次改动 Markdown 相对链接检查，结果为 `All checked markdown links resolve.`。
- 未验证项：独立真实外部 URL 的 `w0-smoke` / `w1a-public-settings-smoke` / `w1b-public-api-smoke` 部署环境执行、W2 `TestW2ManagementProxyOptionsPostgresSmoke` / `TestW2ManagementAuthPostgresSmoke` / `TestW2ManagementProviderOptionsPostgresSmoke` / `TestW2ManagementProviderModelsPostgresSmoke` / `TestW2ManagementRouteStrategiesPostgresSmoke` / `TestW2ManagementGroupOptionsPostgresSmoke` / `TestW2ManagementAccountOptionsPostgresSmoke` 真实 Docker/testcontainers 执行、反向代理切流 smoke、前端品牌加载 smoke、Node public settings 删除证据、W1b Node PG/Redis 对照 smoke、Go W1b / W2 生产路由挂载、W2 授权组 union、授权账户 union、账号标签、group account-options、前端 W2 options smoke、account shell E2E 真实 Docker/testcontainers 复跑、Docker/testcontainers 真实 integration、Node 删除证据和网关测试均待后续代码迁移阶段执行。
...
- 风险 6：W2 管理只读接口如果绕过 Node 当前 `requireAuth` / `requireAdmin` / `my-*` 作用域，会把后台数据裸露出来；因此当前 `proxies/options`、`providers/options`、`route-strategies/options`、`my-route-strategies/options`、`groups/options`、`my-groups/options`、`accounts/options` 和 `my-accounts/options` owner-only 阶段虽已补 Go `requireAuth` 会话鉴权基线、admin-only 管理侧下拉和 self-only 用户侧下拉，也仍必须由 `JUHE_AI_MANAGEMENT_API_ENABLED=false` 默认关闭。显式开启只允许灰度验证已迁移路径，不能替代授权组 union、授权账户 union、账号标签、group account-options、前端 smoke、单 owner 切流和 Node 删除证据。
...
- 完成时间：待后续迁移全部完成后填写。
- 实际完成内容：当前完成 M0 文档规划，W0 Go 常规工程基线已落地，W1a 公开设置读接口已进入 Go 实现中，W1b 已补 catalog/auth、PG auth/log adapter、Redis penalty-window、公开接口日志 snapshot/log builder、Asynq payload/enqueue/handler、public API log 单任务 worker runtime、HTTP shell / capture 契约、499 客户端提前断开捕获、ResponseWriter 可选接口透传、public group CRUD、public route strategy CRUD、public API Key CRUD、public account CRUD 四条真实资源纵切面、默认关闭的生产 router opt-in guard，以及 `w1b-public-api-smoke` 独立 maintenance smoke；W2 当前 `proxies/options`、`providers/options`、`providers/models/options`、`providers/{code}/models`、`route-strategies/options`、`my-route-strategies/options`、`groups/options`、`my-groups/options`、`accounts/options` 和 `my-accounts/options` owner-only 阶段已补 Go schema / 既有表复用 / store / service / handler / `requireAuth` 会话鉴权 / admin / self 作用域 / `JUHE_AI_MANAGEMENT_API_ENABLED` router opt-in guard；Go 系统指标与观测迁移规划已补齐；业务接口尚未正式生产接管。
- 主要改动位置：`docs/migration/`、`docs/plans/计划-0081-Node转Go渐进减法迁移.md`、`backend-go/`。
- 验证结果：W0 常规 Go 验证矩阵、race、lint、漏洞扫描、命令入口、health smoke、public group / public route strategy / public API Key / public account handler / service 非容器专项、W2 `managementauth` / `managementproxies` / `managementproviders` / `managementprovidermodels` / `managementroutestrategies` / `managementgroups` / `managementaccounts` / `httpapi` / `store/postgres` / `config` / `app` 快速测试和跨平台构建已记录；本轮 `sqlc generate`、`go test ./internal/modules/managementaccounts ./internal/httpapi ./internal/store/postgres ./internal/app`、`go test ./...`、`go test -tags=integration ./internal/testkit/integration -run '^$'` 已通过；`go test -v -tags=integration ./internal/testkit/integration -run TestW2Management -count=1` 因 Docker/testcontainers 不可用输出 `SKIP`，覆盖 W2 proxy options、provider options、provider model catalog、route strategy options、group options、account options 和 management auth smoke，真实 PG/Redis/Asynq integration 仍需 Docker 健康环境复跑。Node 对照中 route strategy query guard、API Key route validation 和 system API read burst 已通过；`test:scope` 仍失败在 Node 账户详情 Base URL 断言，需后续单独处理。
