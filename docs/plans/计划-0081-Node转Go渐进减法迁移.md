...
- 风险 5：public group、public route strategy、public API Key、public account 四条资源纵切面和 `JUHE_AI_PUBLIC_API_ENABLED` opt-in guard 只是 W1b 灰度入口，容易被误判为 `/__aipublic__` 已接管；必须保持默认关闭，直到 16 个 CRUD、account shell E2E、PG+Redis smoke、公开日志副作用、反向代理单 owner 切流和 Node 删除证据整体完成。
