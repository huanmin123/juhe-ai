...
- [~] 迁移后台管理只读接口：`GET /__aisys__/api/proxies/options`、`GET /__aisys__/api/providers/options`、`GET /__aisys__/api/providers/models/options`、`GET /__aisys__/api/providers/{code}/models`、`GET /__aisys__/api/route-strategies/options`、`GET /__aisys__/api/my-route-strategies/options`、`GET /__aisys__/api/groups/options` 和 `GET /__aisys__/api/my-groups/options` owner-only 阶段已补 Go PG schema / 既有表复用、store、service、handler、`requireAuth` 会话鉴权基线、admin / self 作用域和默认不接管的 `JUHE_AI_MANAGEMENT_API_ENABLED` router opt-in guard；授权组 union、group account-options、account options、前端 smoke、生产切流和 Node 删除仍未完成。
- [ ] 迁移后台管理写接口和权限链路。
...
| W2 管理只读 | `GET /__aisys__/api/proxies/options`、`GET /__aisys__/api/providers/options`、`GET /__aisys__/api/providers/models/options`、`GET /__aisys__/api/providers/{code}/models`、`GET /__aisys__/api/route-strategies/options`、`GET /__aisys__/api/my-route-strategies/options`、`GET /__aisys__/api/groups/options` 和 `GET /__aisys__/api/my-groups/options` owner-only 阶段 | `go test ./internal/modules/managementauth ./internal/modules/managementproxies ./internal/modules/managementproviders ./internal/modules/managementprovidermodels ./internal/modules/managementroutestrategies ./internal/modules/managementgroups ./internal/httpapi ./internal/store/postgres ./internal/config ./internal/app`、`go test -tags=integration ./internal/testkit/integration -run TestW2Management -count=1` | `proxies/options` 只返回启用代理 `id/name/type/enabled`，`keyword` 前缀搜索，`limit` clamp 到 1-50，错误脱敏；`providers/options` 返回启用供应商的完整 `ProviderDefinition[]`、协议画像、endpoint families 和系统账户默认测试模型偏好，普通用户不能通过 query 指定其他 `systemAccountId`；`providers/models/options` / `providers/{code}/models` 保留模型目录聚合和过滤规则；`route-strategies/options` admin-only，支持 `systemAccountId`、`ids`、`keyword`、`limit=1..100`、`activeOnly`，返回 owner 字段；`my-route-strategies/options` 强制当前用户作用域、忽略 query `systemAccountId` 且不返回 owner 字段；`groups/options` admin-only，支持 `systemAccountId`、`ids`、`keyword`、`providerCode`、`limit=1..50`、`manageableOnly`、`preferDefault`，返回 owner 字段、`accessType=owner` 和 owner permissions；`my-groups/options` 强制当前用户 owner-only 作用域、忽略 query `systemAccountId` 且不返回管理侧系统账户字段；当前分组 options 不覆盖 Node 授权组 union；`juhe_ai_session` 会话 cookie、SHA-256 `token_hash`、`system_sessions` join `system_accounts`、过期 / 停用拒绝和普通用户初始密码拦截符合 Node 当前语义；默认生产 router 不注册，显式 `JUHE_AI_MANAGEMENT_API_ENABLED=true` 才允许灰度挂载 | Go 实现中 | 单元 / handler / store / auth / config / app 快速测试已通过；integration smoke 代码已补，当前 Docker/testcontainers 真实执行输出 `SKIP`，待健康环境复跑 |
...
- 风险 6：W2 管理只读接口如果绕过 Node 当前 `requireAuth` / `requireAdmin` / `my-*` 作用域，会把后台数据裸露出来；因此当前 `proxies/options`、`providers/options`、`route-strategies/options`、`my-route-strategies/options`、`groups/options` 和 `my-groups/options` owner-only 阶段虽已补 Go `requireAuth` 会话鉴权基线、admin-only 管理侧策略 / 分组下拉和 self-only 用户侧策略 / 分组下拉，也仍必须由 `JUHE_AI_MANAGEMENT_API_ENABLED=false` 默认关闭。显式开启只允许灰度验证已迁移路径，不能替代授权组 union、group account-options、account options、前端 smoke、单 owner 切流和 Node 删除证据。
