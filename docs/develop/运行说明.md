...
- 普通日志默认 `20MB` 滚动，保留 `14` 天、最多 `30` 个历史文件；可通过 `JUHE_AI_LOG_LEVEL`、`JUHE_AI_LOG_DIR`、`JUHE_AI_LOG_MAX_FILE_MB`、`JUHE_AI_LOG_RETENTION_DAYS`、`JUHE_AI_LOG_MAX_FILES` 调整。
- 管理后台“日志搜索”的索引查询模式只检索最近 3 天写入 SQLite 索引的普通运行日志；超过 3 天的日志即使日志文件仍保留也不会被索引查询搜到。
- 管理后台“日志搜索”的 `grep 模式` 会直接搜索后端日志目录中当前保留的全部 `.log` 文件；Windows 部署必须安装 `ripgrep`，可用 `winget install BurntSushi.ripgrep.MSVC`、`scoop install ripgrep` 或 `choco install ripgrep`，安装后重启后端并确认 `rg --version` 可执行。
