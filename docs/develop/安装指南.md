...
- `JUHE_AI_RECORD_DATABASE_PATH` 是旧记录库兼容路径；未配置下面两个新路径时，数据集库和统计结果库都会回退到这个文件。
- `JUHE_AI_DATASET_DATABASE_PATH` 可选，保存 `usage_records`、审计、操作日志、运行日志索引和模型检测等高频事实数据。
- `JUHE_AI_STATS_DATABASE_PATH` 可选，保存统计缓存、额度窗口、账号质量、系统监控、表监控和 `stats_job_state` 等查询结果数据。
