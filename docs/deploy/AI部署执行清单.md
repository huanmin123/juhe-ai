...
当前发布拓扑是 Node 主服务加一个 Go `juhe-ai-go-sidecar`。sidecar 内的 F1/F2/F3 分别保留独立 Store、schema 和 owner lease，但只形成一个 PID、日志、容器或 launchd 服务。Node 仍负责网关、账户、主 API、usage、操作日志、公开接口日志、model-check、stats 和 ops。
...
2. 复制 `backend/.env.example` 为 `backend/.env`，填写实际 Origin、稳定 `JUHE_AI_SECRET`（若复用现有业务库必须沿用旧值）、稳定的 F1/F2/F3 owner ID，以及完整 F3 配置。`start.sh` / `start.ps1` 只会生成 `JUHE_AI_SECRET`；不会生成任一 Go owner ID 或 F3 input secret。
