...
当前发布拓扑是 Node 主服务加一个 Go `juhe-ai-go-sidecar`。sidecar 内的 F1/F2/F3/F4 分别保留独立 Store、schema 和 owner lease，但只形成一个 PID、日志、容器或 launchd 服务。Node 仍负责网关、账户、主 API、usage、公开接口日志、model-check、stats 和 ops；操作日志只保留直接签名 RPC producer/read adapter。
...
2. 确认发布包存在且可执行唯一 `backend-go/juhe-ai-go-sidecar`。目标主机还必须提供系统 `rg`，或显式配置 `JUHE_AI_RG_PATH`。
...
7. 常规升级只请求 Node `/__aisys__/health`、`/__aisys__/api/health`（均为 `200`）、F3 `GET /__aiinternal__/health` 与 F4 `GET /__aiinternal__/v1/operation-logs/health`（均为 `204`）以及无 Key 的 `/v1/models`（`401`），再人工查看 `backend/logs/juhe-ai-go-sidecar.log` 的最后 80 行，不得有 `panic` 或 `fatal`。这些是发布包、Node、Go 和网关路由的最小真实检查。
...
5. 常规升级验证 Node 两个 health 为 `200`、F3/F4 sidecar health 均为 `204`、无 Key gateway 为 `401`，并查看 Node 与 sidecar 启动日志。F1/F2 新鲜度和 F3/F4 Node -> Go -> Node 读回只在首次部署、owner/存储变更或故障调查时执行。
