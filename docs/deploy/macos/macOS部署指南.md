...
- 发布包必须包含目标 macOS 架构的 `juhe-ai-jobs`、`juhe-ai-gateway` 和 `juhe-ai-maintenance` 三个二进制。常驻的 `jobs` 承载 F1/F2，`gateway` 承载 F3/F4，`maintenance` 只能执行一次性维护命令。当前候选槽只启动独立 Node 拓扑，并强制复用承流槽的 jobs/gateway owner；它不得创建第二个 F1/F2/F3/F4 owner。任何候选记录都不替代 `current` 切换、反向代理/Edge、生产数据或真实流量验证。
...
`JUHE_AI_AUDIT_LOG_INPUT_SECRET` 与 `JUHE_AI_OPERATION_LOG_INPUT_SECRET` 都不能复用或回退 `JUHE_AI_SECRET`。`start.sh` 启动 Go jobs/gateway，但不会生成 F1/F2/F3/F4 owner ID 或 F3/F4 input secret；任一项缺失时必须失败，不得跳过对应组件继续运行。PostgreSQL performance 模式中 F3 优先使用 `JUHE_AI_AUDIT_LOG_POSTGRES_URL`、F4 优先使用 `JUHE_AI_OPERATION_LOG_POSTGRES_URL`，两者未设置才回退 `JUHE_AI_POSTGRES_URL`；F3 仍使用独立 blob/hot-search 目录。Node 对 F3/F4 的一次性输入确认不阻塞已完成的业务响应。
...
pgrep -af 'juhe-ai-(jobs|gateway)' || true
```

预期是 Node health 为 `200`、jobs/gateway project health 为 `200`、F3/F4 input health 均为 `204`，且两个 Go owner 都来自当前 release。routine release 还要验证无 Key gateway 返回 `401` 并查看 jobs/gateway 启动日志；不能把 launchd loaded 或单个 Node health 当作切流证据。
...
queue 迁移前先建立 token fence，再用 `backend/dist/scripts/operations/drain-redis-streams.js` 独立排空 usage、public API log 和 record maintenance 三条 Stream。F3 审计和 F4 操作日志不进入 Redis，必须分别确认 Go gateway loopback health、owner lease 和写后读回；普通运行日志也不进入 Redis，必须单独确认角色 JSONL 文件 backlog 已由 Go jobs 内 F1 追平并且 cursor/freshness 正常。切换到 `6381` 后它成为新的队列事实源，失败时不得直接把 URL 改回旧 `6380`；state 改为无持久化必须在 queue 连续性验证之后单独执行。

### 4.2 高性能模式双 Go owner

`install-performance-topology.sh` 安装 `juhe-ai-jobs` 和 `juhe-ai-gateway` 两个 launchd 服务：jobs 承载 F1/F2，gateway 承载 F3/F4；`juhe-ai-maintenance` 不常驻。dry-run/apply 都要求 release 内三个 Go 二进制是可执行常规文件；system scope 同时验证读取与执行权限。F1/F2/F3/F4 仍保留独立 Store、schema 和 owner lease，稳定 ID 由 `--instance-id-prefix` 加固定组件名生成。F2 固定 PostgreSQL 模式，优先使用 `JUHE_AI_TABLE_MONITOR_POSTGRES_URL`，否则只继承 `JUHE_AI_POSTGRES_URL`，两者均缺失即失败，不会回退 SQLite、Redis、Node worker 或 queue。

发布包内的运维脚本按文档一律用 `bash docs/deploy/macos/operations/<脚本>.sh ...` 调用；不要假设这些文档脚本本身带可执行位并直接以 `./<脚本>.sh` 运行。`start.sh` 与三个 Go 二进制都必须具备执行权限。

system scope 的 `--apply` 必须由 `sudo` 执行；Node、Go jobs 与 Go gateway 均以 `--service-user` 指定的非 root 用户运行。目标机完成 `pnpm install --prod` 后，release 必须对服务用户递归可读/可执行、但不可写；安装器会拒绝服务用户可写的 release。运行目录、日志和 spool 才可以交给服务用户写入，不能用递归 `chown` 或放宽 release 权限来解决启动失败。

F3 固定 PostgreSQL 模式，稳定实例 ID 为 `<instance-id-prefix>-audit-log`；F4 为 `<instance-id-prefix>-operation-log`。F3 读取优先的 `JUHE_AI_AUDIT_LOG_POSTGRES_URL`，F4 读取优先的 `JUHE_AI_OPERATION_LOG_POSTGRES_URL`，两者未设置才读取 `JUHE_AI_POSTGRES_URL`；F3 另读取可选的 `JUHE_AI_AUDIT_LOG_BUSINESS_SETTINGS_URL`。二者均要求同地址的 loopback input URL/listen address 和各自独立 input secret；缺失、非 loopback、地址不一致或短密钥都会失败。F3/F4 不会回退 `JUHE_AI_SECRET`、SQLite、Redis 或 Node worker。Node service 与 sidecar 统一使用运行槽位的 `$DATA_DIR/audit/blobs` 和 `$DATA_DIR/audit/hot-search`，避免 F3 payload/hot-search 写读目录漂移。

脚本先通过 gateway/control 的 `/__aisys__/health` 和 `/__aisys__/api/health`（后者是 Node DB-service readiness），再启动并验证 Go jobs/gateway project health；F3 `/__aiinternal__/health` 与 F4 `/__aiinternal__/v1/operation-logs/health` 均返回 `204`。`--quick` 只要求一次成功，跳过指标注册和重复稳定轮询。routine cutover 再检查无 Key gateway `401` 和启动日志。F1/F2 新鲜度、F3/F4 写后读回和完整回滚演练只在首次新拓扑、owner/存储变更、故障或回切时进行。不得在脚本中读取 PostgreSQL 凭据、直接查询数据库或把存活误报为数据完成。
...
归档 temporary 服务空间的历史拓扑不能外推到当前发布。当前候选槽必须复用承流槽的 Go jobs/gateway，不能创建第二个 F1/F2/F3/F4 data owner。routine release 完成 control/API/gateway、F1/F2/F3/F4 共享 Go health 和启动日志后，才可通过快速 route 切换上线；它仍不是 `current` 切换、Nginx/Caddy/Edge 变更或生产数据验证。

正式候选必须从最终冻结 commit 生成 `juhe-ai-jobs`、`juhe-ai-gateway`、`juhe-ai-maintenance`，记录 archive SHA-256，并完成 candidate control/API/gateway、F1/F2/F3/F4 共享 Go health 和启动日志检查。Node -> F3/F4 -> Node 读回、稳定观察和 handover rollback 演练只在首次新拓扑、owner/存储变更、故障或回切时进行。
...
2. 使用独立 candidate Node runtime、label、端口、Nginx slot include、upstream suffix、Node instance ID 与 Redis 身份执行 system `--apply`，同时传入 `--go-sidecar-mode reuse --audit-input-port <正式 owner F3 loopback端口> --operation-log-input-port <正式 owner F4 loopback端口>` 复用正式槽 jobs/gateway；candidate 不得创建 F1/F2/F3/F4 的第二个 owner。reuse apply 若发现该 candidate label 的 Go owner job、plist 或 run script 残留会直接拒绝，必须人工确认后清理，禁止自动停止未知 owner。system scope 必须同时传入 `--nginx-config <slot include>` 与 `--nginx-main-config <main config>`，两者不得相同。
3. 对 candidate 请求 control health、API health、无 Key 的 gateway `401`、jobs/gateway project health、F3/F4 input health，并读取启动日志末尾确认没有 `panic` 或 `fatal`。静态 `index.html` 或单个 `200` 不算通过。
