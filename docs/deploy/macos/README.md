...
- 当前高性能 `install-performance-topology.sh` 使用两个 Go owner：`juhe-ai-jobs` 承载 F1/F2，`juhe-ai-gateway` 承载 F3/F4；`maintenance` 仅用于一次性维护。routine release 从最终冻结 release 启动独立 candidate Node 槽，使用 `--go-sidecar-mode reuse --audit-input-port <F3端口> --operation-log-input-port <F4端口> --quick` 复用正式的两个 Go owner，再用 `quick-performance-cutover.sh` 原子切 route。完整 temporary 预演、功能读回和 handover controller 只用于首次新拓扑、事故调查或异常回切。
