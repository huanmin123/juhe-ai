...
- `install-performance-topology.sh`：仅用于高性能模式，在同一台 macOS 上安装独立的 Go F1 `runtime-log-indexer`、Go F2 `table-monitor`、Go F3 `audit-log-writer` LaunchDaemon/LaunchAgent、一个 control、默认 3 个 gateway，以及由 control 看护的 Usage 2 / Log 2 / Stats 1 / Ops 1 worker。三者都有独立的 label、PID、日志、停止和回滚条目。F1 使用 PostgreSQL 和同一日志目录；F2 固定 `JUHE_AI_TABLE_MONITOR_STORE=postgres`；F3 固定 PostgreSQL，使用稳定实例 ID、槽位专用 blob/hot-search 目录和显式 loopback HMAC 输入。三者都不使用 Redis、Node worker 或 queue。脚本仅在 Node `/__aisys__/api/health` 的 DB-service readiness 通过后才启动 F1/F2/F3：F1/F2 验证有界 launchd 存活，F3 还连续验证 `GET /__aiinternal__/health` 为 `204`。这些状态不代替数据验收，生产切流前仍须通过 Node 只读 API 确认 F1/F2 新鲜度，并以真实可审计请求确认 Node -> F3 -> Node 详情读回。逐节点健康、Redis freshness fence 和 PID 拓扑门禁通过后才原子切换已被 Nginx 主配置 include 的配置文件，失败恢复三项 Go 服务、Node 的 launchd、run script 和 Nginx。2026-08-11 已在目标 Mac 隔离 temporary 服务空间完成 system `--apply`、F3 重启接管与 rollback；该证据不授权 `current`、Nginx/Caddy/Edge 或真实流量切换，正式候选仍须从最终冻结 release 重做预演。
...
- 高性能拓扑脚本不会安装 Nginx，也不会修改 Nginx 主配置；`--nginx-config` 必须是主配置已 include 的绝对路径，并应通过 `--nginx-bin` 与 `--nginx-main-config` 明确绑定实际运行实例。生产 apply 还应通过 `--release-dir` 绑定不可变发布目录，脚本会解析物理路径后再生成运行脚本，避免 `current` 并发切换造成进程版本混用。`--dry-run` 不是流程模拟：它只读门禁不可变发布输入，要求真实发布目录、构建产物、两个 Node 预检脚本、`backend/.env` 与三个 Go 二进制均存在，且 F1/F2/F3 都是可执行常规文件；任一缺失必定失败。F3 还要求后续 apply 提供一致的 loopback input URL/listen address、显式 `JUHE_AI_AUDIT_LOG_INPUT_SECRET`、PostgreSQL 和专用 blob/hot-search 目录，缺失绝不回退。`--apply` 才要求 Node、launchd、Nginx 和所有目标端口可用，并执行运行目录创建或服务、Nginx 状态变更。
- 默认单槽路径保持 `bin/performance`、`logs` 与 `shared/usage-spool`。主槽与临时槽并存时，临时槽必须额外提供唯一的 `--runtime-dir` 和 `--nginx-upstream-suffix`：运行根目录必须是 base 物理目录内的真实子目录，运行脚本、launchd 日志和 usage spool 都从该根目录派生；suffix 只能是 `A-Za-z0-9_` 且长度为 1 到 48，生成的 gateway/control Nginx upstream 名称会带上该 suffix。调用方仍须同时使用独立的 release、label prefix、端口、Nginx include 文件、数据库与 Redis 身份；脚本不会替这些外部隔离资源做推断。F1、F2、F3 的实例 ID 都由 `--instance-id-prefix` 与固定服务名组成，必须稳定且唯一；发布输入门禁和 apply 都 fail-fast 检查三个 Go 二进制是发布包内可执行的常规文件，apply 还会在 system scope 下验证服务用户可读/执行。F1 运行脚本优先读取 `JUHE_AI_RUNTIME_LOG_POSTGRES_URL`，F2 优先读取 `JUHE_AI_TABLE_MONITOR_POSTGRES_URL`，随后都只回退 `JUHE_AI_POSTGRES_URL`（含 `backend/.env`）；F3 只读取 `JUHE_AI_POSTGRES_URL` 并显式设置业务设置 URL、input secret 和 loopback listener，缺失即失败，不会切回 SQLite、Redis、Node worker 或旧审计队列。F2/F3 达到有界 launchd 存活不等同于业务完成；不读取凭据、不做直接数据库探测，必须在 Node `/__aisys__/api/health` 已就绪后由部署人员通过 Node 只读 API 检查 F1/F2 新鲜度和 F3 审计详情读回。发布/部署脚本已完成目标 Mac isolated temporary `--apply`、listener、F3 restart 和 rollback 预演；开发 PostgreSQL 闭环和这次临时预演都不能替代正式 release 的重复验证、Nginx/Caddy/Edge 切换或真实流量验收。
