...
- `install-performance-topology.sh`：仅用于高性能模式，在同一台 macOS 上安装独立的 Go F1 `runtime-log-indexer` 与 Go F2 `table-monitor` LaunchDaemon/LaunchAgent、一个 control、默认 3 个 gateway，以及由 control 看护的 Usage 2 / Log 2 / Stats 1 / Ops 1 worker。F1 使用 PostgreSQL 和同一日志目录；F2 固定 `JUHE_AI_TABLE_MONITOR_STORE=postgres`，以稳定实例 ID 取得 owner lease 并向 `juhe_stats` 写快照；两者都不使用 Redis、Node worker 或 queue。该脚本没有 F3 `audit-log-writer` 服务、HMAC 输入或审计读回验证，不能用于 F3 已接管审计的生产候选。脚本仅在 Node `/__aisys__/api/health` 的 DB-service readiness 通过后才启动 F1/F2：F1 验证 launchd 存活，F2 仅验证有界 launchd 存活，生产切流前仍须通过 Node 只读 API 人工确认 F2 snapshot freshness。逐节点健康、Redis freshness fence 和 PID 拓扑门禁通过后才原子切换已被 Nginx 主配置 include 的配置文件，失败恢复两侧 Go 服务及 Node 的 launchd、run script 和 Nginx。F3 macOS 编排须单独实现并在 temporary release 验收后才可应用。
...
- 高性能拓扑脚本不会安装 Nginx，也不会修改 Nginx 主配置；`--nginx-config` 必须是主配置已 include 的绝对路径，并应通过 `--nginx-bin` 与 `--nginx-main-config` 明确绑定实际运行实例。生产 apply 还应通过 `--release-dir` 绑定不可变发布目录，脚本会解析物理路径后再生成运行脚本，避免 `current` 并发切换造成进程版本混用。`--dry-run` 不是流程模拟：它只读门禁不可变发布输入，要求真实发布目录、构建产物、两个 Node 预检脚本、`backend/.env` 与两个 Go 二进制均存在，且 `backend-go/juhe-ai-runtime-log-indexer` 和 `backend-go/juhe-ai-table-monitor` 都是可执行常规文件；任一缺失必定失败。F3 已接管审计的候选仍必须同时具备 F3 二进制、独立 HMAC 配置和三 sidecar 编排；当前脚本不具备这些条件，因此不得对该候选执行 `--apply`。`--apply` 才要求 Node、launchd、Nginx 和所有目标端口可用，并执行运行目录创建或服务、Nginx 状态变更。
- 默认单槽路径保持 `bin/performance`、`logs` 与 `shared/usage-spool`。主槽与临时槽并存时，临时槽必须额外提供唯一的 `--runtime-dir` 和 `--nginx-upstream-suffix`：运行根目录必须是 base 物理目录内的真实子目录，运行脚本、launchd 日志和 usage spool 都从该根目录派生；suffix 只能是 `A-Za-z0-9_` 且长度为 1 到 48，生成的 gateway/control Nginx upstream 名称会带上该 suffix。调用方仍须同时使用独立的 release、label prefix、端口、Nginx include 文件、数据库与 Redis 身份；脚本不会替这些外部隔离资源做推断。F1 `runtime-log-indexer` 和 F2 `table-monitor` 的实例 ID 都由 `--instance-id-prefix` 与固定服务名组成，必须稳定且唯一；发布输入门禁和 apply 都 fail-fast 检查两个 Go 二进制是发布包内可执行的常规文件，apply 还会在 system scope 下验证服务用户可读/执行。F1 运行脚本优先读取 `JUHE_AI_RUNTIME_LOG_POSTGRES_URL`，F2 优先读取 `JUHE_AI_TABLE_MONITOR_POSTGRES_URL`，随后都只回退 `JUHE_AI_POSTGRES_URL`（含 `backend/.env`），缺失即失败，不会切回 SQLite 或 Redis。F2 达到有界 launchd 存活不等同于快照完成；不读取凭据、不做直接数据库探测，必须在 Node `/__aisys__/api/health` 已就绪后由部署人员通过 Node 只读 API 检查 owner lease 和 snapshot freshness。发布/部署脚本已支持，Mac 现场仍待验证；2026-08-09 的开发 PostgreSQL 闭环不能替代 Mac `--apply`、listener、rolling 或 rollback 预演。
