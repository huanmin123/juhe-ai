...
- `install-performance-topology.sh`：仅用于高性能模式，在同一台 macOS 上安装独立的 Go F1 `runtime-log-indexer` 与 Go F2 `table-monitor` LaunchDaemon/LaunchAgent、一个 control、默认 3 个 gateway，以及由 control 看护的 Usage 2 / Log 2 / Stats 1 / Ops 1 worker。F1 使用 PostgreSQL 和同一日志目录；F2 固定 `JUHE_AI_TABLE_MONITOR_STORE=postgres`，以稳定实例 ID 取得 owner lease 并向 `juhe_stats` 写快照；两者都不使用 Redis、Node worker 或 queue。脚本仅在 Node `/__aisys__/api/health` 的 DB-service readiness 通过后才启动 F1/F2：F1 验证 launchd 存活，F2 仅验证有界 launchd 存活，生产切流前仍须通过 Node 只读 API 人工确认 F2 snapshot freshness。逐节点健康、Redis freshness fence 和 PID 拓扑门禁通过后才原子切换已被 Nginx 主配置 include 的配置文件，失败恢复两侧 Go 服务及 Node 的 launchd、run script 和 Nginx。发布/部署脚本已支持，Mac 现场仍待验证。
