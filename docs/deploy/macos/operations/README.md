...
- `install-performance-topology.sh`：仅用于高性能模式，在同一台 macOS 上安装 `juhe-ai-jobs`（F1/F2）和 `juhe-ai-gateway`（F3/F4）两个 LaunchDaemon/LaunchAgent、一个主 control、默认 3 个 gateway，以及由主 control 看护的 Usage 2 / Log 2 / Stats 1 / Ops 1 worker；`--control-count 2` 或 `3` 时新增 control 是只读管理副本，不启动 worker，管理写入和内部派发仍固定主 control。Nginx 先把所有管理请求送主 control；主 control 完成认证、限流后才把签名的纯读请求转给副本。安装器在 Node `/__aisys__/api/health` readiness 通过后启动 jobs/gateway，并连续验证 jobs/gateway project health、F3 `GET /__aiinternal__/health`、F4 `GET /__aiinternal__/v1/operation-logs/health`。高性能候选槽必须明确 `--go-sidecar-mode reuse --audit-input-port <正式 F3 owner 端口> --operation-log-input-port <正式 F4 owner 端口>`，复用两个正式 owner，禁止再启动第二个 Go 数据 writer。候选 Node 读取正式 owner 的审计 blob/hot-search 目录。逐节点健康、Redis freshness fence 和 PID 拓扑门禁通过后才原子切换已被 Nginx 主配置 include 的配置文件；失败恢复受本次 owner/reuse 模式管理的 launchd、run script 和 Nginx。
...
- `quick-performance-cutover.sh`：高性能生产的默认快速切流入口。它只验证已经启动的 candidate control/API/gateway、共享 Go jobs/gateway 与 F3/F4 input health 和启动日志，然后原子替换外层 route fragment、reload Nginx、验证三条公网请求；失败立即恢复原 route。它不启动、停止或重配任何 Node/Go 服务。
...
- `install-wireguard-reconciler.sh`：默认 dry-run 的上述迁移与恢复器安装入口。apply 需要私有 root manifest、两个受控脚本 SHA-256 和由部署环境提供的 root-owned Edge→Kubernetes 入口探针；未传 `--state-dir` 时使用 `<install-dir>/wireguard-reconciler-state`，避免 macOS `/var` 符号链接与 root-only 路径链契约冲突；`--remove --apply` 仅移除恢复器，不回退已经收口到 root-only 的 WireGuard job。
...
- WireGuard reconciler 是唯一的窄例外：它只可对私有 manifest 的全部 Edge 同时满足两次陈旧握手、稳定默认路由、sleep/wake 宽限结束、无维护/发布锁和预算允许时，先处理一个 canary。canary 必须在旧映射已清理后重新观察到 `utun` 映射、连续新握手、传输增量以及外部 Edge→Kubernetes 入口探针成功，才会串行处理余下 Edge；macOS 可复用 `utun` 编号。任一步失败立即停止。它不会把公网 HTTP 失败直接当成 WireGuard 故障。
- 私有 manifest 和外部 probe helper 必须由 root 持有且不可由服务用户、组或其他用户写入。probe helper 只返回 `0=已知健康`、`1=已知 Edge 失败`、`75=未知`；未知不得触发恢复。其实现、指标传输和凭据必须留在部署环境，不在恢复期间临时开放 Prometheus。manifest 用于构成精确 allowlist，不得以通配 label、Shell 片段、环境变量或网络返回值扩展动作目标；wrapper 和配置的源 SHA-256 由 apply 前私有预检写入，不得写入仓库。
...
- candidate 必须使用不可变 release、独立 Node runtime、label、端口、Nginx slot include、upstream suffix、Node instance ID 与 Redis 身份；业务槽连接同一权威业务库，不能把预演 clone 当成生产权威库。candidate 必须显式使用 `--go-sidecar-mode reuse --audit-input-port <正式 F3 owner 端口> --operation-log-input-port <正式 F4 owner 端口>`，共享正式槽 Go jobs/gateway 的 F1/F2/F3/F4 owner、Store、lease、审计 blob/hot-search 目录与 loopback 输入端口，不能创建第二个 Go owner。reuse apply 会检查候选 label 对应的 Go jobs/gateway launchd job、plist 与 run script；任一残留即拒绝，必须先人工确认并清理，绝不自动停止未知 owner。
- 常规发布只需 candidate control health、API health、无 Key 的 gateway `401`、共享 Go jobs/gateway 与 F3/F4 input health 和启动日志无 `panic`/`fatal`；随后由 `quick-performance-cutover.sh` 原子切 route，并验证同样三条公网请求。静态 HTML、单个 `200` 或任一 Go health `204` 仍不能单独放行。
...
该门禁会检查敏感生产常量、只读诊断边界、代理监听/进程/探测身份、PID/cwd/端口/双 health 身份、外部 probe 返回未知或失败时的恢复边界、launchd 安装失败恢复、plist `KeepAlive`、shell 语法和 dry-run；在 Darwin/Linux + gcc 环境还会执行临时原生命令桩的 bootstrap/kickstart/监听/探测失败回滚模拟。
