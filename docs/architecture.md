...
`juhe-ai` 要做成一个轻量、易扩展的本地 OpenAI 兼容中转与账号调度系统。它解决的核心问题不是单纯“管理账号”，而是让客户端长期固定连接一个本地入口，把多套中转地址、多组上游账号、多服务商入口和代理策略统一放到后台调度。

典型使用场景：用户手里有很多中转和上游账户，但在客户端来回切换 Base URL、API Key 或服务商很麻烦；部分客户端切换入口后会出现历史会话不可见、上下文割裂或需要重新配置的问题。`juhe-ai` 通过本地固定入口承接客户端请求，再按 API Key 绑定的分组选择可用账号，让账号切换、错误处理和后续多服务商扩展尽量发生在后台。

系统需要优先保证：

- 客户端入口稳定：客户端只配置本服务 `/v1` 和本地 API Key。
- 会话体验连续：尽量避免用户为了换服务商或账号而频繁改客户端配置，降低会话丢失和上下文割裂感。
- 上游自动切换：目标账号或供应商异常、限流、临时不可用时，按分组、优先级、并发和错误策略切换备用账号。
- 集中可观测：记录请求、用量、错误和命中账号，便于知道问题发生在哪个账号、分组或上游。

整体模块先设计完整，但每期只落必要能力，避免一开始就变成重型系统。
...
错误处理策略以账号为主：账户创建和编辑时把规则列表保存到 `credentials.error_handling_rules`。系统不提供默认错误处理策略；账号规则未命中的上游错误统一触发当前账号临时不可调用并切换账号重试。
