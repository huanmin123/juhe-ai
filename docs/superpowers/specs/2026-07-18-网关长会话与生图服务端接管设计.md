...
- `streamRequestTimeoutSeconds = 120`：上游首个响应和非流式首字节等待上限。
- `streamIdleTimeoutSeconds = 30`：流式上游连续没有 raw chunk 的停顿上限。
- `streamClientTotalWaitTimeoutSeconds = 270`：服务端隐藏切号 / 重试期间的客户端总等待上限。
- `streamMaxLifetimeSeconds = 1800`：单条 SSE 最大存活时间。
...
`noAvailableAccountWaitMs` 只在没有可用账号时生效。只要调度层仍能提供候选账号，不能因为该值到期而停止服务端尝试。

### 3.3 downstream commit

请求维护 `downstreamCommitted` 和 `downstreamBytesWritten`：

- `false`：客户端尚未收到任何不可回滚的响应内容，可以关闭当前上游并换账号。
- `true`：已经写出响应头或正文，不能把第二个账号的完整响应拼接到当前响应。

状态判断不依赖上游 HTTP 状态码或错误体，只依赖网关自身的写出边界。
...
- 当前账号连接中断后不把另一个账号的响应追加到已提交响应。
- 对明确识别的客户端使用现有协议失败事件；通用客户端使用现有协议原生失败 / 断流语义，让客户端重试。
- 已提交阶段不因为上游状态码、错误体或模型返回字段追加新的账号判断。

### 4.3 270 秒语义调整

`streamClientTotalWaitTimeoutSeconds` 改为“无可用账号时的客户端接管等待上限”。当前候选仍可调度时不使用该值阻断服务端救回。已有客户端画像和协议最终失败入口保留，不改变对外错误结构。
...
Images API 非流式响应通常包含 Base64 图片，新增有界 pre-commit buffer：

1. 上游 body 在配置上限内先写入缓冲，不立即向客户端提交。
2. 上游完整结束后一次性提交，避免生成中途失败时返回半截 JSON。
3. 缓冲超过显式上限后立即提交已有数据，后续中断只能交给客户端重试，不能无限增大内存。
4. 缓冲失败时如果仍未提交，释放账号并继续下一个候选账号。

缓冲上限必须复用现有有界响应体配置，不能把完整大文件读入内存。

## 6. 流式响应策略

- `image` lane 使用 120 秒 raw chunk 停顿上限；`text` lane 保持 30 秒。
- 未提交前允许关闭当前上游并切换账号；已有 pre-commit buffer 需要清空。
- SSE 等待账号期间只发送注释心跳，不发送伪造的模型事件、成功事件或错误事件。
- 已提交后只继续当前上游；无法续接时按客户端画像和下游协议结束，不能把新账号流接在旧流后面。
- 已提交且持续有上游数据的长会话不因固定 1800 秒机械中断；绝对寿命只保护未提交的单次尝试。客户端断开、无数据停顿或服务关闭仍是收口条件。

## 7. 跨协议图像 provider

`JUHE_AI_IMAGE_GENERATION_PROVIDER_TIMEOUT_MS` 与 `image` timeout profile 对齐：默认 600 秒，最大 900 秒。provider executor 接收并合并外层 `AbortSignal`，客户端断开或当前尝试切换时及时取消 provider 请求。
...
- `imageIdleTimeoutSeconds`，默认 `120`，范围 `1..3600`。
- `imageUncommittedAttemptMaxLifetimeSeconds`，默认 `3600`，范围 `60..86400`。

不新增 `imageClientTotalWaitTimeoutSeconds`；图像请求继续使用系统级无账号等待值，但在有候选账号时不受其阻断。
...
- 大 Base64 响应超过缓冲上限时按有界内存策略转为客户端重试边界。

## 10. 风险与回退

- 服务端隐藏重试可能增加上游请求次数和成本；审计必须记录尝试序列、账号、是否提交和最终结果，但不记录大 Base64 正文。
- 非流式图像响应的 pre-commit buffer 增加单请求内存，需要固定上限和并发保护。
