...
> 关联：`docs/functions/AI问答设计.md`、`docs/architecture/架构总览.md`、PLAN-0150-20260722T022751000Z
...
- 用户未明确指定宽高或分辨率时，系统提示只要求模型根据用途、内容和构图选择合适的常规尺寸与比例，不写死具体比例，不允许自行选择 2K、4K 或其他超大尺寸。
- “高清、精致、细节丰富”等质量描述只影响 `quality`，不能解释为大尺寸要求。
- 用户明确指定尺寸、分辨率或比例时优先遵循，但服务端仍验证模型的边长、16px 倍数、宽高比和总像素限制。
- 未明确要求大图时，服务端把模型尺寸约束在常规像素档位：最长边不超过 1536px、总像素不超过 1,572,864，且继续满足当前图像模型的最小像素、16px 倍数和最大 3:1 宽高比；这些数值不写入系统提示词。超出时按原比例缩小到合法常规尺寸并记录 `sizeAdjusted=true`，不能把 `size=auto` 原样交给图像上游自由选择超大图。
- 用户明确提出 2K、4K 或具体大尺寸时，才允许进入模型支持的高分辨率范围。
- 模型没有提供尺寸或提供的值无法规范化时，服务端使用 1024x1024 保守兜底；该兜底只处理缺失/非法参数，不覆盖模型已经给出的合法常规比例。
...
- PLAN-0150-20260722T022751000Z 继续追踪本需求，不新建近似重复计划。
