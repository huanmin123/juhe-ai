...
| 层级 | 建议命令 / 验证方式 | 必须覆盖 |
| --- | --- | --- |
| Handler | `go test ./internal/httpapi -run TestPublicAccount -count=1` | shell 注入 handler 后的 `account/list|add|update|del`、strict query/body、GET 分页 coerce、POST 数字不 coerce、可选字段 `null` / 空 notes / nullable schedule、中文错误、测试 token mock、业务响应不返回 `apiKey` / `baseUrl` / `credentials`，请求日志脱敏上游 `apiKey` 和带 userinfo 的 URL；`supportedModels` 省略、显式空数组和非空数组的 presence 必须保持到 service |
