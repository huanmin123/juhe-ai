...
Go public account 在新增 `pending_test` 账户后以 `activation` 调用 Node；更新时仅在 API Key / Base URL 实际变化，或显式提交 `supportedModels` 后以 `configuration` 调用。配置为无默认且 public API 开启时必填的 `JUHE_AI_NODE_INTERNAL_BASE_URL`，只允许 `http` loopback IP literal + 显式端口；`JUHE_AI_NODE_INTERNAL_REQUEST_TIMEOUT` 默认 `2s`、范围 `100ms..10s`；双方使用 trim 后一致的 `JUHE_AI_SECRET`。

内部协议固定为 `POST /__aiinternal__/v1/account-health-check/dispatch`，原始 JSON 最大 `1024 bytes`，字段仅 `accountId/reason`，签名为 HMAC-SHA256（域 `juhe-ai:account-health-check-dispatch:v1\n` + 原始 body），仅 HTTP `202` 成功。Node router 校验 loopback、identity encoding、签名和 strict payload 后调用现有 dispatch。Go 在事务提交后 best-effort 调用，失败不回滚写入、不重试、不构成可靠交付。
...
- `backend-go/internal/store/postgres/publicaccounts.go` 和 `backend-go/internal/store/postgres/queries/w1b_public_accounts.sql`：新增 PostgreSQL public account store / sqlc query，覆盖事务、目标用户 / 分组 / provider profile、列表分页、账号行锁、同名唯一冲突、账号创建、分组绑定创建、模型替换、调度优先级更新、软删除和 summary join；provider profile 查询联表 `providers` 返回并解码 `default_supported_models_json`，供 service 处理字段省略时的默认继承。
...
| 健康任务 bridge | Node/Go 专项 + 跨运行时 integration | loopback、`1024 bytes`、HMAC-SHA256、`activation/configuration`、仅 `202` 成功和写后 best-effort；`TestAccountHealthCheckDispatchNodeE2E` 仅验证 Go client -> Node 生产 router |
...
| 层级 | 建议命令 / 验证方式 | 必须覆盖 |
| --- | --- | --- |
| Handler | `go test ./internal/httpapi -run TestPublicAccount -count=1` | shell 注入 handler 后的 `account/list|add|update|del`、strict query/body、GET 分页 coerce、POST 数字不 coerce、中文错误、测试 token mock、业务响应不返回 `apiKey` / `baseUrl` / `credentials`，请求日志脱敏上游 `apiKey` 和带 userinfo 的 URL；`supportedModels` 省略、显式空数组和非空数组的 presence 必须保持到 service |
| Service | `go test ./internal/modules/publicaccounts -run TestServiceAdd -count=1` | 目标用户 / 目标分组自动创建、provider/profile 启用校验、`type=api_key` 限制、上游 base URL 安全校验、凭据可逆加密、凭据指纹 hash、同名冲突、`pending_test -> active` 拒绝、局部凭据更新、软删除和 `not_found` 幂等；省略模型继承 provider 默认值，显式空数组和空默认值报固定错误，重复名称优先 |
