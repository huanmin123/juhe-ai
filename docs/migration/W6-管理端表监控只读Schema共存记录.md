...
2026-07-22 的 `origin/master` 权威 Goose catalog 为 `000069`。主工作区同时存在尚未提交的 `000070_w2_gateway_model_catalog_chat_snapshot_variants.sql`，该版本属于并行的模型目录 / 聊天快照工作。本切片不得复用 `000070`，也不能在缺少已提交 `000070` 的分支直接添加 `000071`，否则会破坏连续版本目录和并行合并安全。
...
这允许 fresh Go schema 69 正常启动并与 Node 共存。部署层只有在 Node 初始化过上述 PostgreSQL schema 且门禁通过后，才能把三条精确 GET 路径 opt-in 到 Go；否则这些路径继续由 Node 持有。

## 后续 Schema Owner 门禁

- `000070` 合入权威 catalog 后，如减法阶段决定由 Go 接管表监控 schema，再使用当时最新的连续版本新增两张快照表及索引；不能回填或改写已发布 migration。
