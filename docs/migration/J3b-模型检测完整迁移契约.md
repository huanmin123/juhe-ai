...
> 2026-09-21 常驻修订（维护者决定）：`JUHE_AI_J3B_ENABLED` 总开关已移除，模型检测 owner 在 gateway 进程默认装配。handoff/readiness 家族（`JUHE_AI_J3B_BUSINESS_HANDOFF_CONFIRMED / NODE_WRITER_STOPPED / OWNER_EPOCH / CUTOVER_EVIDENCE_PATH / SCHEMA_READY / HEALTH_BOUNDARY_READY / RUNTIME_READY`）全部未配置时按新装 standalone 部署自动认领（与 `runtime.go` 的 `BusinessOwnerAutoClaimed` 同款规则）：handoff 三证置真、epoch 固定 `standalone`、无证据文件校验、无 Redis 时 key-model 准入回退进程内 memory store 且熔断 RuntimeGate 不装配；owner/instance/store/存储路径/凭据密钥均有默认值（datadir 固定名表 `model-check.sqlite3`、hostname 实例 ID、`JUHE_AI_SECRET` 回退、业务库路径跟随主配置 `JUHE_AI_BUSINESS_DATABASE_PATH`、主库连接串回退），专属库文件与 schema 由组合根幂等自举（`bootstrap.EnsureSQLiteJ3bModelCheck`，DDL 单一事实源仍在 maintenance `internal/j3bmodelcheck`，并有端到端验收测试覆盖）。家族内任一成员显式配置即恢复本文全部 fail-closed 门禁与生产切流纪律（专属库仍须外部预置、Redis 仍为硬要求），生产切流路径不受影响；前端模型检测菜单默认常驻，`VITE_JUHE_AI_J3B_ENABLED` 已同步移除，访问控制由管理面权限承担。
> 2026-09-22 单机等价修订（维护者决定）：零配置 standalone 下有/无 Redis 的行为对齐为几乎等价。无 Redis 时账户熔断以进程内 `gateway_dispatch.MemoryCircuitGate` 恒装配（决策语义对齐 `RuntimeCircuitGate` 与 `circuit_runtime` Lua：SUSPECT 确认资格、confirmation/canary 租约互斥、退避阶梯 {3s,5s,10s,30s,60s}、canary 三次成功恢复、unknown 中性；无 generation/ledger 多实例围栏，属单进程单 owner 定位），常驻组件 `J3b account-circuit-memory-gate` 使健康字段 `accountCircuitRuntimeReady` 在单机模式恒为 true——上一修订段中"熔断 RuntimeGate 不装配"的表述自此失效。同批口径修正：Business owner auto-claim 触发家族收窄为五证切流变量（`HANDOFF_CONFIRMED / NODE_WRITER_STOPPED / SCHEMA_READY / OWNER_EPOCH / CUTOVER_EVIDENCE_PATH`），`JUHE_AI_BUSINESS_OWNER / DATABASE_PATH / POSTGRES_URL` 为纯运维变量不再触发五证门禁（显式配置五证仍走原门禁）；gateway 与 jobs 的 Redis namespace 在加载层统一经 `rediscfg.CanonicalRedisNamespace` 剥 `juhe-ai:` 根前缀，全前缀输入不再产生双前缀键空间分裂；jobs `/health` 载荷移除 Solution A 恒定死字段 `modelCheckEnabled`/`modelCheckReady`（前端系统指标页相应减少噪音行）。
> 2026-09-20 材料清理：jobs 侧历史 J3b 实现材料已整体删除（`modelcheckapp/modelcheckcommand/modelcheckhttp/modelcheckpolicy/modelcheckresolver/modelchecksource/modelcheckactive/modelcheckdurable/modelcheckexecutor/modelcheckinput/modelcheckprobe/modelcheckprofile/modelcheckquality/modelcheckstore` 共 14 包，`modelcheckruntime` 修剪为仅剩 `LoadConfig` 硬失败守卫；`modelcheckauth` 因 `proxylatency` 生产引用保留）。本文 §2/§4/§5 等处提及的 jobs 材料路径自此仅作历史对照，不复存在；硬失败义务不变，守卫证据在 `jobs/internal/modelcheckruntime/config.go` 与 `cmd/juhe-ai-jobs/main.go`。
...
### 9.7.38 单机零配置熔断恒装配与健康语义（2026-09-22）

9.7.3 的接线在零配置 standalone（未配置 `JUHE_AI_J3B_CIRCUIT_REDIS_URL` / `JUHE_AI_REDIS_STATE_URL`）下原本不装配 `probeCircuit` 与 projector：探针链熔断准入整段跳过，`accountCircuitRuntimeReady` 恒 false，与配置 Redis 的部署形成可观察差异。2026-09-22 起该分支装配进程内 `gatewaydispatch.MemoryCircuitGate`（`internal/business/gateway_dispatch/memory_circuit_gate.go`）并注册常驻组件 `J3b account-circuit-memory-gate`：健康字段在单机模式恒 true，readiness 门（`!circuitRuntimeEnabled || running`）语义不变。Memory 版决策规则逐条对齐 `RuntimeCircuitGate` 与 `circuit_runtime` Lua（退避阶梯、三次 canary 恢复、租约互斥与读时过期规范化、unknown 中性、SUSPECT 确认资格），差异仅为单进程单 owner 下不需要的多实例围栏（generation/dispatchRevision/ledger/replayOrder）与 escalation 证据链（memory 模式无 projector，不可达）。该组件不对接 Redis、无 Node bridge，重启即重置，与同分支的 memory key-model gate 同定位；配置 Redis 后仍走 9.7.3 的完整 Redis 链路，行为不变。契约测试在 `memory_circuit_gate_test.go`。
