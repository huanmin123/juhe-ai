...
Business SQLite 的 Gateway `schemaReady` 还必须有实际结构证据：执行 `go run ./projects/maintenance/cmd/juhe-ai-maintenance -verify-business-sqlite-schema -business-sqlite-path <Business SQLite>`。该命令以 `mode=ro` 和 `PRAGMA query_only=1` 打开用户数据库，只读取 `sqlite_master`/`PRAGMA table_info`/`PRAGMA foreign_key_list`，并按 `business-sqlite-gateway-v3` 核对 Gateway 已依赖的认证、账户、模型目录、分组、质量策略、调度、enforcement 及 announcement 表、列、关键索引和外键；不执行 DDL。认证范围还必须包含 `system_sessions.created_at` 以及 `system_sessions.system_account_id -> system_accounts.id ON DELETE CASCADE`。公告范围包括 `announcements`、`announcement_reads`、`idx_announcements_public`、`idx_announcements_admin`、`idx_announcements_admin_page`、`idx_announcement_reads_account`，以及公告/已读记录指向 `system_accounts`、`announcements` 的删除关系。输出 `schemaVersion`、`readOnly`、`missingTables`、`missingColumns`、`missingIndexes`、`missingForeignKeys`、`userDatabaseTouched` 与 `ready`，任一缺失以退出码 3 阻断。该只读结构证据不创建 schema、不改变 Node owner，也不解除 B1 或 J3b 的任一 fail-closed 门；Gateway 在 SQLite 模式下收到 `JUHE_AI_J3B_SCHEMA_READY=true` 后仍必须运行共享 contract，不能仅凭环境变量启动。

J3b 专属 SQLite 回填后必须执行只读 readback：设置 `JUHE_AI_MAINTENANCE_J3B_SQLITE_PATH`、`JUHE_AI_MAINTENANCE_J3B_SOURCE_DATASET_PATH`、`JUHE_AI_MAINTENANCE_J3B_SOURCE_STATS_PATH` 后运行 `go run ./projects/maintenance/cmd/juhe-ai-maintenance -verify-j3b-model-check-sqlite-backfill`。命令只读打开 legacy dataset/stats 与专属 target，逐表核对 mandatory/optional 表、行数和按源列投影的 SHA-256 digest，并拒绝共享物理文件；输出 `ready`、`pathsDistinct`、`sourceReadOnly`、`targetReadOnly`、`tables`、`sourceRows`、`targetRows`、`sourceDigest`、`targetDigest`，任何漂移、缺表或路径冲突均以退出码 3 fail-closed。该 readback 是切换/rollback epoch 的证据，不代表 Node active-path-zero 已完成。
