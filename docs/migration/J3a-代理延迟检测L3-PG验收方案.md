...
- projector result role 通过 `J3A_PG_PROJECTOR_URL` 访问同一个一次性 scratch 的 `juhe_business` 与 `juhe_jobs`；必须能完成 projector `CheckContract`、`proxy_profiles` CAS、receipt/cursor 写入，以及由 `proxy_profiles` update trigger 引起的 `accounts`/availability projection 最小读取和 availability dirty upsert，且不得使用持久 dev database。
- fixture、连接串和密码只存在于本机临时环境；不得写入仓库、日志、审计载荷或本文档。

补充兼容规则：Node 旧代理检测会遍历所有启用 provider；当某个 provider 的默认
`base_url` 为空或不是 HTTP(S) URL 时，该 provider 只生成一个 `unknown` 项，不能
使同一代理的其他 provider 候选整体丢失，也不能发起直连或伪造成功。Go direct reader
将该事实冻结为 `Target.ProbeError=target_url_invalid`，executor 只写入脱敏的
`unknown/target_url_invalid` item；该标记不携带原始地址，且禁止 outbound 请求。
...
管理员在直连 `5432` 上创建一次性数据库 `juhe_ai_sub2api_dev_j3a_<name>`、jobs role、business reader role 和外部 `juhe_jobs` schema。`juhe_jobs` 的 owner 必须是 jobs role；业务 schema/表由现行 Node 业务 schema 维护流程提供，不能用 Go jobs schema 初始化掩盖缺失。J3a 表预置统一使用 maintenance 的 `--check-j3a-proxy-latency-postgres` / 显式 `--apply-j3a-proxy-latency-postgres`：检查先返回缺项，apply 只允许由该 schema owner 角色在其已预置 schema 内加法创建 6 表/2 索引，随后检查必须返回 `0`。`Store.EnsureSchema` 不再作为 PostgreSQL scratch 或稳定 runtime 的 bootstrap 入口；稳定 Go owner 始终只调用只读 `Store.CheckSchema`。
...
3. result role：`proxy_profiles`、receipt/cursor 的必要读写；因现役 `proxy_profiles` update trigger，额外仅授予 `accounts`/availability projection 的读取以及 availability dirty 的 `SELECT`/`INSERT`/`UPDATE`，不得扩大为通用业务写入。
4. fixture：至少包含一个启用的 proxy profile、一个启用 provider、一个启用 protocol profile，且字段满足 Node schema 与 J3a reader 的 username-only/password envelope 规则；若存在空 `base_url` 的启用 provider，须验证有效 provider 仍返回，空地址只形成 unknown item。
5. 连接：为两个角色分别生成仅经 PgBouncer `6432` 的 URL；不要把管理员直连 URL 传给测试。
...
2. jobs schema owner 检查；由管理员显式执行的一次性 maintenance `--apply-j3a-proxy-latency-postgres` 仅用于 scratch bootstrap，随后 maintenance `--check-j3a-proxy-latency-postgres` 与 `Store.CheckSchema` 都必须以只读事务通过。稳定 Go owner 启动只允许后者，绝不能把缺表修复为运行期 DDL。
...
维护项目另有 `TestPostgresBootstrapSmoke`，只在显式设置 `JUHE_AI_MAINTENANCE_J3A_BOOTSTRAP_SMOKE_URL` 时运行。它要求直连 `5432`、数据库名以 `juhe_ai_sub2api_dev_j3a_` 开头、`juhe_jobs` schema 已由管理员预置且初始没有 J3a 表/索引，依次验证 maintenance 只读检查、显式 apply、apply 后只读检查，以及对错误列类型和缺失唯一约束的 fail-closed 识别；测试不创建/删除数据库或 schema，结束后仍须由管理员按 scratch 生命周期销毁。

`TestPostgresSchemaContractSmoke` 同样是显式 opt-in，但验证 Go jobs runtime `Store.CheckSchema`：完整结构可通过，故意把 `fence_token` 改为 `TEXT` 后 runtime 必须 fail-closed。它与 maintenance 共享 J3a schema contract，不允许两个 Go 项目对同一 owner schema 各自漂移。
...
2. 管理员以 maintenance 显式 apply 完成 scratch bootstrap 后，Go jobs Store `CheckSchema`、projector `CheckContract` 与业务 schema 权限契约。稳定 runtime 只使用 `CheckSchema`。
...
- Node 管理请求 → Go 执行/投影 → Node 管理 API readback 的真实闭环（Go 从不调用 Node）；
