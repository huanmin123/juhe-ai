...
13. [W5 管理端系统运行设置迁移记录](W5-管理端系统运行设置迁移记录.md)：`GET/PATCH /__aisys__/api/settings` 的白名单、校验、PostgreSQL 事务、统计时区限制、缓存失效、操作日志、验证矩阵和删除门禁。
...
| `W5-管理端全局品牌设置读取记录.md` | W5 `GET/PATCH /settings/global` 管理端品牌设置读写切片；固定管理 API 默认关闭、管理员权限、读写 session、两层 read/write 限流、精确品牌 DTO、PostgreSQL 事务、`settings:global` shared cache version 和 `settings.update_global` operation log；明确排除完整系统运行设置、生产切流和 Node 删除 |
| `W5-管理端系统运行设置迁移记录.md` | W5 `GET/PATCH /settings` 系统运行设置纵切面；固定当前字段白名单、整数范围、PostgreSQL 统计时区在线修改限制、完整快照读取、稳定事务锁、`settings:system` / `settings_updated` 双失效、`settings.update` operation log、真实依赖验证和删除门禁 |
