...
27. [W6 管理端客户端 IP 策略迁移记录](W6-管理端客户端IP策略迁移记录.md)：`allowlist` / `unallowlist` 两条管理写接口的权限、严格请求体、事务锁、缓存失效、操作日志、前端契约和删除门禁。
...
| `W6-管理端客户端IP策略迁移记录.md` | W6 `POST /ip-stats/{ipHash}/allowlist` 与 `POST /ip-stats/{ipHash}/unallowlist` Go opt-in 记录；固定 admin-only、`256 KiB` strict JSON、进程内 mutation guard、PostgreSQL 行锁事务、shared cache version、operation log、前端 request-capture 和真实依赖门禁 |
