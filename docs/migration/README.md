...
2. **迁移分支只在独立 worktree 使用。** 当前长期分支为 `codex/go-migration-continuation`；实际 worktree 绝对路径记录在对应计划中。Agent、测试脚本和子 Agent 必须显式指定该 worktree 作为工作目录，不能依赖终端上一次所在目录。新增并行实现应继续使用独立分支 / worktree，且不得并行编辑同一共享文件。不得对仓库中其他任务的 worktree 执行 `prune` / `remove`；确需清理本任务 worktree 时，必须先核对绝对路径、分支、状态和归属并取得明确授权。
...
推荐顺序固定为：只读检查共享 `master` -> 在独立 worktree 审计范围 -> 将最新 `master` merge 到迁移分支 -> 实现与验证 -> 再次检查 `master` 漂移 -> 必要时再次 merge 与复验 -> 在共享主目录干净且基线稳定时合回 `master` -> 从 `master` 复验并记录证据。
