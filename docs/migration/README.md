...
- [Go 三项目架构基线](Go三项目架构基线.md)：固定 `gateway`、`jobs`、`maintenance` 三个独立 Go 模块的源码、部署、依赖、并发和定时迁移边界。F1/F2 已在 `jobs`，F3/F4 已在 `gateway`；旧 `juhe-ai-go-sidecar` 已删除，新定时功能不得写入 Node worker 或 gateway。
