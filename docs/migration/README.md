...
12. [W5 管理端全局品牌设置读取记录](W5-管理端全局品牌设置读取记录.md)：`GET /__aisys__/api/settings/global` 的 Go opt-in 契约、`publicsettings` / store 复用、管理员权限、只读 session、两层 read 限流、精确品牌 DTO、验证记录和删除门禁。
13. [W6 记录与统计读接口迁移记录](W6-记录与统计读接口迁移记录.md)：记录、日志和统计只读接口迁移记录；当前仅覆盖管理侧 / 个人侧 `usage-window` 固定 31 天窗口 Go opt-in。
14. [W6 System API 限流对齐记录](W6-System-API限流对齐记录.md)：system API IP read / write 与已认证用户 read / write 限流的设置、挂载顺序、Redis / 内存实现、验证和剩余 Node 差异。
15. [测试与验收策略](测试与验收策略.md)：契约测试、回归矩阵、性能验证和网关专项验收。
16. [开发构建部署调整](开发构建部署调整.md)：本地开发、构建、发布包、Docker 和常驻运行的迁移安排。
17. [迁移文档示例](迁移文档示例.md)：后续单模块迁移记录的写法示例。
...
| `W5-管理端全局品牌设置读取记录.md` | W5 `GET /settings/global` 管理端品牌设置读取切片；复用 `publicsettings.Service` / PostgreSQL `global_settings` reader，固定管理 API 默认关闭、管理员权限、只读 session、IP read `600/120`、authenticated user read `300`、`no-store`、`{ data: { appName, appIcon } }` 精确 DTO，以及缺字段 / 非法存储 / reader 错误通用 500；明确排除 `PATCH /settings/global`、`GET/PATCH /settings`、生产切流和 Node 删除 |
