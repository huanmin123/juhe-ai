...
11. [W4 团队与统一授权迁移记录](W4-团队与统一授权迁移记录.md)：系统团队、成员、授权 grant、授权来源展开和最终用户授权的分块迁移记录；当前只覆盖 `POST /system-teams` 团队创建 Go opt-in 灰度能力，不代表 W4、Node `/system-teams` 或 Node `/authorizations` 已接管。
...
| `W4-团队与统一授权迁移记录.md` | 团队与统一授权迁移记录；当前只固定 `POST /system-teams` 团队创建 Go opt-in 灰度能力，后续继续拆团队列表 / 更新 / 成员增删、授权 grant、授权来源展开 / 归还 / 回收 / 到期和删除门禁 |
