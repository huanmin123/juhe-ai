...
| `jobs调度分支冻结清单.md` | T6b 后台调度分支冻结：`background-jobs.ts` 六处模式分支（PG 高性能派生窗口、冷历史范围窗口跳过、余额 owner、账户列表投影开关、key-model 内存恢复 driver 分支）的每 job 每模式参数差异表、Go `jobregistry/schedules()` 现状对照和冻结处置 |
