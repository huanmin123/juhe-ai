...
2. [Go 后端架构基线](Go后端架构基线.md)：目标目录、技术依赖、并发模型和线程安全规则。
3. [存储目标与 SQLite 移除](存储目标与SQLite移除.md)：PostgreSQL + Redis 单模式目标、SQLite 删除范围和数据边界。
4. [模块迁移顺序与减法清单](模块迁移顺序与减法清单.md)：模块迁移波次、删除条件和验收门禁。
5. [测试与验收策略](测试与验收策略.md)：契约测试、回归矩阵、性能验证和网关专项验收。
6. [开发构建部署调整](开发构建部署调整.md)：本地开发、构建、发布包、Docker 和常驻运行的迁移安排。
7. [迁移文档示例](迁移文档示例.md)：后续单模块迁移记录的写法示例。
...
- 影响 Go 目录结构、技术依赖、并发模型、进程模型或存储 owner 时，更新 [Go 后端架构基线](Go后端架构基线.md)。
