...
10. [W3 登录与系统账户迁移记录](W3-登录与系统账户迁移记录.md)：登录、当前用户、会话、改密、验证码和系统账户写接口的分块迁移记录；当前只覆盖 `GET /auth/me` 读切片。
...
| `W3-登录与系统账户迁移记录.md` | 登录、当前用户、会话、改密、验证码和系统账户写接口迁移记录；当前固定 `GET /auth/me` 只读切片和后续拆分门禁 |
