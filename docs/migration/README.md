...
> **现行决策（2026-08-09）。** [完整功能接管与 Node 归档迁移规则](完整功能接管与Node归档迁移规则.md) 取代本目录及历史 M/W 记录中“按 route/job 小切片、长期影子、Go 仅 PostgreSQL + Redis、删除 SQLite、先迁公开 / 管理 HTTP 接口”的优先级。SQLite 与 PostgreSQL/Redis 都是 Go 的正式目标模式；每次只接管一个完整功能，完成后 Node 的整个功能文件集退出活跃路径并归档到 `migration-backup/`。F1“运行日志索引与保留”和 F2“表存储监控采样与保留”已有 Go 完整 owner；F3“原始审计日志持久化与保留”已进入实施，但尚未接管。其他历史记录不得据此推断已接管。

> **当前状态（2026-08-10）。** F1/F2 已完成开发环境闭环；F3 的 Go store、输入 RPC、Node 直投、只读适配器和旧 Node owner 退出已进入当前 `master`，但真实 PostgreSQL smoke 与 Node -> Go -> Node 隔离端到端验收仍是关闭前门禁。完整的已迁移/未迁移清单和下一批顺序见 [迁移状态与后续批次-20260810](迁移状态与后续批次-20260810.md)。
...
3. F3：按 [F3 原始审计日志持久化与保留迁移计划](F3-原始审计日志持久化与保留迁移计划.md) 实施完整审计功能接管；未完成双模式、输入/读取、重启、Node 下线与归档前，Node 仍是唯一 audit owner。
...
12. [F3 原始审计日志持久化与保留迁移计划](F3-原始审计日志持久化与保留迁移计划.md)：当前实施中的完整审计功能边界、Go 直接异步输入、双模式审计事实库、Node 下线/归档、可靠性语义和验收清单；未完成前 Node 仍是唯一 audit owner。
