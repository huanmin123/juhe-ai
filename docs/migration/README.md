...
> **当前状态（2026-08-13）。** F1/F2/F3 已进入正式 Mac 拓扑，F3 生产读回已验证；首次上线发生过硬停机和前端 API-base 构建事故。后续 routine release 固定为独立 candidate、少量真实 HTTP 检查、日志和 route 原子切换；深度 handover 演练只用于首次新拓扑、owner/存储改动、事故或回切。发布快速流程稳定后再进入 F4 操作日志。旧 [2026-08-10 状态](迁移状态与后续批次-20260810.md) 仅作历史快照。
...
4. F4 及后续：先完成 2026-08-12 状态页中的 candidate/handover 发布门禁，再迁移 F4 操作日志、F5 公开接口日志和 F6 使用记录与统计；每个批次都必须重新经过 L1-L4。
...
4. [Go 后端架构基线](Go后端架构基线.md)：目标目录、进程模型、直接异步并发和线程安全规则。
...
> 以下 W1b-W10 链接是旧迁移方案的历史记录索引。其中“当前”“已实现”“Go opt-in”“队列”等措辞只描述各记录写作时的历史状态；它们不构成新的 Go 启动、owner 切换、Node 删除或引入任务队列的授权。后续实施只以上述十三项和“完整功能接管与 Node 归档迁移规则”为准。
