...
- [J3a 代理延迟检测完整迁移契约](J3a-代理延迟检测完整迁移契约.md)：冻结 J3a 的输入、结果、lease、Go 独占投影与 L2 实施边界；Node J3a scheduler、旧 executor、outcome reader/projector 和 business writer 已删除。
- [J3a 代理延迟检测 Node-Go 深度对照报告](../reports/J3a代理延迟检测Node-Go深度对照报告-2026-08-22.md)：记录 Node 历史 oracle、Go 独占执行/投影、手动 adapter 和删除边界；真实 Node→Go 手动进程联调、active-path-zero、生产切换仍未完成。
...
- [J3a 代理延迟检测 L3 PostgreSQL/PgBouncer 验收方案](J3a-代理延迟检测L3-PG验收方案.md)：2026-08-23 dev scratch 经 PgBouncer `6432` required smoke 与 Go business-result projector applied/stale、receipt、cursor、CAS/replay 已通过并清理；真实手动进程联调、生产、active-path-zero、owner handoff 与 L4 仍未完成。
...
> **当前工作基线（2026-08-15）。** `F1/F2` 的 Go 实现在 `jobs`，`F3/F4` 的 Go 实现在 `gateway`。本次已在独立 SQLite 及可销毁 PostgreSQL/PgBouncer 子库完成 F1-F4 store/RPC 验证；这只证明开发基线。Node 仍是对外网关与主要业务 owner，当前不启动新的网关迁移。F4 仍处于切换前状态；任一环境的实际 owner、端口、发布包和回滚点必须由该环境的部署证据单独确认，不能从本仓库代码反推。后台任务后续顺序以 [后台任务迁移总设计与路线图](后台任务迁移总设计与路线图.md) 为准。
...
3. F1/F2：Go 实现与隔离双模式验证完成，下一步仅可按 [Go 三项目部署与验证计划](Go三项目部署与验证计划.md) 做 jobs-only candidate；未取得该环境的 owner handover 证据前，不得停止 Node 相关任务。
4. F3/F4：F3/F4 属于 `gateway` 数据功能，不随当前后台任务迁移启动。F4 仍须执行 [F4 操作日志完整迁移契约](F4-操作日志完整迁移契约.md) 的历史迁移、candidate、切换和回滚门禁后才可进入 L3/L4 生产状态。
5. 后续后台任务：按 [后台任务迁移总设计与路线图](后台任务迁移总设计与路线图.md) 的完整任务批次推进；每个批次重新经过 L1-L4，不以复用已有 jobs 二进制替代 owner 验收。
...
14. [F4 操作日志完整迁移契约](F4-操作日志完整迁移契约.md)：F4 的 32 个 producer、稳定 ID、Go `gateway` RPC、双模式 store/read/retention、pre-cutover archive、历史迁移与生产切换门禁；切换前实现完成不等于生产 owner 已接管。
15. [F4 操作日志切换前验收记录-20260814](F4-操作日志切换前验收记录-20260814.md)：当前提交的开发环境、Go/Node、真实 PostgreSQL/Redis、producer、部署入口和独立复审证据；生产历史迁移与切流仍是未关闭门禁。
> 以下 W1b-W10 链接是旧迁移方案的历史记录索引。其中“当前”“已实现”“Go opt-in”“队列”等措辞只描述各记录写作时的历史状态；它们不构成新的 Go 启动、owner 切换、Node 删除或引入任务队列的授权。后续实施只以上述十五项和“完整功能接管与 Node 归档迁移规则”为准。
