...
9. [W2 管理端只读辅助接口迁移记录](W2-管理端只读辅助接口迁移记录.md)：后台只读 options / catalog 接口的当前契约、已迁移路径、分组授权组只读 union、账户 owner-only 缺口和接管门禁。
...
| `W2-管理端只读辅助接口迁移记录.md` | 管理端只读辅助接口契约、Go 当前实现范围、权限边界、分组授权组只读 union、账户授权缺口和删除门禁 |
