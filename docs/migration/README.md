...
> **现行决策（2026-08-09）。** [完整功能接管与 Node 归档迁移规则](完整功能接管与Node归档迁移规则.md) 取代本目录及历史 M/W 记录中“按 route/job 小切片、长期影子、Go 仅 PostgreSQL + Redis、删除 SQLite、先迁公开 / 管理 HTTP 接口”的优先级。SQLite 与 PostgreSQL/Redis 都是 Go 的正式目标模式；每次只接管一个完整功能，完成后 Node 的整个功能文件集退出活跃路径并归档到 `migration-backup/`。F1“运行日志索引与保留”、F2“表存储监控采样与保留”和 F3“原始审计日志持久化与保留”均已完成 Go 开发 owner 接线与开发环境验证；F3 仍待独立总审和发布部署验收，不能据此推断生产已接管。其他历史记录不得据此推断已接管。

> **当前状态（2026-08-10）。** F1/F2 已完成开发环境闭环；F3 的 Go store、输入 RPC、Node 直投、只读适配器和旧 Node owner 退出已进入当前 `master`，并已通过真实 PostgreSQL smoke 与 Node -> Go -> Node 隔离端到端验证。独立总审及发布/部署验证仍是关闭前门禁。完整的已迁移/未迁移清单和下一批顺序见 [迁移状态与后续批次-20260810](迁移状态与后续批次-20260810.md)。
...
3. F3：按 [F3 原始审计日志持久化与保留迁移计划](F3-原始审计日志持久化与保留迁移计划.md) 完成双模式、输入/读取、重启、Node 下线与归档的开发接管；当前等待独立总审和发布/部署验收，不把开发 owner 结论外推为生产切流。
4. F4 及后续：只在 F3 达到完整接管条件后选择下一个独立功能；完整验证后切换 Go owner，停止 Node 功能，归档完整 Node 文件集并清零 Node 活跃入口；归档 manifest 固定回滚和对照证据。
...
11. [F2 表存储监控采样与保留功能冻结](F2-表存储监控采样与保留功能冻结.md)：F2 Go 唯一 owner、SQLite 专用输出库、PostgreSQL `juhe_stats`、直接异步采样、保留清理、Node 只读边界，以及已在用户授权一次性空库上通过、但不等同生产部署的真实 PostgreSQL smoke。
12. [F3 原始审计日志持久化与保留迁移计划](F3-原始审计日志持久化与保留迁移计划.md)：当前开发验收中的完整审计功能边界、Go 直接异步输入、双模式审计事实库、Node 下线/归档、可靠性语义和验收清单；Node 旧 audit owner 已退出活跃路径，仍待独立总审和发布/部署验收。
> 以下 W1b-W10 链接是旧迁移方案的历史记录索引。其中“当前”“已实现”“Go opt-in”“队列”等措辞只描述各记录写作时的历史状态；它们不构成新的 Go 启动、owner 切换、Node 删除或引入任务队列的授权。后续实施只以本节前十二项和“完整功能接管与 Node 归档迁移规则”为准。
