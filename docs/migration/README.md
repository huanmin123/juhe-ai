...
6. [模块迁移顺序与减法清单](模块迁移顺序与减法清单.md)：模块迁移波次、删除条件和验收门禁。
7. [W1b 外部维护公开接口迁移记录](W1b-外部维护公开接口迁移记录.md)：`/__aipublic__` 外部维护接口的当前契约、Node 对照命令、Go 目标边界和删除门禁。
8. [W2 管理端只读辅助接口迁移记录](W2-管理端只读辅助接口迁移记录.md)：后台只读 options / catalog 接口的当前契约、第一块 proxy options Go 基线和接管门禁。
9. [测试与验收策略](测试与验收策略.md)：契约测试、回归矩阵、性能验证和网关专项验收。
10. [开发构建部署调整](开发构建部署调整.md)：本地开发、构建、发布包、Docker 和常驻运行的迁移安排。
11. [迁移文档示例](迁移文档示例.md)：后续单模块迁移记录的写法示例。
...
| `W2-管理端只读辅助接口迁移记录.md` | 管理端只读辅助接口契约、Go 第一块实现范围、权限边界和删除门禁 |
...
- 影响系统指标、Prometheus、pprof、worker lag、队列状态、PG/Redis/Asynq 观测或前端系统监控契约时，先更新 [Go 迁移指标与观测规划](Go迁移指标与观测规划.md)，不能把 Node `eventLoopLagMs` / `db-service` 字段模拟成 Go 指标。
