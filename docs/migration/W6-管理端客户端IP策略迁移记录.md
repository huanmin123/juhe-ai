...
> 本文记录客户端 IP 统计列表与策略写接口的 Node 当前契约、Go opt-in 实现和删除门禁。`GET /__aisys__/api/ip-stats` 与 `allowlist`、`unallowlist`、`blacklist`、`unblock` 四条 POST 已进入 Go opt-in；`GET /__aisys__/api/ip-stats/{ipHash}/detail`、统计生产 writer / worker 和窗口刷新仍由 Node 负责。Go 代码和测试入口完成不表示真实 PostgreSQL / Redis 已执行通过、生产流量已切到 Go，或 Node IP 管理模块可以删除。
...
- 当前 Node 详情 owner：`backend/src/modules/ip-stats/ip-stats.routes.ts`、`backend/src/storage/client-ip-stats-detail.repository.ts`
...
- 目标 Go 列表 owner：`backend-go/internal/modules/managementclientipstats/`、`backend-go/internal/httpapi/management_client_ip_stats.go`、`backend-go/internal/store/postgres/managementclientipstats.go`
...
- Go 列表只读取 Node 已生成的 PostgreSQL 预聚合结果，不写 `client_ip_registry`、daily、range window、dirty marker 或 `stats_job_state`，也不触发窗口刷新。
...
- Go migration `backend-go/db/migrations/000040_w6_management_client_ip_stats_list.sql` 只补列表需要的 `client_ip_usage_range_windows` range output、两张 dirty 表及列表 / policy / keyword 查询索引。它不创建或迁移 `client_ip_stats_daily`、`client_ip_account_stats_daily`、`client_ip_account_usage_range_windows`，不接管账号详情 reader，也不代表 Go 已有 client IP stats writer / refresh worker。
- Go 请求 SQL 只读 `client_ip_usage_range_windows`、`client_ip_registry`、`client_ip_policies`、两张 dirty 表和 `stats_job_state`；源码门禁禁止读取 `usage_records`、daily 表或在请求路径执行 `SUM`、`COUNT`、`GROUP BY`。
...
`frontend/scripts/smoke/plan0081-real-go-management-smoke.ts` 的默认只读流程已加入 `GET /ip-stats?page=1&pageSize=20&sortField=requestCount&sortOrder=desc`，校验 ready / not-ready DTO、`pageUpperBound`、`hasMore`、日期范围和各用量字段；`frontend/scripts/regression/plan0081-real-go-management-smoke-regression.ts` 使用本地 mock 固定成功、not-ready、HTTP 错误、超时和敏感值不进入错误文本。该真实 smoke 仍要求显式 Go Base URL 与管理员 Cookie，本机未执行真实 listener。
...
try {
  go test -v -tags=integration ./internal/testkit/integration -run '^(TestW6ManagementClientIPPolicyPostgresRedisAsynqSmoke|TestW6ManagementClientIPStatsListPostgresRedisSmoke|TestW6ManagementClientIPStatsNodeWriterGoReaderSmoke)$' -count=1
} finally {
...
`TestW6ManagementClientIPPolicyPostgresRedisAsynqSmoke` 通过真实 PostgreSQL store、Redis version reader / 两层 limiter、Asynq ingest worker 和四条 production handler 组装进程内 router，覆盖策略事务、版本消费、限流 bypass 和 operation log。`TestW6ManagementClientIPStatsListPostgresRedisSmoke` 已加入真实 Goose migration、PostgreSQL + Redis 管理鉴权 / 限流、列表筛选 / 排序 / readiness 和 production SQL `EXPLAIN` 断言，并检查默认与显式 `requestCount desc` 在 custom / generic plan 下使用同一静态 SQL。`TestW6ManagementClientIPStatsNodeWriterGoReaderSmoke` 与 `backend/src/scripts/regression/client-ip-stats-node-writer-go-reader-fixture.ts` 已加入 Node production writer / refresh 到 Go reader 的跨运行时入口；该测试仍在 follow-up 审查中，本机未执行，不记录为已审查通过。follow-up 的验收目标是按“真实 Node schema / writer / refresh -> Goose migrations -> Go reader”顺序执行，覆盖非 UTC 午夜边界、非对称计数，并只向 Node 子进程传递显式环境白名单。三者都使用进程内 `httptest` router，不启动 `app.RunServer` listener；真实 listener、反向代理和前端页面另列门禁。
...
| `e5e238cf6` | Node production writer / refresh 到 Go reader 跨运行时 smoke 初版；后续审查修订中，未执行 |

## 删除门禁

以下条件全部满足前，不得删除 Node 对应列表或写接口：

1. 健康 Docker/testcontainers 或独立环境中真实 PostgreSQL + Redis + Asynq smoke 通过；客户端 IP 列表 production SQL 的真实 `EXPLAIN` 断言必须实际执行，不接受编译通过替代。
...
4. 跨运行时测试按“真实 Node schema / production writer / range refresh -> Goose migrations -> Go reader”顺序通过，覆盖非 UTC 午夜边界、非对称统计和 ready / stale，且 Node 子进程只接收显式环境白名单；Go 不成为 daily、range、dirty 或 stats state writer。
5. 前端 IP 管理页面连接真实 Go 后端完成列表 ready / not-ready、筛选 / 排序 / 分页，以及封禁、解封、加入白名单、移出白名单、两类 `disabledCount=0` 和错误提示 smoke；mutation 脚本必须具备超时迟到提交后的安全清理。
6. 反向代理只把 `GET /ip-stats` 与四条 POST 路径切到 Go 单 owner，并完成回滚演练；未迁移的 `GET /ip-stats/{ipHash}/detail` 和 Node writer / worker 继续由 Node 负责。
7. 生产观测确认列表错误率 / 延迟、PostgreSQL 查询计划 / 行锁等待、Redis 失效失败和 operation log 入队告警正常。
8. 删除 Node 列表 route 和四条写 route 分支后提供静态搜索和回归证据；共享 repository、daily / range writer、dirty / stats state 或详情路径仍被 Node 使用时不得整体删除。

## 当前结论

客户端 IP 列表与四条写路径已形成 Go store、service、HTTP、router/app、前端 request-capture 和对应 integration smoke 代码；列表只读 Node 生产 writer / worker 生成的预聚合结果，allowlist / unallowlist 另有隔离 fixture 安全真实 Go smoke 入口。Go 全量测试 / vet、目标 race、integration 编译 / vet / race 编译、前端 request-capture、mock real-Go-management smoke 和 typecheck 已通过。真实依赖普通模式因 Docker 不可用 `SKIP`，强制模式按预期非零失败，因此真实 PostgreSQL / Redis、EXPLAIN、Node writer -> Go reader 和真实 listener 均未执行通过；真实 allowlist smoke 也因缺少隔离 fixture 参数未执行。生产单 owner 切流、回滚和 Node 删除仍未完成；IP 详情与统计生产 writer / worker 继续由 Node 提供。
