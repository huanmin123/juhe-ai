...
> **2026-08-08 现行边界。** 当前没有受版本控制的 Go 源码或 `go.mod`；本文件中已有依赖版本和“已落地”段落是历史规划 / 记录，不能证明实现存在。Go 功能默认直接异步执行，不以 Redis、Asynq 或通用 Queue Port 为前置；B0 只需分别验证 SQLite 与 PostgreSQL/Redis Store adapter、goroutine 生命周期和资源维度并发。详见 [完整功能接管与 Node 归档迁移规则](完整功能接管与Node归档迁移规则.md)。
...
> 下表保留 2026-08-08 前 PG/Redis-only 方案的依赖与验证记录。当前工作区没有受版本控制的 Go 源码或 `go.mod`；B0 必须重新完成 SQLite 与 PostgreSQL/Redis 的 adapter 基线验证，不能将下表视为当前双模式基线已完成。
