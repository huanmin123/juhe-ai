...
>
> 本文是 Go 后续迁移的边界契约。F1/F2 已物理迁入 `jobs`，F3/F4 已物理迁入 `gateway`；`backend-go` 根目录只保留工作区定义。新迁移的定时功能进入 `jobs`，不再新增到 Node worker 或旧 sidecar。

## 1. 最终项目划分

| 项目 | 运行形态 | 负责 | 不负责 |
| --- | --- | --- | --- |
| `gateway` | 长驻 HTTP 服务 | 管理 API、公开 API、AI 接口桥接上游、请求鉴权、请求级业务处理 | 定时扫描、统计批处理、离线迁移和历史回填 |
| `jobs` | 可独立部署的长驻定时服务 | 账号复制/探活、模型质量检查、OAuth 保活、统计聚合、定时保留清理及其他周期任务 | 对外业务 HTTP、AI 请求代理、一次性人工维护命令 |
...
- 不启动任何 `time.Ticker` 形式的业务定时器，不导入 `jobs` 的任务实现，不在请求结束后偷偷启动无 owner 的 goroutine。
...
2. 按完整功能迁移 Node 定时域：先复制/探活和模型质量等实时外部 I/O，再统计窗口和聚合，最后低优先级维护/保留清理。
...
5. `gateway` 的业务 API 和 AI 桥接迁移按原计划继续；不得为了让 jobs 复用代码而把 gateway 业务 service 放进 shared。
