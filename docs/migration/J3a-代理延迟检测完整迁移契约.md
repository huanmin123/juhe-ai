...
> 状态：J3a 已改为一次性 Go owner：Go 周期 executor、Go manual bridge、Go result projector、receipt/cursor 与 `proxy_profiles` CAS 写回均由 `jobs` 独占；Node J3a scheduler、旧 executor、outcome reader/projector 和 J3a business writer 已删除。Node 仅保留管理 HTTP、权限、操作日志和 Go manual request adapter，不保留 fallback、双 owner、双写或 J3a 回滚路径。当前工作区的定向代码/测试门禁、五个 `go.work` module 的全量 test/race/vet、受控 dev PG CRUD smoke，以及显式 `J3A_PG_PROJECTOR_URL` 的 Go business-result projector applied/stale/receipt/cursor/CAS 集成 smoke 已通过；未提供受控结果库时不得把历史 scratch 记录当作当前通过。生产部署、真实进程 handoff、active-path-zero、重启演练和生产/L4 仍需单独执行，未在本契约中伪称完成。
...
本批不保留 Node/Go 双 owner、双写或运行时 fallback。Go 配置缺失或 Go 服务不可用时请求必须失败，不得回退到 Node。一次性 dev scratch 已验证 Go PgBouncer required smoke 与 Go 业务结果投影；现役 Node 代理管理 PG CRUD/CAS smoke 已验证微秒版本不丢失。真实 Node→Go 手动进程联调、active-path-zero、重启演练和生产/L4 仍需单独执行。

## 8. L2 验收与非目标

L2 必须通过 Go unit/race/vet、Node typecheck 和新增回归：协议/DNS、取消/超时/response cap/full drain、CONNECT header、非 2xx、lease、execution claim 并发、持久 snapshot TOCTOU、重复 request、旧 revision/observed CAS、SQLite 单 writer、Go 结果投影 payload/metadata 双校验以及 Go-only 路径删除检查。当前本地命令已覆盖 proxylatency package/race/vet/jobs 全量、Node typecheck、Go result projector、manual adapter、现役 proxy CRUD/PG smoke 和 Go-only 删除回归；本轮 dev PgBouncer `6432` required smoke、Node PG CRUD/CAS smoke 与 Go business-result projector 已实际通过并清理。真实 Node→Go 手动进程联调、Docker/Redis、Jenkins、GitOps、active-path-zero、owner handoff 和生产/L4 属于后续门禁。
