...
- Go 已迁移 system API limiter 所需的 allowlist 消费能力，以及 `allowlist` / `unallowlist` 两条管理写路由；`/ip-stats` 列表、详情、`blacklist` 和 `unblock` 仍未迁移。写接口边界见 [W6 管理端客户端 IP 策略迁移记录](W6-管理端客户端IP策略迁移记录.md)。
...
- IP policy 剩余列表、详情、blacklist 和 unblock 完成迁移；在此之前必须按路径保持单 owner，不能让 Node 与 Go 同时处理同一条 policy 写路由。
