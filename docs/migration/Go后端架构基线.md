...
| 进程 / 命令 | 职责 | 健康检查 | 退出与重启 |
| --- | --- | --- | --- |
| `juhe-ai server` | HTTP API、静态资源、公开接口、网关入口、轻量 supervisor 和 readiness；W1b `/__aipublic__` 仅在 `JUHE_AI_PUBLIC_API_ENABLED=true` 时 opt-in 挂载，W2 已迁移的 `proxies/options`、`system-accounts`、`system-accounts/options`、`authorization-options/grantee-accounts`、`my-authorization-options/grantee-accounts`、`authorization-options/grantee-teams`、`my-authorization-options/grantee-teams`、`authorization-options/grantee-groups`、`my-authorization-options/grantee-groups`、`providers/options`、`providers/models/options`、`providers/{code}/models`、`route-strategies/options`、`my-route-strategies/options`、`groups/options`、`my-groups/options`、`groups/account-options`、`my-groups/account-options`、`accounts/options`、`my-accounts/options`、`accounts/tags`、`my-accounts/tags`、`DELETE accounts/tags/{tagId}`、`DELETE my-accounts/tags/{tagId}`、`PATCH accounts/{id}/tags` 和 `PATCH my-accounts/{id}/tags` 等管理端辅助路由，以及 W3 已迁移的 `auth/captcha`、`auth/login`、`auth/me`、`auth/change-password`、`auth/logout` 和 `PATCH system-accounts/{id}` reset-password / status-only / profile-only 子动作仅在 `JUHE_AI_MANAGEMENT_API_ENABLED=true` 时 opt-in 挂载，其中系统账户列表读已覆盖 admin-only 管理字段白名单，系统账户 options 已覆盖 admin-only 轻量字段白名单，authorization grantee accounts / grantee teams / grantee groups 已覆盖 admin / my 双路径轻量字段白名单，分组 options / account-options 已覆盖 owner + 授权组只读 union，账户 options 已覆盖 owner + 授权账户只读 union，账户 tags 已覆盖 owner-only 只读、未绑定标签删除、独立 PATCH 标签集合替换和 PATCH 成功后的 operation log 写入基线，reset-password 子动作已覆盖 `super_admin` 权限、Node 兼容密码 hash、目标账户全部 session 删除和敏感日志脱敏，status-only 子动作已覆盖 `super_admin` 权限、`active` / `disabled` 严格枚举、禁止停用最后一个启用 `super_admin`、禁用时删除目标账户全部 session 和状态操作日志，profile-only 子动作已覆盖 `super_admin` 权限、非敏感资料字段更新、最后一个启用 `super_admin` 降级保护和资料字段操作日志；gateway cache invalidation / API Key validation cache 清理未迁移前不能把 system account PATCH 作为生产 owner；相关开关默认关闭 | HTTP health、PG/Redis 基础连通、网关运行态只读检查；W1b 开启时检查 Redis state、Redis queue 和 `JUHE_AI_SECRET`；W2/W3 开启时检查管理端会话 store、session schema、Redis state 和 Redis queue | 优雅关闭 HTTP、取消请求 context，不吞 worker 失败；W1b / W2 / W3 回滚优先关闭开关并恢复路径 owner |
...
- W2 管理端辅助接口已经具备 `requireAuth` 级 Go 会话鉴权基线，并已覆盖 `proxies/options`、`system-accounts`、`system-accounts/options`、`authorization-options/grantee-accounts`、`my-authorization-options/grantee-accounts`、`authorization-options/grantee-teams`、`my-authorization-options/grantee-teams`、`authorization-options/grantee-groups`、`my-authorization-options/grantee-groups`、`providers/options`、`providers/models/options`、`providers/{code}/models`、`route-strategies/options`、`my-route-strategies/options`、`groups/options`、`my-groups/options`、`groups/account-options`、`my-groups/account-options`、`accounts/options`、`my-accounts/options`、`accounts/tags`、`my-accounts/tags`、`DELETE accounts/tags/{tagId}`、`DELETE my-accounts/tags/{tagId}`、`PATCH accounts/{id}/tags`、`PATCH my-accounts/{id}/tags`、`operation-logs`、`operation-logs/{id}`、`my-operation-logs` 和 `my-operation-logs/{id}` 路径：读取 `juhe_ai_session`、按 SHA-256 `token_hash` 查询 `system_sessions`、校验 `system_accounts.status` 和 `expires_at`，并按 Node 语义拦截普通用户初始密码修改；其中系统账户列表读、系统账户 options、管理侧 authorization grantee accounts / grantee teams / grantee groups、管理侧策略路由、分组下拉、分组账号下拉、账户下拉、账号标签下拉和管理侧标签删除必须 admin-only，系统账户 options 和 authorization grantee accounts 只返回 `id/username/displayName/status`，authorization grantee groups 只返回目标 active 用户自己的启用分组轻量摘要，用户侧 `my-*` 路径必须强制当前登录用户作用域且不返回管理侧系统账户字段，模型目录必须保留 OpenAI-compatible / hybrid 聚合和 `hybrid` 自身排除规则。分组下拉 / 分组账号下拉已覆盖 owner + 授权组只读 union 和授权组 `accountIds=[]`，账户下拉已覆盖 owner + 授权账户只读 union、名称包含搜索、授权字段、authorized permissions 和授权实例可调度校验，标签删除已覆盖未绑定标签 204、绑定中 400 和不存在 / 越权 404，独立标签 PATCH 已覆盖 trim / 去重 / 全量替换、授权实例本地标签和 self scope；但仍未覆盖系统账户创建 / 完整更新 / 登录 / 完整会话撤销、授权来源 / grant / 授权写接口、主账户写入 `tags`、OAuth / 导入标签写路径、完整账号 summary 响应、operation log 保留清理和其他写接口 operation log。管理端重置密码、状态更新和资料字段更新属于 W3 reset-password / status-only / profile-only 子动作，不应反向扩大 W2 只读接管范围；其中 status-only / profile-only 仍缺 gateway cache invalidation / API Key validation cache 清理，不能作为完整系统账户 PATCH 接管证据。W3 完整登录 / session 生产接管、`requireAdmin` 和 `my-*` 作用域尚未整体迁移前，生产 server 必须默认关闭 W2 opt-in。任何 `__aisys__/api` 后台路由都不能为了联调绕过会话鉴权、初始密码修改拦截、admin / 普通用户边界、`my-*` 作用域或模型目录可见性边界。
- W3 当前已补 `GET /__aisys__/api/auth/captcha` 验证码发放 / 校验基础、`POST /__aisys__/api/auth/login` 登录 / session 创建小切片、`GET /__aisys__/api/auth/me` 当前用户读、`PATCH /__aisys__/api/auth/me` 当前用户资料更新、`POST /__aisys__/api/auth/change-password` 当前用户改密、`POST /__aisys__/api/auth/logout` 当前会话撤销，以及 `PATCH /__aisys__/api/system-accounts/{id}` reset-password / status-only / profile-only 子动作：captcha 路径不要求登录，使用 Redis state 保存 5 分钟 challenge，返回 PNG data URL，按客户端 IP 60 次 / 分钟限流，校验基础通过 Redis `GETDEL` 一次性消费，并且该 GET 路径必须跳过 system API IP 读限流；login 路径不要求已有登录态，先消费验证码，再通过 Redis state 检查和记录 IP / 用户名登录失败锁，随后校验 active 系统账户和 Node 兼容 PBKDF2-SHA512 密码，成功后在 PG 中创建 token hash session、写入初始 `last_seen_at`、更新 `last_login_at` 并签发 `juhe_ai_session` Cookie，且 session 创建 SQL 必须重新确认 `password_hash` 仍等于刚验证过的值；current-user 读取路径同样读取 `juhe_ai_session` 和 `system_sessions`，但必须允许普通初始密码用户读取当前用户摘要并返回 `mustChangePassword=true`，不能套用普通后台接口的初始密码 403 middleware，且不 touch `last_seen_at`；profile update 路径走管理端写鉴权 middleware，只允许改当前用户 `displayName`，普通初始密码用户继续 403，鉴权通过后按 60 秒最小间隔 touch 当前 session `last_seen_at`，真实变更 best-effort 写 `auth.update_profile` 操作日志，并依赖 PG `idx_system_accounts_display_name_unique_lower` 保证大小写不敏感唯一；change-password 路径不能套普通后台 middleware，必须允许普通初始密码用户只提交新密码，普通改密和管理员改密必须校验旧密码，鉴权阶段同样按 60 秒窗口 touch 当前 session，成功后写 Node 兼容 PBKDF2-SHA512 hash、清除 `mustChangePassword` 并撤销同账号其他 session；logout 路径不要求有效登录态，有 Cookie 时按 SHA-256 `token_hash` 删除当前 session，成功后清除 `juhe_ai_session` Cookie，且不 touch session；reset-password 子动作要求 `super_admin`，只允许 `password/mustChangePassword` 字段，PG 单语句锁定目标账户、更新 Node 兼容密码 hash、按角色归一化 `mustChangePassword` 并删除目标全部 session，operation log 只记录敏感字段“已重置”，不写明文密码；status-only 子动作要求 `super_admin`，只允许 `{ status: "active" | "disabled" }`，拒绝和 reset-password / profile-only 或完整 PATCH 字段混用，PG 单语句锁定启用超级管理员集合和目标账户，禁止停用最后一个启用 `super_admin`，禁用时删除目标账户全部 session，operation log 只记录状态变化和撤销 session 数；profile-only 子动作要求 `super_admin`，只允许 `displayName/description/role/mustChangePassword`，拒绝 `password/status/imageGenerationEnabled/username` 和未知字段混用，PG 单语句锁定启用超级管理员集合和目标账户，禁止降级最后一个启用 `super_admin`，operation log 只记录非敏感资料字段变化。其中 `POST /auth/login` 小切片会创建当前登录 session 并签发 Cookie；但这些仍只是 `JUHE_AI_MANAGEMENT_API_ENABLED=true` 下的 Go opt-in 灰度能力，不代表完整 Cookie 安全部署、系统账户创建 / 完整更新、gateway cache invalidation / API Key validation cache 清理、完整会话撤销、前端 auth / 系统账户页 smoke、生产单 owner 切流或 Node `/auth` / `/system-accounts` 删除已完成。
- W2 account tags PATCH 的 operation log 使用 Asynq `operation-logs` 队列；`JUHE_AI_MANAGEMENT_API_ENABLED=true` 时 Redis queue 是 Go server 启动依赖，配置缺失或 ping 失败必须 fail-fast。单次 PATCH 已提交后日志入队失败才按 best-effort 处理，只记录运行日志，不回滚业务状态。
