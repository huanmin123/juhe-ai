...
- W3 当前已补 `GET /__aisys__/api/auth/me` 当前用户读、`PATCH /__aisys__/api/auth/me` 当前用户资料更新和 `POST /__aisys__/api/auth/logout` 当前会话撤销切片：current-user 读取路径同样读取 `juhe_ai_session` 和 `system_sessions`，但必须允许普通初始密码用户读取当前用户摘要并返回 `mustChangePassword=true`，不能套用普通后台接口的初始密码 403 middleware；profile update 路径走普通后台 middleware，只允许改当前用户 `displayName`，普通初始密码用户继续 403，真实变更 best-effort 写 `auth.update_profile` 操作日志，并依赖 PG `idx_system_accounts_display_name_unique_lower` 保证大小写不敏感唯一；logout 路径不要求有效登录态，有 Cookie 时按 SHA-256 `token_hash` 删除当前 session，成功后清除 `juhe_ai_session` Cookie。这些切片不 touch session、不创建 session、不修改密码或其他系统账户字段，也不代表验证码、登录、改密、完整 Cookie 安全配置或系统账户写接口已经迁移。
