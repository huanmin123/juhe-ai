...
| 进程 / 命令 | 职责 | 健康检查 | 退出与重启 |
| --- | --- | --- | --- |
| `juhe-ai server` | HTTP API、静态资源、公开接口、网关入口、轻量 supervisor 和 readiness；W1b `/__aipublic__` 仅在 `JUHE_AI_PUBLIC_API_ENABLED=true` 时 opt-in 挂载，W2 管理端辅助、标签和 operation log 读路径以及 W3 `auth/captcha`、`auth/login`、`auth/me`、`auth/change-password`、`auth/logout`、`GET /auth/sessions`、`DELETE /auth/sessions/{id}`、`POST system-accounts`、`PATCH system-accounts/{id}` 完整 mixed PATCH 仅在 `JUHE_AI_MANAGEMENT_API_ENABLED=true` 时 opt-in 挂载；`JUHE_AI_MANAGEMENT_AUTH_SESSIONS_ENABLED=true` 且总管理开关关闭时只挂载 `GET /auth/sessions` 和 `DELETE /auth/sessions/{id}`；系统账户列表 / options 保持非敏感字段白名单，当前用户 session list / revoke 保持当前账户作用域和 token hash 不回显，完整 PATCH 已覆盖 `super_admin` 权限、Node 兼容密码 hash、session 撤销、最后一个启用 `super_admin` 保护、gateway runtime cache / API Key validation cache 清理和密码日志脱敏；相关开关默认关闭，不代表生产接管 | HTTP health、PG/Redis 基础连通、网关运行态只读检查；W1b 开启时检查 Redis cache/state/queue 和 `JUHE_AI_SECRET`；W2/W3 全量管理开关开启时检查管理端会话 store、session schema、Redis cache/state/queue 和稳定 `JUHE_AI_SECRET`，任一缺失或不可用均在监听前 fail-fast；W3 session-only 窄开关开启时只额外要求 Redis state，不要求 Redis cache/queue | 优雅关闭 HTTP、取消请求 context，不吞 worker 失败；W1b / W2 / W3 回滚优先关闭开关并恢复路径 owner |
