...
| 进程 / 命令 | 职责 | 健康检查 | 退出与重启 |
| --- | --- | --- | --- |
| `juhe-ai server` | HTTP API、静态资源、公开接口、网关入口、轻量 supervisor 和 readiness；W1b `/__aipublic__` 仅在 `JUHE_AI_PUBLIC_API_ENABLED=true` 时 opt-in 挂载，W2 已迁移的 `proxies/options`、`system-accounts`、`system-accounts/options`、`authorization-options/grantee-accounts`、`my-authorization-options/grantee-accounts`、`authorization-options/grantee-teams`、`my-authorization-options/grantee-teams`、`authorization-options/grantee-groups`、`my-authorization-options/grantee-groups`、`providers/options`、`providers/models/options`、`providers/{code}/models`、`route-strategies/options`、`my-route-strategies/options`、`groups/options`、`my-groups/options`、`groups/account-options`、`my-groups/account-options`、`accounts/options`、`my-accounts/options`、`accounts/tags`、`my-accounts/tags`、`DELETE accounts/tags/{tagId}`、`DELETE my-accounts/tags/{tagId}`、`PATCH accounts/{id}/tags` 和 `PATCH my-accounts/{id}/tags` 等管理端辅助路由，以及 W3 已迁移的 `auth/captcha`、`auth/login`、`auth/me`、`auth/change-password`、`auth/logout` 和 `PATCH system-accounts/{id}` reset-password 子动作仅在 `JUHE_AI_MANAGEMENT_API_ENABLED=true` 时 opt-in 挂载，其中系统账户列表读已覆盖 admin-only 管理字段白名单，系统账户 options 已覆盖 admin-only 轻量字段白名单，authorization grantee accounts / grantee teams / grantee groups 已覆盖 admin / my 双路径轻量字段白名单，分组 options / account-options 已覆盖 owner + 授权组只读 union，账户 options 已覆盖 owner + 授权账户只读 union，账户 tags 已覆盖 owner-only 只读、未绑定标签删除、独立 PATCH 标签集合替换和 PATCH 成功后的 operation log 写入基线，reset-password 子动作已覆盖 `super_admin` 权限、Node 兼容密码 hash、目标账户全部 session 删除和敏感日志脱敏；二者默认关闭 | HTTP health、PG/Redis 基础连通、网关运行态只读检查；W1b 开启时检查 Redis state、Redis queue 和 `JUHE_AI_SECRET`；W2/W3 开启时检查管理端会话 store、session schema、Redis state 和 Redis queue | 优雅关闭 HTTP、取消请求 context，不吞 worker 失败；W1b / W2 / W3 回滚优先关闭开关并恢复路径 owner |
