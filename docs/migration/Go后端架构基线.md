...
- W3 当前仅补 `GET /__aisys__/api/auth/me` 当前用户读切片和 `POST /__aisys__/api/auth/logout` 当前会话撤销切片：current-user 路径同样读取 `juhe_ai_session` 和 `system_sessions`，但必须允许普通初始密码用户读取当前用户摘要并返回 `mustChangePassword=true`，不能套用普通后台接口的初始密码 403 middleware；logout 路径不要求有效登录态，有 Cookie 时按 SHA-256 `token_hash` 删除当前 session，成功后清除 `juhe_ai_session` Cookie。二者不 touch session、不创建 session、不修改用户资料或密码，也不代表验证码、登录、改密、完整 Cookie 安全配置或系统账户写接口已经迁移。
