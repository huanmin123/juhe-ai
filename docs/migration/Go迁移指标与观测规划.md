...
W2 管理只读接口迁移后，每个 options / catalog 都必须有低基数 `routeGroup` 或 `operation`，用于压测和回归确认没有全表扫描。当前已迁移的 options 建议固定操作名为 `proxy_options_list`、`provider_options_list`、`route_strategy_options_list` 和 `my_route_strategy_options_list`；不要把 `systemAccountId`、路由策略 ID、keyword 原文或用户名称放入 Prometheus label。
...
- 新增 Go 指标前先确认本文；如果指标进入用户或管理员可见页面，还要更新 [统计指标与分层聚合设计](../functions/统计指标与分层聚合设计.md) 和 [接口契约与权限矩阵](../functions/接口契约与权限矩阵.md)。
