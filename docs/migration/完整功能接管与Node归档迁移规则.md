...
- 该功能专属的前端调用、脚本、测试、监控与部署入口；
- 两种存储模式下的实现与验证：SQLite 的单 writer 与 owner 边界，PostgreSQL/Redis 的事务和连接池边界。仅尚未迁移的 Node 功能可以继续使用其既有 owner bridge；F3 后被接管功能的 SQLite owner 必须是 Go，不能继续依赖 Node bridge。
...
1. 冻结 Node 的完整功能边界与外部契约，列出所有入口、文件、调用方、数据写入、定时触发和副作用。
2. 在 Go 中完成整个功能及其双模式实现；不注册部分 Go route/job 作为长期并行 owner。
3. 在隔离环境完成该完整功能的契约、两种存储模式、并发、失败、重启和调用方验证。
4. 切换为 Go 的唯一正式 owner，停止 Node 的整个功能入口、scheduler、consumer 和写入。
5. 将 Node 完整功能文件移动到 `migration-backup/node/<feature-id>/`，移除活跃 Node import、路由、启动、部署和测试入口。归档代码不得被运行时、构建、打包或测试自动加载。

完成第 5 步才算“已接管”。不保留 Node fallback、bridge、双写、双 consumer 或仅用于灰度的长期开关。若 Go 功能失败，回滚通过恢复归档文件对应的明确提交和完整 Node 功能 owner 完成，不得混跑两个 owner。
...
归档根目录已提供 [`migration-backup/node/manifest.template.json`](../../migration-backup/node/manifest.template.json)。每次 F4 必须先复制模板为该功能目录的 `manifest.json`，再填入实际文件与验证证据；模板本身不是任何功能已归档的证据。
...
当前共享启动文件（例如 worker 角色装配）不能直接抽走几行。F1 必须先以 Node 行为不变的回归，把运行日志索引与保留的启动注册、服务、测试和配置引用整理为专属文件；只有专属文件集形成后，F2 才实现同一完整功能，F3 关闭 Node 的整个索引功能，F4 再整体归档。若发现 cursor、索引表或 SQLite 文件仍与未迁功能不可隔离，必须扩展至完整共享 owner 或延后该候选，不能保留 Node bridge 作为迁移后依赖。
