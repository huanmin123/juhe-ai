...
本次调研结论同时参考了 Anthropic 官方文档、官方 SDK / GitHub 生态、DeepSeek / GLM / Kimi 等 Anthropic-compatible 入口说明，以及本地参考项目 `sub2api`、`new-api` 的已落地拆分方式。参考项目只用于抽象分层经验，不复制其 Claude Code OAuth 伪装、深度协议互转或全供应商大网关范围。
...
- 第一阶段只做 API Key 直连和 Messages / Models / Count Tokens 的轻量闭环，不做 Claude Code OAuth、Bedrock、Vertex、OpenAI Chat/Responses 到 Anthropic Messages 的自动互转。
...
- `api_key` 加密保存；列表不展示，编辑弹窗可查看和修改。
- `base_url` 默认 `https://api.anthropic.com`，允许用户修改为明确的 Anthropic-compatible 代理地址，但如果不是官方域名，页面必须提示“该地址不是官方 Anthropic 直连”。后续如果要严控官方直连，可增加只允许官方域名的档案。
...
- 账户不显示 OpenAI OAuth、Refresh Token、Access Token、OpenAI Organization、OpenAI Project、Codex Responses、GPT 客户端兼容模式等字段。
- 第一阶段不支持 Anthropic Workload Identity Federation Bearer Token；该能力涉及短期 token、身份联邦和组织级配置，后续单独作为企业认证档案设计。
- 第一阶段不支持 Claude Pro / Claude Code OAuth 账号。参考项目中 Claude Code OAuth 伪装、`anthropic-beta` 组合和 CLI header 模拟不进入本项目官方直连 API Key 方案。

可选高级字段：

- `anthropic_beta`：默认空。只在用户明确配置或客户端明确传入时透传；系统不默认追加 Claude Code 专属 beta。
...
| 官方供应商适配层 `adapters/anthropic` | 官方 Anthropic API Key 上游 URL 拼接、`x-api-key` 替换、`anthropic-version` 默认值、`anthropic-beta` 合并、官方直连诊断字段 | 不实现 Claude Code OAuth / Claude Pro 伪装，不生成 OpenAI 组织 / 项目 / Beta 头，不把第三方兼容入口归为官方 Anthropic |
...
| 客户端画像层 | 第一阶段默认只有普通 Anthropic native / Anthropic SDK 类客户端语义；若后续支持 Claude Code 客户端，也必须作为显式 client profile 设计 | 不把客户端 User-Agent、模型名或某个错误码临时推断成 Claude Code，不把 Codex 可重试语义扩散给 Anthropic native |
...
- 上游认证写入 `x-api-key: <Anthropic API Key>`。
- 上游写入 `anthropic-version`：优先客户端请求头，其次账户默认值 `2023-06-01`。
- `anthropic-beta`：保留客户端显式值，并可追加账户配置的非空值；多个值按逗号去重合并。系统不默认追加 `claude-code`、`oauth` 或 CLI 模拟相关 beta。
...
`connectionType` 是导入协议层字段，用于和后续 WIF、云平台托管 Claude、第三方兼容入口区分。后端落库时仍以 `providerCode + connectionType` 解析 `provider_protocol_profile_id`，再保存 `accounts.type = api_key`。
...
- `sub2api` 对 Anthropic direct、Vertex Anthropic、Bedrock Claude、OpenAI Responses 互转分别建请求构造和转换路径。这说明“官方直连”“云平台托管”“协议兼容代理”应分档案处理。
- `sub2api` 和 `new-api` 都有 OpenAI <-> Claude 深度互转代码，但这类能力涉及工具调用、图片、PDF、thinking、cache、usage 和流式状态机，不能作为本项目第一阶段默认范围。

不采用的参考实现范围：

- 不做 Claude Code OAuth / Claude Pro 账号伪装。
- 不默认追加 Claude Code CLI 专属 `anthropic-beta` 和 `User-Agent`。
...
- 新增默认 Anthropic 分组。
- 前端账户创建在选择 `Anthropic Claude` 后展示 `Anthropic API Key`。
- 后端账户创建、编辑、导入和公开推送接口按 `providerCode=anthropic` 解析官方直连档案。
- 网关本地认证支持 Anthropic native 客户端常用的 `x-api-key` 本地 API Key 入口。
- 新增 Anthropic native 请求适配器，负责路径拼接、请求头过滤、上游认证替换和 raw body 透传。
