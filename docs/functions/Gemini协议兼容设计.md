...
- OpenAI Chat / Responses 和 Anthropic Messages 可以通过 API Key 显式混合路由桥接到 Gemini native 生成类上游；目标规则右侧只暴露 `generate_content`，运行时按下游请求是否流式动态选择 `:generateContent` 或 `:streamGenerateContent?alt=sse`。
- Gemini native `generateContent` / `streamGenerateContent` 可以作为下游来源，通过 API Key 显式混合路由桥接到 OpenAI Chat Completions 上游；下游响应仍渲染为 Gemini JSON / SSE。
- Gemini native `generateContent` / `streamGenerateContent` 也可以通过 API Key 显式混合路由桥接到 Anthropic Messages 上游；下游响应仍渲染为 Gemini JSON / SSE，不合成 Anthropic 响应给 Gemini 客户端。
- 客户端使用什么协议，网关先按什么协议解析：Gemini native 请求默认走 Gemini native 档案；如果命中 API Key 显式混合路由中的 `generate_content|stream_generate_content -> chat_completions` 或 `generate_content|stream_generate_content -> messages` 规则，则由目标 Chat 或 Anthropic Messages 上游承接；OpenAI / Anthropic 请求默认走各自协议候选，只有命中 API Key 显式混合路由中的 `chat_completions|responses|messages -> generate_content` 规则时才进入 Gemini native 目标账号。
...
3. Gemini native `generate_content` / `stream_generate_content` 先匹配当前 API Key 的显式混合路由规则；命中 `upstreamEndpointFamily = chat_completions` 时进入对应 OpenAI Chat 上游账号，命中 `upstreamEndpointFamily = messages` 时进入对应 Anthropic Messages 上游账号，不命中时再走 Gemini native 档案候选。
4. OpenAI Chat / Responses 路径先按 OpenAI 协议解析 source family；命中 API Key 显式混合路由中的 `chat_completions|responses -> generate_content` 规则时，才进入 Gemini native 目标账号。
5. Anthropic Messages 路径先按 Anthropic 协议解析 source family；命中 API Key 显式混合路由中的 `messages -> generate_content` 规则时，才进入 Gemini native 目标账号。
...
- 看到 `/v1beta/models/*` 就在没有 API Key 显式混合路由规则时把 Gemini native 请求转换成 OpenAI Chat。
...
- Codex / OpenAI Responses 客户端通过 API Key 显式混合路由配置 `sourceEndpointFamily = responses`、`upstreamEndpointFamily = chat_completions`，上游模型选择 Gemini 模型。
- OpenAI Chat / Responses 客户端通过 API Key 显式混合路由配置 `sourceEndpointFamily = chat_completions` 或 `responses`、`upstreamEndpointFamily = generate_content`，上游模型选择 Gemini native 模型。
- Anthropic Messages 客户端通过 API Key 显式混合路由配置 `sourceEndpointFamily = messages`、`upstreamEndpointFamily = generate_content`，上游模型选择 Gemini native 模型。
- Gemini native 客户端通过 API Key 显式混合路由配置 `sourceEndpointFamily = generate_content` 或 `stream_generate_content`、`upstreamEndpointFamily = chat_completions`，上游模型选择 GLM、DeepSeek、OpenAI-compatible、GPT API Key 或 Gemini OpenAI Chat 等真实 Chat 模型。
- Gemini native 客户端通过 API Key 显式混合路由配置 `sourceEndpointFamily = generate_content` 或 `stream_generate_content`、`upstreamEndpointFamily = messages`，上游模型选择 Claude / GLM Coding Anthropic / DeepSeek Claude Code 等真实 Anthropic Messages 模型。
...
| OpenAI Responses / Codex | `profile_gemini_openai_chat_v1beta` | Chat Completions | 支持 | 必须在 API Key 显式混合路由配置 `responses -> chat_completions` |
| OpenAI Chat Completions | `profile_gemini_native_v1beta` | Gemini native GenerateContent | 支持范围内 | 必须在 API Key 显式混合路由配置 `chat_completions -> generate_content`，响应还原为 Chat Completion JSON / SSE |
| OpenAI Responses / Codex | `profile_gemini_native_v1beta` | Gemini native GenerateContent | 支持范围内 | 必须在 API Key 显式混合路由配置 `responses -> generate_content`，Responses 状态和 hosted tools 不可保真时走 guidance-first |
| Anthropic Messages | `profile_gemini_native_v1beta` | Gemini native GenerateContent | 支持范围内 | 必须在 API Key 显式混合路由配置 `messages -> generate_content`，响应还原为 Anthropic Messages JSON / SSE |
| Gemini native | `profile_gemini_native_v1beta` | Gemini native | 支持 | 原生直连，不进入模型映射协议转换 |
| Gemini native GenerateContent / StreamGenerateContent | OpenAI 协议 Chat 上游档案 | Chat Completions | 支持范围内 | 必须在 API Key 显式混合路由配置 `generate_content|stream_generate_content -> chat_completions`，响应还原为 Gemini JSON / SSE |
| Gemini native GenerateContent / StreamGenerateContent | Anthropic 协议 Messages 上游档案 | Anthropic Messages | 支持范围内 | 必须在 API Key 显式混合路由配置 `generate_content|stream_generate_content -> messages`，响应还原为 Gemini JSON / SSE |
...
| Codex 显式混合路由 | `/v1/responses` + API Key 规则 `responses -> chat_completions` | 命中 Gemini OpenAI Chat，不命中 Gemini native |
...
- 不实现无映射的自动跨协议转换；OpenAI / Anthropic 到 Gemini native、Gemini native 到 OpenAI / Anthropic 都只能通过 API Key 显式混合路由触发。
...
- OpenAI / Anthropic 到 Gemini native 是否只通过 API Key 显式混合路由中的 `chat_completions|responses|messages -> generate_content` 规则触发。
- Gemini native 到 Chat / Anthropic Messages 是否只通过 API Key 显式混合路由触发，并保持下游 Gemini JSON / SSE 形态。
