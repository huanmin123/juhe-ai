...
> 当前状态：多 Agent 复核定稿（2026-07-22），实现已进入收口复查；执行进度由 [PLAN-0158-20260722T160050118Z](../plans/计划-0158-20260722T160050118Z-AI账户热质量与精准切号实施.md) 跟踪。本文定义目标契约，最终完成仍以计划中的专项、构建、控制面冷启动和 Redis 验证为准。
>
...
当前代码已经包含真实网关 attempt 的热质量生命周期、同层探索 credit / cursor、公共首字截止与墙钟裁剪、账户电路 memory / Redis adapter、控制面 ledger / bridge、恢复边界和有界路由观测摘要。相关专项回归与后端 TypeScript 类型检查已由实施任务记录为通过；这些结果只证明隔离的自动化场景，不替代真实双实例 Redis 验证。

仍在收口的项目包括：dispatch revision 的完整写入路径、控制面冷启动就绪门禁与 maintenance 调度、不可变 route-plan 在主路由路径的最终接入，以及前端最终 typecheck / build 重跑。未完成项不得被本设计视为已验收。
