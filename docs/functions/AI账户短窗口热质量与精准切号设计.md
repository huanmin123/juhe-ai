...
当前实现把完整非 `2xx` 作为当前候选的 opaque 失败，按 `response.ok=false` 排除当前账户并继续统一调度；禁止从状态码、错误类型、正文或普通事件 payload 推导未配置的共享业务状态。维护时必须同时守住以下边界：

- 未配置用户高级规则时，完整 `2xx` 的 JSON/SSE payload 保持透明；所有端点的完整非 `2xx` 排除当前候选并继续调度，不写 Key、账户、IP 避让、热质量失败或上游桶语义状态；候选耗尽后只返回网关统一错误。
...
- preparation / dispatch / routes 必须共享显式策略的状态动作与统一切号决策，任何完整非 `2xx` 都不得因为未命中用户策略而停在当前账户。
- 回归必须同时覆盖“状态码互换不改变共享副作用”“未知 `non-2xx` 排除当前账户并切后备”“用户 `rate_limited/temp_unschedulable/error_disabled` 写状态后仍切后备且不轮换兄弟 Key”“普通 `data.error` 不构成结构失败”“伪造二进制 Content-Type 不跳过 JSON 协议校验”与“语义提交后不拼接第二候选”。
