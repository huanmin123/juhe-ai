...
- 账户新增或编辑弹窗不会因打开、填写 API Key、填写 Base URL 或修改凭据自动请求上游模型目录。只有用户点击“同步上游模型”才发起目录发现；失败不清空草稿、不影响保存、人工测试或后续系统检查。
...
- 用户显式上游模型同步对单把 Key 只有一次 `10s` 截止；不得复用文本生成诊断的 `10s/20s/30s` 阶梯。浏览器断开同步请求时，Node 必须把取消信号传入草稿凭据处理和上游目录请求，停止尚未开始的后续 Key 探测，不再继续占用上游连接。
...
合法跨协议映射按来源协议选择检查形态、按映射目标协议校验上游模型，不能用目标协议直接裁掉来源检查形态。用户显式目录同步不属于检查形态，不参与自动测试或健康结果；其 `requestUrl` 只说明该次同步的受控目录路径。
