...
> 多模型账户的完整目标以 [AI 账户多模型能力健康与精确隔离设计](AI账户多模型能力健康与精确隔离设计.md) 为准：`healthCheckModel` 只作为哨兵和激活模型；请求失败确认与模型能力恢复必须使用失败请求映射后的精确上游模型和 endpoint mode，单模型结论不得直接扩散为整号状态。目标计划完整落地前，本文其余章节仍记录当前单哨兵检查契约。
