...
每个已保存且参与后台检查的 AI 账户最终保存一个检查模型。创建请求可以不提交 `supportedModels` 和 `healthCheckModel`，Node 会持久化供应商默认支持模型，并按“当前系统账户个人默认 > 管理员维护的供应商系统默认 > 协议档案内置默认”解析检查模型；草稿上游目录发现只调整表单推荐，不是保存前置条件：
...
- 当 API Key、Base URL 完整且当前 `supportedModels` 为空时，页面对稳定草稿在 700ms 防抖后静默尝试一次目录发现；编辑账户还要求 API Key 或 Base URL 相对已保存配置发生变化。静默失败不提示、不清空草稿，也不写入账户；用户仍可通过同步按钮显式重试。
- 用户显式执行目录发现时，自动推荐模型必须同时存在于成功上游目录、当前账户支持模型和当前协议可测试目录；它只更新当前未保存表单推荐。多 API Key 账户分别固定每把 Key 探测，所有探测成功后取模型交集；任一 Key 失败时不返回部分目录，避免把某把可调度 Key 不支持的模型写入账户能力边界。
- 任何上游模型目录获取（表单同步、模型目录预检和图像模型目录探针）对单把 Key 都只有一次 `10s` 截止；不得复用文本生成诊断的 `10s/20s/30s` 阶梯，也不得在目录获取失败后继续生成探针。浏览器断开同步请求时，Node 必须把取消信号传入草稿凭据处理和上游目录请求，停止尚未开始的后续 Key 探测，不再继续占用上游连接。
- 后台多 API Key 健康检查按 Key 串行，首个成功立即结束。单个账户检查共享固定 `30s` 总 deadline：目录与生成请求都继承该信号，超时立即取消在途请求并停止尚未开始的 Key；该结果是 `probe_task_failure`，只安排后续复检，不累计健康失败或把账户标为临时不可调用。未完成全池时，进程内短期游标让下一轮从最后尝试 Key 的后一个继续；worker 重启后按账户 ID 的稳定起点开始，避免长期固定扫描池首。例行和请求失败健康检查在获取全局诊断槽前先经过单槽 lane；首次激活与配置检查可使用剩余两个槽，不被慢速大 Key 池饿死。
...
- 除下述精确匹配的列表测试恢复外，测试成功和失败都不直接修改账户、授权实例或来源账户的 `status`、`schedulable`、冷却、最近错误、健康检查事实和运行态。新增和编辑表单草稿测试协议成功时，浏览器仅将当前未保存表单的状态单选预选为 `active`；用户手动选择 `disabled` 时保持停用，只有后续保存才会写入账户。
- 不写入、确认、清理账户失败样本、代理桶、客户端 IP 熔断、客户端 IP 账号回避、Codex turn 账号回避或会话亲和状态。
- 不推进账户内 Key 轮换，也不写入、恢复或摘除 Key 运行态。
- 不把人工测试结果作为创建、编辑、导入、激活或恢复的后端前置凭证；表单中的 `active` 预选仍可由用户改回“待检查”或“停用”。

保存账户的列表测试有且仅有一个恢复例外：任务不是草稿测试，返回真实协议完成的成功结果，任务模型和 `testEndpointMode` 分别严格等于账户保存的 `healthCheckModel` 和 `healthCheckEndpointMode`，并且当前为自有物理账户、未停用、未质量隔离、未过期、当前时间计划允许且持久 `schedulable = false`。写入同时校验测试任务终态、任务选择和账户 `configRevision`；全部匹配时才原子恢复为 `active + schedulable`，并清理冷却、最近错误、健康失败窗口和流式失败运行态。它不写入新的检查成功事实，不改变检查模型、支持模型、Key 运行态或授权实例；任一条件不匹配时任务仍只保留诊断结果。
...
人工测试默认只诊断，不能直接成为已保存账户的状态恢复入口；例外仅包括前述精确匹配的列表测试恢复，以及新增或编辑草稿协议成功后在浏览器本地预选“可调度”。后者不写库、不清理运行态，必须由用户保存后才生效。
...
- 多 API Key 的默认人工账户可用性测试按 Key 顺序串行执行，首个成功即结束；成功前的失败 Key 和成功 Key 组成实际结果，剩余 Key 标记为未测试。人工测试不把“未测试”解释为通过或失败，也不写入任何 Key 运行态。模型目录同步仍必须逐 Key 获取并取交集，不能使用首成功短路。
