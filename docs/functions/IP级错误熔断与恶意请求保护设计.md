...
状态码只能进入审计、使用记录、日志和错误摘要 hash，不能作为默认熔断依据。
...
| 请求错误策略命中 | 否 | 已归属系统级上游错误处理，不能重复处罚来源 |
...
这些窗口只保存达到阈值所需的固定数量样本，样本裁剪必须原地执行，不能在请求热路径展开复制整个 samples 数组。随机无效 token 的 `invalid_token_spray` 熔断打开后，后续已确认无效的新 token 必须先命中 spray 短路，不能继续为每个随机 token 创建新的 token 指纹缓存项。
...
flowchart TD
  ...
  I --> J{"终态结果"}
  J -- "完整成功" --> K["降低或清理来源错误态"]
  J -- "上游账号池失败" --> L["返回网关统一错误或被后续账号救回，不记录来源熔断样本"]
...
| 请求错误策略 | 系统级上游错误强规则 | 优先归属账号状态处理，不计入来源熔断 |
...
如果某个来源在熔断后恢复正常，系统应尽快放行。完整成功可以降低失败计数或清理轻度熔断态，但不能清理其他错误摘要的历史样本到完全不可观测；实现时应保留短窗口自然过期。
