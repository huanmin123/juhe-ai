...
IP 级错误熔断的目标是补上这一层：当同一来源在短窗口内反复产生高置信的认证失败或本地请求校验错误时，网关可以短 TTL 返回本地 OpenAI-compatible `429`，避免继续消耗认证校验、账号池、上游请求、重试链路、审计队列和日志写入成本。
...
IP 级错误熔断是“止损”的策略：当当前来源持续制造高置信本地请求错误时，短时间拒绝继续消耗账号池。
...
| 多账号同一错误签名确认 | 否 | 仍可能是上游公共故障、容量波动或账号池共同问题，只按既有策略透传给客户端；同源同请求短时间重复可进入请求级错误签名缓存，但不返回本地 429 |
...
| 同类本地错误窗口 | `scope + endpoint + normalizedErrorSignature` | 30 秒内同类高置信本地错误 >= 5 次 | 短路该来源，初始 30 秒 |
| 总高置信错误窗口 | `scope` | 60 秒内高置信错误 >= 20 次 | 短路该来源，初始 30 秒 |
...
flowchart TD
  ...
  I --> J{"终态结果"}
  J -- "完整成功" --> K["降低或清理来源错误态"]
  J -- "多账号同签名上游失败" --> L["原样透传或命中请求级错误签名缓存，不记录来源熔断样本"]
...
- `openai-gateway-upstream-dispatch.ts` / `openai-gateway-failure-dispatch.ts`：账号级、容量级、同签名上游失败和未知账号池失败都不计入来源错误熔断；请求级错误签名缓存必须保持上游原始错误语义，不复用 IP 级错误熔断的本地 429。
...
- 回归脚本：新增 `client-ip-error-circuit-regression.ts`，覆盖认证前缺失 Bearer 熔断、重复无效 token 熔断、随机无效 token 不挡有效 token、本地高置信错误熔断、成功恢复、不同 IP / API Key / 分组 / 系统账户隔离、未知同签名上游失败不采样、账号失败不触发、容量失败不触发；在审计保全回归中覆盖审计 payload、审计 queryString 和使用记录 snapshot 的敏感信息脱敏。
...
攻击者可以随机化请求体、模型名或错误类型来规避同类本地错误窗口。总高置信错误窗口用于覆盖这种情况，但阈值应比同类本地错误窗口更高，避免误伤真实调试场景。
