...
> 本文是待实现规划草案，不代表当前代码能力。当前代码只记录部分请求的 `client_ip`，并实现短 TTL IP 级账号回避、IP 级错误熔断和单 IP 并发保护；没有持久 IP 注册表、IP 统计聚合表、IP 封禁策略表、`/__aisys__/api/ip-stats` 管理接口或前端 IP 统计页面。

## 当前状态

- 未实现：`client_ip_registry`、`client_ip_stats_*`、`client_ip_policies`、`client_ip_policy_hits`、管理员 IP 统计页面、持久封禁 / 观察名单和外部 IP 聚合接口都没有进入当前代码。
- 当前可用机制：IP 级账号回避、IP 级错误熔断和单 IP 并发保护都是进程内短 TTL 运行态，不写永久黑名单，也不作为长期用户身份事实。
- 本文件只保留规划思路；后续如需实现，必须先同步后端 schema、worker、网关缓存、管理 API、前端页面、接口契约、SQLite 存储说明和统计文档。
...
- 提供管理员可控的 IP 封禁、临时封禁、解封和观察名单能力，作为 `juhe-ai` 自身网关运维能力。
