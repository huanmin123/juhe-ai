...
- 所有 JPEG、PNG、WebP、GIF 输入统一输出 WebP，保留透明区域，固定质量 82。
- 不再按 75/68/60/55 逐级降质；固定质量 82 后仍超过 3 MiB 时前端不插入、不上传，后端明确拒绝并提示裁剪。单条消息仍最多 5 张，处理后图片合计不超过 15 MiB。
...
- 最终请求体超过 21 MiB 时先压缩历史并重建一次请求；仍超限才返回明确错误。该上限为 5 张、处理后合计 15 MiB 图片转为 base64 后预留传输余量。
