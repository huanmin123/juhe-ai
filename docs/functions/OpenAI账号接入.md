...
新增 / 编辑表单测试固定使用当前表单 `healthCheckModel`；列表测试弹窗打开时不请求候选，直接使用列表行默认值。模型下拉首次展开才读取严格 `{ id, name }` 的可测试模型目录，请求形态下拉只读取当前模型能力；用户可临时选择账户供应商、协议档案和所有者作用域可见的其他启用模型，不受 `supportedModels` 限制。文本模型按协议生成形态测试。OpenAI API Key 纯图片模型的后台激活、周期健康、冷却复测和事前确认只使用受控 `GET /v1/models` 目录探针；只有用户在人工测试中明确接受成本时，才显示 `Images API` 请求形态并通过 `/v1/images/generations` at-most-once 生成单张低质量图片，独立真实 E2E 也必须遵守相同次数门禁。测试弹窗展示异步任务状态、排队等待、诊断过程、成败、返回内容和总耗时；测试固定命中当前账号，并只等待诊断结果、使用记录和审计落地。
