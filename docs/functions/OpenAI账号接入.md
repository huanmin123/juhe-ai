...
- 账户时间计划只作为网关候选过滤条件，不改变手动启用/停用、异常、冷却和到期状态。账号池运行缓存按分钟边界刷新，避免计划切换时间点继续使用旧候选。
...
- Responses 转 Chat Completions bridge 当前不维护本地续链；请求携带非空 `previous_response_id` 时返回本地不支持错误，不进入会话亲和。
