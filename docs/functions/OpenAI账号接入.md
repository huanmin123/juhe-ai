...
- OAuth token 刷新和账户测试会优先使用账户绑定的代理；没有绑定代理时默认直连。账户测试必须复用本地 OpenAI 网关模型请求链路并写入使用记录，不能在测试服务里单独直连上游。后台账号质量主动探测能力已删除；后台冷却复测默认开启，但只复用同一网关模型请求链路去恢复冷却到期的 `rate_limited`、`temporary_unavailable` 账号；单次请求时长和失败后的同账号短重试都沿用全局临时不可调用重试配置。迁移旧账户时不再自动创建或绑定本机固定端口代理，避免换电脑或服务器部署后误连本机端口。
