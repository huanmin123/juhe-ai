...
单次流式响应已输出后，如果本次响应超过输出停顿上限仍没有新的有效输出，或连接读取异常中断，服务端不换账号也不重新请求上游续写，而是按 OpenAI / Codex 可识别的失败事件结束本次 SSE，由客户端按自身上下文决定是否重试；调研结论见 [流式中断与客户端重试调研](流式中断与客户端重试调研.md)。
