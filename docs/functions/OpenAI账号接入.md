...
账户可以配置支持模型列表，选项来自账号所有者可见的所属供应商模型目录；不选择任何模型表示不限制。网关调度时在账号池缓存快照内按客户端请求体里的下游 `model` 做内存过滤，只有不限制账号或精确支持该下游模型的账号才进入上游尝试。因模型不匹配被跳过的账号不算失败，不触发账号错误策略、本地屏蔽或临时不可调用。
