...
> 目录同步边界：上游模型目录由新增 API Key 草稿完整后的自动同步或用户点击“同步上游模型”手动刷新触发，受控内部标记只适用于这些草稿目录操作。人工 / 草稿测试、激活、健康、恢复和调度直接验证账户配置的真实生成或图片 endpoint；上游 `/models` 不开放或返回 `404` 不得阻断这些流程，客户端 `GET /models` / `GET /v1/models` 继续返回本地目录。
...
- 人工测试复用受控的真实 OpenAI 网关请求执行器并写入使用记录与审计，不能在测试服务里单独直连上游。新增和编辑表单测试固定使用当前表单 `healthCheckModel`，不提供临时模型下拉；列表单项测试打开时产生 0 个 options 请求，初始模型和请求形态直接使用列表行。首次展开 / 搜索模型才请求严格 `{ id, name }` 的可测试模型目录；首次展开请求形态只读取当前模型能力，切换模型只清除旧能力结果。候选不受 `supportedModels` 限制。完整诊断在网关为普通客户端改写失败响应时，测试结果的 `responseText`、`responseHeaders`、`message` 和 `errorCode` 仍优先使用最后一次真实上游尝试的有界原始诊断，不能把网关生成的 `upstream_retryable_error` 当成上游返回展示；未捕获上游正文时才保留下游兜底响应。人工测试不保存本次模型，不追加支持模型，也不更新 OAuth 生产额度快照。
...
新增 / 编辑表单测试固定使用当前表单 `healthCheckModel`；列表测试弹窗打开时不请求候选，直接使用列表行默认值。模型下拉首次展开才读取严格 `{ id, name }` 的可测试模型目录，请求形态下拉只读取当前模型能力；用户可临时选择账户供应商、协议档案和所有者作用域可见的其他启用模型，不受 `supportedModels` 限制。文本模型按协议生成形态测试。OpenAI API Key 纯图片模型的人工测试、后台激活、周期健康、冷却复测和事前确认都通过一次 `/v1/images/generations` 请求验证精确图片协议，不重试以避免重复生成和计费；上游 `GET /v1/models` 由新增 API Key 账户凭据完整后的自动同步或用户手动刷新使用，目录不可用不影响这些检查。测试弹窗展示异步任务状态、排队等待、诊断过程、成败、返回内容和总耗时；测试固定命中当前账号，并只等待诊断结果、使用记录和审计落地。

OpenAI 图像账户的当前检查边界：自动激活、健康和恢复与人工测试都走实际 `Images API` 请求形态和协议成功证据，不再调用上游模型目录。上游 `/models` 未开放或返回 `404` 不影响账户使用；模型目录受控转发由新增 API Key 账户凭据完整后的自动同步或用户手动刷新触发。
