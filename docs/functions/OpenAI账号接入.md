...
混合分组中如果同时存在 API Key 账号和 OAuth 账号，某条路径是否可用取决于调度命中的账号类型：OAuth 账号不支持的路径会跳过该账号，仍可由同分组内可用的 API Key 账号承接；如果分组里只有 OAuth 账号，则不支持路径会返回“没有可用的上游账户”一类网关错误，而不是自动协议翻译。
...
- API Key 只绑定调用方自己的分组；统一授权只提供账户或分组的使用权，不能绕过本地分组边界
...
6. API Key 页：创建本地网关密钥并绑定当前系统账户自己的分组；授权账户需要先加入该用户自己的分组后再参与调度。
...
- 后台 worker 另有 `openai-oauth-access-token-refresh` 专职任务，默认每 60 秒扫描所有仍存在、未删除、有 `refresh_token` 且 Access Token 距离过期小于 5 分钟的 OpenAI OAuth 账户，提前刷新并写回凭据；扫描不受 `active`、`disabled`、`error`、`rate_limited`、`temporary_unavailable` 或 `schedulable` 状态影响。后台预刷新只做 token 保活，成功时不恢复调度状态、不清理冷却和最近错误；失败时按退避等待并累计连续失败次数，连续 3 次失败后写入 `status = error`，`last_error_code = oauth_token_refresh_failed`，`last_error_message` 记录最近失败摘要。
...
- OAuth token 刷新和账户测试会优先使用账户绑定的代理；没有绑定代理时默认直连。账户测试必须复用本地 OpenAI 网关模型请求链路并写入使用记录，不能在测试服务里单独直连上游；API Key 账户测试会优先复用最近真实请求形态，模型按显式传入、最近真实请求、账户支持模型、默认模型顺序选择，避免固定测试模型和真实可用模型不一致。API Key 账户的 Responses 测试不发送 `max_output_tokens`，兼容不支持该参数的 OpenAI 兼容上游；管理后台测试弹窗只选择模型，测试输入由后端使用默认探活输入生成。后台账号质量主动探测能力已删除；后台冷却复测固定启用，但只复用同一网关模型请求链路去恢复冷却到期的 `temporary_unavailable` 账号，`rate_limited`、`error`、`disabled` 等其他状态不进入后台复测，不做自愈恢复。账号进入 `temporary_unavailable` 后先按 3 秒进入快速恢复通道，复测失败后按 `3s -> 6s -> 12s -> 24s -> ...` 翻倍；超过快速阈值后退化为慢速恢复通道，单次等待不超过 `defaultTemporaryUnschedulableMinutes` 表达的最大暂停时间。账号进入 `temporary_unavailable` 时记录本轮自动恢复观察起点，进入慢速恢复后若失败发生在 `cooldownAccountRetestMaxBackoffHours` 表达的最长自动恢复观察之后，账号转为 `status = error`、退出调度，并保留最近测试 HTTP 状态、错误码、错误摘要、观察起点和连续失败次数。恢复探活使用 `traffic_source = cooldown_retest` 写入使用记录和审计，避免污染业务统计、账户质量和真实请求形态学习；如果响应里带有 Codex 额度头，额度快照来源也记录为 `cooldown_retest`，不伪装成真实网关流量。迁移旧账户时不再自动创建或绑定本机固定端口代理，避免换电脑或服务器部署后误连本机端口。
...
- 收回授权只把授权状态改成 `revoked`，不删除历史行；被授权用户已绑定的授权账户分组关系会保留但运行时不可用，重新授权同一用户后可按同一稳定授权 ID 恢复使用。历史授权分组 API Key 关系仅作为兼容口径保留，不作为新建绑定路径。
