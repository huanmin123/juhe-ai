...
> 目录同步边界：上游模型目录只由用户点击“同步上游模型”手动刷新触发，受控内部标记只适用于该草稿目录操作。账户弹框打开、字段变更、取消以及测试或确认均不会自动同步；后两者遇到缺少支持模型或检查模型时只提示并中止。人工 / 草稿测试、激活、健康、恢复和调度直接验证账户配置的真实生成或图片 endpoint；上游 `/models` 不开放或返回 `404` 不得阻断已配置完整模型的这些流程，客户端 `GET /models` / `GET /v1/models` 继续返回本地目录。
...
- `gpt-5.6-sol` 作为 `profile_gpt_openai_v1` 的内置默认检查模型。GPT 专属 `DEFAULT_GPT_SUPPORTED_MODELS` 只包含资料完整且当前可见的内置模型；新账户按“当前系统账户个人默认 > 管理员维护的 GPT 系统默认 > 协议档案内置默认”初始化 `healthCheckModel`，个人默认保存到 `provider_default_health_check_models`。默认值只影响新账户，已有账户和后台系统检查始终读取账户自己的检查模型。
...
- 只有配置多个 API Key 时才显示账户内 Key 策略；默认 `failover` 主备，第一行 Key 为主 Key，后续 Key 按顺序作为备用；也可切换为 `round_robin` 轮询或 `weighted_round_robin` 权重轮询。账户内 Key 选择发生在系统已选中该账户之后，不改变分组内账户切号、并发和授权边界。所有请求按 [账户内 API Key 故障隔离设计](账户内APIKey故障隔离设计.md) 在未交付结果时唯一尝试当前可执行 Key，再切后续账户；跨账户物理凭据去重，整个请求最多 64 次真实上游 attempt。未知 HTTP 状态/正文只产生请求内排除，不写共享 Key 状态；图片、音频、文件/资源创建和其他请求同样轮换候选。
...
- 被授权用户不能编辑来源账户、查看敏感凭据、修改来源账户代理 / 并发 / 模型 / 凭据 / 账户追加流式规则，也不能继续转授权；被授权用户可以像使用自有账户一样，在自己的分组内绑定 / 调整授权实例、执行账户测试、停用或恢复自己的授权实例、迁移流量，调整当前分组内优先级、超级优先和降级备用。锁死 / 解除锁死及锁死死亡窗口、锁死重试间隔属于待审核设计，当前账户页和接口尚未提供；若审核落地，上述写入只应影响当前被授权用户的授权实例和分组绑定，不回写授权方原账户。被授权用户不想继续使用个人直授权账户时只能归还个人授权，不删除授权实例账户行或授权方原账户；团队来源授权账户不提供个人归还入口。
...
- API Key 只选择一条路由策略；路由策略可以绑定调用方自己的一个或多个分组，也可以绑定有效授权给调用方的分组；统一授权不提供来源资源编辑、删除或转授权权限，但被授权用户可在自己的授权实例上执行既有使用侧调度操作。
...
- 人工测试复用受控的真实 OpenAI 网关请求执行器并写入使用记录与审计，不能在测试服务里单独直连上游。新增和编辑表单测试固定使用当前表单 `healthCheckModel`，不提供临时模型下拉；列表单项测试打开时立即请求 `{ id, name, testEndpointModes }` 的可测试模型目录，加载完成前或失败后禁止提交，切换模型直接使用该项请求形态。候选不受 `supportedModels` 限制。完整诊断在网关为普通客户端改写失败响应时，测试结果的 `responseText`、`responseHeaders`、`message` 和 `errorCode` 仍优先使用最后一次真实上游尝试的有界原始诊断，不能把网关生成的 `upstream_retryable_error` 当成上游返回展示；未捕获上游正文时才保留下游兜底响应。人工测试不保存本次模型，不追加支持模型，也不更新 OAuth 生产额度快照。
...
新增 / 编辑表单测试固定使用当前表单 `healthCheckModel`；列表测试弹窗打开时立即请求候选，并在返回 `{ id, name, testEndpointModes }` 前禁用提交。用户可临时选择账户供应商、协议档案和所有者作用域可见的其他启用模型，不受 `supportedModels` 限制。文本模型按协议生成形态测试。OpenAI API Key 纯图片模型的人工测试通过一次 `/v1/images/generations` 请求验证精确图片协议；后台激活、周期健康、冷却复测和事前确认通过 `GET /v1/models` 确认目录精确包含检查模型，不产生图片费用。测试弹窗展示异步任务状态、排队等待、诊断过程、成败、返回内容和总耗时；测试固定命中当前账号，并只等待诊断结果、使用记录和审计落地。

OpenAI 图像账户的当前检查边界：自动激活、健康和恢复与人工测试都走实际 `Images API` 请求形态和协议成功证据，不再调用上游模型目录。上游 `/models` 未开放或返回 `404` 不影响已配置完整模型的账户使用；模型目录受控转发只由用户手动刷新触发。
