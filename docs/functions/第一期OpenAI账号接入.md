...
- 收回授权只把授权状态改成 `revoked`，不删除历史行；被授权用户已绑定该授权分组的 API Key 应立即不可继续调度。
- 使用统计按统一授权 ID 聚合展示请求次数、成功次数、失败次数、输入 Token、输出 Token、缓存读取 Token、总 Token、成本、最后使用时间和最近模型。
