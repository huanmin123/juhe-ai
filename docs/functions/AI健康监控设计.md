...
- 每个账户展示当前状态、最近检查、下次检查、三态数量和账户可用率；J1 当前窗口有 outcome 时，最近/下次检查展示对应的最新非 `stale` outcome，不因业务投影字段仅保留旧值而误导用户。
- 正常账户默认每 1 小时主动检查一次，并按全局被动调度策略每轮重新偏移；小时状态条是展示粒度，不要求所有账户在同一分钟执行。
