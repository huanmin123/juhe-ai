...
- Control、全部 Gateway、Usage/Log worker 和 main/temporary 两槽必须共享
  `JUHE_AI_AUDIT_BLOB_ROOT`。PostgreSQL 中的审计元数据与 Redis queue 已跨进程共享时，
  blob 文件不能继续写入 release 或实例私有目录。
- PostgreSQL 审计引用以 `audit_payload_refs` 为唯一事实源；`audit_payload_blobs.ref_count`
  只保留 schema 兼容并固定为 `0`。删除流程先在短事务内原子删除 refs/metadata，提交确认后
  才删除文件；提交结果不确定时保留文件，交给带时间宽限的 orphan 清理收敛。
...
隔离 Linux 以 64 并发运行 600 秒正式混合负载，覆盖普通 Responses/Chat、5K/500K/2M/4M
请求体和 3–30 秒慢 SSE；独立探针同时采样 Control 的两个健康入口与三个 Gateway 直连健康，以及 usage、
统计、账户、审计和运行日志管理接口。最终 `6804/6804` 请求成功，健康最大 `31.69ms`，
管理最大 `80.83ms`，SSE TTFB P99/Max 为 `262.78/394.44ms`；五类 Redis Stream 清零、
usage/audit 精确对账、deadlock 和未知锁均为 `0`，全部 14 个角色在 709 秒资源窗口持续存在。
详细证据见 [高性能同机多进程验证报告](../reports/高性能同机多进程验证报告-2026-07-26.md)。

该结果是隔离发布候选基线，不替代生产 Mac 的双槽切流、长流排空和真实故障注入。
