...
| Log Worker | 2 | 不启用 | public API stream lag；运行日志文件 backlog。F3/F4 由独立 Go sidecar 健康、写后读回和 owner lease 验收，不属于 Log Worker |
...
- 使用 Redis 共享账户并发、熔断、短 TTL 状态和缓存版本，禁止依赖进程内状态实现
  跨 Gateway 全局约束。
- 使用记录生成一次稳定 `event_id` 后异步投递；原始审计在请求终态由 Node 一次签名 RPC 提交给 Go F3，操作日志在管理业务提交后由 Node 一次签名 RPC 提交给 Go F4。三者都不等待旁路恢复后才向客户端输出；F3/F4 不进入 Redis Stream。
...
- 只消费 public API log Redis Stream；原始审计和操作日志分别由 Go F3/F4 以一次签名 RPC 直接 owner，不进入 log worker。
- 所有 public API log 副本在同一 stream 的 consumer group 内竞争消息，数据库主键保证重投幂等。
