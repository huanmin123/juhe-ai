...
- 本地失败隔离：当前账号出现 transport / timeout / read error，或精确客户端画像确认的协议失败时，调度可在语义提交前回避该账号并继续尝试后续账号；通用客户端的完整非 2xx 响应不触发回避。
- 调度事实：前序账号失败，后续账号在同一次请求链路中完整成功，说明本次请求可以通过切号救回来；或者最终失败已经返回客户端，说明客户端后续重试不应无限制继续撞同一个连续失败账号。

IP 级账号回避只基于“失败账号被后续成功账号证明可以绕开”或“最终失败已返回客户端”的调度事实，不基于状态码本身。
...
同一次请求中后续账号完整成功时，会把前序待提交 IP 失败提交到短 TTL 回避状态：
...
如果所有账号都失败并且最终失败已经返回客户端，也会确认本次待提交 IP 失败，但需要达到触发阈值后才真正参与后续排序。这样可以避免同一客户端自动重试时反复命中同一个已连续失败账号；首次确认后仍给该账号一次机会，第二次确认后第三次同 IP 请求才开始回避。
...
| 本地账号屏蔽 | 当前进程短期避让刚失败的账号 | 优先于 IP 回避，是不可调度过滤；全部候选屏蔽时先尝试 API Key 后续分组，无可承接分组则快速失败 |
| 待提交 IP 失败 | 单次请求内失败账号记录 | 后续账号成功或最终失败返回客户端时确认；达到阈值后才参与后续排序 |
...
- `gateway_client_ip_account_failure_confirmed`：后续账号成功或最终失败返回客户端后，提交前序待确认 IP 失败。
...
- `dispatch/upstream-dispatch.ts` / `response/failure-dispatch.ts`：在上游失败并决定跳账号后登记本请求待提交 IP 失败；同请求成功或最终失败返回客户端时再确认。
- `response/finalization.ts` / `routes.ts`：完整成功后清理当前账号回避，并提交前序待确认 IP 失败；最终失败返回客户端前也要确认 pending 失败，流式失败不提前清理成功态。
...
- 账号策略命中、客户端取消、本地校验失败不记录 IP 级账号回避；全失败请求只有在最终失败已经返回客户端时才确认 pending 回避，且达到阈值后才参与排序。
