...
- 不把未知失败升级成账号健康结论。账号是否冷却只交给明确请求错误策略、流熔断、后台复测等强证据链路。
...
- 上游账号错误：由系统级请求错误策略、OAuth token 刷新失败、后台复测等明确规则确认。上游账号错误按既有账号状态链路处理，不依赖 IP 级回避。
...
| 机制 | 作用范围 | 与 IP 级回避的关系 |
| --- | --- | --- |
| 请求错误策略 | 系统级上游错误强规则 | 优先于 IP 回避；命中后按策略冷却、限流或停用 |
