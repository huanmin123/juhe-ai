...
账户列表保持轻量，不返回凭据或完整能力。用户打开单账户测试时，`test-options` 从后端受控读取账户能力和模型映射，响应为模型选项数组；每项包含 `id`、`name`、模型目录的 `supportedApiProtocols`，以及账户上游能力、模型协议和有效模型映射共同计算出的 `testEndpointModes`。

前端加载模型选项后直接绑定同一项里的请求形态，切换模型不再追加单模型能力请求。纯图片模型只返回 `images_json`；没有任何可测试请求形态的模型不进入选项。新增 / 编辑表单同样使用模型选项携带的协议与当前草稿账户能力计算交集，提交测试时后端仍重新校验。
...
- `test-options` 的每个模型选项同时返回目录协议和可测试请求形态，前端切换模型不再发第二次能力请求。
