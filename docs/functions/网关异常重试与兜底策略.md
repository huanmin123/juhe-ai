...
来源级状态不在正式请求热路径执行真实探针，也不同步等待探活结果。首次达到阈值后只异步投递共享的后台健康任务；同一账户 runtime/config generation 的并发来源 join 同一 owner，不能形成探活风暴。control 拒绝、control 网络失败或 worker 未能入队时，必须按对应 fence 结算 `unknown`，不能留下无 owner 的 `dispatchPending`；来源避让保留至后续独立证据或 TTL。探活成功仅精确清理匹配来源 fence；unknown、任务失败和中性 framing 保留短避让，确认 health failure 才进入既有账户健康状态机。只有用户配置的 `retry_next` 策略可授权当前请求重放；后续由客户端主动发起的新请求再按当时配置独立选路。
...
| 任意请求完整非 `2xx` 且未命中显式策略 | 当前 attempt 终态 | 否，保留当前状态、错误码和错误摘要返回 | 不写共享 Key/IP/质量/账户状态；若来源键有效，异步记录来源级短 TTL 避让并最多复用一个账户健康确认 |
| 可重放文本主派发已开始、响应头前 transport failure | 当前 attempt 终态 | 否，返回明确 transport 错误 | 可按有效来源键记录来源级短 TTL 避让；账户健康与 transport 电路仍只由独立探活/独立 transport 证据结算，不在请求回调直接写共享账户状态 |
