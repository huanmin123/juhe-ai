...
- 通用客户端把上游 HTTP 状态、错误对象、错误类型、错误码、完成状态和错误正文视为不透明响应：网关原样转发，不据此重试、切号、轮换账户内 Key、写账号状态或执行响应检查。只有精确识别的 Codex、Claude Code、Gemini CLI 客户端才允许进入各自声明的协议语义和响应检查路径。
...
2. **上游返回响应**
    - 通用客户端不检查 HTTP 是否 `2xx`，也不解析错误对象、错误码、错误类型、错误文案或完成状态来决定调度；状态、响应头和正文按上游原样转发。
    - 通用响应不修改账号状态、不轮换账户内 Key、不建立基于上游语义的局部回避，也不因 `200 + error`、`418 + JSON` 或 SSE `event:error` 自动切号。
    - 精确识别为 Codex、Claude Code 或 Gemini CLI 时，才允许按该客户端画像声明的协议语义和响应检查策略处理；专属规则不能扩散到 `generic_*`。
...
- 中转广告、污染文本和协议内 `200 + SSE` / `200 + JSON` 失败只在精确客户端画像下进入响应语义检查；`generic_*` 客户端完全跳过该管线并原样转发。响应检查只处理完整响应语义帧边界内的策略命中、写下游前服务端重试、运行态避让和审计 metadata，不能绕过客户端画像与下游协议声明的最终失败边界。
...
响应检查策略只能在精确识别的 Codex、Claude Code、Gemini CLI 客户端画像下执行；`clientProfiles` 继续作为策略适用范围维度，但没有显式范围的管理策略也不会扩散到通用客户端。策略必须由错误码、错误类型、错误文案、完成状态、JSON 路径、SSE 原文或输出文本等响应语义触发。最终失败是否使用协议事件由精确 `clientProfile + downstreamProtocol` 控制；通用客户端只会因本地 transport / timeout / capacity 耗尽进入 HTTP 错误或断流，不会因上游响应语义被改写。
...
- 通用客户端只在没有形成完整上游响应的 transport / timeout 耗尽场景返回网关统一错误；一旦收到完整上游响应，就保留其状态、响应头和正文。
- 修改流程后必须补回归，至少覆盖：transport 失败共用请求级同账号确认预算、确认预算用尽后继续扫描所有账号、普通失败只首次投递 `recovery_wait`、普通成功不清账户级运行态、有效后台失败才 `precheck_pending`、账号仍有并发时不直接写 `temporary_unavailable`、generation / lease CAS、同账户和同分组半开单飞、FIFO/abort 清理、零可派发账号等待预算、上游桶运行态避让、SSE 心跳后的双提交边界、Codex / Claude Code / Gemini CLI 协议失败事件、通用上游状态 / JSON / SSE 透明转发、图像 lane 长超时、快速模式只覆盖文本 lane，以及可见输出后不服务端静默拼接。
