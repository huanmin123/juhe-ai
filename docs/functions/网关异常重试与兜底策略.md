...
- 首次有效独立后台探针形成匹配负向结果后，standalone memory / performance Redis 只按 `scopeId + generation + leaseId/fencingToken` 写入该 Attempt 的 `precheck_pending / OPEN`；performance 不能退回空的进程内快照。transport 与 execution capability 各自解释 outcome，完整 HTTP 不按状态码/正文推进 transport。
...
精确探针运行态不写 SQLite：standalone 使用 memory，performance 使用 Redis runtime state；权威 circuit / intent 控制面按完整设计持久化。已落库冷却态只可能来自用户显式策略、迁移前 legacy 或未来专属 account-global owner，并由各自匹配 provenance 的恢复路径处理；当前默认没有自动 account-global owner。完整状态机见 [AI 账户运行态探针恢复设计](AI账户运行态探针恢复设计.md)。
