...
- 已确认的上游瞬态 transport / HTTP 失败先交给统一失败流水线；在文本请求尚未语义提交、墙钟仍有余量且未命中排除条件时，当前物理账户最多额外重试两次，然后再继续后备账户 / 分组。该预算按账户独立计算，不按客户端、Codex、供应商或会话特殊化，也不轮换同账户兄弟 Key。已提交正文中断仍只结束当前响应，不能拼接第二个候选。中间失败不写电路 confirmation、共享质量或 Key/账户状态；独立后台确认仍按其自身边界处理。
