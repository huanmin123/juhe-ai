...
- 未命中显式策略时，完整 HTTP 业务失败只用于救当前安全文本请求和排除本请求已失败 Key/账号；不得建立 API Key / IP / 会话跨请求回避、共享质量失败或 `recovery_wait`。本地可验证的 transport failure 才能进入传输电路与独立后台确认；普通请求不能根据状态码、错误类型或正文建立、续期、升级或清理账户业务运行态。
- 兜底层不维护状态码、错误码或错误文案白名单：状态码和上游错误体只作为诊断字段、审计字段和用户显式配置规则的输入，代码不内置“余额不足”“限流”“某状态码才写状态”之类判断。
- 未知 transport 失败交给统一失败流水线：只有可安全重放的文本主请求已经开始、响应头尚未到达时发生的 transport failure 可以消费同账户安全原地重试；读取异常、正文中断、完整 HTTP、配置首字截止和副作用请求不消费该 token。同账户兄弟 Key 先按请求内池唯一尝试且不消费 token，兄弟 Key 耗尽后才允许重试当前凭据。`temporaryUnschedulableRetryAttempts` 由整次请求共享，不在每个账号、Key、后备分组或 compact 预处理阶段重新发放。中间失败不写电路 confirmation、共享质量或 Key/账户状态；后续候选和后台确认仍按各自独立边界处理。
- 请求进入上游前发现当前账号目标协议不能保真承载某个合法能力时，只记录账号作用域 guidance 并跳过当前账号；它不是上游失败，不写账号状态，不进入状态码 / 错误码策略，也不能在还有同分组账号或后续可承接分组时直接返回给客户端。
- 通用客户端默认不解析上游错误对象、错误类型、错误码、完成状态和错误正文：无副作用文本推理端点只用 `response.ok` 做有界请求级 opaque 接管，同账户当前可执行 Key 按池顺序唯一尝试，再扫描后续账户 / 分组；不使用固定 2-Key 或 4-account 截断，整个请求仍受候选去重、墙钟和最多 64 次真实上游 attempt 的安全上限。图片、音频、文件、资源创建、后台任务、hosted tool 和其他副作用 POST 一旦派发后不隐式重放。只有精确识别的 Codex、Claude Code、Gemini CLI 客户端按协议声明的结构解释本次 attempt；用户或管理员显式配置的账户错误与响应拦截策略只在声明范围内执行。
...
- 多 Key API Key 账户的 Key 级隔离不恢复状态码 / 错误码分类：安全文本推理端点可以仅按 opaque `response.ok=false` 形成当前请求内 Key 排除并按池顺序唯一耗尽，但不能因此写共享 Key 状态。跨账户物理凭据去重，整个请求最多 64 次真实 attempt；完整设计见 [账户内 API Key 故障隔离设计](账户内APIKey故障隔离设计.md)。
- DeepSeek 这类 OpenAI-compatible 第三方也遵守同一兜底层：安全文本完整非 `2xx` 只按 `response.ok=false` 做 opaque 请求级接管；不得读取 401 / 403 / 402 / 429 / 5xx、余额/限流/模型文案或 `finish_reason = insufficient_system_resource` 决定共享动作。`2xx` payload 保持透明，这些字段只可进入诊断或用户显式策略匹配。需要写 `rate_limited`、`temporary_unavailable` 或 `error` 时，只能由用户显式账户错误策略授权；统一边界见 [AI 账户错误语义与状态变更边界](AI账户错误语义与状态变更边界.md)。
...
自动账户 / Key 探针统一使用三态加任务未知：`complete_success` 必须同时满足协议成功和完整 framing，并只按职责及匹配 provenance 激活 / 恢复；`framing_complete_neutral` 表示完整 HTTP/SSE 但协议或业务未成功，只做有界诊断 / 顺延，最多关闭同来源 transport 怀疑；`upstream_failure` 仅代表连接失败、硬超时、读取中断或 framing 未完成的 `transport_incomplete`，是自动路径唯一负向证据；`probe_task_failure/stale/unknown` 不计数、不改状态。用户显式 `temporary_unavailable/rate_limited` 只由 TTL、匹配来源恢复动作或人工恢复清理，普通 / 后台无 provenance 成功不得清理。人工测试和模型检测不参与任何生产状态流转。
...
| 运行态调度降级观察窗口 | `5min` / 最小观察 `60s` | 只有本地可验证 transport failure 可首次投递核实；`runtime_degraded` 和 `latency_degraded` 都只能由后台探针或后台状态评估激活，并由后台探针或手动恢复清理 |
...
flowchart TD
  ...
  E -- "否" --> Q{"语义未提交且请求可安全重放"}
  Q -- "否" --> S["保持 at-most-once 或已提交终止边界"]
  Q -- "是" --> I{"是否为主派发响应头前 transport 且有共享 token"}
  ...
  J -- "否，通用客户端" --> G{"请求是否允许 opaque 接管"}
  G -- "完整 2xx 或无副作用 GET/HEAD" --> K["不解释状态或正文，透明完成"]
  G -- "安全文本完整非 2xx" --> X["只在本请求排除当前 Key/账号并尝试后续候选"]
  G -- "已派发副作用 POST" --> S["保持 at-most-once；返回稳定中性结果或结束已提交响应"]
  J -- "是" --> L["按该客户端声明的协议语义和响应检查处理"]
  L --> R{"语义未提交且请求是否仍可安全重放"}
  R -- "是" --> X["本请求排除当前账号并尝试后续账号"]
  R -- "否" --> S
  X --> P["记录尝试；显式策略按配置执行"]
  P --> T{"后续账号结果"}
  I1 --> C
  T -- "成功" --> U["本次请求救回；opaque 失败仍只留在请求内"]
...
2. **上游返回响应**
    - 通用客户端默认不解析错误对象、错误码、错误类型、错误文案或完成状态；安全文本推理端点只按 `response.ok` 有界接管，GET / HEAD 等无副作用资源读取保持完整响应透明。资源型和未知 POST 一旦派发就保持 at-most-once：完整非 `2xx` 或结果未知时不切 Key / 账户 / 分组；下游未提交时返回网关稳定的 `503/upstream_outcome_unknown`，已提交时只能结束或断流，供应商原始错误只进入有界诊断，不能作为客户端重试指令。
    - 未配置显式策略时，通用响应不修改账号或 Key 共享状态，也不因 `200 + error`、普通事件里的 `data.error`、`418 + JSON` 的具体错误内容或任意错误码自动分类。安全文本推理端点可以仅按完整非 `2xx` 这一 opaque 请求结果轮换本请求内 Key / 账户；这是有界可用性接管，不是上游语义判断。
    - 精确识别为 Codex、Claude Code 或 Gemini CLI 时加载该客户端画像声明的系统默认协议规则；账户所有者或管理员显式创建的策略可以在声明的端点范围内作用于 `generic_*`。显式状态动作按配置执行，但 `retry_next` 仍必须通过请求语义门禁，不能让已经派发的资源型、未知 POST 或其他副作用请求重放。
   ...
   - 只有可安全重放文本的非流式 `2xx` 在首字节前仍可能处于安全切号窗口；首字节等待超过 lane hard timeout 时，网关才能按 transport 边界有界切换后续账号。图片、资源创建及其他副作用请求一旦派发始终只有唯一 attempt，首字等待到期只结束该 attempt，不获得切号资格。
...
3. **精确客户端的上游语义失败**
   ...
   - 只有请求仍是可安全重放文本且下游语义尚未提交时，才可以关闭当前上游并切换后续账号；SSE 注释心跳只设置 `transportCommitted`，不会单独阻断安全文本，但也不能为已派发副作用请求创造重放资格。
...
本地可验证 transport failure 按自身阶段进入统一候选或终止边界；只有符合“可重放文本主派发已开始且仍在响应头前”的失败才先消费整请求安全原地重试 token，正文中断等其他 transport failure 不消费该 token。最终候选 transport 失败可以原子首次投递 `recovery_wait`；安全原地重试的中间失败、完整 HTTP 响应、精确客户端协议结构失败和路由配置截止均不投递恢复。后续普通请求不得合并失败计数、替换 generation、刷新 TTL 或改变 due 时间；普通成功也不能清理账户级事件。
...
- 只消费高置信本地请求错误：本地 JSON 解析失败、模型过滤失败和 OAuth/Codex adapter 本地校验失败。上游结果不计入来源错误熔断；其中 opaque HTTP/协议失败只处理当前请求，本地 transport failure 才能进入来源局部回避和首次后台核实流水线。
...
flowchart TD
  ...
  C -- "可安全重放文本且下游未提交" --> D["关闭当前上游，本次请求排除失败账号，服务端切后续账号 / 后续分组"]
  C -- "副作用已派发" --> Q["保持唯一 attempt；按稳定中性终态结束"]
  D --> M{"服务端是否救回"}
  ...
  E -- "是" --> F["返回成功，清理流失败状态"]
  E -- "否" --> G{"是否已收到可见输出"}
  G -- "否且可安全重放文本" --> H["服务端优先重试；耗尽后按客户端策略或普通失败返回"]
  G -- "否但副作用已派发" --> Q
...
- 是否还能由服务端重试同时取决于请求语义门禁、真实派发状态、下游写出边界、墙钟和 attempt 预算，不按上游错误内容设置例外。同批次输出与失败仍处于预提交缓冲、下游实际写出字节数为零时，只有可安全重放文本才可以继续切号；已派发副作用请求仍保持唯一 attempt。一旦已有内容写给下游，就不静默拼接第二条流，按客户端策略写协议失败事件或断开连接。
- 当前不维护全局流式错误码或终止类型白名单。系统默认的 `response.failed`、`event: error`、完成状态和正文规则必须属于明确 `clientProfile` 的协议策略，通用客户端不得继承；账户或管理端显式策略除外。精确客户端也只有在请求可安全重放、仍有可承接账号 / 分组且满足写出与预算边界时，才优先由服务端救回。
...
`response.incomplete` 在 Codex 客户端中会被解析为可重试流式错误。网关默认把 `clientProfiles = codex` 且 `finishReasons = incomplete` 的 Responses SSE 事件纳入响应检查兜底：只有当前请求属于可安全重放文本且下游尚未提交时，才先走服务端重试 / 切号；副作用型 Responses 派发后不重放，安全文本候选耗尽后才按 Codex 可重试失败事件结束流。
...
| 账户错误处理策略精确命中 `retry_next` | 用户显式策略 | 仅在请求仍可安全重放时切后续 Key/账号；已经派发的副作用请求不得重放 | 状态动作按用户配置独立执行；`retry_next` 本身不推断账户死亡，也不能绕过 at-most-once |
| 安全文本推理完整非 `2xx` 且未命中显式策略 | opaque 请求级接管 | 同账户 Key 按池顺序唯一尝试，再扫后续候选；全请求最多 64 次真实 attempt | 只写本请求排除集合；不写共享 Key/IP/质量/账户状态，不投递 `recovery_wait` |
...
| 完整 HTTP、正文中断、配置首字截止、未派发本地异常或副作用请求异常 | 各自协议、传输或 at-most-once 边界 | 不消费安全原地重试 token；按透明接管、后续候选、已提交终止或单次执行边界处理 | 未命中用户显式策略时不写业务语义状态 |
| 前序本地 transport failure 且后续账号形成完整 framing | 调度救回 + IP 级账号回避 | 否，已完成本次切号 | 当前 IP 可确认前序 transport 局部观察；opaque HTTP/协议失败不进入，普通响应不改变前序账户级运行态 |
...
| lane hard timeout、建连失败、读取中断 | transport failure | 文本语义未提交时按有界确认和候选扫描；副作用 POST 派发后不重放 | 可以进入传输电路；不得按状态码/正文写业务死亡状态 |
...
| 可安全重放文本的所有候选都发生 transport / timeout 或精确客户端语义失败 | 客户端协议最终兜底 | Codex / Claude Code / Gemini CLI 下发各自协议错误事件，由客户端决定是否重试；副作用请求派发后不进入该候选重放循环 | 仅出现 transport failure 的账号可首次投递后台核实；专属语义只影响当前请求，不投递核实，也不扩散到通用客户端 |
...
| 同上游桶多账号短窗 transport failure | 上游桶运行态避让 | 否，影响后续候选排序 | 只有本地 transport failure 可投递后台核实和形成桶证据；同优先级层内避让同桶账号，不禁用代理或上游 |
...
| 客户端取消 | 客户端生命周期 | 否 | 否 |
| 可安全重放文本的精确客户端 SSE 可见输出前发生专属语义失败，服务端仍有可承接账号 | 服务端内部重试 | 本次请求切后续账号 / 后续分组，客户端无感；副作用请求保持唯一 attempt | 当前请求排除失败账号；真实网关流量不因无可见输出直接累计账号流失败计数 |
...
| SSE 可见输出后失败 | 客户端重试 + 通用上游失败处理 | 已写给下游后不服务端静默拼接；已知客户端写协议错误事件，通用客户端断流 | 只有真实读取中断/EOF 等本地 transport 事实才记录 transport 桶；协议/业务失败保持请求级诊断，不避让同桶账号 |
...
- 命中账户错误处理策略或响应拦截策略时按用户配置直接执行状态动作；只有精确 `retry_next` 才表达本次重放意图，但是否重放仍由端点副作用、派发、提交、墙钟和 attempt 预算共同裁决。已经派发的图片、音频、文件/资源、后台任务、hosted tool 和其他副作用请求不得因策略命中而重放。未命中的完整 HTTP 业务失败只处理当前请求，不投递 `recovery_wait`；transport failure 才进入独立后台核实。
- 对精确客户端的可安全重放文本，服务端内部重试优先于客户端重试；可见输出前只有当服务端可承接账号耗尽时，才允许按该客户端策略写最终失败。已派发副作用请求不进入该重试循环，下游未提交时返回稳定 `503/upstream_outcome_unknown`。`response.failed` 事件格式只用于 Codex Responses，Claude Code 和 Gemini CLI 使用各自协议事件；turn 级账号避让仍只允许放在 Codex profile。通用客户端推理端点在 transport / timeout 或完整非 `2xx` 候选耗尽时接收网关 `503/upstream_retryable_error`。
- 不维护上游终止类错误码白名单；精确客户端的可安全重放文本流式语义失败在服务端重试耗尽后按对应客户端策略写协议事件，副作用请求不进入该重试路径。隐藏服务端重试成功时不消耗客户端重试机会，也不写 Codex turn 失败计数。通用客户端收到完整响应后不进入这条语义路径。
- 流式失败不再依赖错误码或阈值直接写账号状态；上游桶运行态和已激活的账号运行态调度降级只作为后续排序辅助，持久状态仍等待确认失败。
- 通用客户端文本推理端点在候选耗尽时返回统一可重试错误；图片和其他副作用 POST 一旦已经派发，无论收到完整响应、结果未知的 transport failure 或正文中断，都不得隐式重放，返回结果遵循其 at-most-once 边界。
- 修改流程后必须补回归，至少覆盖：只有可重放文本主派发已开始、响应头前 transport failure 消费整请求安全原地重试 token；兄弟 Key 先行且不占 token；完整 HTTP、正文中断、配置首字截止、未派发本地异常和副作用请求零消费且无共享副作用；3 Key、6 Key 和大 Key 池唯一穷尽；跨账户物理凭据去重；64-attempt 安全截断；客户端新请求按当前策略重新选号；普通失败不改账户运行态；显式策略仍生效；有效后台失败才 `precheck_pending`；generation / lease CAS；同账户和同分组半开单飞；FIFO/abort 清理；零可派发账号等待预算；SSE 心跳后的双提交边界；协议声明的 `event: error` / `response.failed` 与普通 `data.error` payload 的隔离；无显式策略时的 `2xx` JSON / SSE 透明转发；图片 in-flight 600/3600、at-most-once；音频 / 文件 / 资源 / hosted tool 等副作用 POST 不重放；可见输出后不服务端静默拼接。
...
- `pre_commit_stream_server_retry` / `stream_server_retry_dispatch`：可安全重放文本在下游未提交前的 SSE 失败已进入服务端内部重试和下一轮账号调度；副作用请求不得记录为该事件。
