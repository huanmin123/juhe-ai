...
- 未命中显式策略时，完整 HTTP 业务失败必须以当前 HTTP 状态、上游错误码和有界错误消息直接返回；文本、图片、音频、文件/资源、后台任务和 hosted tool 都不得因此切换 Key、账号或分组。该动作不得建立 API Key / IP / 会话跨请求回避、共享质量失败或 `recovery_wait`，但每个真实请求最多可投递一个按账户限频的固定模型、固定协议健康确认。普通请求不能根据状态码、错误类型或正文直接建立、续期、升级或清理账户业务运行态。
- 兜底层不维护状态码、错误码或错误文案白名单：状态码和上游错误体只作为诊断字段、审计字段和用户显式配置规则的输入，代码不内置“余额不足”“限流”“某状态码才写状态”之类判断。
- 未知 transport 失败交给统一失败流水线，并以明确网关错误终止当前请求；当前 attempt 没有交付结果也不得继续兄弟 Key、账户或后备分组。`temporaryUnschedulableRetryAttempts` 不可作为隐式重放授权。中间失败不写电路 confirmation、共享质量或 Key/账户状态；独立后台确认仍按其自身边界处理。
- 请求进入上游前发现当前账号目标协议不能保真承载某个合法能力时，只记录账号作用域 guidance 并跳过当前账号；它不是上游失败，不写账号状态，不进入状态码 / 错误码策略，也不能在还有同分组账号或后续可承接分组时直接返回给客户端。
- 通用客户端默认不解析上游错误对象、错误类型、错误码、完成状态和错误正文：完整非 `2xx` 及未知 transport 失败都保留在当前 attempt，绝不以 `response.ok` 触发 opaque 接管或扫描后续账户 / 分组。图片、音频、文件、资源创建、后台任务和 hosted tool 不设请求类型例外。只有用户或管理员显式配置且命中的账户错误策略 `retry_next`，才授权在其声明范围内重放；精确识别的 Codex、Claude Code、Gemini CLI 客户端只按协议声明的结构返回本次失败。
...
- 多 Key API Key 账户的 Key 级隔离不恢复状态码 / 错误码分类：各类请求的 opaque `response.ok=false` 都直接返回当前失败，不能形成当前请求内 Key 排除或按池顺序耗尽，也不能写共享 Key 状态。只有显式命中的 `retry_next` 可选择后续 Key。跨账户物理凭据去重；完整设计见 [账户内 API Key 故障隔离设计](账户内APIKey故障隔离设计.md)。
- DeepSeek 这类 OpenAI-compatible 第三方也遵守同一错误边界：完整非 `2xx` 保留实际上游错误，不读取 401 / 403 / 402 / 429 / 5xx、余额/限流/模型文案或 `finish_reason = insufficient_system_resource` 决定共享动作或隐式切换。`2xx` payload 保持透明，这些字段只可进入诊断或用户显式策略匹配。需要写 `rate_limited`、`temporary_unavailable` 或 `error` 时，只能由用户显式账户错误策略授权；统一边界见 [AI 账户错误语义与状态变更边界](AI账户错误语义与状态变更边界.md)。
...
| 参数 | 默认值 | 用途 |
| --- | --- | --- |
| `temporaryUnschedulableRetryAttempts` | `3` | 历史配置，不能作为未知 transport 或任意失败的隐式重放授权；当前自动切换只由精确命中的 `retry_next` 策略授权 |
| `temporaryUnschedulableRetryIntervalSeconds` | `3` | 历史等待参数；未命中 `retry_next` 时不触发重放 |
...
flowchart TD
  ...
  E -- "否" --> R{"是否精确命中 retry_next"}
  R -- "是，且未语义提交" --> X["按显式策略切换一次后续候选"]
  R -- "否" --> W["返回当前 transport 错误，或已提交后断流"]

  D -- "否，收到响应" --> J{"是否为精确客户端画像"}
  J -- "否，通用客户端" --> G{"响应是否成功"}
  G -- "完整 2xx" --> K["不解释正文，透明完成"]
  G -- "完整非 2xx" --> R2{"是否精确命中 retry_next"}
  R2 -- "是，且未语义提交" --> X
  R2 -- "否" --> H["保留当前 HTTP 状态、错误码和错误消息返回"]
  J -- "是" --> L["按该客户端声明的协议语义和响应检查处理"]
  L --> R3{"是否精确命中 retry_next 且未语义提交"}
  R3 -- "是" --> X
...
1. **上游请求抛异常**
   ...
   - 连接/请求 transport failure 是当前 attempt 的明确失败；不消费整请求共享 token，也不尝试同账户兄弟 Key 或后续候选。只有用户显式命中的 `retry_next` 策略可授权安全重放。
...
2. **上游返回响应**
    - 通用客户端默认不解析错误对象、错误码、错误类型、错误文案或完成状态；完整非 `2xx` 或结果未知时都保留当前上游失败，不能排除当前 Key / 账户并继续下一候选。已提交时只能结束或断流。供应商原始错误进入有界诊断和客户端响应，不能作为账户状态或客户端重试指令。
    - 未配置显式策略时，通用响应不修改账号或 Key 共享状态，也不因 `200 + error`、普通事件里的 `data.error`、`418 + JSON` 的具体错误内容或任意错误码自动分类或轮换 Key / 账户。
    - 精确识别为 Codex、Claude Code 或 Gemini CLI 时加载该客户端画像声明的系统默认协议规则；账户所有者或管理员显式创建且命中的 `retry_next` 策略才授权重放。其他显式状态动作只按配置执行。
   ...
   - 各类非流式响应在首字节前仍必须保留当前 attempt；首字节等待超过 lane hard timeout 时，网关返回当前 transport 错误。图片继续使用自己的长时限，但时限到期后同样不切换候选。
...
3. **精确客户端的上游语义失败**
   ...
   - 请求下游语义尚未提交时，精确客户端协议失败仍返回当前失败；只有显式 `retry_next` 策略可关闭当前上游并切换候选。SSE 注释心跳只设置 `transportCommitted`。
...
本地可验证 transport failure 在当前 attempt 终止；不得消费整请求 token、轮换兄弟 Key 或扫描后续候选。只有精确命中的 `retry_next` 策略可授权安全重放。完整 HTTP 与明确协议失败仍可每请求投递一个限频的通用账户健康确认；后续普通请求不得合并失败计数、替换 generation、刷新 TTL 或改变 due 时间。
...
- 只消费高置信本地请求错误：本地 JSON 解析失败、模型过滤失败和 OAuth/Codex adapter 本地校验失败。上游结果不计入来源错误熔断；其中 opaque HTTP/协议失败不进入来源局部回避或 transport `recovery_wait`，但完整 HTTP 失败可触发限频的通用账户健康确认；本地 transport failure 还能进入来源局部回避和传输核实流水线。
...
`response.incomplete` 在 Codex 客户端中会被解析为可重试流式错误。网关只在该事件精确命中用户配置的 `retry_next` 策略时，才在下游尚未语义提交且对应 lane 预算允许时重放；未命中策略时按 Codex 协议呈现当前失败，不自动切换候选。
...
Codex turn 级状态不在正式请求热路径执行额外真实探针，也不因当前失败自动避让账号或重新进入候选。只有用户配置的 `retry_next` 策略可授权当前请求重放；后续由客户端主动发起的新请求再按当时配置独立选路。手动诊断、后台冷却复测等独立探针继续使用 `10s -> 20s -> 30s` 等待档位，但探针结果不替代真实请求调度，也不作为同步切号门槛。
...
| 账户错误处理策略精确命中 `retry_next` | 用户显式策略 | 策略授权后切后续 Key/账号 | `retry_next` 是唯一自动切号授权，不推断账户死亡 |
| 任意请求完整非 `2xx` 且未命中显式策略 | 当前 attempt 终态 | 否，保留当前状态、错误码和错误摘要返回 | 不写共享 Key/IP/质量/账户状态，不投递 `recovery_wait`；每请求最多投递一个限频账户健康确认 |
| 可重放文本主派发已开始、响应头前 transport failure | 当前 attempt 终态 | 否，返回明确 transport 错误 | 只写审计/使用事实，不写电路、质量、Key/账户、IP 或上游桶；独立证据另行处理 |
| 完整 HTTP、正文中断、配置首字截止、未派发本地异常或非文本请求异常 | 当前 attempt 终态与各自诊断边界 | 否；已提交时终止当前响应 | 未命中用户显式策略时不写业务语义状态 |
| 前序本地 transport failure | 当前 attempt 终态 | 否，返回明确 transport 错误 | 只记录当前失败和独立诊断；不因该失败派发后续账号 |
...
| lane hard timeout、建连失败、读取中断 | transport failure | 否，返回当前 timeout / transport 错误 | 可以进入独立传输诊断；不得按状态码/正文写业务死亡状态或切换候选 |
...
| 当前候选发生 transport / timeout，未命中精确策略 | 网关当前错误终态 | 否；下游未提交时返回 HTTP `502/504`，已提交时断流 | 普通请求可投递独立核实；不存在隐式候选扫描 |
| 精确客户端请求发生 transport / timeout 或专属协议失败 | 客户端协议当前错误终态 | 仅精确命中 `retry_next` 时重放；否则按 Codex / Claude Code / Gemini CLI 协议呈现当前失败 | transport 与专属语义不混用；业务请求不直接写状态，也不把专属语义扩散到通用客户端 |
...
- 命中账户错误处理策略或响应拦截策略时按用户配置直接执行状态动作；只有精确动作 `retry_next` 授权自动切号，并且仍受语义提交、墙钟、候选去重和 attempt 预算约束。未命中的完整 HTTP、transport、timeout 和正文中断均只处理当前请求。
...
- 不维护上游终止类错误码白名单，也不维护隐式重试白名单。上游状态、错误码、错误类型和错误文案只能用于精确匹配用户配置的策略，不能自行触发候选切换。
- 流式失败不再依赖错误码或阈值直接写账号状态；上游桶运行态和已激活的账号运行态调度降级只作为后续排序辅助，持久状态仍等待确认失败。
- 未命中显式策略时，任何请求都不扫描候选来隐藏失败；图片和其他可能产生副作用的请求也禁止隐式 at-least-once 重放。产品优先暴露上游或网关的当前错误，修复问题后由客户端发起新请求。
- 修改流程后必须补回归，至少覆盖：未命中策略的完整非 `2xx` 原状态/原错误返回且单次尝试；未知 transport、timeout 和正文中断返回明确当前错误且单次尝试；显式 `retry_next` 精确匹配后才切后备候选；客户端新请求按当前策略重新选号；普通失败不改账户运行态；`2xx` JSON / SSE 的协议校验边界；可见输出后不服务端静默拼接。
...
- `pre_commit_stream_server_retry` / `stream_server_retry_dispatch`：精确命中 `retry_next` 的 SSE 失败在下游未语义提交前进入服务端重放；未命中策略时不记录这些事件。
