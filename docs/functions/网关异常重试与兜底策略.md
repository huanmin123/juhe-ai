...
> 账号“锁死”尚处于 [AI 账户锁死调度机制调研与待审核设计](AI账户锁死调度机制调研与待审核设计.md) 阶段，当前代码未实现锁死字段、运行态或切号否决门。本文的普通候选切换、同账号重试、`speed_first`、熔断、并发和客户端 handoff 仍是现行基线；锁死落地后只能在这些边界之上增加切号否决与共享限频，不能改写既有预算或响应语义。
