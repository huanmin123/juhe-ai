...
- 修改流程后必须补回归，至少覆盖：`invalid_request_error` 切号救回、未知失败立即切号、本地屏蔽生效、全部失败统一网关错误、单账号屏蔽等待释放、代理运行态避让、`response.created` 后缺少终止事件不计数、Codex profile 可见输出前 `event: error` 改写、非 Codex 不触发 Codex 改写、Codex turn 第 4 次切号、可见输出后流失败计数。
