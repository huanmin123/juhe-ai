...
| `cooldownAccountRetestMaxBackoffHours` | `24` | `temporary_unavailable` 后台复测最长自动恢复观察时间；慢速恢复持续失败超过后转异常并停止自动复测 |
...
flowchart TD
  ...
  F --> G{"到期复测是否成功"}
  G -- "是" --> D
  G -- "否，未达到最大暂停" --> H["退避时间翻倍，继续慢速恢复"]
  G -- "否，已达到最大暂停后仍连续失败" --> I["转为 error，停止自动复测"]
```
...
- 慢速恢复通道达到最大暂停时间后，如果在 `cooldownAccountRetestMaxBackoffHours` 表达的最长自动恢复观察内仍连续失败，账号转为 `error`、`schedulable = 0`，并保留最近测试 HTTP 状态、错误码、错误摘要和连续失败次数，供账户页和日志排查。
