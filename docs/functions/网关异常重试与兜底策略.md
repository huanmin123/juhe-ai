...
- 已确认的上游 transport 失败交给统一失败流水线；下游尚未语义提交时排除当前账户并继续后备账户 / 分组，不轮换同账户兄弟 Key。已提交正文中断仍只结束当前响应，不能拼接第二个候选。`temporaryUnschedulableRetryAttempts` 不可作为隐式重放授权。中间失败不写电路 confirmation、共享质量或 Key/账户状态；独立后台确认仍按其自身边界处理。
- 请求进入上游前发现当前账号目标协议不能保真承载某个合法能力时，只记录账号作用域 guidance 并跳过当前账号；它不是上游失败，不写账号状态，不进入状态码 / 错误码策略，也不能在还有同分组账号或后续可承接分组时直接返回给客户端。
- 通用客户端默认不解析上游错误对象、错误类型、错误码、完成状态和错误正文：完整非 `2xx` 只依据 `response.ok=false` 排除当前候选并扫描后续账户 / 分组，不轮换兄弟 Key；用户显式策略的状态动作与当前请求切号彼此独立，只有 `retry_next` 额外授权同账户兄弟 Key 轮换。图片、音频、文件、资源创建、后台任务和 hosted tool 不设请求类型例外。精确识别的 Codex、Claude Code、Gemini CLI 客户端只按协议声明的结构返回本次失败。
...
- 多 Key API Key 账户的 Key 级隔离不恢复状态码 / 错误码分类：各类请求的 opaque `response.ok=false` 都排除当前账户并继续后续账户，不能形成共享 Key 状态；只有显式命中的 `retry_next` 可选择同账户后续 Key。跨账户物理凭据去重；完整设计见 [账户内 API Key 故障隔离设计](账户内APIKey故障隔离设计.md)。
...
| 参数 | 默认值 | 用途 |
| --- | --- | --- |
| `temporaryUnschedulableRetryAttempts` | `3` | 历史配置，不能作为未知 transport 或任意失败的隐式重放授权；完整 HTTP 失败的账户级候选切换不依赖该配置，`retry_next` 只影响同账户兄弟 Key |
| `temporaryUnschedulableRetryIntervalSeconds` | `3` | 历史等待参数；完整 HTTP 失败仍按统一账户级候选路径继续，未命中 `retry_next` 时不轮换同账户兄弟 Key |
...
flowchart TD
  ...
  G -- "完整非 2xx" --> H["按 response.ok=false 排除当前账户并继续候选"]
  H --> T
...
2. **上游返回响应**
    ...
    - 精确识别为 Codex、Claude Code 或 Gemini CLI 时加载该客户端画像声明的系统默认协议规则；账户所有者或管理员显式创建且命中的 `retry_next` 策略只额外授权同一账户兄弟 Key 重放。其他显式状态动作只按配置执行，同时不阻断账户级候选切换。
...
| 账户错误处理策略精确命中任一动作 | 用户显式策略 | 先执行配置的状态动作，再按统一路径切后续账号；仅 `retry_next` 可继续同账户 Key | 用户动作不改变账户级切号准入 |
...
- 命中账户错误处理策略或响应拦截策略时按用户配置直接执行状态动作；状态动作完成后当前请求仍按统一候选规则切号，`retry_next` 只额外授权同账户 Key 轮换，并且仍受语义提交、墙钟、候选去重和 attempt 预算约束。未命中的完整 HTTP 也必须排除当前候选并继续调度。
