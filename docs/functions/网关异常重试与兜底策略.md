...
本文固定 OpenAI 兼容网关在上游异常、账户错误处理策略、未知失败和流式中断时的处理顺序，避免后续维护时把“账户错误处理策略”“本地失败隔离”“流熔断”重复实现或互相抢逻辑。

核心原则：

- 命中上游账号后的错误都先进入账号级运行态屏障：上游请求异常、非 `2xx` 响应、非流式正文中断、流式失败事件或流中断都先用于救当前请求和阻断新流量；默认待确认目标为 `temporary_unavailable`，账户级错误处理策略可以把待确认目标改为 `rate_limited` 或 `error`。
- 兜底层不维护状态码、错误码或错误文案白名单：状态码和上游错误体只作为诊断字段、审计字段和用户显式配置规则的输入，代码不内置“余额不足”“限流”“某状态码才写状态”之类判断。
- 未知失败交给统一失败流水线：任何上游请求异常或非 `2xx` 响应都先按 `temporaryUnschedulableRetryAttempts` / `temporaryUnschedulableRetryIntervalSeconds` 在当前账号原地确认；重试用尽仍失败后才短期屏蔽当前账号、忘记会话亲和、切换后续账号。持久账号状态由后续成功信号、账号事前确认、半开探针、后台复测或人工操作决定。
- 协议兼容恢复先于普通同 body 原地确认：如果非 `2xx` 响应被兼容策略归一为“当前请求体与上游协议状态不兼容”，并且当前请求体存在可恢复协议状态，可以用一次受控请求体变体重试替代同 body 原地重试。该动作只服务当前请求，不作为账号健康判断，也不恢复状态码 / 错误码白名单；没有可生成的变体时继续默认失败流程。
...
- 当当前分组候选账号都被本地屏蔽时，先尝试 API Key 后续可承接分组；没有后续分组时快速返回 `503` 和 `Retry-After`，不再因为“只剩一个号”就旁路屏蔽继续打失败账号。
...
| 本地短暂避让阶梯 | `3s -> 5s -> 10s` | 普通上游失败在原地重试用尽后的进程内调度屏障；每阶到期后只允许一个真实请求半开探测，三阶半开仍失败后升级为事前确认 |
...
| `streamFailureThresholdCount` | `3` | 历史流失败计数参数；真实网关流式失败先进入运行态屏障，确认失败后才写持久状态 |
| `streamFailureThresholdWindowMinutes` | `10` | 历史流失败计数窗口参数；真实网关流式失败不再依赖该窗口直接决定是否写状态 |
...
flowchart TD
  ...
  L --> Q{"是否命中兼容恢复策略"}
  Q -- "是" --> Q1["生成受控 body/header 变体并重试一次"]
  Q -- "否" --> R["有剩余次数则原地重试当前账号"]
  Q1 --> J
...
3. **上游返回 HTTP 非 2xx**
   ...
   - 在普通同 body 原地确认前，先把上游错误体交给兼容策略层判断是否属于请求级协议状态不兼容。命中时可以对当前账号当前上游 URL 执行一次受控 body/header 变体重试，例如删除 Responses `reasoning.encrypted_content` 后重试。
   - 兼容恢复只影响当前请求 attempt，不写账号状态，不作为账户错误处理策略结果，也不记录 IP 级账号回避。恢复 attempt 失败后继续进入默认失败流水线。
...
4. **账号级屏障确认**
   - 运行态屏障只影响新请求选号，不主动迁移或中断已经发往上游的存量请求。
   - 存量请求或受控半开探针完整成功时，清理该账号运行态屏障。
   - 如果账号已经处于运行态屏障，仍在该账号上的存量请求失败时，应减少或跳过剩余同账号原地重试，避免多个并发请求放大失败风暴。
   - 没有成功信号且事前确认、半开探针或流失败确认连续失败后，仍要等当前账号并发归零，才按待确认目标写持久账号状态。
...
- 每个重试用尽仍失败的账号进入当前 Web 进程内短暂避让屏障，首次避让 3 秒。
- 当前请求继续尝试后续候选账号；后续账号成功时，本次请求救回。
- 前序失败账号在原地重试用尽仍失败时进入当前进程运行态屏障；本地屏蔽负责当前进程调度避让，SQLite 状态只在确认失败、后台复测、手动测试或人工操作后写入。
- 后续请求选号时先过滤本地屏蔽账号；避让到期后不直接放开全部并发，只在派发前允许一个请求获得半开探测租约。
- 半开请求完整成功时清理运行态屏障；半开请求失败时进入下一阶避让，阶梯为 `3s -> 5s -> 10s`。
- `10s` 后的半开请求仍失败时，不继续延长短暂避让，升级为 `precheck_pending` 事前确认；事前确认失败且当前账号并发归零后，才写持久 `temporary_unavailable`。
- 代理 profile 已知不可用和账号准备阶段失败如果来自真实网关流量，也按同一套短暂避让阶梯推进；三轮半开仍失败时进入事前确认，不把代理配置失败直接写成账号持久临时不可调用。
- 如果过滤后仍有账号，继续后续代理避让、IP 级回避和 Codex turn 避让。
- 如果当前分组全部候选都被本地屏蔽，先按 API Key 分组绑定顺序尝试后续可承接分组。
- 没有后续可承接分组时立即返回网关统一 `503 service_unavailable`，不返回最后一个上游错误体，并按最早屏蔽释放时间写入 `Retry-After`。

这层屏蔽是进程内易失运行态，不写 SQLite，不作为长期账号健康事实，也不走后台复测。它解决的是“失败账号被大量后续请求继续命中”的调度风暴，而不是账号状态定性；账号状态定性必须等成功信号、事前确认或半开探针给出结论。
...
- 账户错误处理策略、客户端取消、本地校验失败、额度 / 授权 / 模型过滤失败、账号并发满、代理预检跳过和没有后续成功样本的全失败请求，都不能记录 IP 级账号回避。
...
- Codex profile 的最终 `response.failed/upstream_retryable_error` 只暴露统一“上游流式响应在输出前失败，请重试”文案；底层 `incorrect header check`、`error decoding response body`、代理断流或压缩解码细节只进入审计和账号诊断，不直接写给 Codex 客户端。
...
- 中转广告、污染文本和账户级 `200 + SSE` 拦截由 [流式拦截策略配置设计](流式拦截策略配置设计.md) 对应的运行时策略承接；它只处理完整 SSE event 边界内的策略命中、写下游前服务端重试、运行态避让和审计 metadata，不替代非 2xx 账户错误处理策略，也不能绕过 Codex profile 专属重试边界。
...
| `accountClientCompatibility` | `codex_responses`、`openai_standard` | 当前命中账号的兼容能力范围，供响应检查策略收窄适用账号；不能单独决定下游错误格式 |
...
响应检查策略新增两个可选适用范围维度：`clientProfiles` 和 `accountClientCompatibilities`。它们只过滤策略是否适用于当前请求，不能单独构成命中；策略仍必须由错误码、错误类型、错误文案、完成状态、JSON 路径、SSE 原文或输出文本等响应语义触发。账户级响应检查规则可以用当前账号的兼容能力做账号侧适用范围，例如只让具备 `codex_responses` 能力的账号启用 Codex 上游错误拦截；最终是否向下游返回 `upstream_retryable_error` 仍必须由 `clientProfile = codex` 控制，避免普通 OpenAI-compatible 客户端收到 Codex 专属错误语义。
...
Codex turn 级切号在选用备用账号前会执行真实账号探针，探针固定命中当前候选账号并走同一套网关链路。等待档位复用诊断策略 `10s -> 20s -> 30s`，但它不是账号失败确认流程：只有本地超时才在同一候选账号上递进到下一档；一旦探针拿到明确失败结果，就立即淘汰该候选并尝试下一个账号，不按状态码、错误码或错误文案做原地分类重试。

## 策略边界

| 场景 | 处理归属 | 是否同账号重试 | 是否写账号状态 |
| --- | --- | --- | --- |
| 上游返回请求级协议兼容失败且可生成请求体变体 | 兼容策略与请求恢复 | 是，跳过同 body 原地重试，对当前账号当前上游 URL 使用受控变体重试一次 | 否；恢复失败后回到普通失败流水线 |
...
| 前序失败且后续账号成功 | 调度救回 + IP 级账号回避 | 否，已完成本次切号 | 前序失败账号进入运行态屏障；当前 IP 可短 TTL 避让前序账号 |
...
| 所有候选都在本地屏蔽中 | 本地短期调度保护 | 先尝试 API Key 后续分组；无可承接分组时立即失败；到期后单请求半开 | 不新增账号状态；返回 `service_unavailable` 并带 `Retry-After` |
| 短暂避让三轮半开仍失败 | 账号事前确认入口 | 否，交给事前确认探针 | 仍不直接写状态；事前确认失败且并发归零后写 `temporary_unavailable` |
| 同上游桶多账号短窗失败 | 上游桶运行态避让 | 否，影响后续候选排序 | 已命中上游失败的账号仍进入账号运行态屏障；不禁用代理或上游 |
...
- 上游桶运行态失败只影响后续候选排序，不能禁用代理或上游；已实际命中并失败的账号仍进入账号级运行态屏障。
...
- 流式失败不再依赖错误码或阈值直接写账号状态；上游桶运行态只作为后续排序辅助，持久状态仍等待确认失败。
- 普通上游失败全部候选失败时返回网关统一错误，不把最后一个上游错误体返回给客户端。
- 修改流程后必须补回归，至少覆盖：`invalid_request_error` 切号救回并让前序账号进入运行态屏障、未知失败按临时状态配置原地重试且用尽后切号并进入待确认、本地屏蔽生效、存量并发成功后清理屏障、账号仍有并发时不直接写 `temporary_unavailable`、半开探针单飞、全部失败统一网关错误、单账号屏蔽耗尽快速失败且不命中上游、多分组主号池全屏蔽时切后续可承接分组、上游桶运行态避让、`response.created` 后缺少终止事件在下游未提交时服务端切号、Codex profile 服务端重试耗尽后才写 `upstream_retryable_error`、非 Codex 不触发 Codex 改写、Codex turn 可见失败后的账号避让、可见输出后流失败不服务端静默拼接。
...
- `gateway_stream_failure_account_side_effect_enqueued`：流式失败已写入账号状态副作用队列。
