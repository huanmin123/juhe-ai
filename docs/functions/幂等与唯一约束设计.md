...
| `route_strategies` | `id`、`system_account_id + lower(name)` | 同一用户策略路由名称唯一；当前不默认创建策略路由，历史 `is_default` 路由只作为不可删除保护标识。 |
| `api_keys` | `id`、`key_hash`、`system_account_id + lower(name)`、`route_strategy_id WHERE is_default = 1` | 已补同一用户 API Key 名称唯一；`key_hash` 继续保证密钥不重复；当前不默认创建 API Key，历史 `is_default` Key 只作为不可删除保护标识。 |
