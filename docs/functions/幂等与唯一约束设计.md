...
- 本地 SQLite 已离线删除旧的重复 API Key 记录，并直接补建第一批唯一索引；源码不保留启动时清理旧数据或跳过索引的兼容逻辑。
...
- `mutationGuard` 只负责重复提交切入、缓存状态和统一错误语义，不替代业务校验和数据库唯一约束。
...
| `accounts` | `id`、非空 `credential_fingerprint`、`system_account_id + provider_code + lower(name)` | 已补同一用户同供应商账号名称唯一；凭据仍保持全局唯一。 |
...
| `api_keys` | `id`、`key_hash`、`system_account_id + lower(name)` | 已补同一用户 API Key 名称唯一；`key_hash` 继续保证密钥不重复。 |
...
| `team_resource_authorization_grants` | active `resource_type + resource_id + team_id` | 保持。 |
...
| 创建 AI 账户 | `accounts.create` | `provider_code + type + credential_fingerprint`，可附加 owner |
| OAuth 授权码创建账户 | `openai_oauth.create_from_code` | `state + code hash` 或换取 token 后的 `credential_fingerprint` |
| Refresh Token 创建账户 | `openai_oauth.create_from_refresh_token` | `refresh_token fingerprint` |
...
- 新增业务唯一索引前必须先离线检查并清理本地重复数据；源码只保留最新 schema，不在正常启动路径里做旧数据清理或跳过索引兼容。

## 实现落点建议

后端：

- `backend/src/modules/deduplication/deduplication.service.ts`：已落地进程内防重复提交缓存、稳定 hash、TTL 清理和容量上限。
...
- `backend/src/storage/schema.ts`：已补充第一批业务唯一索引。
