...
| `accounts` | `id`、非空 `account_identity_fingerprint`、`system_account_id + provider_code + lower(name)` | 已补同一用户同供应商账号名称唯一；账户身份按供应商、账户类型、上游站点 host[:port] 和凭据全局唯一，单独凭据指纹只做排查索引。 |
...
| 创建 AI 账户 | `accounts.create` | `owner + provider_code + type + name + credential_fingerprint`；数据库最终以 `account_identity_fingerprint` 兜底 |
| OAuth 授权回调创建账户 | `openai_oauth.create_from_code` | `callbackUrl fingerprint` 或换取 token 后的 `account_identity_fingerprint` |
