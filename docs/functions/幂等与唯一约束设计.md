...
| `route_strategies` | `id`、`system_account_id + lower(name)` | 同一用户策略路由名称唯一；默认策略路由按内置默认分组生成，不再限制同一用户只能有一条默认策略路由。 |
| `api_keys` | `id`、`key_hash`、`system_account_id + lower(name)`、`route_strategy_id WHERE is_default = 1` | 已补同一用户 API Key 名称唯一；`key_hash` 继续保证密钥不重复；每条默认路由最多同步一个默认 API Key。 |
