...
| `accounts` | `id`、`system_account_id + provider_code + lower(name)` | 同一用户同供应商账号名称唯一；凭据指纹只做普通排查索引，API Key 和 OAuth 凭据允许重复添加。 |
...
| 创建 AI 账户 | `accounts.create` | `owner + provider_code + type + name + credential_fingerprint`；数据库最终以同一用户同供应商账号名称唯一约束兜底 |
