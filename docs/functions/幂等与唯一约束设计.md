...
| `groups` | `id`、`system_account_id + provider_protocol_profile_id + lower(name)`、`system_account_id + provider_protocol_profile_id WHERE is_default = 1` | 已补同一用户同供应商协议档案分组名称唯一，并限制每个用户每个供应商协议档案只有一个默认分组。 |
...
| `error_policies` | `id` | 系统级全局策略；当前限制最多 200 条，后续可按 `scope_type + protocol_code + provider_code + client_profile + model_pattern + lower(name)` 补管理侧唯一约束。 |
...
| 创建 AI 账户 | `accounts.create` | `owner + provider_protocol_profile_id + type + name + credential_fingerprint`；数据库最终以同一用户同协议档案账号名称唯一约束兜底 |
...
| 创建分组 | `groups.create` | `owner + provider_protocol_profile_id + lower(name)` |
