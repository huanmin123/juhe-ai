...
> **状态：Node + Vue 本地实现完成，生产启用受独立门禁约束。** 已实现 OAuth 2.1 Authorization Code + PKCE、OIDC、Device Flow、个人委托 API 和管理页面；用户侧不展示已授权应用，也不提供主动撤销入口。`backend-go` 不在本次范围；当前 Node 业务库也尚未接入 PostgreSQL driver，因此不得把本实现直接作为 performance/生产模式的发布依据。
...
5. 用户无需管理已授予的应用；授权到期、应用被停用或账号被停用后，token 自动失效。
...
sequenceDiagram
    ...
    J->>J: 到达 7 天绝对到期时间时标记 grant / token 失效
...
轮换请求早于 72 小时必须返回 `token_renewal_not_eligible`，不得创建新 token、更新最近使用时间或消耗轮换额度；到达或超过 grant 硬到期时必须返回 `invalid_token`，且不允许通过轮换恢复。用户不提供授权查询或主动终止入口；授权在到期、应用停用或账号停用时自动失效。
...
- confidential client 的可轮换 `client_secret`。管理员可按应用反复下载含当前值的对接文档；存储保留认证哈希及加密副本，日志不记录明文；
...
所有 Client 只能从自身登记的允许 scope 中申请，用户可以在授权页看到每一项。请求任何 `juhe:*.write` 时，必须在同一个 scope 参数中同时请求对应的 `juhe:*.read`，否则授权码和 Device Flow 均返回 `invalid_scope`。授权服务把用户的 `systemAccountId` 与 grant 绑定；个人委托 API 从 token 中取得该绑定，**完全忽略调用方传入的账户 ID、团队 ID 或拥有者字段**。
...
管理员在“第三方应用”页创建、启用或停用 Client，维护其精确回调地址、可申请 scope 和 Client 类型。每一行均可下载绑定该 Client 的 Markdown 对接文档，文档包含 Client ID、类型、当前状态、精确回调地址、允许 Scope、Issuer、Discovery、JWKS、所有协议端点、PKCE、Device Flow、token 规则及个人委托 API；管理员无需另外复制或整理接入资料。confidential Client 的每次下载还包含当前 `client_secret`；服务端保存其认证哈希及加密副本，重新签发后旧值立即失效，后续下载自动带新值。新字段生效前创建且没有加密副本的 Client 不能恢复旧值，管理员需要重新签发一次。停用 Client 后，其新授权、换码和现有 token 都必须被拒绝。OIDC RS256 签名密钥由平台自动生成并按 7 天周期轮换；旧公钥会在最长 grant 生命周期内继续出现在 JWKS，避免已签发 ID Token 无法验证。Client 管理与既有“公开接口授权 / 外部来源系统”页面并列，名称和说明不得混淆。

### 第三方接入方

第三方直接接收其应用行内下载的绑定文档；confidential Client 的每次行内下载都含当前 `client_secret`。接入文档必须提供 PKCE、state、nonce、错误处理、token 保存和撤销示例，并声明不能把 token 写入前端包、日志、错误上报或 URL。需要替换 Client Secret 时，管理员使用“重新签发”，旧值立刻失效，并下载新的应用专属文档。
...
| 数据 | 关键字段 / 约束 |
| --- | --- |
| `oauth_clients`、回调地址、secret | Client 类型、状态、scope allowlist、精确 redirect URI；secret 保存认证哈希及用于管理员重复下载的加密副本。 |
| 签名密钥 | `kid`、用途、启用/退役时间；ID Token 使用平台自动生成、每 7 天自动轮换的非对称签名密钥。退役公钥在 grant 验证窗口内继续发布到 JWKS。 |
| 授权事务与授权码 | Client、用户、scope、PKCE、nonce、可原样回送的 `state` 加密密文及校验摘要、到期时间；授权码哈希且原子一次消费。 |
| grant、OIDC subject 与 access token | grant 保存不可延长的 `expires_at`；token 保存 `issued_at`、`renewed_from_token_id`、`renewed_at` 和已消费状态。为每位用户保存稳定、不透明、永不复用的 public `sub`；token 仅哈希，在 grant 到期、应用停用或用户停用时自动失效。 |
...
实现默认以 `JUHE_AI_OIDC_ENABLED=false` 关闭。启用时必须配置 `JUHE_AI_OIDC_ISSUER` 和 `JUHE_AI_OIDC_KEY_ENCRYPTION_SECRET`；issuer 必须是生产 HTTPS Origin（仅测试/脚本允许本机 HTTP）。首次 OIDC 协议请求会自动生成签名密钥，之后在到达 7 天周期后的下一次协议请求自动轮换；无需管理员手动操作。未满足条件时保持关闭或显式失败，不能悄悄降级为不安全的授权服务。
...
| 管理界面 | 管理员可创建/启停 Client、按应用重复下载含当前 confidential secret 的文档、重新签发 Client Secret；OIDC 签名密钥由平台每 7 天自动轮换。用户侧不展示已授权应用，也不提供主动终止入口。 | 完成生产角色、审计和运营流程验收。 |
| 存储与日志 | 授权码、access token、device code 仅存哈希；Client Secret 同时存认证哈希及可供管理员文档下载的加密副本；OIDC 私钥、`state`、CSRF 和 `nonce` 加密保存。换码在真实 RS256 签名预检及 nonce 解密成功前不消费一次性 code/device code。 | Node 当前业务库仅支持 SQLite；`JUHE_AI_DATABASE_DRIVER=postgres` 会明确拒绝启动。完成 PostgreSQL driver 后，再做性能模式和生产发布验证。另需完成全链路日志脱敏审计。 |
...
2. 以管理员身份进入 `/oauth-applications`，创建并审核 Client 的精确回调地址和 scope allowlist；不提供签名密钥的手动生成或轮换入口。
3. 第三方从 `/.well-known/openid-configuration` 读取 discovery。客户端按 JWKS 的 `kid` 验签，遇到未知 `kid` 时刷新 JWKS 再重试；标准 OIDC SDK 通常会自动处理这一步。V1 的受控 token 轮换端点通过扩展字段 `juhe_token_renewal_endpoint` 声明，不伪装为标准 `refresh_token`。
