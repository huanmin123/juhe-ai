...
| 状态 | 展示 | 说明 |
| --- | --- | --- |
| `pending` | 不展示余额行 | 已开启但尚无最终结果 |
| `refreshing` | 不展示余额行 | 已有成功余额时继续展示旧余额，否则隐藏 |
| `fresh` | `剩余：$7.31` | 当前查询成功 |
| `unlimited` | `剩余：不限额` | 上游明确报告无限额度 |
| `unsupported` | `余额查询失败` | 后台停止自动轮询，悬浮显示原因 |
...
2. 每轮固定为自愈保留最多 10 个名额，再用剩余容量处理正常到期账户，避免到期 backlog 长期占满 100 条时自愈饥饿。自愈只补充“活动、可调度、已开启、无当前代次快照且刷新时间为空”的账户；已有 `unsupported` 当前快照的暂停账户不进入自愈。SQLite 和 PostgreSQL 都使用进程内 ID 游标按有界页轮转扫描，不能每轮固定从最小 ID 开始。
...
5. 成功时写当前快照并按用户配置周期安排下次刷新；临时失败按第 8 节累计并继续使用同一用户配置周期；确定不支持时写暂停快照并把 `balance_query_next_refresh_at` 置空。
6. 暂停只停止余额后台调度，不修改账户状态、可调度性或用户开关。只有余额查询身份真实变化或人工刷新成功时，才恢复后台周期；同值保存不能解除暂停。
...
- 手动刷新成功时更新 `next_refresh_after`；临时失败写 `failed` 并按账户周期安排重试，明确不支持写 `unsupported` 并暂停自动刷新。
...
- 人工刷新允许对已暂停账户重新检测；成功时解除暂停并恢复用户配置周期，失败时保存本次最终失败语义。
