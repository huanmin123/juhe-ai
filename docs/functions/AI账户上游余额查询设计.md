...
- 配置区只提供“测试查询”按钮和“仅验证当前配置，不会保存”提示。测试结果不在表单内保留，通过页面顶部消息显示成功金额或失败原因。
...
- 临时错误包括网络连接异常、请求取消或超时、HTTP `408`、`429` 和 `5xx`。连续第 1、2 次临时失败只更新 `consecutiveTransientFailures`、`lastTransientErrorMessage`、`lastTransientFailureAt` 和下次刷新时间，不把余额状态改成 `failed`。
...
- 连续第 3 次临时失败才保存 `status='failed'`、清除旧金额并显示最近错误；后续临时失败把计数封顶在 3。任意后台或人工正式刷新成功都删除临时失败字段并恢复当前结果。
- 确定性错误包括 `401/403` 鉴权失败、账户缺少 Base URL 或单一 API Key、自定义查询缺少配置、跨 Origin 路径以及明确的本地配置校验失败。确定性错误不等待阈值，立即保存 `failed` 并清除旧金额。
- 内置适配器会继续按偏好和全量回退顺序识别。只要本轮存在超时、网络、`408/429/5xx` 等临时错误且最终未命中，就按临时错误累计；全部候选都形成确定性不匹配时才立即失败。
...
- 后台临时错误前两次保留上次成功余额或显示“待重试”，连续第 3 次才显示“查询失败”并清空金额；确定性错误立即失败，任意成功清零失败次数。
