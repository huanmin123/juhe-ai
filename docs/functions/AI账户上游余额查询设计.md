...
- 确定性错误包括 `401/403` 鉴权失败、账户缺少 Base URL 或单一 API Key、自定义查询缺少配置、跨 Origin 路径、全部内置适配器确定性不匹配以及明确的本地配置校验失败。后台保存 `status='unsupported'` 和可解释原因，并继续写入下一次刷新时间；错误类型只影响展示与诊断，不决定是否继续调度。
- 多 Key 不属于确定性查询错误。账户写入边界会先自动关闭余额查询，因此 worker 和人工刷新不应收到一个“已启用但包含多个 Key”的有效账户状态。
- 内置适配器会继续按偏好和全量回退顺序识别。只要本轮存在超时、网络、`408/429/5xx` 等临时错误且最终未命中，就按临时错误累计；全部候选都形成确定性不匹配或只返回 `unsupported` 时保存诊断并等待下一周期。
