...
- OAuth 不开放；API Key 账户可以使用单 Key 或多 Key 查询。多 Key 逐 Key 请求只返回掩码明细，只有所有结果明确属于独立 Key 配额且单位一致时才安全合计。

当前 Node 任务所有者支持上述多 Key 查询。`backend-go` 仍处于迁移阶段，本次不改动其 J2 单 Key handover；当 Go 任务所有者开启时，Node 手动刷新入口会明确拒绝多 Key 请求，避免进入不兼容的 Go 契约。Go 迁移完成后需同步逐 Key 输入、结果和投影契约，再取消该门禁。
...
多 Key 且可安全合计时显示 `总剩余：$890.17`；账户级共享额度显示 `共享余额：$890.17`；口径未知、结果部分失败或单位不一致时不显示伪造总额，改显示诊断状态，点击金额查看每个 Key。
...
| 多 Key 部分失败 | `部分失败（8/10）` | 不把成功子集当作总余额 |
| 多 Key 不可合计 | `无法安全合计` | 保留逐 Key 明细和原因 |
...
只有 `type=api_key`、至少一个有效 Key、非授权实例账户可以开启。多 Key 时前端提示逐 Key 查询和安全合计规则；后端根据最终合并后的凭据执行同一规则，不能依赖前端隐藏或关闭开关。
...
| `balance_query_next_refresh_at` | TEXT / timestamptz NULL | `NULL` | 已开启余额的自动刷新到期时间，或首次激活后未配置余额的自动探测意图；业务库中的唯一调度事实 |
...
- 最终有效 API Key 数量必须至少为 1；多 Key 不再自动关闭余额查询，保存后按 Key 查询。
- Key 集合、Base URL、代理或配置变化时刷新代次立即失效，旧查询不能写回新凭据集合。
...
interface RelayBalanceSnapshot {
  ...
  scope?: 'key' | 'account' | 'unknown'
  aggregation?: 'sum' | 'shared' | 'unknown'
  keyCount?: number
  queriedKeyCount?: number
  keyBalances?: Array<{
    keyFingerprint: string
    maskedKey: string
    status: RelayBalanceSnapshot['status']
    remainingUsd?: string
    scope?: 'key' | 'account' | 'unknown'
    errorMessage?: string
...
- `keyBalances` 只写统计库，列表投影必须剥离；明细接口按需返回掩码 Key。逐 Key 记录与当前配置代次整批替换，禁止混代拼接。
...
- 多 Key 的部分失败、口径不一致和未知 scope 是余额诊断，不改变账户状态或用户开关。
...
1. 按索引读取状态可用、启用余额查询、非授权实例、至少一个 API Key 的到期物理账户，worker 单轮总容量为 12 个，其中包含自愈预留名额。
...
7. `account-balance-refresh` 的调度状态只反映本地系统是否完整执行：完整 HTTP、网络或上游超时、中断（包括已收到响应头及部分响应体后的响应体传输中断）、候选截止、适配器不支持与字段解析不匹配都是账户级余额诊断，快照照常保留，整轮任务记为成功，不产生 `partial`、失败计数或告警。租约占用、运行态延期和配置代次过期同样是可恢复的正常调度结果。只有候选扫描、运行态加载、DB/IPC、快照或延期持久化、租约基础设施、程序异常及调度器自身超时等本地错误才允许冒泡为任务失败。未知程序或依赖异常不是外部余额诊断，必须保留原始错误并冒泡；仅在上游请求或响应体读取的传输边界识别到通用网络请求异常时，才显式封装为临时余额诊断。
...
- 仅物理账户所有者或管理员可调用。
- 正式刷新不接收临时配置，账户必须已经保存、启用余额查询且为自有物理账户；人工刷新不受账户状态、可调度性或运行态限制。
...
- 服务端只构造内存账户快照并请求上游，复用当前 Base URL、全部有效 API Key、代理和适配器；多 Key 仍遵守逐 Key 查询与安全合计规则。
...
2. 领取时重新读取账户，只接受 `active`、可调度、未删除、非授权实例、至少一个 API Key、`balance_query_enabled=false`、`balance_query_config_json='{}'` 且配置版本仍当前的账户。时间计划关闭、`disabled`、`pending_test`、`error` 和不可调度账户不发起自动上游请求；意图保留到恢复活动可调度后再领取。
...
- 多 Key 不属于确定性查询错误；worker 和人工刷新可以处理已启用的多 Key 账户，部分失败和未知口径只作为余额诊断。
...
- 创建账户时默认关闭；首次创建链路投递的健康检查成功后可按第 7 节自动开启。保存时选择 `active` 的未配置单 API Key 账户也必须投递一次该检查，但保持 `active` 状态不变；这避免保存前测试或克隆来源状态绕过余额自动探测。
- 编辑配置后如果开启，清空旧适配器快照并立即到期；如果关闭，保留配置但删除 `relay_balance` 快照，列表立即隐藏。
- 创建或编辑多 Key 账户时保留用户明确提交的余额开关；Key 集合变化会清理旧快照并立即启动新的查询代次。
- 多 Key 账户减少为单 Key 后继续按当前开关工作，但旧 Key 快照不得复用到新集合。
- 有效 Key 数量以凭据归一化后的去空白、去重结果为准。标准创建、完整编辑、基础编辑、管理端、自有账户接口、批量编辑和导入最终都进入同一写入规则；不能在路由层用局部凭据或仅靠前端隐藏判断。
- 配置版本递增与余额代次更新在同一业务库事务完成；Key 集合、Base URL、代理或适配器变化后，统计侧旧 `relay_balance` 快照必须被条件写保护淘汰。
...
- 自动刷新只处理到期且可用的物理 API Key 账户，单轮有界且稳定排序。
...
- 新建多 Key 账户即使请求携带余额开启状态也能正常保存，最终保留 `balanceQueryEnabled=true` 和余额刷新时间，并按逐 Key 规则查询。
- 多 Key 草稿点击“测试查询”时逐 Key 请求；只有明确独立 Key 配额才显示总额，否则返回逐 Key 诊断。
- 单 Key 已开启账户增加第二个有效 Key 时保存成功，旧快照失效，新的多 Key 代次按规则查询。
- 多 Key 账户减少为单 Key 后按当前开关继续工作，旧 Key 结果不能写回。
- 前端隐藏、基础编辑、完整编辑、管理端和直接 API 写入都不能绕过后端的 Key scope、权限和代次校验。
...
- 因余额接口不支持、鉴权失败、网络失败或多 Key 部分失败，不自动关闭用户开关；这些都属于余额诊断，不改变账户调度事实。
