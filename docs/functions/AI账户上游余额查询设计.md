...
> 状态：既有核心实现完成；2026-07-12 补充查询修复、`/user/balance` 与新账户非阻塞自动探测设计；2026-07-14 完成多 Key 账户自动关闭实现，待随统一批次发布验证。
>
...
| `unsupported` | `剩余：已暂停` | 当前配置未找到可用余额能力，用户开关保留但后台不再轮询 |
| `failed` | `剩余：查询失败` | 当前查询失败，悬浮显示本次脱敏错误 |

第 3 次连续临时失败后不展示最后一次成功金额，只保留失败状态与错误摘要；前两次临时失败保留已有成功结果。确定性不支持进入 `unsupported` 暂停状态。下一次成功后恢复金额并清零失败序列。
...
内置适配查询顺序固定为：先尝试 `preferredBuiltinAdapter`，再按 `sub2api -> newapi -> litellm -> user_balance` 补齐尚未尝试的规则。任一规则得到 `fresh` 或真正的账户级 `unlimited` 即成功并条件更新偏好；原偏好失败而其他规则成功时，用新规则替换偏好。全部规则都形成确定性不匹配或只返回 `unsupported` 时返回能力不支持并暂停后台调度；只要本轮存在临时错误，就进入连续失败并按账户刷新周期等待下一轮，不误判为不支持。账户 Base URL、API Key 或代理变更后不需要用户重新选择中转类型，下一次查询会自动完成上述回退和偏好修正。
...
3. 以小并发执行查询；自动和手动查询复用现有 `background_job_leases`，以 `account-balance:<accountId>` 为租约 key、30 秒为租期，保证跨进程同一账户同一时刻只有一个查询。
4. 成功时写当前快照并按用户配置周期安排下次刷新；临时失败按第 8 节累计并继续使用同一用户配置周期；确定不支持时写暂停快照并把 `balance_query_next_refresh_at` 置空。
5. 暂停只停止余额后台调度，不修改账户状态、可调度性或用户开关。用户修改并保存账户关键配置，或人工刷新成功时，重新写入下次刷新时间并恢复后台周期。
...
- 仅物理账户所有者或管理员可调用。
- 正式刷新不接收临时配置，账户必须已经保存并启用余额查询且当前可用。
- 请求同步等待外部查询，服务器超时 15 秒；前端图标在请求期间旋转。
- 同一账户已有查询时返回当前 `refreshing` 状态，不重复发起上游请求。
- 成功、无限和失败都返回本次查询结果 DTO；只有本地权限或参数错误使用 4xx。失败 DTO 只用于本次人工反馈，不代表已持久化状态。
- 手动刷新成功时更新 `next_refresh_after`，避免随后被自动任务重复查询；失败时保持原调度时间。
- 前端收到最新快照后必须替换当前页的目标账户对象和列表数组引用，只重渲染该行余额；禁止为了显示新金额重新请求整张账户列表。当前列表使用 `shallowRef`，不能只修改数组内部对象的 `balanceSnapshot`。
- 列表人工刷新与后台失败判定分离：人工刷新成功时保存新快照、清零后台临时失败次数并更新当前单元格；人工刷新失败时立即通过顶部消息返回本次错误，但不写失败快照、不累计后台失败次数、不推进自动刷新时间，也不覆盖列表中已有金额。
- 人工刷新允许对已暂停账户重新检测；成功时解除暂停并恢复用户配置周期，失败时保持暂停。
...
- 服务端只构造内存账户快照并请求上游，复用当前 Base URL、单 API Key、代理和适配器。
...
- 临时错误包括网络连接异常、请求取消或超时、HTTP `408`、`429` 和 `5xx`。连续第 1、2 次临时失败只更新 `consecutiveTransientFailures`、`lastTransientErrorMessage`、`lastTransientFailureAt`，不把余额状态改成 `failed`；第 1、2、3 次及后续失败都按账户 `intervalMinutes` 安排下一次查询，不另行覆盖用户配置周期。
...
- 连续第 3 次临时失败才保存 `status='failed'`、清除旧金额并显示最近错误；后续临时失败把计数封顶在 3，并继续按账户配置周期查询。任意后台或人工正式刷新成功都删除临时失败字段并继续用户配置周期。
...
- 内置适配器会继续按偏好和全量回退顺序识别。只要本轮存在超时、网络、`408/429/5xx` 等临时错误且最终未命中，就按临时错误累计；全部候选都形成确定性不匹配或只返回 `unsupported` 时才暂停。
- 失败次数随现有 `relay_balance` 快照 JSON 保存，不新增数据库列、不重建非业务 schema。配置关闭或更换后删除旧快照，自然清零失败序列。
- 暂停状态复用当前业务字段：`balance_query_enabled=1` 表示用户意愿仍开启，`balance_query_next_refresh_at=NULL` 表示后台暂停，`relay_balance.status='unsupported'` 保存可解释原因。用户保存 Base URL、API Key、代理或余额配置时重新到期；列表人工刷新成功也解除暂停。
...
- NewAPI `unlimited_quota=true` 不再显示“无限”；其他适配器仍未命中时显示“已暂停”，用户开关保持开启且后台不再轮询。
- 确定性不支持只暂停余额调度，不关闭用户开关；修改账户配置或人工刷新成功后恢复。
- 列表人工刷新不会重复并发请求；失败时立即提示本次错误，但不写失败快照、不累计后台失败次数、不推进调度时间或覆盖已有金额。
