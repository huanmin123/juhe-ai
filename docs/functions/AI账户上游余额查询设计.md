...
内置适配查询顺序固定为：先尝试 `preferredBuiltinAdapter`，再按 `sub2api -> newapi -> litellm -> user_balance` 补齐尚未尝试的规则。任一规则得到 `fresh` 或真正的账户级 `unlimited` 即成功并条件更新偏好；原偏好失败而其他规则成功时，用新规则替换偏好。全部规则都形成确定性不匹配或只返回 `unsupported` 时返回能力不支持并暂停后台调度；只要本轮存在临时错误，就进入连续失败和退避，不误判为不支持。账户 Base URL、API Key 或代理变更后不需要用户重新选择中转类型，下一次查询会自动完成上述回退和偏好修正。
...
4. 成功时写当前快照并按用户配置周期安排下次刷新；临时失败按第 8 节累计和退避；确定不支持时写暂停快照并把 `balance_query_next_refresh_at` 置空。
...
- 后台临时错误前两次保留上次成功余额或显示“待重试”，连续第 3 次才显示“查询失败”并清空金额，重试间隔按 15/30/60 分钟退避；任意成功清零失败次数并恢复用户周期。
