...
`Responses -> Chat` 成功转换后，切号时必须使用两个已经确定的结果：

- `SwitchTarget`：实际上游供应商、模型、协议族和精确 endpoint mode；当前 bridge 的上游载体是 `chat_sse`。
- `ConvertedContext`：已恢复并归一化的 Chat / canonical 上下文，包含可迁移的 messages、工具调用、工具结果和必要 reasoning，不再依赖原始 Responses 状态。

因此，客户端仍然是 `Responses` 只说明下游返回要渲染为 Responses；它不再决定切号候选的协议。只要上下文已经物化为 Chat，下一账号可以是没有 Responses 能力、也没有相同 mapping 左侧的 Chat 账号，但必须支持相同 RHS 模型、`chat_completions` 和精确 Chat mode。候选请求必须从 `ConvertedContext` 重新序列化，不能把原始 Responses body 或上一账号已经改写的最终 body 直接发给下一账号。

相反，外部原生 `previous_response_id`、仍需原上游解开的 `encrypted_content` 或未完成的 Responses 状态链不能被当作 Chat 上下文。此类请求只能继续原生 Responses 的同供应商 / 兼容 profile 账号；没有可承接账号时受控失败，不得为了切号把状态交给 Chat 或其他供应商。

该切号契约的完整字段、冻结时点、供应商例外和验收矩阵见 [切号时有效上游目标与上下文迁移设计](切号时有效上游目标与上下文迁移设计.md)。
...
2. 每次实际派发前，按本次选中的账号从规范请求体或已经冻结的 `ConvertedContext` 渲染上游请求；切号重试时不得复用上一账号改写后的最终 body，但必须复用同一份已经物化的上下文和 `SwitchTarget`。
...
- 显式 Responses -> Chat 映射账号一旦完成上述转换，后续未提交切号以 Chat 目标为准；候选不必支持客户端 Responses，但必须能够序列化同一份 Chat / canonical 上下文。
