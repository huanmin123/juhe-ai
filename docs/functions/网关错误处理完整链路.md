...
真实网关流量里的 transport / timeout 失败先救当前请求，后由独立后台探针确认账号健康。通用客户端不解析错误对象、错误类型、错误码、完成状态或文案；完整非 `2xx` 默认原样返回，只有账户所有者或管理员显式配置的账户错误策略命中并声明 `retry_next` 时才切换账户，并且不在同一账户内换 Key 重放。精确识别的 Codex、Claude Code、Gemini CLI 客户端可进入各自声明的系统默认协议语义，但也不能把通用 HTTP 状态码或供应商文案隐式升级为跨账户重放规则。
...
flowchart TD
  ...
  H -- "收到完整响应，generic_*" --> J["推理端点仅按 response.ok 接管；资源 POST 原样转发"]
...
通用客户端的非流式响应不解析正文语义；明确推理端点的非 `2xx` 可按 `response.ok` 接管，资源型或未知 POST 原样转发：
...
通用客户端的完整非 `2xx` 默认不解析 `invalid_request_error`、`rate_limit_exceeded`、余额文案、未知错误 JSON 或供应商自定义状态，而是复制上游状态、安全响应头和正文后结束调度。账户所有者显式配置的账户错误策略可以匹配响应并执行其声明动作；只有命中并声明 `retry_next` 时，当前请求才会切换到账户候选。

- 完整 HTTP 非 `2xx` 未命中显式 `retry_next` 时，不切换账户，也不轮换同一账户内 API Key。
- 未命中显式账户错误策略或显式响应拦截时，不建立账户运行态、不投递 `recovery_wait`、不写账号状态。
- 资源型和未知 POST 同样默认不切号，也不确认 half-open 成功。
...
审计和使用记录仍可记录上游状态和有界摘要。显式 `retry_next` 或 transport / timeout 切换耗尽后由网关返回最终失败；未触发切换的完整 HTTP 非 `2xx` 保留原始状态和正文，由客户端自行决定是否重试。
...
| 通用客户端收到完整非 `2xx` | 上游完整 HTTP 失败 | 默认否；仅显式账户错误策略命中 `retry_next` 时切账户 | 系统自动路径不写状态；用户显式账户错误或响应拦截策略命中时按配置执行 |
| 通用客户端资源型或未知 POST 收到完整响应 | 上游透明响应 | 否，原样转发状态 / 响应头 / 正文 | 否 |
| 精确客户端命中显式响应 / 账户策略 | 客户端专属语义 + 用户规则 | `semanticCommitted = false` 时按策略执行 | 按用户显式配置；不得扩散到通用客户端 |
| 精确客户端命中异常完成语义 | 客户端专属响应语义 | 语义未提交时可按客户端契约重试；提交后不能拼接 | 系统默认语义只影响本次请求，不写账户运行态；只有用户显式策略可执行其配置动作 |
...
2. 通用客户端不解析错误语义；完整非 `2xx`、`200 + error` 和 SSE 失败事件默认保持透明，只有显式账户错误策略命中 `retry_next` 才允许因完整 HTTP 失败切号。
