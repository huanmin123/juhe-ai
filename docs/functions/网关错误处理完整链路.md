...
- 多 IP、共享 API Key、高并发和恶意错误请求下，哪些机制只影响来源，哪些机制会因为真实命中上游账号而写账号状态。
...
只要请求已经命中上游账号，上游错误就写当前命中的账号状态。状态码、错误码和错误文案只用于诊断、审计和用户显式账号规则输入，代码不再内置“余额不足”“限流”“某状态码才写状态”这类判断。
...
flowchart TD
  ...
  F5 --> G["逐个账号请求上游"]
  G --> H{"上游结果"}
  H -- "请求异常" --> I["记录失败；按临时状态配置原地重试，用尽后短期屏蔽当前账号、写账号状态、切后续账号"]
  H -- "HTTP 非 2xx" --> J["记录失败；按临时状态配置原地重试，用尽后按规则或通用策略写账号状态并切号"]
...
- 响应头前等待和 `2xx` 响应首字节前等待都受 `streamRequestTimeoutSeconds` 表达的上游首包等待上限约束；超时发生在下游未收到任何响应内容前，按上游请求异常写当前账号状态并切换后续账号。
...
非流式 `2xx` 过程中如果客户端取消或连接断开，按客户端生命周期记录 `client_aborted`，释放账号并发槽，不写账号失败状态。非流式首字节已经写给下游后，如果正文中途异常，网关不能在同一个 HTTP 响应里安全重放半个 JSON 或半段响应，因此不做服务端透明换号；网关会记录 `upstream_body_interrupted`，忘记当前会话亲和，短期本地避让该账号，写当前账号状态，并主动打断下游连接，让客户端按网络读取失败自行重试。
...
| `response.created`、`response.in_progress`、metadata、心跳后中断 | 失败，但未产生可见输出 | 写当前账号状态 |
| 首段前超时、首段后无任何上游新数据、上游 EOF、缺少终止事件 | 失败 | 写当前账号状态 |
| 上游 `response.failed` 或通用 `event: error` | 失败 | 写当前账号状态 |
| 已输出文本、reasoning、工具参数或可改变 assistant 状态的 output item 后失败 | 失败 | 写当前账号状态；同时记录上游桶运行态用于后续候选排序 |
...
可见输出前失败时，只有精确命中 Codex profile、Responses SSE，并解析到普通 JSON 对象格式的 `x-codex-turn-metadata.turn_id` 时，才改写为 Codex 可重试的 `response.failed/upstream_retryable_error`。普通 OpenAI-compatible 客户端不伪造 Codex 可重试码，也不触发 Codex 第 4 次 turn 级切号。无论是否命中 Codex profile，流式失败都会写当前账号状态。

可见输出后失败时，服务端不能安全重放半段回答或工具参数。网关会写当前账号状态，并把账号记入上游桶运行态：同一 `proxy`、`baseUrl` 或 `provider` 在短窗口内影响多个不同账号时，后续请求优先避开同桶账号。`cyber_policy` 是 Codex profile 下已输出后仍允许改写为客户端重试的显式例外。

## 非 2xx 如何处理

非 `2xx` 上游响应不再有通用代码内置错误特征规则，也不会因为 `invalid_request_error`、状态码或错误文案进入特殊分支。上游错误体只作为账号规则输入和排障摘要；未命中显式账号规则时按通用上游失败写当前账号 `temporary_unavailable`。
...
4. 原地重试用尽仍失败后，立即忘记当前账号的会话亲和，并把当前账号加入本进程短 TTL 本地屏蔽。
5. 先匹配账号自带 `credentials.error_handling_rules`；账号自带规则默认不配置，只有用户手动添加时才参与匹配。配置入口只允许填写非 `2xx` HTTP 状态码，状态码或错误码字段都不能把 `200` 这类成功码写进错误策略；`200 + SSE` 协议级失败由流式失败机制处理。
6. 命中账号策略时，按策略异步写 `rate_limited`、`temporary_unavailable` 或 `error`；未命中策略或命中 `retry_next` 时，按通用上游失败写 `temporary_unavailable`。
...
10. 后续请求进入候选排序时会先过滤本地屏蔽账号；如果所有候选都被屏蔽，则等待最早屏蔽释放或等待预算耗尽。
11. 等待释放后有账号可用则继续调度；等待超时仍无账号可用时，返回“所有上游账户正在临时隔离，请稍后重试”。

`invalid_request_error` 的处理也遵守这套顺序：

- 如果后续账号成功，本次请求应救回，前序账号短期屏蔽且已写本次失败状态。
- 如果所有账号都返回失败，仍返回网关统一错误，不把上游 `invalid_request_error` 原文直接给客户端。
- 无论是否命中账号自带错误策略，只要已经命中上游账号并发生上游错误，都写当前账号状态；显式策略只决定写成限流、临时不可调用还是异常。
...
| 上游请求异常、连接失败、EOF | 上游账号调用失败 | 按临时状态重试配置原地确认，用尽后短期屏蔽当前账号并切号 | 写临时不可调用 |
| 非 `2xx` 命中账号错误策略 | 账号规则 + 当前失败 | 按临时状态重试配置原地确认，用尽后短期屏蔽当前账号并切号 | 按策略 |
| 非 `2xx` 未命中账号策略或命中 `retry_next` | 通用上游失败 | 按临时状态重试配置原地确认，用尽后短期屏蔽当前账号并切号 | 写临时不可调用 |
| 后续账号成功 | 单账号或通道可绕开 | 已救回 | 前序失败账号已写状态；可提交当前 IP 回避 |
| 多账号均失败 | 账号池短期不可用或公共上游失败 | 已扫完候选 | 命中过上游的账号都已写状态，返回网关统一错误 |
| 所有候选都处于本地屏蔽 | 本进程短期调度保护 | 等待释放或超时 | 不新增账号状态 |
| `200 + SSE` 可见输出前失败 | 流式协议失败 | 不服务端换账号 | 写临时不可调用 |
| `200 + SSE` 可见输出后失败 | 流式协议失败 | 不服务端换账号 | 写临时不可调用，并记录上游桶运行态 |
| 慢客户端背压 | 下游读取慢 | 否 | 否 |
| 客户端断开 | 客户端生命周期 | 否 | 否 |
...
- 代理 profile 已知不可用。
- 上游失败写账号状态、账号错误策略等路径也可以同步设置本地屏蔽，加速当前进程避让。
...
- 该状态不写 SQLite，服务重启或进程重启后丢失；持久账号状态由真实上游账号失败、账号策略、后台复测或人工操作写入。

### 上游桶运行态失败

上游桶运行态失败只影响候选排序，不自动禁用代理、不自动禁用上游；已经实际命中上游并失败的账号仍会按通用失败写状态。
...
不同 IP 不会因为来源维度本身凭空写账号状态；但只要某次请求实际命中上游账号并发生上游错误，该账号会写入本次失败状态。为了避免大量请求继续打到刚失败的账号，进程内本地账号屏蔽会在短 TTL 内影响同进程后续来源。
...
这些来源和排序状态本身不写账号 `status`、`schedulable`、`cooldown_until` 或 `last_error_code`；其中本地账号屏蔽只存在于当前 Web 进程内，TTL 到期或进程重启后恢复。自有账户和分组授权账户按命中 `account_id` 屏蔽；账户授权实例也按自己的实例 `account_id + 使用方系统账户 + 本地分组 + 最终授权 ID` 屏蔽，避免授权方原账户或其他授权实例被同一个短 TTL 屏蔽误伤。

账号会被写入持久状态的情况：

- 真实命中上游账号后的请求异常、非 `2xx`、非流式正文中断和流式失败；账户授权实例只写当前命中的实例账户行，不写授权方原账户，也不写 `group_accounts.local_*`。
- 账号错误策略显式命中时，按规则写 `rate_limited`、`temporary_unavailable` 或 `error`；未命中规则或命中 `retry_next` 时按通用失败写 `temporary_unavailable`。
- 后台恢复探活只负责自动恢复和退避，不负责把冷却态升级为永久异常。
- 管理员手动操作。

也就是说，IP 维度用于隔离来源和优化排序；账号状态只由真实命中上游账号后的错误或显式管理动作写入。
...
2. 如果是非 `2xx` 或上游请求异常，当前账号应该先进入进程内短期屏蔽、写入账号状态，并继续切后续账号。
3. 如果错误体包含 `invalid_request_error`，不要直接判断请求不可恢复，也不要把它作为特殊分支；仍按统一失败流水线处理。
4. 如果后续账号成功，本次请求应救回；前序失败账号仍应保留短期本地屏蔽并写入本次失败状态。
...
8. 流式失败不再按可见输出、状态码、错误码或阈值决定是否写账号状态；只要命中上游账号后失败，就写当前账号状态。
9. 客户端取消、慢客户端背压、本地容量满都不代表账号坏。
10. 单 IP 错误熔断只消费高置信本地请求错误，不消费上游账号失败。
11. 写账号状态的核心边界是“是否真实命中上游账号后发生错误”；不要恢复状态码、错误码或错误文案硬编码判断。
