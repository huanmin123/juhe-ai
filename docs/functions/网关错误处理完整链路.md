...
| 精确客户端命中异常完成语义 | 客户端专属响应语义 | 各类请求在语义未提交且协议/预算门禁满足时可按客户端契约重试；提交后不能拼接 | 系统默认语义只影响当前请求，不写账户运行态；只有用户显式策略可执行其配置动作 |
...
| 精确客户端 SSE 结构失败且语义未提交 | 流式协议失败 | 各类请求都优先服务端切后续账号 / 后续分组；耗尽后按客户端策略兜底 | 只影响当前请求；不得投递账户核实或写共享质量/状态，除非同时存在独立 transport failure 或用户显式动作 |
...
2. 通用客户端不解析错误语义；各类请求的非 `2xx` 仅按 `response.ok` 有界轮换 Key / 账号，`200 + error` 和普通 SSE payload 保持透明。opaque 路径不得触发共享账号副作用，用户显式状态策略也不改变自动切号准入。
