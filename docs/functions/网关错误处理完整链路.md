...
flowchart TD
  ...
  F5 --> G["逐个账号请求上游"]
  G --> H{"上游结果"}
  H -- "请求异常" --> I["记录失败，短期屏蔽当前账号，写账号状态，切后续账号"]
  H -- "HTTP 非 2xx" --> J["记录失败，短期屏蔽当前账号，按规则或通用策略写账号状态"]
...
4. 先匹配账号自带 `credentials.error_handling_rules`；账号自带规则默认不配置，只有用户手动添加时才参与匹配。配置入口只允许填写非 `2xx` HTTP 状态码，状态码或错误码字段都不能把 `200` 这类成功码写进错误策略；`200 + SSE` 协议级失败由流式失败机制处理。
5. 命中账号策略时，按策略异步写 `rate_limited`、`temporary_unavailable` 或 `error`；未命中策略或命中 `retry_next` 时，按通用上游失败写 `temporary_unavailable`。
...
| 上游请求异常、连接失败、EOF | 上游账号调用失败 | 立即短期屏蔽当前账号并切号 | 写临时不可调用 |
| 非 `2xx` 命中账号错误策略 | 账号规则 + 当前失败 | 立即短期屏蔽当前账号并切号 | 按策略 |
| 非 `2xx` 未命中账号策略或命中 `retry_next` | 通用上游失败 | 立即短期屏蔽当前账号并切号 | 写临时不可调用 |
