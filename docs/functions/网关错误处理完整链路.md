...
真实网关流量里的 transport / timeout 失败先救当前请求，后由独立后台探针确认账号健康。通用客户端不解析错误对象、错误类型、错误码、完成状态或文案；对明确的推理端点只用 `response.ok` 判断本次 attempt 是否成功，完整非 `2xx` 在下游尚未提交时可切下一账户，整个请求跨后备分组最多尝试 4 个不同账户，并且不在同一账户内换 Key 重放。资源创建和未知 POST 不获得跨账户重放能力，完整响应原样返回。精确识别的 Codex、Claude Code、Gemini CLI 客户端可进入各自声明的系统默认协议语义；账户所有者或管理员显式配置的错误策略和响应拦截在通用推理端点同样生效。
...
通用客户端的完整非 `2xx` 默认不解析 `invalid_request_error`、`rate_limit_exceeded`、余额文案、未知错误 JSON 或供应商自定义状态。明确推理端点仅依据 `response.ok=false` 在有界候选内接管；账户所有者显式配置的账户错误策略可以额外匹配响应并执行其声明动作。资源创建型和未知 POST 复制上游状态、安全响应头和正文后结束调度，不读取正文执行策略，避免为检查策略消费或重放一次性资源请求。

- 推理端点可以切换不同账户，但整个请求最多尝试 4 个不同账户；不得轮换同一账户内 API Key。
- 未命中显式账户错误策略或显式响应拦截时，不建立账户运行态、不投递 `recovery_wait`、不写账号状态。
- 资源型和未知 POST 不切号、不执行账户错误或响应检查策略，也不确认 half-open 成功。
...
审计和使用记录仍可记录上游状态和有界摘要。推理端点接管耗尽后返回统一可重试 `503`；不可重放端点保留原始状态和正文，由客户端自行决定是否重试。
...
| 通用客户端推理端点收到完整非 `2xx` | 请求级不透明失败 | 是，按 `response.ok` 有界切账户，不在同账户换 Key | 系统自动路径不写状态；用户显式账户错误或响应拦截策略命中时按配置执行 |
...
2. 通用客户端不解析错误语义；明确推理端点的非 `2xx` 仅按 `response.ok` 有界切号，资源型 POST、`200 + error` 和 SSE 失败事件保持透明，所有路径都不得触发账号副作用。
