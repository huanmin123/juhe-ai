...
新建账户默认和需要重新确认连接配置的账户进入 `pending_test`，在后台检查通过前保持 `schedulable = false`。管理页面的新建表单只提供“待检查”和“停用”；要在未检查时承担风险，操作者必须在创建后通过“恢复可调度”进行显式确认。该确认仍受账户时间计划和套餐过期状态约束，且不改变导入或内部创建路径的默认待检查语义。
