...
本文固定 AI 账户后台检查失败后的复检、终止和人工操作语义，避免账户长期停留在没有明确终点的 `pending_test` 或长期不可用状态。多模型 active 账户的精确能力状态以 [AI 账户多模型能力健康与精确隔离设计](AI账户多模型能力健康与精确隔离设计.md) 为准，模型能力层不得写 `accounts.status`。
...
- `framing_complete_neutral`：framing 完整但协议 / 业务未成功；对 transport 电路和冷却恢复保持中性。`pending_test` 激活把它作为账户连接可用性失败；active 周期哨兵、请求派生探针和质量确认只把它作为匹配 Route / Attempt 的 execution 失败。
- `upstream_failure`：仅限连接失败、硬超时、读取中断或 framing 未完成的 `transport_incomplete`；它是 transport 状态机负向证据，也是 pending_test 的连接失败或 active 精确能力的一次 execution 失败，不能从子 scope 升级整号。
...
- `last_health_check_*` 保存最近一次诊断；pending_test 的 `transport_incomplete` 与 `framing_complete_neutral` 都增加激活失败计数，任务故障和 stale/unknown 只更新有界调度。active 的相同结果只更新 v1 哨兵和精确能力，不授权账户状态转换。
...
字段为空表示当前没有连续的待检查连接可用性失败窗口。生产 PostgreSQL 上线前必须执行 `000042_w1b_account_health_failure_window.sql`；运行时代码不提供旧结构兼容或自动补列。完整多模型目标不新增 SQLite 长期 adapter，最终由 Go + PostgreSQL owner 维护。

状态终止错误码：

| 场景 | 错误码 |
| --- | --- |
| 待检查从首次独立连接可用性失败起满 24 小时 | `account_activation_check_timeout` |
...
- pending_test 激活写回继续使用 `config_revision` 和真实成功时间守卫；配置已变化或检查期间出现更新成功信号时，旧失败不得覆盖新状态。active 精确能力结果使用 Route / Attempt revision、generation 和 ledger CAS；业务成功只写 observation，不能终结 active intent。
- 终态转换 SQL 同时校验当前状态，已被人工停用或其他任务改变的账户不得被迟到结果改写。

## 8. 验证

- `account-pending-test-regression`：完整 HTTP / 协议失败与独立 transport 失败的 1 小时复检、24 小时终态、重新检查和异常恢复；不存在人工跳过激活旁路。
