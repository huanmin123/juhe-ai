...
- `server.ts` 继续承载 `/__aisys__`、`/*`、`/v1/*`、静态资源、DB service 和 worker 看护。
- `public-api.ts` 只承载 `/__aipublic__/*`，默认不提供后台页面和网关入口。
...
| 进程 | 职责 | 禁止事项 |
| --- | --- | --- |
| 主 Web/网关进程 | 后台静态资源、`/__aisys__/api/*` 代理、网关请求、网关运行态、DB service / worker 看护 | 不挂载 `/__aipublic__`，不解析公开接口 body，不执行公开接口鉴权 |
