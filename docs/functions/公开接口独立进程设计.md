...
- 不把公益站用户、排行榜业务快照、IP 到用户归属、消耗聚合公开读取或公益接入信息放进 `sub2api-lite`。
...
flowchart LR
  ...
  PublicApi --> PublicSnapshot["公开对账快照 / 索引读模型"]
...
| 接口 | 数据来源 | 请求路径禁止 |
| --- | --- | --- |
| API Key 列表 | 当前用户控制面索引查询或后续公开对账快照 | 返回批量 API Key 明文、扫描使用记录 |
| 路由策略列表 | 当前用户策略路由索引和分组绑定摘要 | 返回账号凭据、绕过策略归属校验 |
| 分组列表 | 当前用户分组索引和绑定数量摘要 | 返回上游凭据、全量展开账号详情 |
| AI 账户列表 | 当前用户账户索引和分组绑定摘要 | 返回上游 API Key、OAuth token、代理密码 |

推荐演进：

- 第一阶段：独立 `public-api` 进程仍复用现有 repository，但必须保持查询窗口、分页上限和索引约束。
- 第二阶段：worker 刷新 `public_*` 对账快照表或独立公开快照 SQLite 文件，`public-api` 读快照，不直接读核心业务表的大范围窗口。
...
反向代理层仍建议独立限流公开域名，尤其是 `/__aipublic__/account/*`、`/__aipublic__/route-strategy/*`、`/__aipublic__/api-key/*` 和 `POST` 写入路径。
...
| 公开对账快照滞后 | 返回快照滞后字段或回退受控索引查询；不现场扫描明细 |
...
- 主端口 `http://127.0.0.1:3000/__aipublic__/api-key/list` 返回 404 或明确不可用。
- 公开端口 `http://127.0.0.1:8788/__aipublic__/api-key/list` 使用正确来源 token 返回成功。
...
- API Key、路由策略、分组和 AI 账户公开列表不扫描使用明细，不临时聚合统计。
