...
- 新增或调整新版本响应污染检查、广告插入、OpenAI v1 Chat / Responses、SSE / JSON、上游强制流式、非流式 JSON 成功响应检查、其他协议适配器或统一响应语义帧时，优先更新 [响应语义检查管线设计](响应语义检查管线设计.md)。[流式拦截策略配置设计](流式拦截策略配置设计.md) 只记录旧版本 `200 + SSE` 专项能力；新版本不迁移旧策略数据，也不做运行时兼容。落地代码后再同步 [流式中断与客户端重试调研](流式中断与客户端重试调研.md)、[网关异常重试与兜底策略](网关异常重试与兜底策略.md) 和 [接口契约与权限矩阵](接口契约与权限矩阵.md)。
