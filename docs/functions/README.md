...
- 新增或调整账户短窗口热质量、同层唯一候选一轮、请求级精准切号、账户电路单飞、受控同层探索、route coordinator 账户结果，或可调度到临时不可调度、长期恢复、恢复验证和渐进放量的完整生命周期时，优先更新 [AI 账户短窗口热质量与精准切号设计](AI账户短窗口热质量与精准切号设计.md)，并按影响同步 [策略路由设计](策略路由设计.md)、[普通路由速度优先延迟切换设计](普通路由速度优先延迟切换设计.md)、[AI 账户运行态探针恢复设计](AI账户运行态探针恢复设计.md)、[AI 账户健康状态收敛设计](AI账户健康状态收敛设计.md)、[账号健康检测设计](账号健康检测设计.md) 和 [网关异常重试与兜底策略](网关异常重试与兜底策略.md)。
