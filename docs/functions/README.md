...
- [高性能模式同机多进程拓扑设计](高性能模式同机多进程拓扑设计.md)
...
- 新增或调整当前 Node performance 模式的同机多 Gateway、Usage / Log consumer 副本、Nginx 分流、进程实例 ID、旁路降级或 launchd 编排时，优先更新 [高性能模式同机多进程拓扑设计](高性能模式同机多进程拓扑设计.md)。跨物理主机才进入 [分布式部署与用户分片设计](分布式部署与用户分片设计.md)。
