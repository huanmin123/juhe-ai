...
- [AI 账户多模型能力健康与精确隔离设计](AI账户多模型能力健康与精确隔离设计.md)：定义模型映射后的精确健康作用域、按需探针单飞、局部阻断、半开恢复、整号全覆盖仲裁和完整前后端落地边界。
...
- 新增或调整多模型账户的模型级健康作用域、请求失败按模型触发探针、单飞、局部阻断、半开恢复、部分能力展示或账户级全覆盖仲裁时，优先更新 [AI 账户多模型能力健康与精确隔离设计](AI账户多模型能力健康与精确隔离设计.md)，再按影响同步检查模型、周期健康、运行态探针、错误语义、接口、存储、前端和部署文档。
...
- 新增或调整 Gemini API Key / Google OAuth、Gemini native `generateContent` / `streamGenerateContent` / `countTokens` / Interactions、`gemini-cli` 网关兼容、Gemini OpenAI compatibility 边界、Gemini 模型目录、Gemini usage / price 口径或 Gemini 账户测试时，优先更新 [Gemini 账号接入](Gemini账号接入.md)、[Gemini 协议兼容设计](Gemini协议兼容设计.md)、[厂商模型目录更新与清洗指南](厂商模型目录更新与清洗指南.md)、[模型价格与用量统计口径](模型价格与用量统计口径.md) 和 [自定义模型与模型映射设计](自定义模型与模型映射设计.md)。
- 新增或调整 xAI / Grok API Key、OpenAI Chat / Responses 兼容档案、Grok 模型目录、Grok usage / price 口径或 xAI 账户测试时，优先更新 [xAI / Grok 账号接入](xAI账号接入.md)、[厂商模型目录更新与清洗指南](厂商模型目录更新与清洗指南.md)、[模型价格与用量统计口径](模型价格与用量统计口径.md)、[AI 账户上游接口能力设计](AI账户上游接口能力设计.md) 和 [自定义模型与模型映射设计](自定义模型与模型映射设计.md)。
