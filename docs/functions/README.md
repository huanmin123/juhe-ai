...
- [上游 API Key 账户安全识别落地方案](上游APIKey账户安全识别落地方案.md)
...
- 新增或调整上游 API Key 类型 AI 账户的提示注入、本地执行风险、危险工具调用、安全识别开关、请求 / 响应侧安全阻断或安全命中审计时，优先更新 [上游 API Key 账户安全识别落地方案](上游APIKey账户安全识别落地方案.md)，再按影响同步 [请求处理分层设计](请求处理分层设计.md)、[原始审计日志设计](原始审计日志设计.md)、[安全与日志策略](安全与日志策略.md)、[接口契约与权限矩阵](接口契约与权限矩阵.md) 和 [SQLite 存储说明](SQLite存储说明.md)。
