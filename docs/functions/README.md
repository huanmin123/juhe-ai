...
- [AI 账户锁死调度机制调研与待审核设计](AI账户锁死调度机制调研与待审核设计.md)（待审核，未实现）
...
- “AI 账户锁死”仍处于需求审核阶段，当前只参考 [AI 账户锁死调度机制调研与待审核设计](AI账户锁死调度机制调研与待审核设计.md) 的调研结论和待决策项；未获得审核结论前，不得把锁死字段、锁死运行态或 `lock_policy` 状态写入实现。
