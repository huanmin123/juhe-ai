...
- [AI 账户多模型能力健康与精确隔离设计](AI账户多模型能力健康与精确隔离设计.md)：定义最终派发描述、凭据级能力作用域、按需探针单飞、局部阻断、半开恢复、派生全能力门禁和完整前后端落地边界；模型能力层不写整号状态。
...
- [Node 管理接口按需读写设计](../superpowers/specs/2026-07-28-Node管理接口按需读写设计.md)（仅 Node + Vue；Go 未触碰）
- [核心功能设计](核心功能设计.md)
...
- 新增或调整多模型账户的凭据级健康作用域、请求失败按最终派发描述触发探针、单飞、局部阻断、半开恢复、部分能力展示或派生全能力门禁时，优先更新 [AI 账户多模型能力健康与精确隔离设计](AI账户多模型能力健康与精确隔离设计.md)，再按影响同步检查模型、周期健康、运行态探针、错误语义、接口、存储、前端和部署文档。
