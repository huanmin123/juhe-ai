...
- 新增或调整账户短窗口热质量、同层唯一候选一轮、请求级精准切号、账户电路单飞、受控同层探索或 route coordinator 账户结果时，优先更新 [AI 账户短窗口热质量与精准切号设计](AI账户短窗口热质量与精准切号设计.md)，并按影响同步 [策略路由设计](策略路由设计.md)、[普通路由速度优先延迟切换设计](普通路由速度优先延迟切换设计.md)、[AI 账户运行态探针恢复设计](AI账户运行态探针恢复设计.md) 和 [网关异常重试与兜底策略](网关异常重试与兜底策略.md)。
