...
- 新增或调整 `200 + SSE` 内容污染、广告插入、流式拦截策略配置、供应商内置 preset 或账户级流式规则时，先更新 [流式拦截策略配置设计](流式拦截策略配置设计.md)；落地代码后再同步 [流式中断与客户端重试调研](流式中断与客户端重试调研.md)、[网关异常重试与兜底策略](网关异常重试与兜底策略.md) 和 [接口契约与权限矩阵](接口契约与权限矩阵.md)。
