...
- 一个分组就是一个号池；首选号池暂时不可用、全部账号繁忙、模型不匹配或授权账户失效时，调用方可以在真实派发前自动尝试备用号池。普通上游失败不能在已选分组内做跨组透明重放；只有写下游前命中的配置化流式拦截在当前号池账号耗尽时，才可按绑定优先级尝试备用号池。
...
- 允许的跨分组行为包括按优先级切换候选号池、写下游前配置化流式拦截耗尽当前号池后的后备切换，以及记录安全诊断摘要，例如尝试过哪些分组、失败 reason code 和最终命中分组。
- 不允许跨分组账号混排、跨分组负载均衡、跨分组借号、跨分组共享队列或跨分组共享分组级等待状态；普通上游失败和已写下游输出后的失败不得跨分组透明重放。
...
- 多分组切换主要发生在分组选择 / 调度预检阶段：如果 A 号池没有正常可调度账号，或本次请求在 A 号池内没有账号能通过路径能力 / 客户端 profile / 模型预检，才按优先级尝试 B 号池。一旦进入某个分组内真实上游派发，普通上游失败只在该分组内按账号切换、短期屏蔽、错误策略和恢复探活处理，不做服务端透明跨分组重放；配置化流式拦截在写下游前命中、允许服务端重试且 A 号池账号耗尽时，可以按绑定优先级尝试 B 号池。
...
| 已进入真实上游派发 | 选定分组后，普通上游请求失败、全部账号失败或已写下游输出后的流式中断都由该分组内账号切换和错误策略处理，不透明切到其他号池重放；写下游前配置化流式拦截耗尽当前号池时可按绑定优先级尝试后备号池 |
...
- 真实上游派发开始后，不因 A 分组普通上游失败而跨号池服务端重放；分组内账户会按既有恢复、冷却和切号策略自动处理。写下游前配置化流式拦截耗尽 A 号池时，可以按绑定优先级进入后续号池，并在审计 metadata 中记录切换原因。
