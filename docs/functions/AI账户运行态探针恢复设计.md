...
- 后台系统探针统一采用三类事实：完整协议成功、已经形成真实上游尝试但未成功、未形成有效上游尝试。前两类分别推进成功或失败状态机；任务取消、执行器内部错误、检查模型配置异常等未形成有效上游尝试的结论必须丢弃，不能累计失败、建立软阻断或推进下一次确认轮次。分类依据是是否形成真实上游尝试，不维护状态码、错误码或错误文案白名单。
