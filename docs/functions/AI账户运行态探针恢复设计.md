...
- 每个进程内的自动完整诊断共享最多 3 路门禁，不能随批量设置或同进程多个队列叠加放大；server 运行态恢复探针也独立限制为最多 3 路。不同 worker 不引入 Redis 全局诊断锁，依靠任务归属、启动错峰和 DB service 优先级隔离。冷却复测、Key 级冷却复测和速度优先恢复探针在 ops-worker 启动后分别延迟 60、65、75 秒执行首轮，避免与 stats-worker 启动期窗口刷新同时争用 DB service。探针 DB service 请求窗口为 30 秒，超时日志必须携带具体 operation 类型。
