...
本文用于说明 GPT API Key 和 OpenAI 兼容 API Key 账户，在同一个账户内配置多个上游 API Key 后的故障隔离、切号和后台恢复逻辑。当前第一阶段已落地：多个上游 API Key 归属同一个账户，支持轮询 / 权重选择，支持 Key 级运行态、失败摘除、账户内可用 Key 过滤、当前请求切后续账户、后台单次探测恢复和账户摘要字段。

后续仍可增强的部分包括：编辑弹窗逐 Key 状态展示、批量恢复 Key 状态、手动检测全部 Key、使用记录 / 审计中的 Key 指纹排障字段，以及更完整的运维页面。
...
当前请求命中账户 A 的 Key-1
Key-1 失败
先把 Key-1 记入网关进程内短避让和失败风暴观察
当前请求切到后续账户，不继续遍历账户 A 的其他 Key
如果只是单一来源、短时波动或近期仍有成功，不写全局 Key 运行态
只有跨 IP / 持续失败经过观察期且失败率足够高，才把 Key-1 写入不可调度或冷却
...
- 适用范围按供应商能力和账户凭据类型双重判断：GPT 和 OpenAI 兼容供应商的 `api_key` 账户，并且账户内实际存在 2 个及以上 Key 时，才启用 Key 级故障隔离。GPT OAuth、OpenAI OAuth、Codex OAuth、授权刷新失败、客户端兼容分支和未来非 API Key 供应商都不进入此状态机。
...
- 真实网关流量不能单次失败就写全局 Key 不可用；高并发、单一客户端 IP、单类请求或短时上游波动只应先进入进程内短避让和来源级避让。跨 IP 失败也必须经过最小观察期，并结合近期成功和失败率判断，避免把其他正常调用方可用的 Key 池打穿。
...
flowchart TD
  ...
  D --> E["Key-1 进入本地短避让和失败风暴观察"]
  E --> F{"跨 IP / 持续失败是否通过观察期、成功证据和失败率门槛"}
  F -- "否" --> G["不写全局 Key 状态"]
  F -- "是" --> H["写入 Key-1 运行态屏障 / 冷却 / 待恢复"]
  G --> I["当前请求排除账户 A"]
  H --> I
...
- 非流式或流式已经向下游输出内容后失败时，不能在同一个响应里静默拼接另一个 Key 的结果；只标记当前 Key 后续避让，让客户端或下一次请求恢复。
...
| 上游 attempt 失败但仍在请求级兼容恢复内 | 不立即写 Key 状态，等待恢复 attempt 结果 |
| 账户错误处理策略命中 `retry_next` 或未命中规则 | 网关真实流量先进入本地短避让和失败风暴观察；跨 IP / 持续失败通过观察期、近期成功和失败率门槛后，当前 Key 进入 `temporary_unavailable` 待恢复 |
| 账户错误处理策略命中 `temporary_unavailable` | 网关真实流量先进入本地短避让和失败风暴观察；跨 IP / 持续失败通过观察期、近期成功和失败率门槛后，当前 Key 进入 `temporary_unavailable` 待恢复 |
| 账户错误处理策略命中 `rate_limited` | 网关真实流量先进入本地短避让和失败风暴观察；跨 IP / 持续失败通过观察期、近期成功和失败率门槛后，当前 Key 进入 `rate_limited` 待恢复 |
| 账户错误处理策略命中 `error` | 代表错误策略已明确要求硬处理，当前 Key 可以立即进入 `error`，等待后台慢速探测或人工恢复 |
...
为了避免 100 个新 Key 全部异常但只能靠用户请求逐个发现，建议新增轻量异步检测：
...
- 对外 API Key：仍只绑定分组和授权调用方；不绑定上游 Key，也不感知上游 Key 明文。
