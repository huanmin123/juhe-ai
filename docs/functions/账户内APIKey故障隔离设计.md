...
当前请求命中账户 A 的 Key-1
Key-1 失败
立刻把 Key-1 标记为不可调度或冷却
当前请求切到后续账户，不继续遍历账户 A 的其他 Key
下一次再选到账户 A 时，跳过 Key-1，使用账户 A 内其他可用 Key
账户 A 内所有 Key 都不可用时，账户 A 才整体不可承接
后台探测异常 Key，成功后恢复该 Key
```
...
flowchart TD
  ...
  D --> E["标记 Key-1 进入运行态屏障 / 冷却 / 待恢复"]
  E --> F["当前请求排除账户 A"]
  F --> G["尝试同分组后续账户或后续可承接分组"]
  G --> H{"后续账户是否成功"}
  H -- "成功" --> I["本次请求救回；Key-1 后台探测恢复"]
  H -- "失败或无后续账户" --> J["返回网关统一失败；不继续遍历账户 A 的其他 Key"]
```
...
| 账户错误处理策略命中 `retry_next` 或未命中规则 | 当前 Key 进入 `temporary_unavailable` 待恢复 |
| 账户错误处理策略命中 `temporary_unavailable` | 当前 Key 进入 `temporary_unavailable` 待恢复 |
| 账户错误处理策略命中 `rate_limited` | 当前 Key 进入 `rate_limited` 待恢复 |
| 账户错误处理策略命中 `error` | 当前 Key 进入 `error`，等待后台慢速探测或人工恢复 |
