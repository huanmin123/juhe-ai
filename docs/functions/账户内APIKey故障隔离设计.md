...
本文用于说明 GPT API Key 和 OpenAI 兼容 API Key 账户，在同一个账户内配置多个上游 API Key 后的故障隔离、切号和后台恢复逻辑。当前口径是：多个上游 API Key 归属同一个账户，支持轮询 / 权重选择、Key 级运行态、失败摘除、账户内可用 Key 过滤、当前请求切后续账户、后台单 Key 探测恢复和账户摘要字段；人工测试只返回 Key 诊断明细，不初始化、摘除或恢复 Key 运行态。
...
建议不要在每次 Key 状态变化时立即覆盖 `accounts.status`。账户物理状态继续表达账户本身的停用、异常、到期、待检查等事实；Key 池不可用作为 `effectiveAvailability` 的派生阻断原因返回给网关和前端。只有当所有 Key 长时间探测失败并超过自动恢复观察窗口时，才考虑把账户标记为硬异常，且必须保证后台 Key 探测成功后可以恢复该派生阻断。
...
- `pending_test` 账户进入后台激活检查；激活检查按系统 Key 池策略验证账户，至少一个 Key 成功时账户可进入 `active`，失败 Key 由后台探针写入 Key 运行态并进入恢复队列。
...
- 创建账户先保存为 `pending_test`，后台激活检查按系统策略检查 Key 池；至少一个 Key 成功后账户才进入 `active`，失败 Key 写入 Key 运行态并等待后台恢复。
...
生产 `runtimeStateDriver=redis` 时，真实网关失败只写 `accountId + 完整 Key 指纹` 的 Redis 短暂避让，不直接写数据库全局 Key 运行态。避让按 3 / 5 / 10 秒推进，计数最多保留 10 分钟；真实成功清理短态。Key 选择使用一次批量读取合并数据库持久探测状态与 Redis 短态，禁止逐 Key 查询 Redis，也禁止 Redis 故障时回退写数据库全局状态。

后台冷却复测、人工测试等非普通网关流量仍按各自持久状态契约处理。探针任务自身异常或没有形成真实上游尝试时，不得写 Key 失败。
...
- 不支持同一次请求遍历账户内全部 Key。
- 不为 Key 级状态引入分布式锁；Redis 仅承接生产网关短 TTL transient avoidance，持久探测状态仍由业务库管理。
