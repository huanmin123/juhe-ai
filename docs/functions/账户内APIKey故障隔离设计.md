...
- 安全文本推理请求在同一账户内按当前池顺序唯一尝试所有可执行 Key；每个真实发送前同时检查跨账户物理凭据去重、整个请求 64 次 attempt 上限、总墙钟和最终响应预留。图片、音频、文件/资源创建、后台任务、hosted tool 和其他副作用 POST 发送后不得进入该轮换。
...
| 账户错误处理策略精确命中 Key 动作 | 只执行用户明确配置的 Key 副作用；动作包含 `retry_next` 仅表达本次重放意图，仍须通过可安全重放文本、未语义提交、派发阶段及预算门禁；已派发副作用请求不得重放 |
