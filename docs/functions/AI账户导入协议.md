...
- 导出会保留账户级 `healthCheckModel` 和 `healthCheckEndpointFamily`；再次导入时检查模型必须属于同一账户的 `supportedModels`，检查协议族仅允许 `chat_completions`、`responses`、`messages`、`generate_content` 且必须启用对应 JSON endpoint mode，否则预览失败。导入省略检查协议族时按新账户默认规则解析。
...
| `healthCheckEndpointFamily` | 否 | 后台健康检查协议族；允许 `chat_completions`、`responses`、`messages`、`generate_content`。省略时 GPT 官方优先 Responses，OpenAI-compatible 优先 Chat，Anthropic 优先 Messages，Gemini Native 优先 GenerateContent。 |
