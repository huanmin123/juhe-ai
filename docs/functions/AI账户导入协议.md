...
- 需要把 Gemini native `generateContent` / `streamGenerateContent` 请求桥接到 Chat Completions 上游时，在目标 OpenAI 协议 Chat 上游账号的 `modelMappings` 中显式配置 `sourceEndpointFamily: "generate_content"` 或 `"stream_generate_content"`，并配置 `upstreamEndpointFamily: "chat_completions"`；账户真实能力仍在 `credentials.supported_endpoint_modes` 中填写 `chat_json`、`chat_sse`。不要配置 Gemini native 到 `responses`、`messages`、`count_tokens` 或 `embed_content` 的映射。
- `providerProtocolProfileId = profile_gemini_openai_chat_v1beta` 时，`modelMappings` 只允许 OpenAI Chat 直连、OpenAI Responses / Codex 显式映射到 Chat Completions，或 Gemini native `generate_content` / `stream_generate_content` 显式映射到 Chat Completions；禁止 `messages -> chat_completions`，也禁止 OpenAI Chat / Responses 转 Gemini native。
- `tags` 用于账户快速分类，单个账户最多 24 个标签，单个标签最长 40 个字符；空白标签会被忽略，同一账户内大小写重复标签会去重。
- `modelMappings` 的 `sourceModel` 是客户端请求模型；`chat_completions` / `responses` 来源必须来自目标账户所有者可见的 OpenAI 协议模型池，`messages` 来源必须来自 Anthropic 协议模型池，`generate_content` / `stream_generate_content` 来源必须来自 Gemini 协议模型池；`upstreamModel` 是该账户实际转发模型，专用供应商必须来自当前供应商模型目录，通用 OpenAI-compatible 账号可来自 OpenAI 协议模型池。
...
- Codex Responses 请求只会命中具备 Codex Responses 兼容能力或显式 `responses -> chat_completions` 模型映射的账号；GPT / 通用 OpenAI 原生 Responses 账号必须具备 `responses_sse`，GLM Coding、DeepSeek 和 Gemini OpenAI Chat bridge 账号必须具备 `chat_sse` 并由本地 bridge 改写到上游 Chat Completions。Gemini native `streamGenerateContent` 桥接到 Chat 上游时同样要求目标账号具备 `chat_sse`。
