...
| `modelMappings` | 否 | 账号模型别名列表，只允许当前账号供应商模型目录内的同协议模型名改写；条目包含 `sourceModel`、`sourceEndpointFamily`、`upstreamModel`、`upstreamEndpointFamily`、`enabled`。跨供应商或跨协议映射请使用混合供应商账户。 |
...
- 需要把 OpenAI Responses 转到 Chat Completions、OpenAI / Gemini 转到 Anthropic Messages、Anthropic / OpenAI 转到 Gemini native，或 Gemini native 转到 OpenAI / Anthropic 时，请使用混合供应商账户配置真实上游和协议转换；账户 `modelMappings` 不承接这些跨协议桥接。
