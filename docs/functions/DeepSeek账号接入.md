...
本文记录 DeepSeek 供应商的接入方案、账户创建类型、协议档案、网关透传边界、模型目录、验证结果和后续实现注意事项。当前代码已落地 DeepSeek OpenAI-compatible 独立供应商，支持 Chat Completions JSON / SSE；Codex Responses -> Chat SSE 由 API Key 显式混合路由规则桥接到 DeepSeek OpenAI-compatible 分组；同时新增 DeepSeek Anthropic v1 Messages 档案用于 Claude Code / Anthropic 客户端画像直连。真实上游验证结果见本文“验证记录”。
...
- Codex Responses 桥接通过 API Key 默认客户端画像和显式混合路由规则启用，账户底层 endpoint modes 仍保存真实上游能力 `chat_json`、`chat_sse`
...
- 已落地 API Key 显式混合路由下的 DeepSeek Responses -> Chat SSE 桥接，覆盖 `/v1/responses` 入站、上游 `/v1/chat/completions` 改写、function tools、`web_search` guidance、`input_image` data URL、`reasoning_content`、截断失败和 SSE error 事件。
...
- 本地 mock AI 回归已覆盖 Chat JSON、Chat SSE、同协议模型映射、非兼容 `/responses` 拒绝、API Key 显式混合路由 bridge、`web_search` guidance 且不命中上游、cache usage、`reasoning_content`、`insufficient_system_resource` 和上游 `Content-Length`。
...
OpenAI-compatible 档案当前只声明 Chat Completions 语义，不把 Responses 当成默认能力。Codex bridge 是 API Key 显式混合路由适配：只有规则把 OpenAI Responses 来源指向 DeepSeek OpenAI-compatible 目标分组，且目标账号真实支持 `chat_sse` 时生效。`/beta` 下的 Chat Prefix Completion 和 FIM Completion 作为 DeepSeek 供应商 beta 能力记录，不扩展成通用 OpenAI Responses 档案。
...
- 下游客户端画像由 API Key 默认客户端画像或请求自动识别决定，不能单独决定上游协议档案
- 同一个 DeepSeek 上游 Key 如需同时承担普通 OpenAI Chat 和 Codex bridge，可以放在同一真实 Chat 能力账号中，由 API Key 显式混合路由决定哪些下游请求桥接到该分组
...
- 普通 OpenAI SDK / Responses 请求不由 DeepSeek 承接；只有 API Key 显式混合路由命中 DeepSeek OpenAI-compatible 目标分组时，才接收 `/responses` 或 `/v1/responses` 并改写到上游 `/chat/completions`
...
- 默认 DeepSeek 分组承接 DeepSeek Chat Completions 请求；当 API Key 显式混合路由把 Codex Responses 来源指向 DeepSeek OpenAI-compatible 分组，且请求为 Codex Responses SSE 时，可通过本地 bridge 承接 Codex `/responses`
...
- 一个 API Key 可以同时绑定 DeepSeek 与其他供应商分组；每次请求优先由 API Key 显式混合路由或模型目录、协议档案定位到目标 DeepSeek 账号池，再只在该 Key 已绑定的目标档案分组内调度
...
- DeepSeek 账户普通测试从 `/v1/chat/completions` 入口验证真实上游能力；需要验证 Codex bridge 时，通过绑定该分组的 API Key 显式混合路由从 `/v1/responses` 入口进入本地 bridge，并由 bridge改写到上游 `/v1/chat/completions`
...
- Claude Code / Anthropic-compatible 作为下游客户端画像由 API Key 或请求识别处理，不作为 DeepSeek 账户表单开关
...
| endpoint mode 误放开 | 普通 `/responses`、FIM、Prefix beta、Images、count_tokens 被路由到 DeepSeek | OpenAI 档案默认只开 `chat_json`、`chat_sse`；Codex bridge 只在 API Key 显式混合路由命中时接收 `/responses` |
