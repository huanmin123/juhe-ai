...
本文承接原 `docs/architecture/架构总览.md` 中偏具体功能、字段、接口、页面行为和运行策略的内容。架构总览只保留模块边界和跨模块关系；具体实现要求统一以本文、[第一期 OpenAI 账号接入](第一期OpenAI账号接入.md)、[SQLite 存储说明](SQLite存储说明.md) 和相关专题文档为准。

## 功能边界总览

- 第一阶段只实现 OpenAI 供应商，账户类型支持 OpenAI OAuth 和 OpenAI API Key。
- 系统账户负责后台登录、角色权限和数据隔离。
- AI 账户保存上游凭据、调度状态、代理、并发、错误策略、透传开关和授权关系。
