...
- 系统账户负责后台登录、角色权限和数据隔离。
- AI 账户保存上游凭据、调度状态、代理、并发、错误策略和授权关系；透传不作为账号开关，由供应商网关策略统一决定。
...
AI 账户使用授权用于把某个上游账户的“使用权”授予另一个系统账户。授权不改变账户所有者，不复制凭据，也不产生新的上游账户。
...
- 被授权用户不能查看账户完整敏感凭据，只能看到列表需要的基础摘要、状态、可用性和必要来源标识。
- 授权账户的“测试”如果保留，应按一次使用动作处理，必须以当前用户为调用方写入使用记录和统计。
...
- 授权收回后，被授权用户的旧分组关联可以保留为失效状态，调度时必须重新校验有效授权。
...
- 每行建议返回 `permissions` 摘要，例如 `can_use`、`can_edit`、`can_delete`、`can_authorize`、`can_view_credentials`。
...
分组是调度和使用授权边界。账户主动选择归属分组，API Key 绑定分组。
...
- 一个 API Key 先绑定一个自有分组；分组授权完成后，也允许绑定授权给自己的分组。
...
- 可共享账户默认只包含分组所有者自有账户；如果分组里包含其他用户授权来的账户，第一阶段不能随分组继续共享，避免绕过账户授权“不转授权”的规则。
...
- 每行建议返回 `permissions` 摘要，例如 `can_use`、`can_edit`、`can_delete`、`can_authorize`、`can_manage_accounts`。
...
- API Key 可绑定自有分组；分组授权能力完成后，也可绑定授权给自己的分组。
...
- 授权账户和授权分组调用都按调用方写入明细，资源所有者不能直接查看被授权用户的请求快照、响应快照、客户端 IP 或业务内容，只能通过对应授权管理查看聚合用量。
...
- 账户授权需要同时维护调用方口径和账户归属口径：调用方统计按 `usage_records.system_account_id` 聚合；账户列表真实账户用量按 `account_owner_system_account_id + account_id` 聚合；账户授权管理弹窗按 `account_owner_system_account_id + account_authorization_id` 聚合。
- 分组授权需要同时维护调用方口径和分组归属口径：调用方 API Key 和个人统计按 `usage_records.system_account_id` 聚合；分组所有者总用量按 `group_owner_system_account_id + group_id` 聚合；分组授权管理弹窗按 `group_owner_system_account_id + group_authorization_id` 聚合。
