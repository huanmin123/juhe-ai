...
> 状态：方案待评审，尚未实施。本文定义模型检测配置、定时检查、质量处罚、健康监控联动、质量隔离状态和自动恢复的目标契约；当前实现事实仍以[模型检测设计](模型检测设计.md)、[AI 健康监控设计](AI健康监控设计.md)和[AI 账户错误语义与状态变更边界](AI账户错误语义与状态变更边界.md)为准。
...
1. 手工检查与定时检查使用同一份互斥检测 profile：深度检测关闭时只运行快速检查，开启时只运行深度检查；“手工检测处罚”开关默认开启，只控制手工检查是否修改账户，不隐藏已经形成的质量事实。
...
- “检测配置”使用 `SettingOutlined` 图标和可读 tooltip，不只靠图标表达含义。
...
| 字段 | 类型 | 默认值 | 后端边界 | 说明 |
| --- | --- | --- | --- | --- |
| 深度检测 | 开关 | 关闭 | `profile=quick\|full` | 关闭时只使用快速检查，开启时只使用深度检查；手工、定时和质量恢复统一继承该 profile。 |
...
- 默认 `profile=quick`，包括手工、定时和质量恢复；只有用户在配置浮窗中显式开启深度检测并保存后，后续运行才使用 `profile=full`。系统不推荐默认开启深度检测。
...
- 支持一次选择多个账户；批量新增时每个账户使用自己的默认检查模型，创建后可逐行编辑模型和间隔。
...
列表由后端分页和稳定排序，不能加载全部账户后前端分页。删除计划不自动清除已经生效的处罚；用户需通过“立即检查”形成合格结果，或执行带确认和操作日志的人工解除。
...
- `stats-worker` 按稳定游标增量消费模型检测完成事实，写 `account_quality_health_hourly`，API 请求不扫描模型检测明细。
- 统计库重建时从保留期内的 `model_check_runs` 重建质量小时表。
- 质量小时表使用与健康小时表相同的保留期和账户删除清理边界。
- 游标只有在目标统计写成功后推进；worker 重放依靠 `(account_id, stat_hour)` 和 `model_check_run_id` 幂等。
...
penalty_threshold
penalty_action
isolation_recovery_interval_minutes
revision
created_at
updated_at
```
...
system_account_id
applied_action           -- fallback / quality_isolate / disable
generation
trigger_run_id UNIQUE
trigger_schedule_id
policy_revision
...
configured_action
actual_action
enforcement_status       -- not_triggered / applied / already_effective / skipped / pending_retry / failed
before_status
after_status
before_fallback_enabled
after_fallback_enabled
before_super_priority_enabled
after_super_priority_enabled
...
`model_quality_decisions` 是处罚展示和幂等执行的权威事实；每个 `run_id` 只能有一条。模型检测列表和详情必须按当前页 run IDs 批量合并决策摘要，禁止逐行 N+1 查询。dataset run 的安全摘要可用于日志保留和离线展示，但不能替代业务库中的处罚状态机事实；两边不同步时由后台按 run ID 补齐，详情优先返回权威决策并标记同步状态。
...
`model_check_runs` 增加触发来源和策略快照字段，或增加一张与 run 一对一的有界 metadata 表。实现只需要满足当前 Node 模型检测链路的职责边界和现有数据集兼容性：
...
`model_quality_decisions` 位于业务事实域，不能依赖 dataset 外键级联。run 批量淘汰成功后必须投递对应 decision 历史清理；当前仍被 `account_quality_enforcements` 引用的处罚信息已经在 enforcement 行中保存完整当前状态，decision 可以按 run ID 清理。清理投递失败要进入有界重试，不能让不可见的历史 decision 无限制增长。
...
- claim 后生成带 `generation` 的恢复任务；执行前后都校验处罚代次和账户配置版本。
...
管理侧和用户侧分别复用现有 `/model-checks`、`/my-model-checks` 权限边界，建议新增：

```text
GET    /model-checks/quality-policy
PUT    /model-checks/quality-policy
GET    /model-checks/schedules
POST   /model-checks/schedules
PATCH  /model-checks/schedules/:id
DELETE /model-checks/schedules/:id
POST   /model-checks/schedules/:id/run-now
```

用户侧对应 `/my-model-checks/...`。管理端必须指定具体 `systemAccountId`；用户侧忽略客户端传入的系统账户 ID。所有写接口只允许操作目标系统账户的自有账户，并使用 `revision` / `generation` 做乐观并发控制。
...
操作日志记录成功的策略保存、计划增删改、处罚应用、人工解除和自动恢复，不记录只读列表和普通轮询。安全字段至少包括：

- actor、system account、account、schedule、run 和 trace ID。
- 策略 revision、处罚 action、score、threshold、level。
- 状态前后值、处罚 generation、恢复 due time。
...
- 深度检测开关只出现在检测配置浮窗，默认关闭；快速与深度互斥，关闭只跑快速，开启只跑深度，运行中不自动升级或降级 profile。
- 阈值默认 70、最小 40、最大 100，且只接受整数；处罚默认降级备用；质量隔离恢复默认 10 分钟且最小 10 分钟，所有时间输入只接受整数分钟。
- 快速检查右侧存在定时检查按钮，可新增、编辑、暂停、立即执行、删除和分页查看自有账户计划。
...
- “手工检测处罚”默认开启；开启时自有账户手工检测可执行处罚并联动健康监控，关闭时不产生任何生产副作用。
