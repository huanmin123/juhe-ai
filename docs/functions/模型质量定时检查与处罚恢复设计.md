...
| 题库测试（选填，最多 3 题） | 多选（远程搜索） | 空 | `customQuestionIds`，JSON 数组长度 1..3、去重、每题必须已 `approved`；NULL 或 `[]` = 不启用 | 选择全局共享题库中已通过审核的题目；题库环节在检测套件最末执行，答错从百分制总分扣减，详见 [模型检测设计](模型检测设计.md) §5.7。 |
...
- 题库选择遵循字段级差量：PATCH 未携带 `customQuestionIds` 表示不修改，携带空数组表示清空并不再执行题库环节；提交的题目 id 由服务端校验必须存在且已通过审核。
...
- 模型字段硬不匹配、协议结构硬冲突，以及连续两次同专项契约的 GPT-5.6 Juice 强异常等 `suspicious` 硬证据不受阈值放宽影响，始终视为质量不达标。
...
- 普通账户候选只返回账户下拉直接展示和资格判断所需字段，不读取或附带 `modelCheckModels`。只有用户展开已选账户的模型下拉时，前端才以 `accountId=<账户 ID>&limit=1` 定点读取按支持模型和有效模型映射计算后的 `modelCheckModels`；手工检测与定时检查都只展示这个集合。保存计划时后端再次执行相同校验，禁止绕过页面写入无效账户 / 模型组合。
- 检查间隔：只接受整数分钟，默认 `60`，最小 `10`，最大 `10080`。
- 深度检测：每条计划独立配置 `profile=quick|full`，默认快速；深度模式耗时更长并消耗更多 Token。
- 题库测试：每条计划独立配置 `customQuestionIds`（选填，最多 3 题）；选择器远程搜索仅返回已通过审核的题目，空 = 不启用，语义与手动配置一致。
...
- 新增区的账户候选只在账户下拉首次展开或搜索时读取，并对搜索做短延迟合并、取消旧请求和迟到响应隔离；普通候选不附带模型关系。只有用户展开模型下拉时，前端才通过 `accountId=<accountId>&limit=1` 精确补取当前账户的 `modelCheckModels`，并按 owner、账户和请求代次缓存 / 隔离结果，不预取全部账户模型。
...
详情同时展示题库测试小计：已配置题库时显示“题库测试 x/31，扣 y 分”并支持逐题展开判定与回答摘录，数据来自 `resultSummary.customQuiz`；未配置题库时不出现该小计。
...
```text
hard_failure = 模型字段硬不匹配或协议硬冲突，或 GPT-5.6 Juice 强异常在两个同账户、同模型、同版本且同 SHA-256 专项契约的可比 full 运行中连续复现
below_threshold = score < penalty_threshold
quality_failed = hard_failure OR below_threshold
```

- 上游 HTTP 200 且固定内容、结构、确定性行为、生成式 canary 或已证实 Token 比例灌水校验失败时，必须先反映到 `score`，再按上述阈值统一决定处罚；不会因为来源是 Juice、Token 或 canary 而绕开分数。
- 题库测试扣分计入百分制总分后按同一阈值参与判定与处罚联动：基础探针全对 100 分时，3 题中 1 题答错为 100−11=89 仍达标；只配置 1 题且答错为 100−31=69，低于默认阈值 70，会按配置触发处罚。题库判定与扣分边界见 [模型检测设计](模型检测设计.md) §5.7。
- 仅有缺字段、未校准统计提示、网络/协议失败或缺少可比基线时为证据不足，不能伪造为内容失败或直接处罚。
...
- claim 直接返回计划行内的 profile、阈值、处罚方式、恢复周期和题库配置；题库题目 id 一并在 claim 时冻结进不可变 `ScheduledPayload`，计划后续编辑只影响下一轮，不读取 `model_quality_policies`。
...
- 恢复检查的题库配置按处罚来源取值：`config_source='schedule'` 时优先读取关联计划（`config_source_id`）的 `custom_question_ids`，该列为空时回落手动策略配置；其余来源直接使用手动策略配置。取不到任何配置时题库环节按未配置处理。
...
模型检测页的轻量静态 `GET /model-checks/options` 可以在首屏读取，但结果使用页面模块级内存缓存复用，切换 owner 或再次进入页面不重复请求；用户主动刷新时才强制更新。质量策略、定时计划和账户候选不并入该静态响应，也不得因此首屏预取。运行目标、可信对比、历史筛选和定时计划账户统一使用专用 `GET /model-checks/account-options`：只在对应账户下拉展开或搜索时调用，普通 `purpose=run|history|schedule` 响应不读取或返回模型关系；`schedule` 只允许当前 owner 的自有物理账户。用户展开模型下拉后，前端才以 `accountId=<当前账户>&limit=1` 定点读取该账户的 `modelCheckModels`，同一上下文并发请求使用 singleflight，切换账户、owner、关闭弹窗或 KeepAlive 停用时中止并丢弃旧响应。前端不得为了填充模型下拉再拉取通用账户详情或供应商全部模型。
