...
> 当前实现事实：曾创建的 `backend/src/storage/runtime/*` 只包含 `planned` descriptor，没有被生产入口调用，也没有实现本文列出的业务 Port，已于 2026-07-22 清理。当前 Node 运行时仍由既有 repository、DB service、worker、cache / runtime-state / queue factory 组成；本文的 Port / Adapter 结构是后续迁移约束，不代表仓库中已经存在统一 `StorageRuntime`。
...
- 运行模式选择只允许存在于已经承载真实消费者的装配或基础设施边界；不创建没有消费者、只返回状态描述的占位 runtime。
...
当前 Node 过渡模块如果仍需同时支持两种运行模式，对应业务 Port 才实现两套有真实行为的 adapter：
...
新增长期能力默认直接进入 Go + PostgreSQL + Redis。无论属于哪条迁移路径，都必须由已确认的生产消费者和业务 Port 契约驱动；只有 descriptor、没有方法实现和调用方的 adapter 不算已落地能力，也不应保留。
...
当前不存在统一 `StorageRuntime` 对象。未来只有在迁移某个真实业务消费者时，才按以下顺序落地装配：

1. 从调用方的业务事务和查询场景定义最小 Port，并用契约测试固定行为。
2. 实现目标运行模式需要的真实 adapter；Node 过渡模块按当前兼容范围实现，Go 长期模块只实现 PostgreSQL + Redis。
3. 在该消费者的 composition root 注入 Port，迁移调用后验证旧直接入口不再被引用。
4. 删除被替代的 repository 分支和临时白名单；不保留空接口、`planned` 状态对象或无调用方的全局容器。

装配规则：

- 只有真实 composition root 或基础设施初始化允许读取 `runtimeConfig.runtimeMode`、`databaseDriver`、`cacheDriver`、`runtimeStateDriver` 和 `queueDriver` 来选择实现。
- 业务 service、routes、DB service operation handler 不新增数据库类型分支。
- 旧 repository 函数在迁移期可以作为 Port adapter 内部实现细节复用，但对外只暴露 Port。
- 完成具体业务域收敛后，相关 driver 分支只允许出现在 adapter、静态守卫和明确的 fail-fast 边界。

## 代码落点

以下目录只在存在真实 Port、adapter 和消费者时按需创建，不预先铺设空骨架：
...
backend/src/storage/
  ...
  adapters/
    ...
    postgres-redis/
      ...
      ...
  composition/
    system-account-storage.ts       # 仅在系统账户消费者迁移时创建
...
2. **消费者阶段**：选择一个真实业务入口，先定义最小 Port 和契约测试，再实现 adapter 与注入；不提前创建全局 runtime 骨架。
...
7. **收敛阶段**：确认新 Port 已有生产消费者后，删除被替代的 driver 分支，扩大静态守卫，文档和测试矩阵改以 Store Port 为单位。
...
- 新 DB 操作默认先定义 Port；Node 过渡模块按仍需支持的模式实现 adapter，Go 长期模块只实现 PostgreSQL + Redis adapter。
