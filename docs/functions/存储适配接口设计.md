...
> 执行计划见 [PLAN-20260629T035152001Z 存储适配接口收敛](../plans/计划-20260629T035152001Z-存储适配接口收敛.md)。

> 迁移方向更新（2026-08-08）：Go 必须以 Store Port 收口 SQLite 与 PostgreSQL 的 schema、事务与方言差异，并以 Runtime / 直接异步执行边界选择本地或 Redis runtime adapter；业务规则不得按 driver 分叉。SQLite 模式不要求 Redis，缺失 Redis 也不能静默禁用 Go。当前 Node Redis Streams 仅服务于未迁 Node 功能；新 Go 完整功能不引入 Queue Port，直接提交 Store 并从持久事实恢复。详见 [完整功能接管与 Node 归档迁移规则](../migration/完整功能接管与Node归档迁移规则.md)。
