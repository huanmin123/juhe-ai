...
- `api_keys.group_id` 仍指向唯一分组；新建和编辑时只能绑定 API Key 所属系统账户自己的分组，当前 API Key 模型不保留授权分组绑定字段。
