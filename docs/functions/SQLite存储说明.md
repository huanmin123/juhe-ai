...
- `protocols` 保存协议族和版本，当前为 `openai/v1`；`protocol_endpoint_families` 保存协议下的端点族，当前包含 `chat_completions` 和 `responses`；`providers` 保存供应商身份和父子关系，当前为通用 `openai` 供应商和 `gpt.parent_code = openai` 子供应商；`provider_protocol_profiles` 把供应商绑定到协议版本并保存默认 `base_url`、默认测试模型、账户类型和能力，当前默认档案为 `profile_openai_openai_v1` 和 `profile_gpt_openai_v1`；`provider_protocol_profile_families` 保存档案启用的端点族能力。账户凭据 JSON 中的 `supported_endpoint_modes` 保存单个上游实际支持的 Chat / Responses 与 JSON / SSE 组合，网关候选筛选和账户测试都按该字段执行。
...
- `api_keys.availability_schedule_json` 保存 API Key 时间计划；为空表示不设置计划，API Key 完全按 `api_keys.status` 手动启停。启用计划后，background worker 按当前时间维护 `api_keys.availability_schedule_active` 派生字段，不覆盖人工启停 `status`。网关只读取落库后的人工状态和派生计划状态，不在请求链路解析计划 JSON。
