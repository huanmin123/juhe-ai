...
- `accounts.health_check_model` 保存账户必填检查模型，必须属于最终 `account_supported_models` 并满足协议最小检查能力。active 周期健康、真正账户全局运行态恢复、账号级冷却复测和 Key 恢复探针严格读取该字段，缺失或非法时记录配置异常，不回退其他模型；`pending_test` 激活改读 ready Catalog 的持久 activation selection，该模型只用于候选优先级。`accounts` 不保存 `default_test_model` 或 `health_check_enabled`；默认检查模型只在新账户创建阶段解析。
- `accounts.health_check_failure_started_at` 是旧 schema 的兼容诊断字段，只记录当前 activation selection 首个精确 execution 失败的诊断时间，不再拥有状态转换权限。`pending_test` 的 `framing_complete_neutral` 与 `upstream_failure(transport_incomplete)` 都只写对应 Route / Attempt 能力事实；任一 current definition / binding 的 `complete_success` 可完成激活，连续 24 小时无成功只写 `activation_unconfirmed / owner_action_required` 告警，不得写 `account_activation_check_timeout` 或整号 `error`。`probe_task_failure`、配置 / 解密异常、stale 和 unknown 不改能力状态；selection owner 负责有界重排。重新检查和异常恢复都只进入不可调度的 `pending_test`，把旧 selection 置 stale，并按当前 publication 重建或唤醒唯一后台检查，不能直接写为 `active`。自有账户在任意非 `disabled` 状态都可人工停用。
...
账户添加和编辑可以维护账号自己的错误处理规则；账户凭据中的 `error_handling_rules` 缺省为空，未命中规则时走通用失败处理。规则只决定用户显式命中后的目标动作是只切号、限流、临时不可调用还是异常；状态写入保留 policy provenance / generation / CAS，当前请求仍按统一的未交付结果规则切换候选。自有账户、分组授权账户和账户授权实例只有显式策略、`pending_test` 激活 owner、来源匹配的用户状态恢复，或已注册且真正账户全局的专属 owner 可以写各自 `accounts.status / cooldown_until / last_error_code / last_error_message`。普通请求、周期哨兵和精确 transport / model capability 探针只推进匹配 Route / Attempt，不写账户行。人工单账户测试无论成功或失败都不写账户、授权实例、Key、健康、错误、冷却或运行态字段；项目不提供多账户批量测试。完整流程见 [AI 账户运行态探针恢复设计](AI账户运行态探针恢复设计.md) 与 [网关异常重试与兜底策略](网关异常重试与兜底策略.md)。

账号质量窗口里的“频繁失败”只来自可信真实网关 transport / 账号依赖样本。质量刷新任务不会直接改 `accounts.status`；它只按固定批次生成真实失败 Attempt 的精确确认候选。确认探针按调用方系统账户、分组和授权实例上下文执行，使用失败 Attempt 的 ProbeRecipe；结果最多推进匹配 `scopeId + generation` 的精确 transport / model capability 运行态。无论失败次数多少，都不得把 active 账户写为整号 `temporary_unavailable`。
