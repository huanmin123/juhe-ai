...
- 账户级错误处理策略保存在 `accounts.credentials.error_handling_rules`，用于表达账户所有者对上游响应的显式管理意图。策略可按其声明范围匹配 `generic_*` 安全推理端点的完整响应；这不等于系统默认信任状态码/正文。只有精确 `retry_next` 表达本次重放意图，且仍受端点副作用与语义提交门禁；图片、音频、文件/资源创建等副作用 POST 派发后不得因其他动作隐式重放。规则保存启用状态、名称、优先级、状态码 / 错误码 / 错误类型 / 关键字匹配条件、动作和限流恢复策略。
