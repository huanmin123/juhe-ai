...
- 智谱官网新增的 Coding Plan OpenAI Responses `/api/v1` 在当前 seed 中尚未启用；后续必须新增独立 `profile_glm_coding_openai_responses_v1` 与 `provider_protocol_profile_families` 行，不能把现有 GLM Chat 档案直接扩成 Responses。模型协议能力、账户 `supported_endpoint_modes` 和健康检查形态也要在验证通过后分别写入，详见 [GLM 三协议适配方案](../plans/计划-20260908T162203434Z-GLM三协议适配方案.md)。
...
- AI 账户名称最长 128 个字符，由后端写入入口强制校验，前端表单同步限制输入长度。`account_name_search_documents` 每个账户保存一条 NFKC + 小写后的规范化名称；`account_name_search_terms` 保存该规范化名称的去重 1/2/3 连续字符词项，字段包含 `account_id`、`system_account_id`、`term` 和 `created_at`。每个账户最多约 `128 + 127 + 126 = 381` 个词项，规模按账户数线性增长。账户创建、改名、授权实例物化、授权实例改名和账户逻辑删除会同步维护这两张表；AI 账户列表的名称包含匹配先通过 `term + system_account_id + account_id` 索引定位候选账号 ID，再通过 `account_name_search_documents.normalized_name LIKE` 做最终连续片段校验，避免把非连续词项误判为命中。历史库升级后如需补齐已有账户词项：原 Node 命令 `pnpm --filter juhe-ai-backend maintenance:rebuild-account-name-search` 已随 Node 后端归档删除，Go maintenance 暂无等价重建 CLI（2026-09-20 标注；与 `rebuild-usage-stats` 同属 BUG-0175 D-219 登记的 rebuild CLI 缺口，usage 统计重建部分另见 BUG-0182），当前升级后词项只由账户创建/改名路径增量维护；列表请求路径不得为了补历史数据扫描 `accounts` 主表。
- `accounts.health_check_endpoint_mode` 是不可空精确请求形态，允许 Chat / Responses / Messages / GenerateContent 的 JSON / Streaming，以及图片模型的 `images_json`；与 `health_check_model` 一起定义后台系统探针。运行时直接使用保存值，不读取旧字段或做协议猜测；图片形态只能在模型目录证明检查模型支持 `images` 时保存。GPT 文本模型默认 `responses_sse`，纯图片模型默认 `images_json`，其他协议档案默认对应 JSON mode。既有 SQLite 数据库在初始化时若 CHECK 约束缺少 `images_json`，在事务内重建 `accounts` 表并保留数据、索引和触发器；重建或外键检查失败必须回滚并中止启动。
...
- `accounts.cooldown_retest_failure_count`、`cooldown_retest_observation_started_at`、`cooldown_retest_last_at` 和 `cooldown_retest_last_status_code` 保存 `temporary_unavailable` / `rate_limited` 后台复测的连续独立 `transport_incomplete` 次数、本轮自动 transport 恢复观察起点、最近复测时间和最近 HTTP 诊断状态。只有连接失败、硬超时、读取中断或 framing 未完成才增加计数并推进 7 天窗口：前五次按 `3/6/12/24/48` 秒策略基线复测，第六次起按全局被动策略重新偏移到 `1..5` 分钟。完整 HTTP / SSE framing 但协议未成功和已执行的 unknown 只在该策略窗口内顺延，任务失败不计数，stale 队列项由围栏丢弃。超过 `cooldownAccountRetestMaxBackoffHours`（默认 `12` 小时）后进入长期不可用阶段，按全局被动策略以 1 小时基线重新偏移复测；从首次 transport 失败起满 7 天后的再次独立 transport 失败才原子转为 `error`、关闭调度、清空冷却，并写 `cooldown_retest_observation_timeout` 和明确错误摘要。用户显式 `temporary_unavailable` / `rate_limited` 只能由 TTL、匹配来源恢复动作或人工恢复清理，普通或无 provenance 成功不得越权清理。
...
- `audit_logs`：保存进入原始审计的客户端请求事件元数据，用于后台页面检索；完全成功请求的主记录长期保留，最近 1 小时全量热保留 payload，超过热窗口后仅 10% 稳定样本继续保留 payload，其余主记录降为 `metadata_only`。
...
- `audit_logs` 写入热窗口内的全部完全成功请求，以及所有失败、异常、客户端中断、流式中断和重试后成功链路；热窗口后未命中 10% 稳定采样的成功记录只裁剪 payload，不删除主记录。每次请求的计量事实仍由 `usage_records` 保底。
...
- `route_strategies.is_default` 标识系统默认策略路由；默认数据初始化和系统账户创建都会为每个系统账户幂等补齐所有内置供应商的缺失默认分组，新插入行固定为 `enabled = 1`、`is_default = 1`，已有默认分组不覆盖也不改变启停状态；系统账户创建再为除“默认混合供应商分组”以外的每个默认分组创建默认普通路由。`api_keys.is_default` 标识系统默认 API Key；启动种子、系统账户创建和带账户作用域的 API Key 列表读取会为每条非混合默认普通路由补齐一个默认 API Key。混合供应商路由属于高级路由，必须由用户自己创建策略、绑定分组并配置对应混合供应商账户。
...
- `defaultTemporaryUnschedulableMinutes = 2`：账户首次进入临时不可调用或限流时的默认冷却时长，也用于其他普通临时不可调度流程；冷却账号后台复测的慢速通道以 `1..5` 分钟为策略基线并按全局被动策略重新偏移，不受该设置截断。
...
- 原始审计默认启用，不提供系统设置开关；2026-09-21 起 `JUHE_AI_AUDIT_LOG_ENABLED` 部署开关亦已移除，审计写入面常驻。
- 完全成功请求默认先全量进入最近 1 小时热保留窗口；超过热窗口后稳定桶命中的 `10%` 成功样本继续保留 payload，未命中长期采样的成功审计由后台清理 attempts 和 payload 引用并降为 `metadata_only`，主记录不删除。
...
- 默认启用最近 1 小时热保留窗口，审计日志页面可直接搜索热窗口内的原始内容。普通成功请求超过热窗口后只有命中默认 10% 稳定采样的记录继续保留 payload；未命中长期采样的成功记录由后台热窗口清理任务降为 `metadata_only`。部署环境变量可以调整或关闭成功热窗口与长期采样，但不能关闭问题链路审计。
