...
- 覆盖系统账户、团队、授权、代理、错误策略、AI 账户、分组、API Key、公告、使用记录、模型检测、审计日志、操作日志、运行日志、公开接口日志、外部来源系统、流式拦截策略、IP 统计与封禁策略、后台记录清理目标、账号质量、用量统计、系统监控和表空间监控。
...
- admin 作为超级管理员通过管理权限查看和操作全局业务数据；Mockdata 不通过“把资源授权给 admin”实现全局可见，也不生成 `资源归属人 = 被授权人` 的自授权业务记录或运行时授权。
- 分组会同时生成个人分组和高并发 AI 分组；高并发分组会带调度策略样本、专用 API Key、专用账户和对应使用记录，用于分组管理页验收。
- 授权样例必须覆盖个人直授权、团队授权、AI 账户授权、分组授权、有效授权、暂停授权、过期授权、回收授权和归还授权；授权调用方至少覆盖研发、测试、运维、财务和观察用户，不能只围绕单一用户或单一团队造数。
- 授权分组只作为分组列表、我的授权和授权用量统计样本展示；API Key 只能绑定调用方自己的本地分组，不能直接绑定授权方分组。`mockdata-summary.json` 会通过 `apiKeyBindingRule` 和 `authorizationSamples[].bindableToApiKey` 显式标记这个边界。
- 授权调用方的 Mock API Key 绑定的是该调用方自己的默认分组；授权账户样本会放入调用方本地分组，授权分组消耗样本只用于统计和审计展示，不代表 API Key 绑定路由。
...
- 公开接口日志、后台清理目标、IP 封禁策略、流式拦截策略和外部来源系统也必须使用上述清理标识，保证重复执行不会叠加旧样本。
...
- 授权列表中不应出现 admin 被授权记录，也不应出现资源归属人给自己授权的记录。
- 使用记录、审计日志、操作日志、运行日志均可按 `mockdata` 或 `造数` 检索。
- 公开接口日志、外部来源系统、流式拦截策略、IP 统计、IP 封禁策略、后台清理目标、用量统计、AI 性能监控、授权用量、API Key 额度窗口、系统指标趋势和表空间监控均有近 31 天数据。
- 模型检测历史会覆盖管理端与用户侧可见路径，包含运行中、已完成、失败和已取消样本，以及可信对比样本，用于模型检测页筛选、列表和详情验收。
- `backend/data/mockdata-summary.json` 中的 active API Key 可用于本地网关请求验证；其中 `authorizationSamples` 里的分组授权用于验证授权展示和授权统计，不应作为 API Key 绑定候选。
