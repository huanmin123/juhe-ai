...
- 先确定账户集合。
- series 的显式 `accountIds` 只按权限读取账户标签元数据，再读取对应小时缓存；不得为了补标签或排序读取 `usage_rank_snapshots`。排行快照仅用于 base 的默认前 10 账户池。
...
| PostgreSQL 关键词搜索按账户类别逐条往返 | 高延迟环境中一次输入触发多次串行查询 | 自有账户、账户授权实例、分组授权和来源账户名命中在一条候选 SQL 内合并、去重和限量，再统一执行一次窄账户标签投影 |
