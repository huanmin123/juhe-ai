...
| 入口 | 当前路由 | 当前接口 | 可见范围 |
| --- | --- | --- | --- |
| 用户侧 | `/my-ai-performance` | `GET /__aisys__/api/my-stats/ai-performance` | 当前登录系统账户名下的自有 AI 账户和授权实例账户 |
| 用户侧账户选项 | `/my-ai-performance` | `GET /__aisys__/api/my-stats/ai-performance/accounts` | 当前登录系统账户名下的自有 AI 账户和授权实例账户基础选项 |
...
- 用户侧只看自己名下的 AI 账户行，包含自有账户和别人授权给自己的授权实例账户；归属人原账户不会混入授权实例曲线。
- 管理侧只允许管理员访问，支持 `systemAccountId` 筛选指定用户；未指定时读取全局 `account` 统计缓存和全局排行快照。
- 统计视图按账户行维度读取；授权实例账户与归属人原账户分别统计，互不混入。授权方查看被授权消耗走授权团队 / 用户消耗明细，不通过 AI 性能监控合并到来源账户曲线。
...
| 账户归属校验缺失 | 越权查看他人账号趋势 | 用户侧 AI 账户性能按当前登录用户名下的 `system_account_id + account_id` 聚合，只能读取当前用户自己的账户行；别人授权给自己的 AI 账户会以授权实例账户身份进入用户侧，但归属人的原账户不会进入。管理侧由管理员权限和 `systemAccountId` 筛选决定范围 |
...
- 用户侧进入 `AI性能监控`，默认日期范围为最近 3 天，默认账户池为自己最近 7 天活跃前 10 个 AI 账户行，包含自有账户和授权给自己的授权实例账户。
- 管理员在管理模式进入 `AI性能监控`，可按用户筛选或查看全部用户全局账户性能，默认日期范围为最近 3 天。
- 别人授权给自己的 AI 账户以当前用户自己的授权实例账户进入默认账户池、搜索结果和 `accountIds` 临时追加结果；授权方原账户不会进入被授权人的用户侧页面。授权实例与授权方原账户分别统计，互不混入。
