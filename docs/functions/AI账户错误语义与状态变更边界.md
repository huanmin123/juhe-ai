...
未知上游响应只能终止当前请求，允许的动作只有：

- 记录 attempt、审计、耗时和有界的原始状态、错误码与错误摘要；
- 完整非 `2xx` 在未修改下游响应时直接返回原状态、正文和安全响应头；
...
只有账户 `credentials.error_handling_rules` **实际命中且动作是 `retry_next`** 才授权当前请求尝试另一个 Key、账户或分组。`cooldown`、`disable` 等其他显式动作可以改变账户状态，但当前请求仍返回该次失败；未匹配策略、系统默认响应检查、混合质量检查、speed-first 首字截止和请求墙钟都不授权候选切换。
...
“下游尚未提交”只说明网关仍能返回完整失败响应，不构成重放授权。图片、音频、文件/资源创建、后台任务、hosted tool 和普通文本都遵循同一规则：完整非 `2xx`、transport failure、timeout、响应体中断、`2xx` 协议结构失败与流式预提交失败，均在当前候选终止并暴露可诊断事实；不得以可用性为由切 Key、账户或后备分组。
...
自动候选切换是显式账户错误策略 `retry_next` 的专属能力。它仍必须受请求级去重、总墙钟和 attempt 预算约束；不得因未知错误产生重复 token、重复生成、重复外部副作用或重复计费。用户显式策略的 `cooldown`、`rate_limited`、`error_disabled` 等非 `retry_next` 动作只负责账户状态，不授权当前请求切换。
