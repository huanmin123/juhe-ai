...
>
> 多模型账户的最小故障作用域、局部阻断和整号全覆盖仲裁见 [AI 账户多模型能力健康与精确隔离设计](AI账户多模型能力健康与精确隔离设计.md)。无法证明整号作用范围时必须停留在精确模型能力层，不能因单模型独立探针失败写整个账户状态。
...
“下游尚未提交”只证明可以保持客户端响应完整，不能证明上游没有执行或计费。产品在这里明确采用**可用性优先的有界 at-least-once**：图片生成、图片编辑、image lane Responses、音频、文件/资源创建、后台任务、hosted tool 和其他请求，与普通文本使用同一候选切换规则。当前候选没有交付可用结果时，无论是完整非 `2xx`、transport failure、timeout、响应体中断还是精确协议确认的失败，都在请求预算内排除当前候选并继续 Key、账户或后备分组。该决策不读取状态码、错误码或错误正文来决定“是否切号”。

图片 lane 使用独立长时限：默认首响应上限 600 秒、流式 idle 上限 120 秒、单次未提交 attempt 上限 3600 秒，整请求候选切换墙钟由 `imageRequestWallTimeoutSeconds` 控制，默认 3600 秒。图片整请求墙钟与文本固定 270 秒墙钟、AI 对话 `generate_image` 工具的 `chatImageGenerationTotalTimeoutSeconds` 相互独立；配置可以按实际生成服务调整到 10–15 分钟或更长。整请求墙钟只在候选切换决策点阻止继续派发，不机械中断仍处于图片专用 attempt 时限内的在途请求。普通路由 `cost_first / speed_first` 的文本首 token 截止、慢样本和 `latency_degraded` 永远不作用于图片。图片专用时限只决定当前 attempt 何时失败，不改变统一候选切换规则：下游尚未语义提交时继续下一候选；下游已经写出真实协议事件、正文或可见输出时只能结束或断开当前响应，不能拼接第二个候选的结果。

统一候选切换属于明确的有界 at-least-once 可用性模式，可能产生重复 token、重复生成、重复外部副作用或重复计费；产品确认这些是低概率且可接受的代价，优先避免因当前账户失败而直接终止请求。所有请求都必须受请求级候选去重、总墙钟和 attempt 预算约束。用户显式策略的 `temp_unschedulable`、`rate_limited`、`error_disabled` 等动作仍只负责账户状态；它们不承担“是否允许当前请求切号”的授权职责，也不能另建一套切号旁路。

普通文本保留同一凭据的受控原地 transport 重试 token；这是文本链路的额外延迟优化，不是统一候选切换的准入门禁。图片和其他请求无需消费该 token，也能在当前 attempt 未交付结果时继续兄弟 Key、账户或后备分组。所有再次派发都必须检查候选去重、墙钟和最终响应保留时间。中间失败只形成审计/使用记录事实并可投递限频的独立健康检查，不写电路 confirmation、共享质量、Key/账户状态、客户端 IP 避让、代理或上游桶；共享状态必须等待独立探针确认。
...
流式失败只允许在精确协议适配器中按协议声明的事件结构识别，例如 SSE 事件名 `event: error`、Responses 的 `response.failed` 或图片协议的 `image_generation.failed`。普通 SSE 事件的 `data` JSON 仅包含 `error`、`data.error`、`metadata.error`、`code` 或 `message` 字段时只是普通 payload，不能被猜测为失败终态；事件携带的 code、type、message 和 HTTP 状态同样不可信。结构失败只证明本次 attempt 失败，不授权共享账户状态。下游尚未语义提交时，各类请求都可丢弃未提交事件并继续下一候选；输出后必须保留已提交内容、丢弃原始失败事件并稳定结束，不能拼接网关错误或透明重放。`generic_*` 客户端仍保持事件透明。
...
- 图片 lane 的未知 HTTP、请求体已发送后的断连、timeout 和 `2xx` 正文中断都会排除当前候选并尝试后备候选；
- 图片在 `speed_first` 策略下不得创建文本首 token timer、慢样本或速度切号；直接图片和账户模型映射升级后的图片跨过文本 270 秒后失败时，仍必须在图片整请求墙钟内尝试健康后备候选；
- 音频、文件、资源创建、后台任务和 hosted tool 等请求与普通文本使用同一候选切换规则；
