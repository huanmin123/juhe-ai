...
普通文本保留同一凭据的受控原地 transport 重试 token；这是文本链路的额外延迟优化，不是统一候选切换的准入门禁。图片和其他请求无需消费该 token，也能在当前 attempt 未交付结果时继续兄弟 Key、账户或后备分组。所有再次派发都必须检查候选去重、墙钟和最终响应保留时间。中间失败只形成审计/使用记录事实，不写电路 confirmation、共享质量、Key/账户状态、客户端 IP 避让、代理或上游桶。
