...
最终取舍：API Key 透传不是“所有入站头都裸转”，而是“body 真透传 + Header 最小清洗 + 认证替换 + 账号级 OpenAI 头覆盖”。这样既保留客户端请求语义，又避免把本地网关、代理链路、客户端 SDK 内部追踪和错误组织 / 项目信息带给上游。
...
| API Key 上游 header | `backend/src/modules/gateway/openai-gateway-upstream.ts` | 复制客户端语义头，过滤危险头、代理链路头、SDK / tracing 噪声，替换认证，并从账号凭据写入 OpenAI 组织 / 项目 / Beta |
| API Key 表单 | `frontend/src/views/accounts/AccountApiKeySection.vue` | API Key 账号支持可选 OpenAI 组织、项目和 Beta 配置 |
| 回归验证 | `backend/src/scripts/openai-api-key-passthrough-regression.ts` | 覆盖 raw body、Header 过滤、账号级覆盖和非透传 JSON fallback |
...
OpenAI OpenAPI spec 当前公开 base URL 是 `https://api.openai.com/v1`；`POST /responses` 示例使用 `Content-Type: application/json` 和 `Authorization: Bearer $OPENAI_API_KEY`。`OpenAI-Organization`、`OpenAI-Project`、`OpenAI-Beta` 属于公开 API 可用的附加头，但在账号池中转里不应默认相信任意下游客户端传入的组织 / 项目头。
...
| `juhe-ai` 当前建议 | 过滤危险头、代理链路、本地认证、`OpenAI-Organization` / `OpenAI-Project` 和常见噪声前缀；保留客户端 API 语义头；账号配置可覆盖 `OpenAI-Organization` / `OpenAI-Project` / `OpenAI-Beta` | 保留真实请求语义，同时让账号池边界更稳定 |
...
| `Portkey` | provider headers 来自 provider options；客户端额外头来自 `forwardHeaders` | 组织、项目、Beta 更应该归属于账号配置，而非调用方随意指定 |
...
原因：这两个头决定上游账号归属上下文。账号池中转里，下游客户端只拥有本地 API Key，不应该能用自己的请求头影响被调度上游账号所属组织或项目。它们应从账号凭据配置写入。

### 可保留但允许账号覆盖

- `OpenAI-Beta`

原因：`OpenAI-Beta` 更多是公开 API 功能语义，例如启用某些 beta API。客户端传入时可以保留；如果账号凭据里配置了 `openai_beta`，账号配置优先。
...
OpenAI API Key 账号凭据新增三个可选字段：

| 字段 | 上游 Header | 默认值 | 用途 |
| --- | --- | --- | --- |
| `openai_organization` | `OpenAI-Organization` | 空 | 当上游 API Key 需要显式指定组织时填写 |
| `openai_project` | `OpenAI-Project` | 空 | 当上游 API Key 需要显式指定项目时填写 |
| `openai_beta` | `OpenAI-Beta` | 空 | 当账号需要固定启用某个 OpenAI beta header 时填写 |

这三个字段不参与凭据指纹唯一性。唯一性仍以 `api_key` 明文归一化后计算，避免同一把上游 key 被多个系统账户重复添加。
...
| 前端复杂度 | 企业网关配置更多 | 我们只加三个可选字段，不暴露 header 规则 | 保持轻量，但不隐藏必要账号配置 |
...
- `OpenAI-Beta` 保留客户端语义，但账号凭据可覆盖。
- API Key 账号表单增加 `OpenAI 组织`、`OpenAI 项目`、`OpenAI Beta` 三个可选字段。
- API Key 回归脚本覆盖 raw body 字节级保留、危险头过滤、账号级覆盖、`Idempotency-Key` 保留和非透传 JSON fallback。
...
| P2 | 增加账号级“Header 预览”只读调试 | 如果用户经常误配 org/project/beta，可在测试结果里展示脱敏后的上游 Header 摘要 |
...
| 单元 / 脚本 | 客户端带 `OpenAI-Organization` / `OpenAI-Project`，账号也配置对应字段 | 上游只使用账号配置值 |
| 单元 / 脚本 | 客户端带 `OpenAI-Beta`，账号未配置 | 上游保留客户端值 |
| 单元 / 脚本 | 客户端带 `OpenAI-Beta`，账号配置 `openai_beta` | 上游使用账号配置值 |
...
4. `OpenAI-Organization` / `OpenAI-Project` 从账号配置写入，不从客户端透传。
5. `OpenAI-Beta` 兼顾客户端功能语义和账号级覆盖。
