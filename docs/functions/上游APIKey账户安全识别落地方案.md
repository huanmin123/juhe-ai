...
> 设计状态：已暂缓，不进入当前实现计划。
> 本文仅保留“上游 API Key 类型 AI 账户安全识别”评估过程和放弃原因；当前不新增字段、接口、前端入口或网关运行逻辑。
> 若未来重新评估，必须先重写目标、收益、可用性边界和验证方式，不能直接按本文旧方案落地。

## 暂缓结论

当前决定暂缓并放弃本轮实现，不把该能力纳入当前产品闭环。主要原因：

- 网关侧安全识别不是独立可信安全边界，尤其同账户自识别无法可靠证明不可信上游没有故意放行。
- 流式场景已经写出客户端后无法无缝修复，只能失败结束；这会明显影响体验。
- 每次命中候选都增加一次上游模型调用，带来延迟、成本、限流和可用性压力。
- fail-closed 会放大账号池不可用风险，fail-open 又削弱安全收益，默认策略难以平衡。
- 服务器本身不执行本地命令，真正危险执行仍发生在 Codex、Cursor、Claude Code、MCP Client 等客户端侧；客户端工具审批、沙箱和权限隔离才是主要防线。
- 以当前项目轻量中转定位看，实现、维护和排障成本高于短期收益。

后续维护口径：

- 不新增 `api_key_account_safety_*` 字段。
- 不新增安全识别模型选择、阻断处置、命中记录入口或相关前端 UI。
- 不在网关请求 / 响应 / 流式路径接入安全识别调用。
- 不因为本文存在而把“杀毒 / 安全识别”视为待实现功能。
- 如未来只做低风险能力，优先考虑“响应检查策略”或“审计观察”一类已有链路，而不是重新引入完整安全裁决闭环。
...
基于上述资料，当前方案保留“AI 账户级开关 + 当前账户安全识别模型 + 阻断处置策略”，但需要补强七个边界：

1. 安全识别模型不是另选一个 AI 账户，而是在当前被保护账户内选择一个更低成本、可结构化输出的模型。页面可以推荐低成本模型，但必须固定模型 ID 和安全配置，模型升级后需要重新跑安全回归。低价但不稳定的模型不适合作为最终裁决来源。
2. 同账户识别降低配置复杂度和跨账户可用性风险，但它不是独立信任边界。如果上游来源本身恶意并故意让识别模型放行，当前方案不能宣称可以完全对抗；第一版定位是降低提示注入、危险工具调用和本地执行风险，而不是替代可信第三方安全审计。
3. 静态规则只做候选提取，不能直接替代语义裁决。只要账户开启安全识别，安全裁决就是该账户本轮输出放行前置条件；安全识别模型内部重试后仍不可用时，当前账户本轮视为不可用，不能把未裁决的输出返回客户端。
4. 安全裁决必须带证据包：用户授权摘要、非用户注入信号、候选危险动作、目标路径和工具名。识别模型需要判断“哪里得到用户授权”，不能只输出一个风险分数。
5. 流式模式要做风险敏感缓冲。工具调用参数、shell 代码块和疑似危险命令在完整组装并裁决前不能直接写给客户端；普通文本可以按现有流式路径继续转发。
6. 默认处置应是短期避让，不是 `temporary_unavailable`。内容有毒或安全识别不可用都不直接等价于账号持久故障，默认只在网关运行态短时间避开当前账户，让网关或客户端重试后切其他账户。
7. `error` 和 `disable_account` 是人工偏强策略。它们应该返回不可重试错误让客户端停止，并在列表悬浮提示中展示安全原因，避免用户误以为是网络失败或上游限流。

## 非目标

- 不做本地杀毒软件，不扫描用户电脑文件，不阻止客户端本机绕过网关直接执行命令。
- 不替代 Codex、Cursor、Claude Code、MCP Client 等本机工具审批、沙箱和权限隔离。
- 不提供独立可信第三方安全审计；同账户安全识别只是在当前上游账户内换用指定模型做语义裁决。
...
静态规则不能准确判断“危险操作是否被用户授权”。第一版应支持为每个开启安全识别的 API Key 类型 AI 账户在当前账户内选择一个安全识别模型，用低成本模型做语义裁决。

配置规则：

- 开启安全识别时，用户只需要选择当前账户可用的“安全识别模型”，不选择另一个 AI 账户。
- 安全识别请求使用当前被保护账户的 base URL、凭据、代理和供应商协议档案发起，只把模型替换为用户选择的安全识别模型。
- 安全识别请求必须走当前账户配置的真实调用链路，包括 base URL、凭据、代理、请求头过滤、供应商协议档案和上游请求准备；不能绕过账号配置改用全局客户端或默认直连。
- 安全识别请求标记为内部安全裁决流量，不递归触发安全识别，不参与安全识别调用自身的账号调度、切号、账户错误处理策略或后台复测。
- 安全识别调用自身不切号；如果内部重试后仍不可用，外层网关应丢弃当前账户本轮输出，按当前账户安全识别不可用写短期避让运行态，再重新选择其他可用账户。
- 安全识别模型和被保护业务模型可以相同，但页面应建议选择同账户内更低成本、稳定支持结构化输出的模型。
- 安全识别模型只接收有界摘要、风险候选和必要上下文，不发送完整审计 payload、完整历史日志或完整大文件。
- 安全识别模型必须支持稳定结构化输出；后端只接受合法 JSON 裁决，解析失败按“安全识别不可用”处理。
...
- 安全识别不执行本地命令、文件操作或工具调用，因此不产生本机执行副作用；但它会产生一次额外上游模型调用，必须计入延迟、上游 token 成本和上游限流风险。
- 安全识别调用应在使用记录或审计 metadata 中标记为内部安全裁决流量，便于区分用户业务请求和安全识别成本。
- 安全识别模型调用必须有独立超时和重试预算；默认建议最多总尝试 2 次，只对网络异常、超时、上游 `429/5xx`、连接中断或返回非合法 JSON 做短退避重试。
- 安全识别重试仍然只使用当前账户和当前安全识别模型，不切换其他账户，不把识别失败直接写成持久账号异常。
- 安全识别最终不可用时，当前账户本轮不可放行；默认写 `safety_review_unavailable` 短期避让诊断，让外层调度尝试其他账户。
...
- 经过内部重试仍无法得到合法结构化裁决时，当前账户本轮视为安全识别不可用。
- `observe`、`block_high_risk` 和 `strict_agent` 都不能放行未裁决输出；区别只体现在已有合法裁决后的记录或阻断阈值。
- 安全识别不可用默认不执行 `error` 或 `disable_account` 持久副作用，也不触发账户错误处理策略或后台复测；它写入短期避让运行态，原因码为 `safety_review_unavailable`。
- 外层网关应丢弃当前账户本轮上游响应，在尚未写出下游内容时重新调度其他可用账户；没有其他可用账户时返回本地安全识别不可用错误。
...
短期避让运行态建议复用现有“本地账号短期屏蔽 / 响应语义检查命中避让”的思路：
...
| `api_key_account_safety_model` | text / null | `null` | 当前账户内用于安全裁决的模型，仅开启安全识别时必填 |
...
- API Key 账号开启安全识别时必须同时提交安全识别模型和阻断处置策略。
...
- 提供证据：为安全识别模型准备候选动作、风险类型、风险等级和目标摘要；安全识别不可用时不再用静态规则放行当前账户输出。
...
| 安全识别内部重试后仍不可用 | 不返回当前账户输出，写 `safety_review_unavailable` 短期避让，未写下游前由外层网关重新调度其他账户 |
...
- 不返回当前账户已经产生但未通过安全裁决的输出。
- 不按内容阻断策略把账户持久写为异常或停用；只写 `safety_review_unavailable` 短期避让运行态。
- 未写出下游 body 前，外层网关可以重新进入候选选择，避开当前账户并尝试其他可用账户。
- 已写出下游 body 后，不能静默拼接第二条上游流，只能写安全识别不可用失败事件并结束。
- 如果没有其他可用账户，返回本地 OpenAI 兼容错误，错误码为 `api_key_account_safety_review_unavailable`。
...
{
  "error": {
    "message": "当前 API Key 类型 AI 账户安全识别暂不可用，未返回未裁决内容",
    "type": "service_unavailable",
    "code": "api_key_account_safety_review_unavailable"
  }
}
```
...
| `safety_review_unavailable` | 当前账户安全识别不可用；未写下游前由网关避开当前账户重试其他账户，没有其他账户时返回本地失败 |

## 审计与使用记录

使用记录保存轻量字段：

- `safety_action`：`observe`、`blocked`、`review_unavailable`
...
- `safety_decision`：`allow`、`observe`、`block`、`unavailable`
- `safety_model`
- `safety_review_attempts`
- `safety_review_failure_reason`
- `safety_avoidance_reason`：例如 `safety_blocked`、`safety_review_unavailable`
...
- 开关：`启用安全识别`
- 安全识别模型：从当前账户可用模型中选择，建议默认低成本且稳定支持结构化输出的模型。
...
```json
{
  ...
  "apiKeyAccountSafetyModel": "safety-model",
...
- `apiKeyAccountSafetyEnabled = true` 时，`safetyModel` 和 `blockAction` 必填。
- `safetyModel` 必须来自当前账户可用模型目录或用户可见模型选项。
- `blockAction` 只能是 `short_avoidance`、`error` 或 `disable_account`，默认 `short_avoidance`。
- 安全裁决请求必须标记为内部流量，不能递归触发安全识别，也不能进入常规网关账号调度。
...
- 安全识别模型调用必须有独立超时、最大输入 token 预算和最大输出 token 预算；超时后按“安全识别不可用”策略处理。
- 安全识别模型调用默认最多总尝试 2 次，单次超时、退避间隔和最大耗时由后端常量控制，不在第一版页面暴露；重试只用于安全裁决内部稳定性，不能形成跨账户切换或无限重放。
...
- 安全识别调用失败只按“当前账户安全识别不可用”策略处理，不能把当前账户写为持久异常、限流、普通临时不可用或触发后台复测。
- 安全识别不可用短期避让必须只影响当前账户的安全识别调度资格，不写入 `accounts.cooldown_until`，也不参与普通上游健康复测。

## 与现有模块关系

| 模块 | 关系 |
| --- | --- |
| AI 账户 | 新增 API Key 类型账号的安全识别配置；OAuth 不适用；安全识别模型来自当前 API Key 账户 |
| 分组 | 不持有策略，只影响命中哪个账号 |
| 本地网关 API Key | 不持有策略，仍负责调用方身份、额度和分组路由 |
| 账户错误处理策略 | 无关系；安全阻断不是上游非 `2xx` 错误 |
| 响应语义检查策略 | 响应语义检查处理 `200 + SSE` / `200 + JSON` 协议污染；安全识别处理本地执行风险，两者可以共存但命中 metadata 要区分 |
...
以下内容仅作为历史方案复盘参考，不代表当前待实现或待验证事项。若未来重新立项，必须重新评估并重写验证要求。

- API Key 类型账号能保存、读取和展示安全识别配置。
- 开启安全识别时必须选择当前账户内的安全识别模型。
- 开启安全识别时可选择阻断处置策略，默认是 `short_avoidance`。
- 安全裁决请求使用当前账户的凭据、base URL、代理和协议档案，只替换为安全识别模型，不能递归触发安全识别。
- 安全识别模型必须返回合法结构化 JSON；非法 JSON 按安全识别不可用处理。
- 安全识别模型调用失败或非法 JSON 时只在内部最多重试固定次数；内部调用不能切换其他账户，重试后仍失败时当前账户本轮不可用并触发外层重新调度。
- 安全识别不可用时不能放行当前账户输出；未写下游前必须避开当前账户尝试其他可用账户，没有其他账户时返回 `api_key_account_safety_review_unavailable`。
...
- 安全识别不可用命中后终止当前账户尝试，不改变账号持久状态，只写 `safety_review_unavailable` 短期避让运行态；网关应优先在服务端重新调度其他账户。
...
以下分阶段内容已不进入当前实现计划，仅保留为方案演进记录。

### 第一阶段：配置与观察

- 新增 API Key 类型账号安全识别字段。
- 前端要求开启时选择当前账户内的安全识别模型。
...
- 外部分类器如果作为增强组件接入，不得反向调用被检测账户；当前账户安全识别模型仍按本方案使用当前 API Key 账户发起内部裁决请求。
