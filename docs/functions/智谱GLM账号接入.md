...
> 2026-09-09 三协议复核（方案阶段）：智谱官方现已在 Coding Plan 接入工具表中明确列出 OpenAI Response 协议及独立端点 `https://open.bigmodel.cn/api/v1`。当前 Go/前端仍只实现下方三个历史档案，尚未注册原生 GLM Responses；本次先固化新增独立 Responses 档案的方案，代码、seed、模型白名单和导入枚举均待后续实施与验证。本文旧段落中“没有原生 Responses”的表述仅代表复核前的实现事实，不能继续作为官网能力结论。

## 范围

> 目录同步边界：GLM 的上游模型目录只由用户手动刷新账户草稿调用，内部标记仅保护该受控转发。账户弹框打开、字段变更、取消以及测试或确认不会自动同步；测试或确认遇到缺少支持模型或检查模型时只提示并中止。人工 / 草稿测试、激活、健康、恢复和调度直接验证 Chat 或 Messages endpoint；目录 `404` 或未开放不能阻断已配置完整模型账户的使用，客户端模型目录仍由本地返回。

本文记录智谱 GLM 供应商的接入结论、账户创建类型、协议档案、网关透传边界、Codex bridge、模型目录和后续实现注意事项。当前代码已落地 `glm` 供应商、通用 GLM API Key、GLM Coding Plan OpenAI Chat 和 GLM Coding Plan Anthropic v1 三个协议档案；官网新增的 OpenAI Responses 端点尚未进入运行代码。计划新增独立 `profile_glm_coding_openai_responses_v1`，并保留现有 Chat-only bridge 的显式语义。运行事实以“当前实现”与“方案阶段目标”两栏区分。
...
- Codex / OpenAI 客户端必须使用 Responses 协议时，适配目标是独立 `profile_glm_coding_openai_responses_v1` 直连 `/api/v1/responses`。现有 `responses -> chat_completions` 仅作为 Chat-only 账号的显式 bridge，不能冒充原生 Responses，也不能通过 API Key 或策略路由隐式开启。
...
- 通用 GLM API、GLM Coding Plan OpenAI Chat、GLM Coding Plan Anthropic v1 和目标 Responses 档案必须使用不同 `provider_protocol_profile_id`，不能只靠同一个 `glm` 供应商和不同 `base_url` 混用，否则分组、额度、模型目录、账号测试和排障都会混在一起。
...
| `profile_glm_coding_openai_responses_v1`（方案目标，待实现） | `glm` | `openai/v1` | `https://open.bigmodel.cn/api/v1` | GLM Coding Plan Key | OpenAI Responses / Codex | `responses_json`、`responses_sse` | 不启用隐式 bridge |

OpenAI Chat 两套现有档案的账户真实能力仍只声明 `chat_completions`；Anthropic v1 档案只声明 Messages / Models，不默认写入 `message_token_counting`。Responses 档案只有在代码落地且通过 Mock + 最小真实请求后，才可写入 `responses_json/responses_sse`。普通 GLM 账户不会隐式启用 Codex bridge；需要把 Responses / Codex Responses 转到 GLM Chat 时，必须通过账号模型别名显式声明 `responses -> chat_completions`，账户真实 endpoint modes 仍保持 `chat_json/chat_sse`。
...
当前选择 `智谱 GLM` 后展示三个已落地接入类型；Responses 目标档案在代码实施完成前不得出现在可提交导入枚举中：
...
| GLM Coding Plan OpenAI Responses Key（方案目标，待实现） | `api_key` | `profile_glm_coding_openai_responses_v1` | OpenAI Responses / Codex | `api_key`、`base_url` | 待模型白名单验证 |
...
- `credentials.supported_endpoint_modes` 省略时，当前两种 GLM OpenAI Chat 接入类型默认 `['chat_json', 'chat_sse']`；目标 Responses 档案实施后单独默认 `['responses_json', 'responses_sse']`，不得按 `glm` 供应商统一回填。
...
- GLM 账号不再保存账户级客户端兼容开关；当前通用 GLM 和 GLM Coding OpenAI Chat 档案只保存真实 `chat_json/chat_sse` 上游能力。Responses 目标档案实施后才保存真实 `responses_json/responses_sse`，Codex 客户端画像仍由网关内部识别。
- 管理 API、导入协议和公开外部账号推送当前只接收 `providerProtocolProfileId = profile_glm_general_openai_v1 | profile_glm_coding_openai_v1 | profile_glm_coding_anthropic_v1`。代码实施完成并通过验收后，才将 `profile_glm_coding_openai_responses_v1` 加入契约；旧接入类型别名不能解析。
...
- 通用 GLM 账户和 GLM Coding OpenAI Chat 档案只具备 Chat Completions 请求能力。目标 Responses 档案直连 GLM `/api/v1/responses`；Chat-only 账号如需承接 Responses / Codex Responses，仍须在普通账号模型别名中显式声明 `responses -> chat_completions`，并继续只保存真实 Chat 上游配置。
...
GLM OpenAI Chat 档案复用 OpenAI v1 Chat Completions 协议适配器；方案目标的 Responses 档案复用通用 OpenAI v1 Responses adapter，但两者必须按独立 profile 和 Base URL 选择。
...
方案目标的原生 Responses 边界：

- 命中 `profile_glm_coding_openai_responses_v1` 且模型目录明确声明 `responses` 时，`POST /responses` 与 `POST /v1/responses` 直连 `https://open.bigmodel.cn/api/v1/responses`（或同协议代理根地址），不经过 Chat bridge。
- Responses profile 只允许 `responses_json`、`responses_sse`；不能因客户端请求形态把 Chat modes 写入该档案，也不能把 Chat profile 的 Base URL 改成 `/api/v1`。
...
本节只描述 Chat-only 档案的兼容 bridge。原生 Responses 目标档案已发布独立 `/api/v1` 方案后，仍不能把 bridge 当成原生能力；两者要在 profile、endpoint modes、模型能力、审计和指标中分开。

当命中的仍是 GLM Chat 档案且需要兼容 Codex 客户端时，Responses 请求才通过普通 GLM OpenAI v1 账户的模型别名显式表达：下游仍看见 Responses SSE，账号真实上游调用 GLM Chat Completions SSE。API Key 和策略路由层不再保存 `responses -> chat_completions` 显式规则。
...
- GLM Coding bridge 不等于 GLM 原生支持 OpenAI Responses；Chat 账户的 `supported_endpoint_modes` 仍保存 `chat_json/chat_sse`。原生 Responses 只能命中独立 Responses profile。
...
GLM 默认只创建一个供应商默认分组：默认 GLM 分组。通用 GLM API、GLM Coding OpenAI Chat、目标 Responses 和 GLM Coding Anthropic Messages 的差异由 AI 账户接入类型和 `provider_protocol_profile_id` 表达，不再拆成多个默认分组。
...
- 方案目标的 Responses 档案实施后，必须增加独立 `/v1/responses` JSON/SSE 测试，确认请求命中 `/api/v1/responses`；该测试与 Chat bridge 分开计账和审计。
...
| 范围 | 必补事项 | 风险 |
| --- | --- | --- |
| 官方资料 | 已核对 Coding Plan 工具接入表、Codex、Claude Code 和最新模型页：官方列出 Chat、Anthropic Messages、Responses 三个不同 Base URL；Responses 字段、事件和模型白名单仍需真实验证。 | 页面正文对 Responses 展开不足；若未按独立 profile 处理，容易把 `/responses` 错发到 Chat URL 或误把 bridge 当原生能力。 |
...
| 供应商档案 | 在现有三个档案外新增目标 `profile_glm_coding_openai_responses_v1`、Responses profile family 和单个默认 GLM 分组，不复用 GPT、通用 OpenAI 或官方 Anthropic 档案。 | 只靠 `base_url` 区分会导致模型目录、价格、账号测试和错误排障混用。 |
...
| Base URL | 通用档案默认 `https://open.bigmodel.cn/api/paas/v4/`，Coding Chat 档案默认 `https://open.bigmodel.cn/api/coding/paas/v4`，Coding Anthropic 档案默认 `https://open.bigmodel.cn/api/anthropic`，目标 Responses 档案默认 `https://open.bigmodel.cn/api/v1`；路径拼接必须验证不会生成重复 `/v1`、漏掉 `/chat/completions`、`/responses` 或误拼 Anthropic `/messages`。 | OpenAI SDK 的 `base_url` 语义与上游 HTTP API 原始路径容易混淆；Chat 与 Responses 根地址不同，错误复用 helper 会导致错端点。 |
| Endpoint mode | 现有 GLM Chat 档案默认仅 `chat_json/chat_sse`，Anthropic 档案仅 Messages；目标 Responses 档案单独允许 `responses_json/responses_sse`。Chat-only 账号的 `responses -> chat_completions` 继续是显式 bridge。 | 如果沿用 GPT API Key 默认四项能力，Responses 请求可能被误认为上游原生支持；如果把 bridge 隐藏在 profile 里，路由和审计无法判断该请求为什么跨协议。 |
...
| 错误处理 | 通用客户端完整 `2xx` / 普通 JSON / SSE payload 透明；所有请求的完整非 `2xx` 只按 `response.ok=false` 做内容无关的请求级候选接管，transport / timeout 进入统一错误输入。图片、文件和其他请求在语义未提交时同样切换候选。只有精确客户端画像或用户显式策略才允许解释声明范围内的 `finish_reason` / Coding Plan 错误码。 | 直接按状态码或错误正文硬编码限流 / 异常会破坏协议中立边界。 |
| 账号切换与恢复 | GLM Chat/Anthropic 继续按现有探针；目标 Responses 增加独立 JSON/SSE 探针。原生 Responses 的不透明状态不得在 GPT、GLM 或不同 GLM profile 间自动换号；Chat bridge 仍标记 `responses -> chat_completions`。 | Coding Plan 测试会消耗套餐；后台批量探测或错误恢复如果不节流，会把套餐额度浪费用在恢复任务上。 |
...
| 前端体验 | 账户创建页、编辑页、批量导入说明、模型选择、测试连接、错误提示、状态标签和日志筛选要区分“通用 GLM API Key”、“GLM Coding Plan Chat Key”、“GLM Coding Plan Responses Key”和“GLM Coding Anthropic Key”；目标档案未实施前不得出现在可提交枚举。 | 用户把 Key 填错档案时，如果前端只显示“API Key”，无法定位是 Key 无效还是接入类型选错。 |
| Mock 与验证 | Mock AI 回归脚本和真实烟测需覆盖四档案目标矩阵、Chat JSON/SSE、Messages JSON/SSE、Responses JSON/SSE、未配置映射的 Chat 账号拒绝原生 Responses、显式 Responses -> Chat bridge、错误响应、无 usage 流式、错档案 Key、分组硬边界和统计落库。 | 只测保存账户会漏掉网关调度、协议转换、切号、恢复、usage 和统计链路的集成问题。 |
...
- [ ] 新增 `profile_glm_coding_openai_responses_v1`，绑定 `https://open.bigmodel.cn/api/v1`、`responses` family 和 `responses_json/responses_sse`。
- [ ] 同步 Go seed、账户 endpoint mode 归一化、URL 构造、后台健康/模型探针、身份 header 白名单和前端 profile/import/export 枚举。
- [ ] 为模型目录建立独立 Responses 能力白名单；在真实证据前不把全部 GLM 模型标记为 `responses`。
- [ ] Mock 覆盖 Responses JSON/SSE、usage/reasoning/tool/error、原生 Responses 与 Chat bridge 分流，以及 GPT/GLM 原生 Responses 不透明状态换号拒绝。
- [ ] 用最小 Coding Plan 请求验证候选模型和真实事件语义，通过后才 Promote 为可用能力。
...
- 直接 `/v1/responses`：2026-06-20 对 `vsllm.com/v1` 代理的历史请求未按 Codex Responses 协议成功返回；这只能说明当时代理路径未验证通过，不能否定智谱官方当前公布的 `/api/v1` Responses 端点。官方端点仍需按本计划重新做 Mock 与最小真实请求验证。
...
- 方案目标的 `profile_glm_coding_openai_responses_v1` 仅在模型目录声明 `responses` 且探针确认 `/api/v1/responses` 后可启用；其 JSON/SSE 测试、usage/reasoning/tool 语义和不透明状态换号防火墙必须与 Chat bridge 分开验收。
