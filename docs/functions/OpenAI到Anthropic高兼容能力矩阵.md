...
- 未配置本地 runtime / provider / adapter / executor 的 `code_interpreter`、`computer`、`image_generation`，以及所有需要服务端执行的 MCP 请求。
