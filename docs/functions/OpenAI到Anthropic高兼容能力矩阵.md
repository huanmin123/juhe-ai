...
| OpenAI hosted tools | 已逐类返回正常 agent guidance 或本地承接；`web_search` 不再走本地预取模拟；`file_search`、`image_generation`、Responses `tool_search` + `namespace` function 本地展开、Responses `code_interpreter` mock runtime、Responses `mcp` mock proxy 和 MCP `local_runtime` allowlist executor 第一段已有路径 | `tool_search` 本地展开只处理请求内已声明的 namespace function，并恢复 Responses `function_call.namespace`，不伪造 `tool_search_call` / `tool_search_output`；`code_interpreter` mock runtime 只在显式 mock 模式下固定 `code_interpreter_call` JSON / SSE 外形，不执行代码、不请求 Anthropic、不代表真实沙箱；`mcp` mock proxy 只在显式 mock 模式下命中固定 `mock-mcp` allowlist，返回 `mcp_list_tools`、`mcp_approval_request` 或 `mcp_call`，不连接远程 MCP、不回显 authorization、不代表真实 MCP proxy；MCP `local_runtime` 已具备 allowlist executor 第一段，JSON 非流式免批路径会先请求 Anthropic 让模型选择导入后的 MCP client tool，再执行 MCP JSON-RPC `initialize` / `tools/list` / `tools/call`，并把 `tool_result` 回灌给 Anthropic 生成最终回答，支持 authorization 白名单透传、`allowed_tools` 过滤和输出截断；默认 approval 或非免批工具本地返回 `mcp_approval_request`，不上游、不调用 `tools/call`；但尚未完成多轮工具循环、持久 approval、HTTP-SSE 真实远程回归和 UI 管理；`image_generation` 首批支持 Responses generate JSON / SSE，provider 可选择 OpenAI Images API 或 OpenAI Responses API 图像工具形态；provider 未配置时返回 OpenAI 形态 guidance 且不命中 Anthropic；provider 审核失败返回 Responses `status=failed` 并保留稳定错误码；Responses `computer` / `code_interpreter` / `mcp` 与 Chat `code_interpreter` 的无 adapter guidance 已有 mock 覆盖；code_interpreter / computer / MCP 真实运行时仍需要对应本地运行时或上游原生等价能力后才能启用；Chat 搜索模型路径不提供 Responses 的完整工具控制和 sources 能力 |
...
| `metadata` / `user` / `safety_identifier` / `prompt_cache_key` | Anthropic `metadata.user_id`、网关本地亲和或审计 metadata | L1/L5 | `safety_identifier` 优先映射为 Anthropic `metadata.user_id`，否则使用 `user`；`prompt_cache_key` 只作为本地亲和 / 审计信号，不承诺 Anthropic prompt cache；业务 `metadata` 不扩散到 Anthropic |
...
| `mcp` / remote MCP | Anthropic MCP connector 或网关 MCP proxy；mockai 阶段可走固定 allowlist 的本地 mock proxy；`local_runtime` 已进入 allowlist executor 第一段 | L2/L3/L4 | 需要 server allowlist、auth、approval 和审计映射；默认返回 agent guidance；Responses 无 adapter guidance 已有 mock 覆盖且不请求 Anthropic；Responses `mcp=mock` 已覆盖固定 `mock-mcp` allowlist、`allowed_tools` 过滤、approval request、`mcp_call` JSON / SSE、authorization 不回显和未授权 server 本地拒绝；Responses `mcp=local_runtime` 未配置 executor 时本地 503，`connector_id` 本地 400；配置 allowlist executor 后，JSON 非流式免批路径会把 MCP tools/list 导入 Anthropic client tools，由 Anthropic `tool_use` 触发 MCP JSON-RPC `tools/call`，再把 `tool_result` 回灌给 Anthropic 生成最终回答，authorization 仅入 allowlist server，并支持输出截断；默认 approval 或非免批工具本地返回 `mcp_approval_request`，不上游、不调用 `tools/call`；SSE 当前仍走本地 Responses 直返路径；真实 MCP proxy 首批只承接 `server_url` 远程 MCP，OpenAI `connector_id` 需要独立 connector adapter，不能用 remote proxy 伪装；多轮工具循环、持久 approval、HTTP-SSE 远程回归和 UI 管理仍按 [OpenAI 托管工具运行时设计](OpenAI托管工具运行时设计.md) 推进 |
...
| Function tools | 多工具 JSON/SSE、指定工具、required、none、`parallel_tool_calls=false` 映射、Responses `max_tool_calls` 本地拒绝、Responses `allowed_tools` function 子集、非对象 / 非法 JSON arguments 保留、tool result、orphan / duplicate tool result、thinking + 强制 tool_choice 冲突拒绝、流式 `input_json_delta` 参数分片 |
| Hosted tools | Responses / Chat `web_search` guidance 且不命中上游、Responses `computer` / `code_interpreter` / `mcp` guidance 且不命中上游、Chat `code_interpreter` guidance 且不命中上游、Responses `code_interpreter=mock` JSON / SSE 本地 `code_interpreter_call` 且不执行代码不上游、Responses `mcp=mock` JSON / SSE 本地 `mcp_list_tools` / `mcp_approval_request` / `mcp_call` 且不连接远程 MCP 不上游、Responses `mcp=local_runtime` JSON 免批模型选择工具 + MCP `tools/call` + 输出截断、Responses `mcp=local_runtime` 默认 approval 本地 `mcp_approval_request`、Responses `mcp=local_runtime` SSE 本地直返、Responses `tool_search` + `namespace` JSON / SSE 本地展开、image_generation 无 provider guidance、image_generation Images provider JSON / completed-only SSE / partial SSE 成功路径、image_generation Responses provider JSON / SSE 成功路径、provider `moderation_blocked` / 非 JSON / 超大响应体 / timeout failed response、edit / mask / 历史复用 guidance、file_search 未知 vector store 本地失败、file_search 本地检索成功路径、vector store file `in_progress` / `failed` 生命周期 |
...
| 状态 / Compact | `conversation` 本地拒绝、`background=true` 本地拒绝、`store=true` 本地拒绝、`truncation=auto` 本地拒绝、`context_management` 本地拒绝、`compaction` / `compaction_summary` 恢复、`/responses/compact` 返回 `response.compaction`、不直转 Anthropic、跨边界 snapshot 拒绝 |
