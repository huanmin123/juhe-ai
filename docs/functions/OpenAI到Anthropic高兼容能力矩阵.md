...
| Responses JSON / SSE 文本 | 已支持 | hosted tool 事件仍未覆盖；`include=reasoning.encrypted_content`、`include=message.output_text.logprobs`、`include=web_search_call.action.sources`、`include=code_interpreter_call.outputs`、`include=computer_call_output.output.image_url`、`include=message.input_image.image_url` 和 `top_logprobs>0` 在 Anthropic bridge 下本地拒绝，不静默忽略 |
...
| OpenAI hosted tools | 已逐类返回正常 agent guidance 或本地承接；`web_search` 不再走本地预取模拟；`file_search`、`image_generation` 和 Responses `tool_search` + `namespace` function 本地展开已有首批路径 | `tool_search` 本地展开只处理请求内已声明的 namespace function，并恢复 Responses `function_call.namespace`，不伪造 `tool_search_call` / `tool_search_output`；`image_generation` 首批支持 Responses generate JSON / SSE，provider 可选择 OpenAI Images API 或 OpenAI Responses API 图像工具形态；provider 未配置时返回 OpenAI 形态 guidance 且不命中 Anthropic；provider 审核失败返回 Responses `status=failed` 并保留稳定错误码；Responses `computer` / `code_interpreter` / `mcp` 与 Chat `code_interpreter` 的无 adapter guidance 已有 mock 覆盖；code_interpreter / computer / MCP 真实运行时仍需要对应本地运行时或上游原生等价能力后才能启用；Chat 搜索模型路径不提供 Responses 的完整工具控制和 sources 能力 |
...
| Responses 其他输出扩展 `include` | 按本地工具运行时补齐，或本地拒绝 | L2/L3/L4 | `file_search_call.results` 已由本地 File Search 运行时承接；`web_search_call.action.sources`、`code_interpreter_call.outputs`、`computer_call_output.output.image_url`、`message.input_image.image_url` 当前没有等价输出结构或运行时，命中时本地 OpenAI 错误拒绝，不静默返回缺少 include 数据的成功响应 |
