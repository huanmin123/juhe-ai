...
供应商能力层不能承诺 100% 语义无损。OpenAI hosted tools、Responses encrypted reasoning、Files / Vector Stores、image generation、computer use、code interpreter、MCP 等能力不是简单字段同构。长期方案必须是：
...
- 未配置本地 runtime / adapter / executor 的 `code_interpreter`、`computer`，以及所有需要服务端执行的 MCP 请求。`image_generation` 不读取独立 provider 开关，由当前 API Key 的 Images 路由是否存在决定可用性。
...
| L4 | Agent guidance / 受控不执行 | 当前不能可靠承接，例如 encrypted reasoning 原文验证、跨供应商 compact snapshot、服务端 MCP 执行 | hosted/native tool 无执行器时返回正常 agent guidance；非法历史、权限或状态边界问题返回对应协议错误；都不请求不支持的上游 |
...
| OpenAI hosted tools | 已逐类返回正常 agent guidance 或本地承接；`web_search` 不再走本地预取模拟；`file_search`、`image_generation`、Responses `tool_search` + `namespace` function 本地展开、Responses `code_interpreter` mock runtime、Responses `code_interpreter` local_runtime Python worker 首段、Responses `computer` mock runtime、Responses `computer` local_runtime adapter 首段和 Responses `computer` HTTP sandbox adapter bridge 首段已有路径；MCP 固定为 guidance | `tool_search` 本地展开只处理请求内已声明的 namespace function，并恢复 Responses `function_call.namespace`，不伪造 `tool_search_call` / `tool_search_output`；`code_interpreter` mock runtime 只在显式 mock 模式下固定 `code_interpreter_call` JSON / SSE 外形，不执行代码、不请求 Anthropic、不代表真实沙箱；`code_interpreter` local_runtime 首段会把 Responses `code_interpreter` 转为 Anthropic `python` client tool，由网关独立 Python 子进程执行后回灌 `tool_result`，支持 stdout / stderr logs、超时、输出截断、安全 env 不泄漏、文件产物元数据摘要、符合上限产物写入本地 Files 后的下载接口，以及 `/v1/containers/{container_id}/files` 兼容壳；`computer` mock / local_runtime / HTTP sandbox adapter 只在显式配置下承接，并保持不上游、截图正文省略和敏感字段脱敏；`image_generation` 支持 Responses generate JSON / SSE，携带当前请求授权回到本机 Images 网关路由，无可用路由时返回网关不可用错误；Responses `mcp` 不提供 mock、local_runtime、proxy、approval、execution record 或管理 UI，命中时返回 OpenAI 形态 guidance，提示客户端使用本地 MCP、调用方 agent 自行执行或切换到原生支持该能力的上游，不请求 Anthropic、不连接远程 MCP、不伪造工具结果 |
...
| Chat / Responses 顶层 `moderation` | OpenAI 输入 / 输出审核配置，无 Anthropic bridge 等价策略 | L4 | 顶层 `moderation` 存在且非 `null` 时本地 OpenAI 错误拒绝；注意 `image_generation` tool 内部 `moderation` 由 Images 网关路径单独承接 |
...
| `mcp` / remote MCP | 不由网关服务端承接；客户端本地 MCP 或原生支持 MCP 的上游自行处理 | L4 | 固定返回 agent guidance，不读取 MCP runtime 开关，不配置 server allowlist / auth / approval / execution record / transport / UI，不输出网关生成的 `mcp_list_tools`、`mcp_call` 或 `mcp_approval_request`；本轮不请求 Anthropic、不连接远程 MCP server、不伪造工具结果 |
...
| `image_generation` | 当前 API Key 的本地 Images 路由 | L3 | Anthropic Messages 不产生图像；桥接层生成或抽取 revised prompt 后，携带当前请求授权调用本机 `/v1/images/generations`，由 `API Key -> 路由策略 -> 分组 -> gpt-image-2 API Key 账户` 决定可用性；Responses JSON / SSE 返回 `image_generation_call`；无可用图片路由时按网关不可用错误返回；`moderation_blocked`、非 JSON / 缺失结果、超大响应体、超时返回 Responses failed object；edit / mask / 历史图像复用已覆盖 guidance |
...
| 输出图片 / `image_generation_call` | 本地 Images 网关路由返回后渲染 OpenAI image item | L3 | 与 Messages 文本响应分离；成功路径输出 base64 `result` 和 `revised_prompt`；失败路径不输出图片，保留 OpenAI 风格 `error.type` / `error.code` / `moderation_details`；审计不保存非流式 `result` 或流式 `partial_image_b64` 正文；流式支持 completed 事件和最终 `response.completed`；上游 Images / Responses 差异由账户协议档案和网关转换处理，不再由图片 provider 环境变量选择 |
...
| image_generation result | 不适用于 Chat text | `image_generation_call` / image output item | 已支持 Responses generate JSON / SSE 的本机 Images 网关路由 |
