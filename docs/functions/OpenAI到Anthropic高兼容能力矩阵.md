...
| OpenAI hosted tools | 已逐类返回正常 agent guidance 或本地承接；`web_search` 不再走本地预取模拟；`file_search`、`image_generation`、Responses `tool_search` + `namespace` function 本地展开、Responses `code_interpreter` mock runtime、Responses `mcp` mock proxy 和 MCP `local_runtime` allowlist executor 第一段已有路径 | `tool_search` 本地展开只处理请求内已声明的 namespace function，并恢复 Responses `function_call.namespace`，不伪造 `tool_search_call` / `tool_search_output`；`code_interpreter` mock runtime 只在显式 mock 模式下固定 `code_interpreter_call` JSON / SSE 外形，不执行代码、不请求 Anthropic、不代表真实沙箱；`mcp` mock proxy 只在显式 mock 模式下命中固定 `mock-mcp` allowlist，返回 `mcp_list_tools`、`mcp_approval_request` 或 `mcp_call`，不连接远程 MCP、不回显 authorization、不代表真实 MCP proxy；MCP `local_runtime` 已具备 allowlist executor 第一段，JSON 非流式免批路径会先请求 Anthropic 让模型选择导入后的 MCP client tool，再执行 MCP JSON-RPC `initialize` / `tools/list` / `tools/call`，支持 authorization 白名单透传、`allowed_tools` 过滤和输出截断；默认 approval 或非免批工具本地返回 `mcp_approval_request`，不上游、不调用 `tools/call`；但尚未完成二次 Anthropic `tool_result` 回灌、持久 approval、HTTP-SSE 真实远程回归和 UI 管理；`image_generation` 首批支持 Responses generate JSON / SSE，provider 可选择 OpenAI Images API 或 OpenAI Responses API 图像工具形态；provider 未配置时返回 OpenAI 形态 guidance 且不命中 Anthropic；provider 审核失败返回 Responses `status=failed` 并保留稳定错误码；Responses `computer` / `code_interpreter` / `mcp` 与 Chat `code_interpreter` 的无 adapter guidance 已有 mock 覆盖；code_interpreter / computer / MCP 真实运行时仍需要对应本地运行时或上游原生等价能力后才能启用；Chat 搜索模型路径不提供 Responses 的完整工具控制和 sources 能力 |
