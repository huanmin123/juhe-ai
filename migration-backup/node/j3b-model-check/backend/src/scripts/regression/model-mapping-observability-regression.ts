...
const auditLogMappersSource = readProjectFile('backend/src/storage/audit-log-f3-mappers.ts')
...
assertIncludes(auditLogMappersSource, 'sourceEndpointFamily: text(row.attempt_source_endpoint_family)', 'F3 审计尝试 mapper 必须返回 attempt sourceEndpointFamily')
assertIncludes(auditLogMappersSource, 'upstreamEndpointFamily: text(row.attempt_upstream_endpoint_family)', 'F3 审计尝试 mapper 必须返回 attempt upstreamEndpointFamily')
assertIncludes(auditLogMappersSource, 'attempt_source_endpoint_family', 'F3 审计 read-only mapper 必须返回 attempt sourceEndpointFamily')
assertIncludes(auditLogMappersSource, 'attempt_upstream_endpoint_family', 'F3 审计 read-only mapper 必须返回 attempt upstreamEndpointFamily')
