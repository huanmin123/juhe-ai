...
export function shouldRefreshGeminiOAuthCredentials(
  ...
  const expiresAt = resolvedGeminiOAuthExpiresAt(credentials, expiresAtFallback)
  ...
  if (expiresAtMs === undefined) {
    throw new Error('Gemini OAuth credentials.expires_at必须是带 Z 或数值 offset 的 RFC3339 时间')
  }
  return expiresAtMs - Date.now() <= geminiOAuthRefreshLeadMs
}
...
function dispatchAccountWithCredentials(
  ...
  const expiresAt = resolvedGeminiOAuthExpiresAt(credentials, account.expiresAt)
...
function resolvedGeminiOAuthExpiresAt(
  credentials: Record<string, unknown>,
  expiresAtFallback?: string
): string | undefined {
  if (credentials.expires_at !== undefined) {
    return requiredRfc3339Instant(credentials.expires_at, 'Gemini OAuth credentials.expires_at')
  }
  if (expiresAtFallback === undefined) return undefined
  return requiredRfc3339Instant(expiresAtFallback, 'Gemini OAuth account.expiresAt')
}
