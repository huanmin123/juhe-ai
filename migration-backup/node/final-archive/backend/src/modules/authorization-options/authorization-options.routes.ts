...
authorizationOptionsRouter.get('/grantee-accounts', async (req, res, next) => {
  try {
    ...
    const options = await listAuthorizationGranteeAccountsAsync(access, query)
    res.json(ok(options))
  } catch (error) {
    next(error)
  }
...
authorizationOptionsRouter.get('/grantee-teams', async (req, res, next) => {
  try {
    const access = getRequestAccessScope()
    const query = parseAuthorizationOptionListOptions(req.query)
    const options = await listAuthorizationGranteeTeamsAsync(access, query)
    res.json(ok(options))
  } catch (error) {
    next(error)
  }
...
authorizationOptionsRouter.get('/grantee-groups', async (req, res, next) => {
  ...
  try {
    const access = getRequestAccessScope()
    const groups = await listAuthorizationGranteeGroupsAsync(access, options)
    res.json(ok(groups))
  } catch (error) {
    next(error)
  }
