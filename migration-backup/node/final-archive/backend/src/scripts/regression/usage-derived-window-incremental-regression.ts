...
assert.match(postgresAiRefresh, /LIMIT \$\{aiPerformanceDirtyClaimLimit\}[\s\S]+FOR UPDATE SKIP LOCKED/, 'AI 性能摘要每轮应有有界 dirty account claim')
...
assert.match(postgresQuotaRefresh, /LIMIT \?[\s\S]+FOR UPDATE OF dirty SKIP LOCKED/, 'quota 窗口刷新必须有动态 scope 预算')
assert.match(postgresQuotaRefresh, /dirty\.generation = claimed\.generation/, 'quota dirty 清理必须使用 generation CAS')
assert.match(postgresQuotaRefresh, /request_quota_hourly_window_scope_bindings/, 'quota 窗口刷新必须读取精确 scope 绑定')
assert.doesNotMatch(postgresQuotaRefresh, /CROSS JOIN windows/, 'quota 窗口刷新不得对每个 scope 计算所有活跃窗口')

assert.match(overviewSource, /LIMIT \$\{usageOverviewDirtyCandidateLimit\}[\s\S]+snapshotRowCount \+ estimatedRows > usageOverviewRolloverSnapshotRowBudget/, '概览窗口刷新必须按窗口行预算限制单轮 dirty scope 数')
