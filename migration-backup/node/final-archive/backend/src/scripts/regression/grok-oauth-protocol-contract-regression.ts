...
assertRoute(routesSource, 'expectedConfigRevision: z.number().int().min(1)', 'Grok OAuth 手动刷新与重授权必须要求客户端配置版本')
assertRoute(routesSource, 'runWithProviderOAuthRefreshLock(', 'Grok OAuth 手动刷新与重授权必须加入跨进程刷新锁')
assert.ok((routesSource.match(/current\.configRevision !== parsed\.data\.expectedConfigRevision/gu) ?? []).length >= 3, 'Grok 手动刷新与两条重授权路径都必须在上游请求前拦截版本变化')
...
assertRoute(routesSource, 'rotateOAuthCredentialsAsync({', 'Grok OAuth 凭据写回必须使用用途专用窄写 repository')
assertRoute(routesSource, 'oauthRotationReceipt(updated', 'Grok OAuth 凭据更新只返回最小 mutation 回执')
assert.equal(routesSource.includes('updateAccountAsync'), false, 'Grok OAuth 凭据更新不得复用账户宽写')
assert.equal(routesSource.includes('sanitizeAccountCredentialCarrierResponse'), false, 'Grok OAuth 凭据更新不得返回完整账户载体')
