...
assert.equal(redisStreamDrainContracts.length, 3, '排空工具必须覆盖三条仍由 Node 拥有的可靠队列流')
assert.equal(new Set(redisStreamDrainContracts.map((item) => item.streamKey)).size, 3, '三条流 key 不得重复')
assert.equal(new Set(redisStreamDrainContracts.map((item) => item.groupName)).size, 3, '三个 consumer group 不得重复')
...
assert.equal(drained.drained, true, '三条流都为空时应允许完成排空')
