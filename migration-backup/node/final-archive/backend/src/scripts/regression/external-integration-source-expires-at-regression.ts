...
try {
  ...
  const clearedSource = externalSources.updateExternalIntegrationSource(source.id, { expectedUpdatedAt: source.updatedAt, expiresAt: null })
  assert(clearedSource?.mutation.updatedAt, '来源系统 expiresAt 清空必须返回新版本')
  assert.equal(externalSources.findExternalIntegrationSource(source.id)?.expiresAt, undefined, '来源系统 expiresAt 必须用 null 显式清空')
  ...
  assert.throws(() => externalSources.updateExternalIntegrationSourceToken(source.id, token.id, {
    expectedUpdatedAt: externalSources.findExternalIntegrationSource(source.id)?.tokens.find((item) => item.id === token.id)?.updatedAt ?? '',
    expiresAt: '2026-02-31T00:00:00'
  ...
  const tokenBeforeClear = externalSources.findExternalIntegrationSource(source.id)?.tokens.find((item) => item.id === token.id)
  assert(tokenBeforeClear?.updatedAt, 'Token 详情必须携带 PATCH 版本')
  const clearedToken = externalSources.updateExternalIntegrationSourceToken(source.id, token.id, {
    expectedUpdatedAt: tokenBeforeClear.updatedAt,
    expiresAt: null
  })
  assert(clearedToken?.mutation.updatedAt, '来源系统 token expiresAt 清空必须返回新版本')
  assert.equal(externalSources.findExternalIntegrationSource(source.id)?.tokens.find((item) => item.id === token.id)?.expiresAt, undefined, '来源系统 token expiresAt 必须用 null 显式清空')
