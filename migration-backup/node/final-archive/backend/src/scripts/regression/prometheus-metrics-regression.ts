...
const upstreamCompleted = startHttpMetricRequest('/v1/chat/completions', 'POST', 1_200)
finishHttpMetricRequest(upstreamCompleted, 503, 'completed', 1_250, 'upstream')
...
assert.match(rendered, /juhe_ai_http_requests_total\{[^}]*failure_scope="gateway"[^}]*method="POST"[^}]*outcome="completed"[^}]*route_group="gateway"[^}]*status_class="5xx"[^}]*\} 1/)
assert.match(rendered, /juhe_ai_http_requests_total\{[^}]*failure_scope="upstream"[^}]*method="POST"[^}]*outcome="completed"[^}]*route_group="gateway"[^}]*status_class="5xx"[^}]*\} 1/)
...
assert.match(rendered, /juhe_ai_http_failure_scope_metrics_enabled\{service="juhe-ai"\} 1/)
assert.match(
  rendered,
