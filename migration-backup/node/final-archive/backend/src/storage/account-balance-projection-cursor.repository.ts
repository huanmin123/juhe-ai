...
export async function currentAccountBalanceProjectionCursorAsync(client: DatabaseClient, consumerKey: string): Promise<AccountBalanceProjectionCursor | undefined> {
  return cursor(await client.one<Row>(`SELECT ${storedObservedAt(client)} AS observed_at,outcome_id FROM ${table(client)} WHERE consumer_key=?`, [key(consumerKey)]))
}
...
export async function advanceAccountBalanceProjectionCursorAsync(client: DatabaseClient, consumerKey: string, next: AccountBalanceProjectionCursor): Promise<boolean> {
  ...
  return await client.transaction(async (tx) => {
    const existing = cursor(await tx.one<Row>(`SELECT ${storedObservedAt(tx)} AS observed_at,outcome_id FROM ${table(tx)} WHERE consumer_key=?${tx.driver === 'postgres' ? ' FOR UPDATE' : ''}`, [consumer]))
...
function storedObservedAt(client: DatabaseClient): string {
  return client.driver === 'postgres'
    ? `to_char(observed_at::timestamptz AT TIME ZONE 'UTC', 'YYYY-MM-DD"T"HH24:MI:SS.US"Z"')`
    : 'observed_at'
}
