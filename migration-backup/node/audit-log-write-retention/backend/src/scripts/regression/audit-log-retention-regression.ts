...
function assertAuditPayloadCleanupUsesAsyncFiles(): void {
  ...
  assert(recordMaintenanceSource.includes('auditRetainedDataCleanupBatchSizeLimit = runtimeConfig.background.recordMaintenanceAuditCleanupBatchSize'), '审计保留维护任务必须限制从 data-retention 继承的大批量参数')
  assert(recordMaintenanceSource.includes('auditRetainedDataCleanupMaxBatchesLimit = runtimeConfig.background.recordMaintenanceAuditCleanupMaxBatches'), '审计保留维护任务必须限制单次追赶批次数，避免长时间占住 record-maintenance consumer')
