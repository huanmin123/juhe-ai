...
if ($ownerLockEnabled.Trim().Equals('true', [System.StringComparison]::OrdinalIgnoreCase)) {
  ...
  node scripts/validate-owner-manifest.mjs --require-deployment-epoch=$ownerLockEpoch --require-node-version=$nodeVersion --require-schema-version=63 deploy/owner-manifest.json
  if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
